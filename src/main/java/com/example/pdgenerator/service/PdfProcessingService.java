...
public class PdfProcessingService {
    ...
    /**
     * Get relevant factor guide content for a job series
     */
    public String getRelevantFactorGuide(String jobSeries, String duties, String gsGrade) {
        String fullText = getCachedPdf("factor_guide");
        // Use keywords from jobSeries, duties, and grade to select relevant paragraphs
        String[] keywords = { jobSeries, gsGrade };
        if (duties != null && !duties.isEmpty()) {
            keywords = Stream.concat(Arrays.stream(keywords), Arrays.stream(duties.split("\\s+"))).toArray(String[]::new);
        }
        return extractRelevantSections(fullText, keywords);
    }
}