...
public class PdService {
    ...
    public String buildPrompt(PdRequest request) {
        try {
            // Build prompt using request data
            String prompt = buildComprehensivePrompt(request);
            // Call OpenAI and get response
            String aiResponse = callOpenAI(prompt);
            // Extract total points and grade from AI response
            int totalPoints = extractTotalPoints(aiResponse);
            String aiGrade = extractGSGrade(aiResponse);
            String gradeRange = calculateGradeRange(totalPoints);
            // Ensure consistency
            if (!aiGrade.equals(request.getGsGrade())) {
                // If mismatch, override with AI grade
                request.setGsGrade(aiGrade);
            }
    ...
    Duties/Responsibilities Context: %s
        ...
        **FAIR LABOR STANDARDS ACT DETERMINATION:**
        ...
        jobSeries, subJobSeries, federalAgency, subOrganization, lowestOrg, historicalData,
        federalAgency, subOrganization, lowestOrg, subJobSeries, jobSeries, subJobSeries, gsGrade, supervisoryLevel,
    ...
    private String calculateGradeRange(int totalPoints) {
        // Implement your logic to calculate the grade range based on total points
        ...
        if (totalPoints >= 1900) {
            ...
            return "GS-11 or below";
        }
    }
    // Helper to extract total points from AI response
    private int extractTotalPoints(String response) {
        java.util.regex.Pattern p = java.util.regex.Pattern.compile("Total Points: (\\d+)");
    ...
    // Helper to extract GS Grade from AI response
    private String extractGSGrade(String response) {
        java.util.regex.Pattern p = java.util.regex.Pattern.compile("Final Grade: (GS-\\d+)");
        java.util.regex.Matcher m = p.matcher(response);
        if (m.find()) {
            return m.group(1);
        }
    ...
    private String calculateFinalGrade(int totalPoints) {
        // Implement your logic to calculate the final grade based on total points
        // This is a placeholder implementation
        if (totalPoints >= 1900) {
            return "15";
        } else if (totalPoints >= 1800) {
