...
private int getMaxLevelForFactor(String factor) {
    ...
    // Fix HEADER section: add newlines between fields
    text = text.replaceAll(
        "(\\*\\*HEADER\\*\\*\\s*\\n?)(Job Series: [^\\n]+)(Position Title: [^\\n]+)(Agency: [^\\n]+)(Organization: [^\\n]+)(Lowest Organization: [^\\n]+)(Supervisory Level: [^\\n]+)",
        "$1$2\n$3\n$4\n$5\n$6\n$7"
    );

    // Fix missing newlines after each header field (if not already present)
    ...
    // Fix bad spacing in factor headers (bold and non-bold)
    text = text.replaceAll(
    ...
    // Fix missing spaces after colons in header fields
    text = text.replaceAll("Job Series:([A-Za-z0-9\\-]+)", "Job Series: $1");
    text = text.replaceAll("Position Title:([A-Za-z0-9 \\-]+)", "Position Title: $1");
    text = text.replaceAll("Agency:([A-Za-z0-9 .\\-]+)", "Agency: $1");
    text = text.replaceAll("Organization:([A-Za-z0-9 .\\-]+)", "Organization: $1");
    text = text.replaceAll("Lowest Organization:([A-Za-z0-9 .\\-]+)", "Lowest Organization: $1");
    text = text.replaceAll("Supervisory Level:([A-Za-z0-9 .\\-]+)", "Supervisory Level: $1");

    // Optional: Ensure two newlines before and after major section headers
