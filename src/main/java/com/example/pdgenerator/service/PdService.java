...
""",
    ...
    section.append("YOU MUST FOLLOW THIS EXACT FORMAT FOR EVERY SINGLE FACTOR - NO EXCEPTIONS:\n\n");
    
    section.append("CORRECT EXAMPLES (COPY THIS EXACT FORMAT):\n");
    section.append("**Factor 1 – PROGRAM SCOPE AND EFFECT Level 1-3, 550 Points**\n\n");
    section.append("The incumbent oversees critical operations. This position has significant impact.\n\n\n");
    section.append("**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level 4-4, 100 Points**\n\n");
    section.append("The incumbent interacts with senior personnel. These contacts require effective relationships.\n\n\n");
    section.append("**Factor 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS) Level 4-4, 125 Points**\n\n");
    section.append("The purpose is to collaborate and share information. The incumbent must communicate effectively.\n\n\n");
    
    section.append("SPACING RULES (MANDATORY):\n");
    section.append("1. Header line: **Factor N – NAME Level X-Y, ZZZ Points**\n");
    section.append("2. One blank line\n");
    section.append("3. Description: 2-5 sentences\n");
    section.append("4. Two blank lines before next factor\n\n");
    
    section.append("FORBIDDEN PATTERNS - NEVER OUTPUT THESE:\n");
    section.append("❌ Factor1 (missing space)\n");
    section.append("❌ Factor4A (missing space after Factor)\n");
    section.append("❌ Level1-3,550 (no spaces)\n");
    section.append("❌ Level4-4,100 (no spaces)\n");
    section.append("❌ G:1-3 (never include G: prefix)\n");
    section.append("❌ G:4-4 (never include G: prefix)\n");
    section.append("❌ NameLevel (no space between name and Level)\n\n");
    
    section.append("REQUIRED OUTPUT:\n");
    section.append("✓ Factor 1 (with space)\n");
    section.append("✓ Factor 4A (with space after Factor)\n");
    section.append("✓ Level 1-3, 550 (spaces around dash and comma)\n");
    section.append("✓ Level 4-4, 100 (spaces around dash and comma)\n");
    section.append("✓ NAME Level (space between name and Level keyword)\n");
    section.append("✓ Never write G: in any output\n\n");
    
    section.append("═══════════════════════════════════════════════════════\n\n");

    // List the 6 GSSG factors with their locked levels and points
    // CRITICAL: Strip G: prefix BEFORE putting into the template that goes to AI
    for (int i = 1; i <= 6; i++) {
        if (i == 4) {
            ...
            System.out.println("[GSSG DEBUG] Factor 4A: raw='" + lvlA + "', clean='" + lvlA_clean + "', points=" + ptsA);
            ...
            System.out.println("[GSSG DEBUG] Factor 4B: raw='" + lvlB + "', clean='" + lvlB_clean + "', points=" + ptsB);
        ...
        System.out.println("[GSSG DEBUG] Factor " + i + ": raw='" + level + "', clean='" + level_clean + "', points=" + points);
    ...
    section.append("⚠️ ABSOLUTELY MANDATORY - YOUR OUTPUT MUST MATCH THESE RULES:\n");
    section.append("1. Generate ALL 6 factors (1, 2, 3, 4A, 4B, 5, 6) - never skip any\n");
    section.append("2. EVERY factor header MUST be: **Factor N – NAME Level X-Y, ZZZ Points**\n");
    section.append("3. ALWAYS include spaces: 'Factor 1' not 'Factor1'\n");
    section.append("4. ALWAYS include spaces: 'Factor 4A' not 'Factor4A'\n");
    section.append("5. ALWAYS include spaces: 'Level 1-3, 550' not 'Level1-3,550'\n");
    section.append("6. ALWAYS include spaces: 'Level 4-4, 100' not 'Level4-4,100'\n");
    section.append("7. ALWAYS include space between NAME and Level: 'CONTACTS Level' not 'CONTACTSLevel'\n");
    section.append("8. NEVER include 'G:' prefix anywhere in your output\n");
    section.append("9. ONE blank line after header, then description, then TWO blank lines before next factor\n");
    section.append("10. Do NOT alter any levels or point values - copy them exactly\n");
    section.append("11. Description: 2-5 sentences only, complete thoughts\n\n");

    String builtPrompt = section.toString();
    System.out.println("[GSSG PROMPT] Full prompt built, length: " + builtPrompt.length());
    return builtPrompt;
}
...
""",
    ...
    // === SPECIAL HANDLING FOR 4A AND 4B - MUST COME FIRST ===
    // Handle all malformed Factor 4A variants (with or without spaces, commas, G: prefixes, bold markers)
    text = text.replaceAll("(?mi)\\*{0,2}\\s*Factor\\s*4A\\s*[-–—]?\\s*[^\\n]*?(?:NATURE[^\\n]*?)?Level\\s*G?:?\\s*([0-9]+-[0-9]+)\\s*,?\\s*(\\d{1,6})\\s*Points\\s*\\*{0,2}",
        "\n\n**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level $1, $2 Points**\n\n");
    
    // Handle all malformed Factor 4B variants
    text = text.replaceAll("(?mi)\\*{0,2}\\s*Factor\\s*4B\\s*[-–—]?\\s*[^\\n]*?(?:PURPOSE[^\\n]*?)?Level\\s*G?:?\\s*([0-9]+-[0-9]+)\\s*,?\\s*(\\d{1,6})\\s*Points\\s*\\*{0,2}",
        "\n\n**Factor 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS) Level $1, $2 Points**\n\n");
    ...
    // === FIX FACTOR HEADERS - AGGRESSIVE MULTI-LAYER APPROACH ===
    
    // Layer 1: Fix completely malformed headers (no spaces anywhere)
    // Pattern: **Factor4A – NAME Level4-4,100 Points** -> **Factor 4A – NAME Level 4-4, 100 Points**
    text = text.replaceAll("(?m)\\*\\*Factor([0-9]+)([A-Z]?)\\s*–\\s*([^\\n]*?)Level([0-9]+-[0-9]+),(\\d+)\\s*Points\\*\\*", 
        "**Factor $1$2 – $3 Level $4, $5 Points**");
    
    // Layer 1b: Handle when there's already partial spacing
    text = text.replaceAll("(?m)\\*\\*Factor([0-9]+)([A-Z]?)\\s+–\\s+([^\\n]*?)Level([0-9]+-[0-9]+),(\\d+)\\s+Points\\*\\*", 
        "**Factor $1$2 – $3 Level $4, $5 Points**");
    
    // Layer 2: Fix missing space after "Factor" - catches Factor4A -> Factor 4A, Factor4B -> Factor 4B
    text = text.replaceAll("(?m)\\*\\*Factor([0-9])([AB])(\\s*)–", "**Factor $1$2 –");
    text = text.replaceAll("(?m)\\*\\*Factor([0-9])(\\s*)–", "**Factor $1 –");
    
    // Layer 3: Fix "Level" with no space after it (LevelX-Y,Z -> Level X-Y, Z)
    text = text.replaceAll("(?m)Level([0-9]+-[0-9]+),(\\d+)\\s+Points", "Level $1, $2 Points");
    
    // Layer 4: Remove any remaining "G:" prefixes from levels
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+),(\\d+)", "Level $1, $2");
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+)\\s+Points", "Level $1 Points");
    
    // Layer 5: Ensure space before Level keyword
    text = text.replaceAll("(?m)([A-Z])Level\\s+([0-9]+-[0-9]+)", "$1 Level $2");
    
    // Layer 6: Fix missing dash spacing in headers
    text = text.replaceAll("(?m)(Factor\\s+[0-9]+[A-Z]?)([–-])\\s*([A-Z])", "$1 – $3");
    
    // Layer 7: Ensure all factor headers end with **
    text = text.replaceAll("(?m)(\\*\\*Factor\\s+[0-9]+[A-Z]*\\s*–\\s*[^\\n]*?Level\\s+[0-9]+-[0-9]+,\\s*\\d+\\s*Points)(\\*{0,2})$", "$1**");
    
    // Layer 8: Ensure one blank line after factor header before description
    text = text.replaceAll(
        "(?m)(\\*\\*Factor\\s+[0-9]+[A-Z]*\\s*–\\s*[^\\n]*?Level\\s+[0-9]+-[0-9]+,\\s*\\d+\\s*Points\\*\\*)\\n(?!\\n)",
        "$1\n\n"
    );
    
    // Layer 9: Final cleanup pass specifically for 4A and 4B - ensure perfect spacing
    text = text.replaceAll("(?m)\\*\\*Factor\\s*4\\s*A\\s*–", "**Factor 4A –");
    text = text.replaceAll("(?m)\\*\\*Factor\\s*4\\s*B\\s*–", "**Factor 4B –");
    text = text.replaceAll("(?m)(Factor 4[AB])\\s*–\\s*PERSONAL CONTACTS", "$1 – PERSONAL CONTACTS");
    text = text.replaceAll("(?m)PERSONAL CONTACTS\\s*\\(\\s*NATURE OF CONTACTS\\s*\\)", "PERSONAL CONTACTS (NATURE OF CONTACTS)");
    text = text.replaceAll("(?m)PERSONAL CONTACTS\\s*\\(\\s*PURPOSE OF CONTACTS\\s*\\)", "PERSONAL CONTACTS (PURPOSE OF CONTACTS)");

    // Fix evaluation summary lines
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Total Points:?\\s*:?\\s*(\\d+)\\s*\\*{0,2}", "**Total Points: $1**");
