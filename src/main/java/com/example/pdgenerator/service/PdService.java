...
private String buildComprehensivePromptWithPDFs(PdRequest request) {
    String jobSeries = request.getJobSeries();
    String positionTitle = request.getPositionTitle(); // <-- Use the actual position title
    String federalAgency = request.getFederalAgency();
    String subOrganization = request.getSubOrganization();
    String lowestOrg = request.getLowestOrg();
    ...
    System.out.println("  positionTitle: " + positionTitle); // <-- Log the correct variable
...
private int getMaxLevelForFactor(String factor) {
    ...
    // These MUST be done first and in this exact order
    
    // Fix Job Series running into Position Title
    text = text.replaceAll("(Job Series:\\s*GS-\\d+)([A-Z][a-z])", "$1\n\n$2");
    text = text.replaceAll("(Job Series:\\s*GS-\\d+)(Position Title:)", "$1\n\n$2");
    
    // Fix Position Title running into Agency
    text = text.replaceAll("(Position Title:[^\\n]{1,80}?)(Agency:)", "$1\n\n$2");
    
    // Fix Agency running into Organization (CRITICAL - catches "JusticeOrganization")
    text = text.replaceAll("(Agency:[^\\n]{1,80}?)(Organization:)", "$1\n\n$2");
    text = text.replaceAll("(Department of [A-Z][a-z]+)([A-Z][a-z]+(?:ion|ment))", "$1\n\nOrganization: $2");
    
    // Fix Organization running into Lowest Organization
    text = text.replaceAll("(Organization:[^\\n]{1,80}?)(Lowest Organization:)", "$1\n\n$2");
    
    // Fix Lowest Organization running into Supervisory Level
    text = text.replaceAll("(Lowest Organization:[^\\n]{1,80}?)(Supervisory Level:)", "$1\n\n$2");
    
    // Fix Supervisory Level running into Grade
    text = text.replaceAll("(Supervisory Level:[^\\n]{1,80}?)(Grade:)", "$1\n\n$2");
    
    // Fix Grade running into section headers
    text = text.replaceAll("(Grade:\\s*GS-\\d+)(\\*\\*[A-Z])", "$1\n\n$2");
    
    // ===== STEP 2: Fix MALFORMED FACTOR HEADERS =====
    // This is the MOST CRITICAL FIX
    
    // Fix "Level 1-7125, 0 Points" -> "Level 1-7, 1250 Points"
    // Pattern: Level [factor]-[level][EXTRA_DIGITS], [WRONG] Points
    text = text.replaceAll(
        "Level\\s*(\\d)-(\\d)(\\d{3,4}),\\s*\\d+\\s*Points",
        "Level $1-$2, $3 Points"
    );
    
    // Also fix cases where level is double digit (unlikely but possible)
    text = text.replaceAll(
        "Level\\s*(\\d+)-(\\d)(\\d{3,4}),\\s*\\d+\\s*Points",
        "Level $1-$2, $3 Points"
    );
    
    // ===== STEP 3: Format all section headers with proper spacing =====
    ...
    for (String header : sectionHeaders) {
        // Match any variation of the header (bolded or not, with/without colons)
        String pattern = "\\*{0,2}\\s*" + header + "\\s*:?\\s*\\*{0,2}";
        text = text.replaceAll("(?i)" + pattern, "\n\n**" + header + "**\n\n");
        
        // Fix if section header runs directly into content (no newline)
        // e.g., "**CONDITIONS OF EMPLOYMENT**The incumbent"
        String fixPattern = "(\\*\\*" + header + "\\*\\*)([A-Z][a-z])";
        text = text.replaceAll(fixPattern, "$1\n\n$2");
    }
    
    // ===== STEP 4: Format factor headers =====
    // Ensure proper format: **Factor N – Name Level X-Y, NNNN Points**
    text = text.replaceAll(
        "(?m)^\\*{0,2}\\s*Factor\\s*(\\d+)\\s*[–-]\\s*([^\\n*]+?)\\s*Level\\s*(\\d+)-(\\d+),\\s*(\\d{1,4})\\s*Points\\s*\\*{0,2}",
        "\n\n**Factor $1 – $2 Level $3-$4, $5 Points**\n\n"
    );
    
    // Also catch factor headers that aren't at line start
    text = text.replaceAll(
        "([^\\n])\\*{0,2}\\s*Factor\\s*(\\d+)\\s*[–-]\\s*([^\\n*]+?)\\s*Level\\s*(\\d+)-(\\d+),\\s*(\\d{1,4})\\s*Points\\s*\\*{0,2}",
        "$1\n\n**Factor $2 – $3 Level $4-$5, $6 Points**\n\n"
    );
    
    // ===== STEP 5: Fix evaluation summary lines =====
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Total Points:\\s*(\\d+)\\s*\\*{0,2}", "\n\n**Total Points:** $1\n\n");
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Final Grade:\\s*(GS-\\d+)\\s*\\*{0,2}", "**Final Grade:** $1\n\n");
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Grade Range:\\s*([\\d\\-+]+)\\s*\\*{0,2}", "**Grade Range:** $1\n\n");
    
    // ===== STEP 6: Cleanup excessive whitespace =====
    text = text.replaceAll("\\n{4,}", "\n\n\n");  // Max 2 blank lines
    text = text.replaceAll("[ \\t]+\\n", "\n");    // Remove trailing spaces
    text = text.replaceAll("\\n[ \\t]+", "\n");    // Remove leading spaces
    text = text.replaceAll("[ ]{2,}", " ");        // Remove double spaces
