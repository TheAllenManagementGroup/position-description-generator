...
""",
    ...
    section.append("7. NEVER include 'G:' prefix anywhere in your output\n");
    section.append("8. ONE blank line after header, then description, then TWO blank lines before next factor\n");
    section.append("9. Do NOT alter any levels or point values - copy them exactly\n");
    section.append("10. Description: 2-5 sentences only, complete thoughts\n\n");
...
""",
    ...
    // === FIX FACTOR HEADERS - AGGRESSIVE CLEANUP ===
    
    // Step 0: Fix malformed factor headers like "Factor4A" -> "Factor 4A" and "Level4-4,100" -> "Level 4-4, 100"
    // This catches what the AI sometimes generates despite instructions
    text = text.replaceAll("(?m)\\*{0,2}Factor([0-9]+)([A-Z]?)", "**Factor $1$2");
    text = text.replaceAll("(?m)Level([0-9]+-[0-9]+),(\\d+)", "Level $1, $2");
    text = text.replaceAll("(?m)\\*\\*Factor\\s+([0-9]+)([A-Z]?)\\s+–", "**Factor $1$2 –");
    
    // Step 1: Fix "Factor1" -> "Factor 1", "Factor4A" -> "Factor 4A", etc. (add space after Factor)
    text = text.replaceAll("(?m)\\*{0,2}\\s*Factor([0-9]+)([A-Z]?)", "**Factor $1$2");
    
    // Step 2: Remove "G:" prefix from levels: "G:1-3,550" -> "1-3,550"
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+),(\\d+)", "Level $1, $2");
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+)\\s+Points", "Level $1 Points");
    
    // Step 3: Ensure proper spacing in factor headers: "Level 1-3,550" -> "Level 1-3, 550"
    text = text.replaceAll("Level\\s+([0-9]+-[0-9]+),(\\d+)\\s+Points", "Level $1, $2 Points");
    
    // Step 4: Close the header with ** and ensure proper line break
    // Pattern: **Factor X – NAME Level Y-Z, PTS Points** but make sure it's properly closed
    text = text.replaceAll("(?m)(\\*\\*Factor\\s+[0-9]+[A-Z]*\\s*–\\s*[^\\n]*?Level\\s+[0-9]+-[0-9]+,\\s*\\d+\\s*Points)(\\*{0,2})", "$1**");
    
    // Step 5: CRITICAL - Ensure blank line after factor header, before description
    // This regex finds factor headers and ensures they're followed by exactly one blank line
    text = text.replaceAll(
