...
private String replaceFactor4A4BHeaders(String aiOutput, PdRequest request) {
    ...
    for (int i = 0; i < lines.length; i++) {
        ...
        // Check if this line is a Factor 4A header (any variation)
        boolean is4AHeader = (lineLower.contains("factor") && lineLower.contains("4") && lineLower.contains("a") &&
                             lineLower.contains("personal contacts") && lineLower.contains("level") && 
                             lineLower.contains("points"));
        
        // More specific check: must have "4a" or "4 a" together
        boolean has4A = lineLower.matches(".*factor[\\s]*4[\\s]*a.*") && lineLower.contains("personal");
        
        if (has4A && is4AHeader) {
            System.out.println("[replaceFactor4A4BHeaders] Replacing 4A line: " + line);
            result.append(correct4AHeader);
            replaced4A++;
        }
        // Check if this line is a Factor 4B header (any variation)
        else if (lineLower.contains("factor") && lineLower.matches(".*factor[\\s]*4[\\s]*b.*") && 
                 lineLower.contains("personal") && lineLower.contains("level") && lineLower.contains("points")) {
            System.out.println("[replaceFactor4A4BHeaders] Replacing 4B line: " + line);
    ...
    if (replaced4A == 0) {
        System.err.println("⚠️ CRITICAL: No Factor 4A headers were replaced!");
    }
    if (replaced4B == 0) {
        System.err.println("⚠️ CRITICAL: No Factor 4B headers were replaced!");
    }
