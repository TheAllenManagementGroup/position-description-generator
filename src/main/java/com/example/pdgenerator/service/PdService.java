...
""",
    ...
    // Remove any "G:" prefix used internally (so headers show "1-3" not "G:1-3" or "G:1-3,550")
    text = text.replaceAll("\\bG:\\s*([0-9]+-[0-9]+)\\b", "$1");
    // Also remove stray "G:" before levels with comma: "G:1-3,550" -> "1-3,550"
    text = text.replaceAll("G:\\s*([0-9]+-[0-9]+)\\s*,\\s*(\\d+)", "$1, $2");
    ...
    // General formatting for factor lines that AI may produce (place header on its own line)
    text = text.replaceAll(
    ...
    try {
        ...
        while (m.find()) {
            ...
            if (factorId.equals("4A")) {
                ...
                name = (idx >= 1 && idx <= 6) ? getSupervisoryFactorName(idx) : ""; // empty name will be safe fallback
        ...
        // Don't fail formatting entirely on regex issues; log and continue
