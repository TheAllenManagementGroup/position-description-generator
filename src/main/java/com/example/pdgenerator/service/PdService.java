...
public String fixPDFormatting(String pdText) {
    ...
    // === ULTRA-AGGRESSIVE FIX FOR FACTOR 4A AND 4B - MULTIPLE STRATEGIES ===
    
    // Strategy 1: Direct string replacements (most reliable) - do these FIRST
    ...
    // Strategy 2: Fix Level formatting for 4A/4B specifically
    text = text.replace("Level4-4,100 Points", "Level 4-4, 100 Points");
    text = text.replace("Level4-4,125 Points", "Level 4-4, 125 Points");
    text = text.replace("Level 4-4,100 Points", "Level 4-4, 100 Points");
    text = text.replace("Level 4-4,125 Points", "Level 4-4, 125 Points");
    text = text.replace("Level4-4, 100 Points", "Level 4-4, 100 Points");
    text = text.replace("Level4-4, 125 Points", "Level 4-4, 125 Points");
    
    // Strategy 3: Regex for any malformed Factor4A or Factor4B patterns
    text = text.replaceAll("\\*\\*Factor(4[AB])\\s*–", "**Factor $1 –");
    text = text.replaceAll("Factor(4[AB])\\s*–", "Factor $1 –");
    
    // Strategy 4: Generic Level formatting fixes
    text = text.replaceAll("Level([0-9])-([0-9]),([0-9]+)\\s+Points", "Level $1-$2, $3 Points");
    text = text.replaceAll("Level\\s+([0-9])-([0-9]),([0-9]+)\\s+Points", "Level $1-$2, $3 Points");
    
    // Strategy 5: Remove G: prefix
    ...
    // === FINAL PASS: Fix any remaining Factor headers ===
    // This regex specifically looks for Factor 4A and Factor 4B patterns
    text = text.replaceAll("(?m)(\\*\\*Factor\\s*)4A(\\s*– PERSONAL CONTACTS \\(NATURE OF CONTACTS\\) Level\\s*)4-4,\\s*100(\\s*Points\\*\\*)", 
        "$1" + "4A" + "$2" + "4-4, 100" + "$3");
    text = text.replaceAll("(?m)(\\*\\*Factor\\s*)4B(\\s*– PERSONAL CONTACTS \\(PURPOSE OF CONTACTS\\) Level\\s*)4-4,\\s*125(\\s*Points\\*\\*)", 
        "$1" + "4B" + "$2" + "4-4, 125" + "$3");
    
    // Ensure proper bold formatting for all factors
    text = text.replaceAll("(?m)^\\*{0,2}(Factor [0-9]+[AB]? – [^\\n]+ Level [0-9]+-[0-9]+, [0-9]+ Points)\\*{0,2}$", 
        "**$1**");
    ...
    // Ensure two blank lines before next factor
    text = text.replaceAll("(?m)([.!?])\\n(\\*\\*Factor [2-9]+[AB]? – )", "$1\n\n\n$2");

    // Fix evaluation summary
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Total Points:?\\s*:?\\s*(\\d+)\\s*\\*{0,2}$", 
        "**Total Points: $1**");
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Final Grade:?\\s*:?\\s*(GS-\\d+)\\s*\\*{0,2}$", 
        "**Final Grade: $1**");
    text = text.replaceAll("(?m)^\\*{0,2}\\s*Grade Range:?\\s*:?\\s*([\\d\\-+]+)\\s*\\*{0,2}$", 
        "**Grade Range: $1**");
    ...
    // === ONE MORE SAFETY CHECK: Final cleanup of Factor4A and Factor4B ===
    text = text.replace("Factor4A", "Factor 4A");
    text = text.replace("Factor4B", "Factor 4B");
