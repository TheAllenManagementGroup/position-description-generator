...
""",
    ...
    try {
        ...
        if (response.statusCode() == 200) {
            ...
            // === DEBUG: Log raw output from OpenAI ===
            String rawOutput = fullContent.toString();
            System.err.println("\n========== RAW OUTPUT FROM OPENAI ==========");
            System.err.println("Total length: " + rawOutput.length() + " chars");
            System.err.println(rawOutput);
            System.err.println("========== END RAW OUTPUT ==========\n");
            System.err.flush();
            
            // === DEBUG: Extract and log all factor headers found ===
            System.err.println("\n========== FACTOR HEADERS IN RAW OUTPUT ==========");
            java.util.regex.Pattern factorPattern = java.util.regex.Pattern.compile(
                "(?m)^.*Factor\\s+[0-9A-Z]+.*Level.*Points.*$"
            );
            java.util.regex.Matcher factorMatcher = factorPattern.matcher(rawOutput);
            int factorCount = 0;
            while (factorMatcher.find()) {
                factorCount++;
                System.err.println("[Factor " + factorCount + "]: " + factorMatcher.group());
            }
            System.err.println("Total factors found: " + factorCount);
            ...
            System.err.println("\n========== FORMATTED OUTPUT ==========");
            System.err.println(formattedPD);
            System.err.println("========== END FORMATTED OUTPUT ==========\n");
            System.err.flush();
            
            // === DEBUG: Extract and log all factor headers after formatting ===
            System.err.println("\n========== FACTOR HEADERS IN FORMATTED OUTPUT ==========");
            java.util.regex.Matcher formattedFactorMatcher = factorPattern.matcher(formattedPD);
            int formattedFactorCount = 0;
            while (formattedFactorMatcher.find()) {
                formattedFactorCount++;
                System.err.println("[Factor " + formattedFactorCount + "]: " + formattedFactorMatcher.group());
            }
            System.err.println("Total formatted factors found: " + formattedFactorCount);
    ...
    // --- SUPERVISORY FACTOR HEADER FIXES ---
    // Match supervisory factor headers like "**Factor4A – ..." and normalize spacing
    text = text.replaceAll("(?m)\\*{0,2}\\s*Factor\\s*([0-9]+[A-Z]?)\\s*[–-]\\s*", "\n\n**Factor $1 – ");
    
    // Fix factor headers that are missing asterisks or have formatting issues
    // Pattern: Factor X – NAME Level G:level,points Points
    text = text.replaceAll(
        "(?m)^(?:\\*{0,2})?\\s*(Factor\\s+[0-9A-Z]+\\s*–\\s*[^\\n]*?Level\\s+G:[0-9]+-[0-9]+,\\d+\\s*Points)\\s*\\*{0,2}\\s*$",
        "**$1**"
    );
    
    // Ensure proper spacing after factor headers: header + blank line + content
    text = text.replaceAll(
        "(?m)(\\*\\*Factor\\s+[0-9A-Z]+\\s+–\\s+[^\\n]+?Points\\*\\*)\\n(?!\\n)",
        "$1\n\n"
    );
    
    // Remove the "G:" prefix in factor headers so "G:1-3,550" becomes "1-3, 550"
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+),(\\d+)", "Level $1, $2");
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+)\\s+Points", "Level $1 Points");

    // Ensure 'Factor' always has a single space before the number/letter (handles "Factor1", "Factor4A")
    text = text.replaceAll("(?i)\\bFactor\\s*([0-9]+)([A-Za-z]?)\\b", "Factor $1$2");
    ...
    // --- Replace factor header names with authoritative supervisory names when a factor header is detected ---
    try {
        java.util.regex.Pattern pat = java.util.regex.Pattern.compile(
            "(?m)^\\*{0,2}\\s*Factor\\s+([0-9]{1,2}[A-Z]?)\\s*[–-]\\s*[^\\n]*?Level\\s+([0-9]+-[0-9]+)\\s*,\\s*(\\d{1,5})\\s*Points\\s*\\*{0,2}",
            java.util.regex.Pattern.CASE_INSENSITIVE
        );
        java.util.regex.Matcher m = pat.matcher(text);
        StringBuffer out = new StringBuffer();
        while (m.find()) {
            String factorId = m.group(1).toUpperCase(); // e.g., "4A" or "1"
            String level = m.group(2);
            String pts = m.group(3);
            String name;
            if (factorId.equals("4A")) {
                name = "PERSONAL CONTACTS (NATURE OF CONTACTS)";
            } else if (factorId.equals("4B")) {
                name = "PERSONAL CONTACTS (PURPOSE OF CONTACTS)";
            } else {
                int idx = 0;
                try {
                    idx = Integer.parseInt(factorId.replaceAll("[^0-9]", ""));
                } catch (Exception ex) { idx = -1; }
                name = (idx >= 1 && idx <= 6) ? getSupervisoryFactorName(idx) : "";
                if (name == null || name.isBlank()) name = "Factor " + factorId;
            }
            String replacement = "**Factor " + factorId + " – " + name + " Level " + level + ", " + pts + " Points**";
            m.appendReplacement(out, java.util.regex.Matcher.quoteReplacement(replacement));
        }
        m.appendTail(out);
        text = out.toString();
    } catch (Exception e) {
        System.err.println("fixPDFormatting: factor header normalization failed: " + e.getMessage());
    }

    // Ensure factor descriptions start on a new line after blank line
    text = text.replaceAll(
        "(?m)(\\*\\*Factor\\s+[0-9A-Z]+\\s+–\\s+[^\\n]+?Points\\*\\*)\\n\\n([A-Z])",
        "$1\n\n$2"
    );
