...
private String buildComprehensivePromptWithPDFs(PdRequest request) {
    String jobSeries = request.getJobSeries();
    String subJobSeries = request.getSubJobSeries();
    ...
    System.out.println("  subJobSeries: " + subJobSeries);
...
%s
""",
formattingInstructions, relevantFactors, factorGuide, seriesGuide,
...
private int getMaxLevelForFactor(String factor) {
    ...
    // Ensure each header field is on its own line, even if LLM runs them together
    ...
    // Also fix headers that are not bolded at all
    text = text.replaceAll(
