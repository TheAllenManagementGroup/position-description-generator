...
private int getMaxLevelForFactor(String factor) {
    ...
    public void streamPD(PdRequest request, PrintWriter writer) {
        try {
            ...
            if (response.statusCode() == 200) {
                ...
                System.out.println("[DEBUG] Formatted PD output:\n" + formattedPD);
    ...
    // --- AGGRESSIVE HEADER variable splitting: force each field onto its own line ---
    // Handles run-together and missing spaces
    ...
    // --- AGGRESSIVE Factor header fix: split level and points if run together (e.g., 1-81550 -> 1-8, 1550) ---
    // Handles "Level 1-81550" or "Level 1-8155, 0 Points"
    text = text.replaceAll(
        "(Level\\s*[0-9]+-[0-9])([0-9]{3,4})(,?\\s*[0-9]+\\s*Points)",
        "$1, $2$3"
    );
    ...
    text = text.replaceAll(
        "(?m)^\\s*\\*?\\*?\\s*Factor\\s*([0-9])\\s*[–-]?\\s*([^\\n*]+?)\\s*Level\\s*([0-9]+)-([0-9]+),\\s*([0-9]{2,4})\\s*Points\\s*\\*?\\*?",
