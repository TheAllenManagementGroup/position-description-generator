...
""",
    ...
    section.append("⚠️ CRITICAL: YOU MUST GENERATE ALL 6 FACTORS - DO NOT SKIP ANY!\n\n");
    section.append("═══════════════════════════════════════════════════════\n");
    section.append("LOCKED FACTOR VALUES - GENERATE ALL 6 FACTORS:\n");
    section.append("═══════════════════════════════════════════════════════\n\n");

    // Factors 1-3: AI generates with template
    for (int i = 1; i <= 3; i++) {
        String factorNum = String.valueOf(i);
        String level = factorLevels.get(factorNum);
        Integer points = factorPoints.get(factorNum);
    ...
    // ========== FACTOR 4A: DETERMINISTIC HEADER ==========
    String lvlA = factorLevels.getOrDefault("4A", "4-2");
    lvlA = lvlA.startsWith("G:") ? lvlA.substring(2) : lvlA;
    Integer ptsA = factorPoints.getOrDefault("4A", 50);
    
    // Build the EXACT header that will be inserted into the final output
    String factor4AHeader = String.format("**FACTOR 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level %s, %d Points**",
                                          lvlA, ptsA);
    
    // Tell AI to copy this EXACT header
    section.append("⚠️ FOR FACTOR 4A, USE THIS EXACT HEADER (COPY IT VERBATIM):\n");
    section.append(factor4AHeader).append("\n\n");
    section.append("(Write 2-5 sentences explaining this subfactor level)\n\n\n");

    // ========== FACTOR 4B: DETERMINISTIC HEADER ==========
    String lvlB = factorLevels.getOrDefault("4B", "4-2");
    lvlB = lvlB.startsWith("G:") ? lvlB.substring(2) : lvlB;
    Integer ptsB = factorPoints.getOrDefault("4B", 75);
    
    String factor4BHeader = String.format("**FACTOR 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS) Level %s, %d Points**",
                                          lvlB, ptsB);
    
    section.append("⚠️ FOR FACTOR 4B, USE THIS EXACT HEADER (COPY IT VERBATIM):\n");
    section.append(factor4BHeader).append("\n\n");
    section.append("(Write 2-5 sentences explaining this subfactor level)\n\n\n");

    // Factors 5-6: AI generates with template
    for (int i = 5; i <= 6; i++) {
        String factorNum = String.valueOf(i);
        String level = factorLevels.get(factorNum);
        Integer points = factorPoints.get(factorNum);
        ...
        section.append(String.format("**Factor %d – %s Level %s, %d Points**\n\n",
            i, getSupervisoryFactorName(i), level, points));
        section.append("(Write 2-5 sentences explaining this factor level here)\n\n\n");
    }
    ...
    section.append("⚠️ CRITICAL INSTRUCTIONS:\n");
    section.append("- For Factor 4A and Factor 4B: COPY THE EXACT HEADERS SHOWN ABOVE\n");
    section.append("- DO NOT change the Factor 4A or Factor 4B headers - they are pre-formatted\n");
    section.append("- For all other factors: Follow the template format shown\n");
    section.append("- Write 2-5 sentences for each factor\n");
    section.append("- Add TWO blank lines after each factor's description\n");
...
""",
    ...
    /**
 * Post-process AI output to enforce correct 4A/4B headers
 * Call this in streamPD AFTER getting rawOutput but BEFORE fixPDFormatting
 */
private String enforce4A4BHeaders(String aiOutput, PdRequest request) {
    Map<String, String> factorLevels = request.getFactorLevels();
    Map<String, Integer> factorPoints = request.getFactorPoints();
    ...
    System.out.println("[enforce4A4BHeaders] Correct 4A header: " + correct4AHeader);
    System.out.println("[enforce4A4BHeaders] Correct 4B header: " + correct4BHeader);
    
    // Replace ANY variation of Factor 4A header with the correct one
    // This regex captures the entire Factor 4A line regardless of formatting
    aiOutput = aiOutput.replaceAll(
        "(?i)\\*{0,2}\\s*FACTOR\\s*4\\s*A\\s*[–—-]\\s*PERSONAL CONTACTS[^\\n]*?\\*{0,2}",
        correct4AHeader
    );
    
    // Replace ANY variation of Factor 4B header with the correct one
    aiOutput = aiOutput.replaceAll(
        "(?i)\\*{0,2}\\s*FACTOR\\s*4\\s*B\\s*[–—-]\\s*PERSONAL CONTACTS[^\\n]*?\\*{0,2}",
        correct4BHeader
    );
    
    System.out.println("[enforce4A4BHeaders] Headers enforced");
    
    return aiOutput;
}

/**
 * Updated streamPD method with enforce4A4BHeaders call
 * Insert this in your existing streamPD method
 */
public void streamPD(PdRequest request, PrintWriter writer) {
    try {
        ...
        // Validate prompt formatting before sending to OpenAI
        ...
        messages.add(new Message("system",
            ...
            "1. For Factor 4A and Factor 4B: COPY THE EXACT HEADERS PROVIDED IN THE PROMPT\n" +
            "2. DO NOT modify Factor 4A or Factor 4B headers in any way\n" +
            "3. The Factor 4A and Factor 4B headers are already perfectly formatted - just copy them\n" +
            "4. For all other factors: Follow standard formatting with proper spacing\n"
        ));
        ...
        if (response.statusCode() == 200) {
            ...
            // === ENFORCE 4A/4B HEADERS (NEW STEP) ===
            System.out.println("[streamPD] Enforcing correct 4A/4B headers...");
            rawOutput = enforce4A4BHeaders(rawOutput, request);
            
            // === EMERGENCY INLINE FIXES ===
            System.out.println("[Post-Stream] Checking for formatting issues...");
            ...
            // Count issues
            int factor4ACount = rawOutput.split("Factor 4A", -1).length - 1;
            int factor4BCount = rawOutput.split("Factor 4B", -1).length - 1;
            
            System.out.println("[Post-Stream] Found Factor 4A: " + factor4ACount + " times");
            System.out.println("[Post-Stream] Found Factor 4B: " + factor4BCount + " times");
            
            // Log samples
            String[] postFixLines = rawOutput.split("\n");
            for (String line : postFixLines) {
                if (line.contains("Factor 4A") || line.contains("Factor 4B") || 
                    line.contains("FACTOR 4A") || line.contains("FACTOR 4B")) {
                    System.out.println("[Post-Stream Sample] " + line);
                }
            }

            // NOW call fixPDFormatting
            System.out.println("[streamPD] Calling fixPDFormatting...");
            String formattedPD = fixPDFormatting(rawOutput);
            System.out.println("[streamPD] fixPDFormatting completed, output length: " + formattedPD.length());
...
public String fixPDFormatting(String pdText) {
    ...
    System.out.println("[fixPDFormatting] Starting with text length: " + pdText.length());

    // Normalize line endings first
    String text = pdText.replaceAll("\\r\\n?", "\n");

    // Normalize invisible whitespace
    text = text.replace("\u00A0", " ").replace("\u200B", "");

    // Remove lines with only asterisks
    text = text.replaceAll("(?m)^\\s*\\*+\\s*$\\n?", "");

    // === CRITICAL FIX: Aggressively fix Factor 4A and 4B spacing ===
    
    // Fix Factor4A (no space) -> Factor 4A (with space)
    // This handles all variants: Factor4A, Factor 4A, Factor  4A
    text = text.replaceAll("(?i)Factor\\s*4A", "Factor 4A");
    
    // Fix Factor4B (no space) -> Factor 4B (with space)
    text = text.replaceAll("(?i)Factor\\s*4B", "Factor 4B");
    
    // Fix any other concatenated factors (Factor1, Factor2, etc.)
    text = text.replaceAll("(?i)Factor([0-9]{1,2})([^A-Z0-9-]|$)", "Factor $1$2");
    
    System.out.println("[fixPDFormatting] After Factor spacing fixes");
    
    // === Fix Level spacing issues ===
    
    // Fix Level4-4,100 -> Level 4-4, 100
    // This handles: Level4-4,100 | Level4-4, 100 | Level 4-4,100
    text = text.replaceAll("(?i)Level\\s*([0-9]+)\\s*[-–—]\\s*([0-9]+)\\s*,?\\s*([0-9]+)\\s*Points?", 
                          "Level $1-$2, $3 Points");
    
    System.out.println("[fixPDFormatting] After Level spacing fixes");
    
    // === Remove any "G:" prefixes ===
    text = text.replaceAll("(?i)\\bG:\\s*", "");
    
    // === Fix the dash after Factor number ===
    // Normalize various dashes to en-dash with proper spacing
    text = text.replaceAll("(?i)(Factor\\s+[0-9]{1,2}[AB]?)\\s*[-–—:\\s]+", "$1 – ");
    
    System.out.println("[fixPDFormatting] After dash normalization");
    
    // === Specific fixes for 4A and 4B canonical names ===
    // Ensure the full header format is correct for Factor 4A
    text = text.replaceAll(
        "(?mi)\\*\\*Factor\\s+4A\\s*–[^\\n]*?PERSONAL CONTACTS[^\\n]*?\\*\\*",
        "**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level 4-4, 100 Points**"
    );
    
    // Ensure the full header format is correct for Factor 4B
    text = text.replaceAll(
        "(?mi)\\*\\*Factor\\s+4B\\s*–[^\\n]*?PERSONAL CONTACTS[^\\n]*?\\*\\*",
        "**Factor 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS) Level 4-4, 125 Points**"
    );
    
    System.out.println("[fixPDFormatting] After 4A/4B canonical name fixes");
    
    // === Bold all factor headers ===
    text = text.replaceAll(
        "(?m)^(\\s*)(Factor\\s+[0-9]{1,2}[AB]?\\s+–\\s+.*?Level\\s+[0-9]+-[0-9]+,\\s*\\d+\\s*Points)\\s*$",
        "$1**$2**"
    );
    
    // === Spacing around factor headers ===
    // Blank line after each factor header
    text = text.replaceAll(
        "(?m)(\\*\\*Factor\\s+[0-9]{1,2}[AB]?\\s+–\\s+.*?Points\\*\\*)\\n(?!\\n)",
        "$1\n\n"
    );
    
    // Two blank lines before next factor header
    text = text.replaceAll(
        "(?m)([.!?)]\\s*)\\n\\s*(\\*\\*Factor\\s+[0-9]{1,2})",
        "$1\n\n\n$2"
    );
    
    System.out.println("[fixPDFormatting] After spacing fixes");
    ...
    for (String header : sectionHeaders) {
        text = text.replaceAll(
            "(?i)\\*{0,2}\\s*" + java.util.regex.Pattern.quote(header) + "\\s*:?\\s*\\*{0,2}",
            "\n\n**" + header + "**\n\n"
        );
    }
    ...
    System.out.println("[fixPDFormatting] Completed. Final length: " + text.length());
    
    // === FINAL SAFETY CHECK: Log any remaining issues ===
    if (text.contains("Factor4A") || text.contains("Factor4B")) {
        System.err.println("⚠️ WARNING: Still found 'Factor4A' or 'Factor4B' after formatting!");
    }
    if (text.matches("(?s).*Level\\s*4-4\\s*,\\s*100\\s*Points.*") && 
        text.matches("(?s).*Level4-4.*")) {
        System.err.println("⚠️ WARNING: Found 'Level4-4' without space!");
    }
