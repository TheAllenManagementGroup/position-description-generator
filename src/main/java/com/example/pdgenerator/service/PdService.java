...
public class PdService {
    ...
    // Calculate the target point range for the accepted grade
    String targetPointRange = getPointRangeForGrade(gsGrade);
    int targetMinPoints = getMinPointsForGrade(gsGrade);
    int targetMaxPoints = getMaxPointsForGrade(gsGrade);

    String handbookText = getHandbookText();
    ...
    return String.format("""
        ...
        CRITICAL GRADE CONSTRAINT: This position MUST be classified as %s with total points between %d and %d points. 
        You MUST assign factor levels and points that result in a total within this range.
        ...
        **MAJOR DUTIES 3 (with realistic percentages)**
        ...
        IMPORTANT: Assign factor levels that total between %d and %d points to achieve %s classification.
        ...
        **FAIR LABOR STANDARDS ACT DETERMINATION:**
        ...
        """,
        gsGrade, targetMinPoints, targetMaxPoints, // Grade constraint
        jobSeries, subJobSeries, federalAgency, subOrganization, lowestOrg, historicalData, supervisoryLevel, handbookText,
        gsGrade, targetPointRange, // Target grade info
        federalAgency, subOrganization, lowestOrg, subJobSeries, gsGrade, subJobSeries, // Header info
        targetMinPoints, targetMaxPoints, gsGrade, // Factor evaluation constraint
        targetMinPoints, targetMaxPoints, gsGrade, targetPointRange // Summary constraint
    );
}
...
private int getMinPointsForGrade(String grade) {
    ...
    switch (grade.toUpperCase()) {
        ...
        case "GS-15": return 4055;
        default: return 2355; // Default to GS-11 minimum
    }
}

private int getMaxPointsForGrade(String grade) {
    if (grade == null) return 2750;
    ...
    switch (grade.toUpperCase()) {
        ...
        case "GS-15": return 5000; // High ceiling for GS-15
        default: return 2750; // Default to GS-11 maximum
    }
}
    /**
