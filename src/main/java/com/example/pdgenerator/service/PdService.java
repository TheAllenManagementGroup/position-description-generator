...
public Map<String, String> getDefaultFactorLevelsForGrade(String grade, String supervisoryLevel) {
    ...
    String normGrade = normalizeGsGrade(grade); // NEW: normalize input grade like "GS-14 (target)" -> "GS-14"

    if (isSup) {
        System.out.println("[getDefaultFactorLevelsForGrade] Generating GSSG 6 factors for grade " + normGrade);
        switch (normGrade) {
            case "GS-15":
                levels.put("1", "G:1-5");
                levels.put("2", "G:2-3");   // Top setting still 2-3 in GSSG
                levels.put("3", "G:3-5");
                levels.put("4A", "G:4-4");
                levels.put("4B", "G:4-4");
                levels.put("5", "G:5-6");
    ...
    System.out.println("[getDefaultFactorLevelsForGrade] Generating standard 9 factors for grade " + normGrade);
    ...
    private String normalizeGsGrade(String grade) {
    if (grade == null) return "GS-13";
    java.util.regex.Matcher m = java.util.regex.Pattern.compile("GS-\\s*0?(\\d{1,2})").matcher(grade.toUpperCase());
    if (m.find()) {
        return "GS-" + m.group(1);
    }
...
private int getMaxPointsForTargetGrade(String grade) {
    switch (normalizeGsGrade(grade)) {
        case "GS-15": return 9999;
        case "GS-14": return 4050;
        case "GS-13": return 3600;
        case "GS-12": return 3150;
        case "GS-11": return 2750;
        case "GS-9": case "GS-09": return 2100;
        case "GS-7": case "GS-07": return 1600;
        default: return 1350;
    }
}

private void adjustGssgFactorsToTargetGrade(Map<String, String> levels, Map<String, Integer> points, String targetGrade) {
    int min = getMinPointsForTargetGrade(targetGrade);
    int max = getMaxPointsForTargetGrade(targetGrade);
    int targetMid = (min + max) / 2;
    ...
    int safety = 0;
    while (Math.abs(targetMid - total) > 25 && safety++ < 100) {
        String[] order = (total < targetMid) ? incOrder : decOrder;
        boolean changed = false;

        for (String fid : order) {
            String cur = levels.get(fid);
            if (cur == null) continue;

            String curClean = cur.startsWith("G:") ? cur.substring(2) : cur;
            String next = (total < targetMid)
                ? getNextHigherGssgLevel(fid, curClean)
                : getNextLowerGssgLevel(fid, curClean);

            if (next == null) continue;

            int oldPts = points.getOrDefault(fid, getGssgPointsForLevel(fid, curClean));
            int newPts = getGssgPointsForLevel(fid, next);
            int delta = newPts - oldPts;

            if ((total < targetMid && delta > 0) || (total > targetMid && delta < 0)) {
                levels.put(fid, "G:" + next);
                points.put(fid, newPts);
                total += delta;
                changed = true;
                if (Math.abs(targetMid - total) <= 25) break;
            }
        }
...
// NEW: next level helpers for GSSG
private String getNextHigherGssgLevel(String factorId, String current) {
    int cur = parseLevelNum(current);
    int max = getGssgMaxLevel(factorId);
    if (cur < max) return base4Id(factorId) + "-" + (cur + 1);
    return null;
}

private String getNextLowerGssgLevel(String factorId, String current) {
    int cur = parseLevelNum(current);
    if (cur > 1) return base4Id(factorId) + "-" + (cur - 1);
    return null;
}

private int getGssgMaxLevel(String factorId) {
    String f = factorId.toUpperCase();
    if ("4A".equals(f) || "4B".equals(f)) return 4;
    switch (f) {
        case "1": return 5;
        case "2": return 3;
        case "3": return 5;
...
private String base4Id(String factorId) {
    // returns "4" for 4A/4B so resulting string is "4-<n>"
    return factorId.toUpperCase().startsWith("4") ? "4" : factorId.replaceAll("[^0-9]", "");
}

// Also make FES helpers symmetric for decrease
private String getNextLowerLevel(String factor, String currentLevel) {
    String[] parts = currentLevel.split("-");
    int current = Integer.parseInt(parts[1].replaceAll("[^0-9]", ""));
    int min = 1;
    return (current > min) ? factor + "-" + (current - 1) : null;
}
    ...
    if (levels == null || points == null || levels.isEmpty()) return;

    // If any level starts with "G:" treat as GSSG (supervisory)
    boolean isGssg = levels.values().stream().anyMatch(v -> v != null && v.startsWith("G:"));
    if (isGssg) {
        adjustGssgFactorsToTargetGrade(levels, points, targetGrade); // requires helper
        return;
    }

    // FES (9-factor) logic
    int targetMin = getMinPointsForTargetGrade(targetGrade);
    int targetMax = getMaxPointsForTargetGrade(targetGrade); // requires helper
    int currentTotal = points.values().stream().mapToInt(Integer::intValue).sum();

    // Try to increase to at least the minimum
    if (currentTotal < targetMin) {
        String[] boostOrder = {"1", "5", "2", "4", "3", "6", "7", "8", "9"};
        int safety = 0;
        boolean changed;
        do {
            changed = false;
            for (String factor : boostOrder) {
                ...
                String nextLevel = getNextHigherLevel(factor, currentLevel); // requires helper
                if (nextLevel == null) continue;

                int oldPoints = points.getOrDefault(factor, 0);
        ...
        } while (currentTotal < targetMin && changed && ++safety < 100);
    }

    // If above the band, decrease toward the max
    if (currentTotal > targetMax) {
        String[] lowerOrder = {"6", "4", "2", "3", "5", "1", "7", "8", "9"};
        int safety = 0;
        boolean changed;
        do {
            changed = false;
            for (String factor : lowerOrder) {
                if (currentTotal <= targetMax) break;
                String currentLevel = levels.get(factor);
                if (currentLevel == null) continue;

                String prevLevel = getNextLowerLevel(factor, currentLevel); // requires helper
                if (prevLevel == null) continue;

                int oldPoints = points.getOrDefault(factor, 0);
                int newPoints = getPointsForFactorLevel(factor, prevLevel);
                if (newPoints < oldPoints) {
                    levels.put(factor, prevLevel);
                    points.put(factor, newPoints);
                    currentTotal += (newPoints - oldPoints);
                    changed = true;
                }
            }
        } while (currentTotal > targetMax && changed && ++safety < 100);
    }
}
