...
/**
 * Stream PD generation to client - UPDATED VERSION
 */
public void streamPD(PdRequest request, PrintWriter writer) {
    try {
        ...
        if (response.statusCode() == 200) {
            ...
            // ============================================================
            // === STEP 1: REPLACE FACTOR 4A/4B HEADERS DETERMINISTICALLY ===
            // ============================================================
            System.out.println("[streamPD] Step 1: Replacing Factor 4A/4B headers deterministically...");
            rawOutput = replaceFactor4A4BHeaders(rawOutput, request);
            
            // ============================================================
            // === STEP 2: EMERGENCY INLINE FIXES ===
            // ============================================================
            System.out.println("[streamPD] Step 2: Emergency inline fixes for remaining issues...");
            
            // Fix any remaining Factor spacing issues (for non-4A/4B factors)
            ...
            // Additional 4A/4B safety fixes (in case regex missed something)
            ...
            System.out.println("[streamPD] Found Factor 4A: " + factor4ACount + " times");
            System.out.println("[streamPD] Found Factor 4B: " + factor4BCount + " times");
            
            // Log samples
            String[] postFixLines = rawOutput.split("\n");
            for (String line : postFixLines) {
                if (line.toUpperCase().contains("FACTOR 4A") || line.toUpperCase().contains("FACTOR 4B")) {
                    System.out.println("[streamPD] Sample after fixes: " + line);
                }
            }

            // ============================================================
            // === STEP 3: CALL fixPDFormatting ===
            // ============================================================
            System.out.println("[streamPD] Step 3: Calling fixPDFormatting...");
...
/**
 * NUCLEAR FIX: Replace ONLY Factor 4A and 4B headers deterministically
 * Add this method to your PdService class
 */
private String replaceFactor4A4BHeaders(String aiOutput, PdRequest request) {
    ...
    // Ensure clean format (remove any existing prefixes)
    lvlA = lvlA.replaceAll("^4[AB]?-", "");
    if (!lvlA.contains("-")) {
        lvlA = "4-" + lvlA;
    } else if (!lvlA.startsWith("4-")) {
        lvlA = "4-" + lvlA.split("-")[1];
    }
    ...
    lvlB = lvlB.replaceAll("^4[AB]?-", "");
    if (!lvlB.contains("-")) {
        lvlB = "4-" + lvlB;
    } else if (!lvlB.startsWith("4-")) {
        lvlB = "4-" + lvlB.split("-")[1];
    }
    Integer ptsB = factorPoints.getOrDefault("4B", 125);
    
    // Build the EXACT correct headers (all uppercase for FACTOR)
    ...
    System.out.println("[replaceFactor4A4BHeaders] Correct 4A header: " + correct4AHeader);
    System.out.println("[replaceFactor4A4BHeaders] Correct 4B header: " + correct4BHeader);
    
    // Count existing malformed headers
    int before4ACount = countOccurrences(aiOutput, "(?i)\\*{0,2}\\s*FACTOR\\s*4\\s*A");
    int before4BCount = countOccurrences(aiOutput, "(?i)\\*{0,2}\\s*FACTOR\\s*4\\s*B");
    
    System.out.println("[replaceFactor4A4BHeaders] Found " + before4ACount + " Factor 4A headers before replacement");
    System.out.println("[replaceFactor4A4BHeaders] Found " + before4BCount + " Factor 4B headers before replacement");
    
    // Replace Factor 4A header - catches ALL variations
    aiOutput = aiOutput.replaceAll(
        "(?i)\\*{0,2}\\s*FACTOR\\s*4\\s*A\\s*[–—-]\\s*[^\\n]*?(?:Level|level)[^\\n]*?(?:Points?|points?)\\s*\\*{0,2}",
        correct4AHeader
    );
    
    // Replace Factor 4B header - catches ALL variations  
    aiOutput = aiOutput.replaceAll(
        "(?i)\\*{0,2}\\s*FACTOR\\s*4\\s*B\\s*[–—-]\\s*[^\\n]*?(?:Level|level)[^\\n]*?(?:Points?|points?)\\s*\\*{0,2}",
        correct4BHeader
    );
    
    // Verify replacements worked
    int after4ACount = countOccurrences(aiOutput, "\\Q" + correct4AHeader + "\\E");
    int after4BCount = countOccurrences(aiOutput, "\\Q" + correct4BHeader + "\\E");
    
    System.out.println("[replaceFactor4A4BHeaders] Successfully replaced " + after4ACount + " Factor 4A headers");
    System.out.println("[replaceFactor4A4BHeaders] Successfully replaced " + after4BCount + " Factor 4B headers");
    
    if (after4ACount == 0) {
        System.err.println("⚠️ WARNING: Factor 4A header replacement failed - trying fallback");
    ...
    if (after4BCount == 0) {
        System.err.println("⚠️ WARNING: Factor 4B header replacement failed - trying fallback");
        aiOutput = aiOutput.replaceAll(
            "(?i)\\*{0,2}[^\\n]*?FACTOR\\s*4\\s*B[^\\n]*?PERSONAL CONTACTS[^\\n]*?\\*{0,2}",
            correct4BHeader
        );
...
/**
 * Helper method to count regex pattern occurrences
 */
