...
private String buildCompleteFactorSectionForPD(PdRequest request) {
    ...
    // Always recalculate factor levels and points to match assigned grade and range
    String gsGrade = request.getGsGrade() != null ? request.getGsGrade() : "GS-13";
    String supervisoryLevel = request.getSupervisoryLevel() != null ? request.getSupervisoryLevel() : "Non-Supervisory";

    // If missing or incomplete, generate defaults for assigned grade
    if (factorLevels == null || factorPoints == null || factorLevels.size() != 9 || factorPoints.size() != 9) {
        System.out.println("Generating default factor levels for " + gsGrade);
    ...
    // If calculated grade does not match assigned grade, adjust factors upward
    if (!calculatedGrade.equals(gsGrade)) {
        System.out.println("Adjusting factors: current=" + totalPoints + "pts (" + calculatedGrade + "), target=" + gsGrade);
        ...
        gradeRange = getGradeRange(gsGrade);
        calculatedGrade = gsGrade;
        System.out.println("After adjustment: " + totalPoints + "pts = " + calculatedGrade);
    }

    // Lock these values into the request for downstream use
    ...
    section.append("YOU MUST GENERATE ALL 9 FACTORS IN THIS EXACT FORMAT:\n\n");
    ...
    for (int i = 1; i <= 9; i++) {
        ...
        if (level != null && points != null) {
            // Show exact header format the AI must use
            section.append(String.format("**Factor %d – %s Level %s, %d Points**\n\n", 
                i, factorNames[i-1], level, points));
            
            // Provide detailed instructions for content
            section.append(String.format(
                "Write 2-3 detailed paragraphs (6-8 sentences total) explaining why this position requires Level %s:\n" +
                "- Paragraph 1: Describe the specific knowledge/skills/responsibilities at this level\n" +
                "- Paragraph 2: Explain why this level fits %s rather than %s (lower) or %s (higher)\n" +
                "- Paragraph 3: Reference specific duties that demonstrate this factor level\n\n",
                level, 
                calculatedGrade, 
                getLowerGrade(calculatedGrade), 
                getHigherGrade(calculatedGrade)
            ));
        }
    }

    // Add summary section with exact format
    section.append("\nAFTER ALL 9 FACTORS, ADD THIS SUMMARY:\n\n");
    ...
    section.append("CRITICAL: Do NOT skip any factors. Do NOT use placeholders like '[Pending]'. ");
    section.append("Generate complete paragraphs for ALL 9 factors with the exact levels and points shown above.\n");
