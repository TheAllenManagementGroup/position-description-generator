...
private int getMaxLevelForFactor(String factor) {
     ...
     */
    public String fixPDFormatting(String pdText, int totalPoints, String gsGrade, String gradeRange) {
    if (pdText == null || pdText.trim().isEmpty()) return "";

    String text = pdText.trim();

    // --- Robust HEADER section fix ---
    // Split all header fields, even if run together or missing spaces/newlines
    ...
    // --- Factor header spacing and bolding ---
    // Fix missing spaces between level and points (e.g., 1-7125 -> 1-7, 1250)
    text = text.replaceAll(
        "(Level\\s*[0-9]+-[0-9]+)([0-9]{3,4})", "$1, $2"
    );
    // Ensure all factor headers are bolded and formatted
    text = text.replaceAll(
        "(?m)^\\s*\\*?\\*?\\s*Factor\\s*([0-9])\\s*[–-]?\\s*([^\\n*]+?)\\s*Level\\s*([0-9]+)-([0-9]+),\\s*([0-9]+)\\s*Points\\s*\\*?\\*?",
        "**Factor $1 – $2 Level $3-$4, $5 Points**"
    );
    // Ensure two blank lines before each factor header
    text = text.replaceAll("([^\n])\\n\\*\\*Factor", "$1\n\n**Factor");

    // --- Section header spacing and bolding ---
    // Ensure section headers are bolded and have two blank lines before and after
    text = text.replaceAll(
        "\\*\\*(HEADER|INTRODUCTION|MAJOR DUTIES|FACTOR EVALUATION - COMPLETE ANALYSIS|FACTOR EVALUATION|EVALUATION SUMMARY:?|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES DETERMINATION|FAIR LABOR STANDARDS ACT DETERMINATION)\\*\\*",
        "\n\n**$1**\n\n"
    );
    ...
    // --- Ensure EVALUATION SUMMARY fields are present and correct ---
    // Remove any existing EVALUATION SUMMARY section
    String summaryPattern = "(?s)\\*\\*EVALUATION SUMMARY\\*\\*.*?(\\*\\*|$)";
    text = text.replaceAll(summaryPattern, "");

    // Insert correct summary at the end
    String correctSummary = String.format(
        "\n\n**EVALUATION SUMMARY**\n\n**Total Points: %d**\n\n**Final Grade: %s**\n\n**Grade Range: %s**\n\n",
        totalPoints,
        gsGrade,
        gradeRange
    );
    text = text.trim() + correctSummary;

    // Final cleanup: remove any accidental multiple newlines again
    text = text.replaceAll("\\n{3,}", "\n\n");
