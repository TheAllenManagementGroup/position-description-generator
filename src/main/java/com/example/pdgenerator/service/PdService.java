...
public String fixPDFormatting(String pdText) {
    ...
    // start with the existing normalized pipeline
    ...
    text = text.replaceAll("(?i)([0-9]{1,6})Points\\b", "$1 Points");
    text = text.replaceAll(",\\s*(?=\\d+\\s*Points)", ", ");

    // canonicalize common section headers
    ...
    // general factor header bolding and spacing
    text = text.replaceAll("(?m)^(\\s*)(Factor\\s+[0-9]{1,2}[A-Z]?\\s+–\\s+.*?Level\\s+[0-9]+-[0-9]+,\\s*\\d+\\s*Points)\\s*$", "$1**$2**");
    ...
    // === FINAL TARGETED PASSES FOR FACTOR 4A / 4B (deterministic) ===
    // Replace any single-line header variants for Factor 4A with canonical header (preserve captured level and points)
    try {
        java.util.regex.Pattern p4a = java.util.regex.Pattern.compile(
            "(?mi)^\\s*\\*{0,2}\\s*Factor\\s*4A\\b[^\\n]*?Level\\s*([0-9]+-[0-9]+)\\s*,?\\s*(\\d{1,6})\\s*Points\\s*\\*{0,2}\\s*$"
        );
        java.util.regex.Matcher m4a = p4a.matcher(text);
        StringBuffer sb4a = new StringBuffer();
        while (m4a.find()) {
            String lvl = m4a.group(1);
            String pts = m4a.group(2);
            String replacement = "\n\n**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level " + lvl + ", " + pts + " Points**\n\n";
            m4a.appendReplacement(sb4a, java.util.regex.Matcher.quoteReplacement(replacement));
        }
        m4a.appendTail(sb4a);
        text = sb4a.toString();

        // If AI produced a header without explicit Level/Points on same line, replace loose "Factor 4A" lines
        java.util.regex.Pattern p4aLoose = java.util.regex.Pattern.compile("(?mi)^\\s*\\*{0,2}\\s*Factor\\s*4A\\b[^\\n]*\\*{0,2}\\s*$");
        text = p4aLoose.matcher(text).replaceAll("\n\n**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level 4-4, 100 Points**\n\n");
    } catch (Exception e) {
        System.err.println("fixPDFormatting: 4A replacement failed: " + e.getMessage());
    }

    try {
        java.util.regex.Pattern p4b = java.util.regex.Pattern.compile(
            "(?mi)^\\s*\\*{0,2}\\s*Factor\\s*4B\\b[^\\n]*?Level\\s*([0-9]+-[0-9]+)\\s*,?\\s*(\\d{1,6})\\s*Points\\s*\\*{0,2}\\s*$"
        );
        java.util.regex.Matcher m4b = p4b.matcher(text);
        StringBuffer sb4b = new StringBuffer();
        while (m4b.find()) {
            String lvl = m4b.group(1);
            String pts = m4b.group(2);
            String replacement = "\n\n**Factor 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS) Level " + lvl + ", " + pts + " Points**\n\n";
            m4b.appendReplacement(sb4b, java.util.regex.Matcher.quoteReplacement(replacement));
        }
        m4b.appendTail(sb4b);
        text = sb4b.toString();

        java.util.regex.Pattern p4bLoose = java.util.regex.Pattern.compile("(?mi)^\\s*\\*{0,2}\\s*Factor\\s*4B\\b[^\\n]*\\*{0,2}\\s*$");
        text = p4bLoose.matcher(text).replaceAll("\n\n**Factor 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS) Level 4-4, 125 Points**\n\n");
    } catch (Exception e) {
        System.err.println("fixPDFormatting: 4B replacement failed: " + e.getMessage());
    }

    // Safety: fix run-together tokens that sometimes escape earlier passes
    text = text.replaceAll("(?i)Factor4A", "Factor 4A");
    text = text.replaceAll("(?i)Factor4B", "Factor 4B");
    text = text.replaceAll("(?i)Level4-4\\s*,?\\s*100", "Level 4-4, 100");
    text = text.replaceAll("(?i)Level4-4\\s*,?\\s*125", "Level 4-4, 125");
    ...
    // final diagnostic: if still contains exact bad tokens, log warning
    if (text.matches("(?s).*Factor4A.*|.*Factor4B.*|.*Level4-4.*")) {
        System.err.println("⚠️ fixPDFormatting: residual 4A/4B spacing artifacts remain.");
    }
