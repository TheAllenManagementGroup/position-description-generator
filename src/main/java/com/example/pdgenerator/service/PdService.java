...
public class PdService {
    ...
    String totalPoints = request.getTotalPoints() != null ? request.getTotalPoints().toString() : null;
    ...
    // --- CRITICAL: If factor levels/points/totalPoints/gradeRange are missing, generate them for the assigned grade ---
    boolean needsDefaultFactors = request.getFactorLevels() == null || request.getFactorPoints() == null
        || request.getFactorLevels().size() != 9 || request.getFactorPoints().size() != 9
        || totalPoints == null || gradeRange == null;

    if (needsDefaultFactors) {
        ...
        // Lock these into the request for downstream use
        ...
        totalPoints = String.valueOf(calculatedTotalPoints);
        gradeRange = calculatedGradeRange;
    }

    // Build complete factor section with locked values (now guaranteed to exist)
...
private String buildCompleteFactorSectionForPD(PdRequest request) {
    ...
    for (int i = 1; i <= 9; i++) {
        ...
        section.append("Write 3-5 sentences following the example format above:\n");
        section.append("What specific knowledge/skills/responsibilities this level requires\n");
        section.append("Specific duty examples demonstrating this level\n\n");
    }
