...
""",
    ...
    section.append("🚨🚨🚨 CRITICAL: READ THIS EXAMPLE CAREFULLY 🚨🚨🚨\n\n");
    section.append("YOU MUST COPY THIS EXACT SPACING FOR FACTOR 4A AND 4B:\n\n");
    section.append("✅ CORRECT:\n");
    section.append("**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level 4-4, 100 Points**\n\n");
    section.append("Notice: 'Factor 4A' has a SPACE between Factor and 4A\n");
    section.append("Notice: 'Level 4-4, 100' has spaces around the comma\n\n");
    section.append("❌ WRONG (DO NOT DO THIS):\n");
    section.append("**Factor4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level4-4,100 Points**\n\n");
    section.append("This is WRONG because there's no space after 'Factor' and no spaces in 'Level4-4,100'\n\n");
    
    section.append("═══════════════════════════════════════════════════════\n\n");

    // List the 6 GSSG factors
    for (int i = 1; i <= 6; i++) {
        if (i == 4) {
...
""",
    ...
    try {
        ...
        // ENHANCED SYSTEM MESSAGE with explicit formatting rules
        messages.add(new Message("system",
            "You are an expert federal HR classification specialist. " +
            "CRITICAL FORMATTING RULES YOU MUST FOLLOW:\n" +
            "1. When writing Factor 4A, you MUST write 'Factor 4A' (WITH a space after Factor)\n" +
            "2. When writing Factor 4B, you MUST write 'Factor 4B' (WITH a space after Factor)\n" +
            "3. NEVER write 'Factor4A' or 'Factor4B' (without space)\n" +
            "4. When writing Level values, you MUST include spaces: 'Level 4-4, 100' NOT 'Level4-4,100'\n" +
            "5. Example CORRECT format: **Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level 4-4, 100 Points**\n" +
            "6. Example WRONG format: **Factor4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level4-4,100 Points**\n\n" +
            "You MUST generate COMPLETE, DETAILED position descriptions with NO placeholders, NO outlines, NO incomplete sections. " +
...
public String fixPDFormatting(String pdText) {
    ...
    // === NUCLEAR OPTION: Fix Factor4A and Factor4B with multiple passes ===
    // Pass 1: Direct string replacement (most reliable)
    text = text.replace("**Factor4A", "**Factor 4A");
    text = text.replace("**Factor4B", "**Factor 4B");
    text = text.replace("Factor4A", "Factor 4A");
    text = text.replace("Factor4B", "Factor 4B");
    
    // Pass 2: Fix Level formatting for 4A/4B specifically
    text = text.replace("Level4-4,100", "Level 4-4, 100");
    text = text.replace("Level4-4,125", "Level 4-4, 125");
    text = text.replace("Level 4-4,100", "Level 4-4, 100");
    text = text.replace("Level 4-4,125", "Level 4-4, 125");
    
    // Pass 3: Generic Level formatting fixes
    text = text.replaceAll("Level([0-9])-([0-9]),([0-9]+)", "Level $1-$2, $3");
    text = text.replaceAll("Level ([0-9])-([0-9]),([0-9]+)", "Level $1-$2, $3");
    
    // Pass 4: Remove G: prefix anywhere it appears
    text = text.replace("Level G:", "Level ");
    text = text.replaceAll("Level\\s+G:", "Level ");

    // Pass 5: Ensure proper spacing in factor headers using line-by-line processing
    String[] lines = text.split("\n");
    StringBuilder result = new StringBuilder();
    for (String line : lines) {
        // Check if this is a factor header line
        if (line.contains("Factor") && line.contains("Level") && line.contains("Points")) {
            // Ensure "Factor " has space after it
            line = line.replaceAll("Factor([0-9]+)([AB]?)\\s*–", "Factor $1$2 –");
            // Ensure "Level " formatting
            line = line.replaceAll("Level\\s*([0-9]+-[0-9]+)\\s*,\\s*([0-9]+)", "Level $1, $2");
            // Ensure proper bold markers
    ...
    // Fix Grade and Agency in header
    text = text.replaceAll("(Grade:[^\n]+)(Agency:)", "$1\n\n$2");
    text = text.replaceAll("(Agency:[^\n]+)(Organization:)", "$1\n\n$2");

    // Fix FACTOR EVALUATION header
    ...
    // Ensure blank line after factor header
    text = text.replaceAll("(?m)(\\*\\*Factor [0-9]+[AB]? – [^\\n]+ Level [0-9]+-[0-9]+, [0-9]+ Points\\*\\*)\\n([A-Z])", "$1\n\n$2");
    
    // Ensure spacing between factors
