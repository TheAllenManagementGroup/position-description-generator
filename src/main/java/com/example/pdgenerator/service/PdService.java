...
private int getMaxLevelForFactor(String factor) {
    ...
    // ===== CRITICAL: Fix run-together header fields FIRST =====
    // These must be done in order, and very aggressively
    text = text.replaceAll("(Job Series:\\s*GS-\\d+)([A-Z])", "$1\n\n$2");
    text = text.replaceAll("(Position Title:[^\\n]{1,100}?)([A-Z][a-z]+:)", "$1\n\n$2");
    text = text.replaceAll("(Agency:[^\\n]{1,100}?)(Organization:)", "$1\n\n$2");
    text = text.replaceAll("(Organization:[^\\n]{1,100}?)(Lowest Organization:)", "$1\n\n$2");
    text = text.replaceAll("(Lowest Organization:[^\\n]{1,100}?)(Supervisory Level:)", "$1\n\n$2");
    text = text.replaceAll("(Supervisory Level:[^\\n]{1,100}?)(Grade:)", "$1\n\n$2");
    text = text.replaceAll("(Grade:\\s*GS-\\d+)([A-Z*])", "$1\n\n$2");
    
    // Fix department names that get split (e.g., "Department of JusticeOrganization")
    text = text.replaceAll("(Department of [A-Z][a-z]+)([A-Z][a-z]+ (?:Division|Office))", "$1\n\nOrganization: $2");
    
    // ===== AGGRESSIVE: Fix malformed factor headers =====
    // Fix "Level 1-8155, 0 Points" -> "Level 1-8, 1550 Points"
    // The pattern catches: Level [factor]-[level][POINTS], WRONG Points
    text = text.replaceAll(
    ...
    for (String header : sectionHeaders) {
        // Remove existing asterisks and ensure proper bold formatting with spacing
