...
public String fixPDFormatting(String pdText) {
    ...
    // normalize invisible whitespace that breaks regex matches (NBSP, zero-width)
    text = text.replace("\u00A0", " ").replace("\u200B", " ");

    // remove lines that contain only asterisks
    text = text.replaceAll("(?m)^\\s*\\*+\\s*$\\n?", "");

    // remove stray "G:" prefixes (conservative)
    text = text.replaceAll("(?i)\\bG:\\s*", "");

    // Ensure a single space after "Factor" even when concatenated (handles Factor4A, Factor4B, Factor01 etc.)
    text = text.replaceAll("(?i)\\bFactor[\\s\\u00A0\\u200B]*([0-9]{1,2}[A-Za-z]?)\\b", "Factor $1");

    // Normalize any dash/em/en/— variants and ensure "Factor X – Name" spacing
    text = text.replaceAll("(?m)(?i)(Factor\\s+[0-9]{1,2}[A-Za-z]?)\\s*[-–—:\\u2014\\u2013\\s]+\\s*", "$1 – ");

    // Normalize Level spacing and ensure comma + space before Points
    // handle forms like "Level4-4,125", "Level 4-4,125Points", "Level4-4, 125 Points"
    text = text.replaceAll("(?i)Level\\s*([0-9]+)\\s*[-–—]?\\s*([0-9]+)\\s*,?\\s*([0-9]{1,6})\\s*Points", "Level $1-$2, $3 Points");
    // also handle missing "Points" spacing if the model joined token (e.g., "125Points")
    text = text.replaceAll("(?i)([0-9]{1,6})Points\\b", "$1 Points");

    // Make sure there is a space after commas before numbers (general safety)
    text = text.replaceAll(",\\s*(?=\\d+\\s*Points)", ", ");

    // Ensure Factor 4A / 4B canonical names even if model produced different wording
    text = text.replaceAll("(?mi)\\bFactor\\s*4A\\b[\\s–—:\\-]*[^\\n]*?\\bPERSONAL CONTACTS\\b[^\\n]*", "Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS)");
    text = text.replaceAll("(?mi)\\bFactor\\s*4B\\b[\\s–—:\\-]*[^\\n]*?\\bPERSONAL CONTACTS\\b[^\\n]*", "Factor 4B – PERSONAL CONTACTS (PURPOSE OF CONTACTS)");

    // Wrap factor header lines in bold (exactly) when they contain Level and Points
    text = text.replaceAll("(?m)^(\\s*)(Factor\\s+[0-9]{1,2}[A-Z]?\\s+–\\s+.*?Level\\s+[0-9]+-[0-9]+,\\s*\\d+\\s*Points)\\s*$", "$1**$2**");

    // Ensure a blank line after each bolded factor header
    text = text.replaceAll("(?m)(\\*\\*Factor\\s+[0-9]{1,2}[A-Z]?\\s+–\\s+.*?Points\\*\\*)\\n(?!\\n)", "$1\n\n");

    // Ensure two blank lines between factor descriptions and the next Factor header
    text = text.replaceAll("(?m)([\\.!?\\)\\\"]+)\\n\\s*(\\*\\*Factor\\s+[0-9]{1,2})", "$1\n\n\n$2");

    // Normalize section headers to bold and spacing
    ...
    for (String header : sectionHeaders) {
        text = text.replaceAll("(?i)\\*{0,2}\\s*" + java.util.regex.Pattern.quote(header) + "\\s*:?\\s*\\*{0,2}", "\n\n**" + header + "**\n\n");
    }

    // Fix Grade/Agency header run-ons
    ...
    // Final safety passes for concatenated tokens
    text = text.replaceAll("Factor4A", "Factor 4A");
    text = text.replaceAll("Factor4B", "Factor 4B");
    text = text.replaceAll("Level4-", "Level 4-");
    text = text.replaceAll(",(\\d{1,6})\\s*Points", ", $1 Points");
