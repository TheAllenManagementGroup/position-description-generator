...
""",
    ...
    // Handle many malformed variants produced by the model (missing spaces, missing commas, "G:" prefixes, lack of bold markers)
    text = text.replaceAll("(?mi)\\*?\\s*Factor\\s*4A\\s*[-–—:\\s]*[^\\n]*?Level\\s*G:?\\s*([0-9]+-[0-9]+)\\s*,?\\s*(\\d{1,6})\\s*Points\\s*\\*?",
        "\n\n**Factor 4A – PERSONAL CONTACTS (NATURE OF CONTACTS) Level $1, $2 Points**\n\n");
    text = text.replaceAll("(?mi)\\*?\\s*Factor\\s*4B\\s*[-–—:\\s]*[^\\n]*?Level\\s*G:?\\s*([0-9]+-[0-9]+)\\s*,?\\s*(\\d{1,6})\\s*Points\\s*\\*?",
    ...
    // Layer 2: Fix missing space after "Factor" - catches Factor4A -> Factor 4A
    text = text.replaceAll("(?m)\\*\\*Factor([0-9])([A-Z])(\\s*)–", "**Factor $1$2 –");
