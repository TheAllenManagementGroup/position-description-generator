...
""",
    ...
    section.append("CORRECT EXAMPLES (COPY THIS EXACT FORMAT WITH SPACES):\n");
    ...
    section.append("SPACING RULES (MANDATORY - FOLLOW EXACTLY):\n");
    section.append("1. 'Factor 1' (space between Factor and number)\n");
    section.append("2. 'Factor 4A' (space between Factor and 4A - NOT Factor4A)\n");
    section.append("3. 'Factor 4B' (space between Factor and 4B - NOT Factor4B)\n");
    section.append("4. 'Level 1-3, 550' (spaces around comma - NOT Level1-3,550)\n");
    section.append("5. 'Level 4-4, 100' (spaces around comma - NOT Level4-4,100)\n");
    section.append("6. 'Level 4-4, 125' (spaces around comma - NOT Level4-4,125)\n");
    section.append("7. One blank line after header, then description, then TWO blank lines before next factor\n\n");
    
    section.append("FORBIDDEN PATTERNS - NEVER OUTPUT THESE:\n");
    section.append("❌ Factor1 (missing space after Factor)\n");
    section.append("❌ Factor4A (missing space after Factor)\n");
    section.append("❌ Factor4B (missing space after Factor)\n");
    section.append("❌ Level1-3,550 (no spaces)\n");
    section.append("❌ Level4-4,100 (no spaces)\n");
    section.append("❌ Level4-4,125 (no spaces)\n");
    section.append("❌ G:1-3 (never include G: prefix)\n");
    section.append("❌ G:4-4 (never include G: prefix)\n\n");
    
    section.append("REQUIRED OUTPUT (COPY EXACTLY):\n");
    section.append("✓ Factor 1 (WITH space)\n");
    section.append("✓ Factor 4A (WITH space after Factor)\n");
    section.append("✓ Factor 4B (WITH space after Factor)\n");
    section.append("✓ Level 1-3, 550 (WITH spaces)\n");
    section.append("✓ Level 4-4, 100 (WITH spaces)\n");
    section.append("✓ Level 4-4, 125 (WITH spaces)\n");
    section.append("✓ Never write G: anywhere\n\n");
    ...
    section.append("⚠️ ABSOLUTELY MANDATORY - YOUR OUTPUT MUST MATCH THESE EXACT SPACING RULES:\n");
    section.append("1. Generate ALL 6 factors (1, 2, 3, 4A, 4B, 5, 6) - never skip any\n");
    section.append("2. ALWAYS write 'Factor 4A' (WITH space) - NEVER 'Factor4A' (without space)\n");
    section.append("3. ALWAYS write 'Factor 4B' (WITH space) - NEVER 'Factor4B' (without space)\n");
    section.append("4. ALWAYS write 'Level 4-4, 100' (WITH comma and space) - NEVER 'Level4-4,100'\n");
    section.append("5. ALWAYS write 'Level 4-4, 125' (WITH comma and space) - NEVER 'Level4-4,125'\n");
    section.append("6. NEVER include 'G:' prefix anywhere in your output\n");
    section.append("7. ONE blank line after header, then description, then TWO blank lines before next factor\n");
    section.append("8. Do NOT alter any levels or point values - copy them exactly as shown above\n");
    section.append("9. Description: 2-5 sentences only\n\n");
...
public String fixPDFormatting(String pdText) {
    ...
    // === CRITICAL FIX FOR 4A AND 4B - MUST COME FIRST ===
    // These patterns MUST match Factor4A and Factor4B (no space) explicitly
    
    // Fix Factor4A (no space between Factor and 4A)
    text = text.replaceAll(
        "(?m)\\*{0,2}Factor4A\\s*[-–—]?\\s*([^\\n]*?)Level([0-9]+-[0-9]+),([0-9]+)\\s*Points\\*{0,2}",
        "**Factor 4A – $1Level $2, $3 Points**"
    );
    
    // Fix Factor4B (no space between Factor and 4B)
    text = text.replaceAll(
        "(?m)\\*{0,2}Factor4B\\s*[-–—]?\\s*([^\\n]*?)Level([0-9]+-[0-9]+),([0-9]+)\\s*Points\\*{0,2}",
        "**Factor 4B – $1Level $2, $3 Points**"
    );
    
    // Fix Factor 4A with space but bad Level formatting
    text = text.replaceAll(
        "(?m)\\*{0,2}Factor\\s+4A\\s*[-–—]?\\s*([^\\n]*?)Level([0-9]+-[0-9]+),([0-9]+)\\s*Points\\*{0,2}",
        "**Factor 4A – $1Level $2, $3 Points**"
    );
    
    // Fix Factor 4B with space but bad Level formatting
    text = text.replaceAll(
        "(?m)\\*{0,2}Factor\\s+4B\\s*[-–—]?\\s*([^\\n]*?)Level([0-9]+-[0-9]+),([0-9]+)\\s*Points\\*{0,2}",
        "**Factor 4B – $1Level $2, $3 Points**"
    );
    ...
    // Layer 1: Fix completely malformed headers (no spaces anywhere) - UPDATED FOR 4A/4B
    // Pattern: **Factor4A – NAME Level4-4,100 Points** -> **Factor 4A – NAME Level 4-4, 100 Points**
    text = text.replaceAll("(?m)\\*\\*Factor([0-9]+)([AB])\\s*–\\s*([^\\n]*?)Level([0-9]+-[0-9]+),([0-9]+)\\s*Points\\*\\*", 
        "**Factor $1$2 – $3Level $4, $5 Points**");
    
    text = text.replaceAll("(?m)\\*\\*Factor([0-9]+)\\s*–\\s*([^\\n]*?)Level([0-9]+-[0-9]+),([0-9]+)\\s*Points\\*\\*", 
    ...
    // Layer 3: Fix "Level" with no space after it (Level4-4,100 -> Level 4-4, 100)
    text = text.replaceAll("(?m)Level([0-9]+-[0-9]+),([0-9]+)\\s+Points", "Level $1, $2 Points");
    
    // Layer 4: Remove any remaining "G:" prefixes from levels
    text = text.replaceAll("Level\\s+G:([0-9]+-[0-9]+),([0-9]+)", "Level $1, $2");
    ...
    text = text.replaceAll("(?m)(Factor\\s+[0-9]+[AB]?)([–-])\\s*([A-Z])", "$1 – $3");
    
    // Layer 7: Ensure all factor headers end with **
    text = text.replaceAll("(?m)(\\*\\*Factor\\s+[0-9]+[AB]*\\s*–\\s*[^\\n]*?Level\\s+[0-9]+-[0-9]+,\\s*[0-9]+\\s*Points)(\\*{0,2})$", "$1**");
    
    // Layer 8: Ensure one blank line after factor header before description
    text = text.replaceAll(
        "(?m)(\\*\\*Factor\\s+[0-9]+[AB]*\\s*–\\s*[^\\n]*?Level\\s+[0-9]+-[0-9]+,\\s*[0-9]+\\s*Points\\*\\*)\\n(?!\\n)",
        "$1\n\n"
    );
