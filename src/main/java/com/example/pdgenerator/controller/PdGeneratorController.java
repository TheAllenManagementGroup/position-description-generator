...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> factors) {
    try {
        ...
        if (response == null || response.isBlank()) {
            return Map.of("error", "AI returned empty response");
        }

        String jsonResponse = extractJsonFromResponse(response);
        if (jsonResponse == null || jsonResponse.isBlank()) {
            return Map.of("error", "Failed to extract JSON from AI response");
        }
        ...
        try {
            ...
            return Map.of("error", "Failed to parse AI response JSON: " + e.getMessage());
        }
        ...
        for (Map.Entry<String, Map<String,String>> entry : aiLevels.entrySet()) {
            ...
            Integer points = null;
            if (FACTOR_POINTS.containsKey(factorNum)) {
                points = FACTOR_POINTS.get(factorNum).get(level);
            }
            if (points == null) points = 0;
            totalPoints += points;

            Object contentObj = factors.get("Factor " + factorNum);
