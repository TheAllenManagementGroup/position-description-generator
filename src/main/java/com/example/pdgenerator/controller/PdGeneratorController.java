...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> factors) {
    ...
    try {
        ...
        prompt.append("You are an OPM HR expert. Review each factor content and assign the correct level. ")
              .append("Return JSON only with keys 'Factor 1', 'Factor 2', ..., each containing {\"level\":\"X-Y\",\"rationale\":\"brief explanation\"}.\n");

        prompt.append("CONTENT={");
        factors.forEach((k,v) -> {
            if(v != null && !v.toString().isBlank()) {
                String content = v.toString().replace("\"","'");
                if(content.length() > 1000) content = content.substring(0,1000) + "..."; // truncate long content
                prompt.append(k.replace("Factor ","")).append(":\"").append(content).append("\",");
            }
        });
        ...
        String response;
        try {
            response = callOpenAIWithTimeout(prompt.toString(), 30); // 30 sec timeout
        } catch(Exception e) {
            e.printStackTrace();
            return Map.of("error", "AI call failed: " + e.getMessage());
        }
        ...
        if(jsonResponse == null || jsonResponse.isBlank() || !jsonResponse.trim().startsWith("{")) {
            return Map.of("error", "AI returned invalid or empty JSON");
        }

        ObjectMapper mapper = new ObjectMapper();
        Map<String, Map<String,String>> aiLevels = mapper.readValue(
                jsonResponse, new TypeReference<Map<String, Map<String,String>>>() {}
        );
        ...
        for(Map.Entry<String, Map<String,String>> entry : aiLevels.entrySet()) {
            try {
                String factorNum = entry.getKey().replace("Factor ","").trim();
                Map<String,String> data = entry.getValue();
                String level = data.get("level").trim();

                Map<String,Integer> factorMap = FACTOR_POINTS.get(factorNum);
                if(factorMap == null) continue; // skip unknown factor

                Integer points = factorMap.get(level);
                if(points == null) {
                    // Try removing leading zeros: e.g., "1-07" -> "1-7"
                    level = level.replaceFirst("-0", "-");
                    points = factorMap.get(level);
                ...
                finalFactors.put("Factor " + factorNum, Map.of(
                        ...
                        "content", factors.getOrDefault("Factor " + factorNum, "").toString(),
                        "level", level,
                        "points", points,
                ...
                System.err.println("Error processing factor " + entry.getKey() + ": " + e.getMessage());
            }
        }
        ...
        e.printStackTrace();
        return Map.of("error", "Failed to update factor points/grade: " + e.getMessage());
    }
}
