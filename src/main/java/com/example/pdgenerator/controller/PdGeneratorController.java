...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> factors) throws Exception {
    ...
    // Build mutable factor points map
    final Map<String, Map<String, Integer>> FACTOR_POINTS = new HashMap<>();
    FACTOR_POINTS.put("1", Map.of("1-1",200,"1-2",350,"1-3",500,"1-4",750,"1-5",950,"1-6",1250,"1-7",1550,"1-8",1850,"1-9",2210));
    ...
    // Build AI prompt with only factor content
    ...
    factors.forEach((k,v) -> {
        if (!v.toString().isBlank()) {
            prompt.append(k.replace("Factor ","")).append(":\"").append(v.toString().replace("\"","'")).append("\",");
        }
    });
    if (prompt.charAt(prompt.length()-1) == ',') prompt.deleteCharAt(prompt.length()-1);
    prompt.append("}");

    // Call AI with short timeout
    String response = callOpenAIWithTimeout(prompt.toString(), 25);

    String jsonResponse = extractJsonFromResponse(response);
    ObjectMapper mapper = new ObjectMapper();
    Map<String, Map<String,String>> aiLevels = mapper.readValue(jsonResponse, new TypeReference<>() {});

    // Calculate points and build final factors
    int totalPoints = 0;
    ...
    for (Map.Entry<String, Map<String,String>> entry : aiLevels.entrySet()) {
        ...
        int points = FACTOR_POINTS.get(factorNum).get(level);
        totalPoints += points;
    ...
    // Assign grade range & final grade
...
private String calculateGradeRange(int totalPoints) {
    if (totalPoints >=  855 && totalPoints <= 1100) return "0855-1100";
    if (totalPoints >= 1105 && totalPoints <= 1350) return "1105-1350";
    if (totalPoints >= 1355 && totalPoints <= 1600) return "1355-1600";
    if (totalPoints >= 1605 && totalPoints <= 1850) return "1605-1850";
    if (totalPoints >= 1855 && totalPoints <= 2100) return "1855-2100";
    if (totalPoints >= 2105 && totalPoints <= 2350) return "2105-2350";
    if (totalPoints >= 2355 && totalPoints <= 2750) return "2355-2750";
    if (totalPoints >= 2755 && totalPoints <= 3150) return "2755-3150";
    if (totalPoints >= 3155 && totalPoints <= 3600) return "3155-3600";
    return "Unknown";
}

private String calculateFinalGrade(int totalPoints) {
    if (totalPoints >=  855 && totalPoints <= 1100) return "GS-05";
    if (totalPoints >= 1105 && totalPoints <= 1350) return "GS-07";
    if (totalPoints >= 1355 && totalPoints <= 1600) return "GS-09";
    if (totalPoints >= 1605 && totalPoints <= 1850) return "GS-11";
    if (totalPoints >= 1855 && totalPoints <= 2100) return "GS-12";
    if (totalPoints >= 2105 && totalPoints <= 2350) return "GS-12";
    if (totalPoints >= 2355 && totalPoints <= 2750) return "GS-13";
    if (totalPoints >= 2755 && totalPoints <= 3150) return "GS-14";
    if (totalPoints >= 3155 && totalPoints <= 3600) return "GS-15";
    return "Unknown";
}
