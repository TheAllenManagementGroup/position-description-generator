...
public class PdGeneratorController {
    ...
    private boolean testOpenAIConnection() {
        try {
            ...
            OpenAIRequest testRequest = new OpenAIRequest("gpt-4", testMessages, false);
...
// Controller method - replace your existing generateEvaluationStatement method
...
public void generateEvaluationStatement(@RequestBody Map<String, String> body, HttpServletResponse response) throws Exception {
    System.out.println("=== EVALUATION STATEMENT GENERATION STARTED ===");
    ...
    try {
        ...
        System.out.println("Generating evaluation statement for: " + positionTitle + " (" + gsGrade + ")");

        // Send status update
        writer.println("data: {\"status\":\"Generating evaluation statement...\"}\n");
        writer.flush();

        // Build the evaluation statement prompt
        String prompt = buildEvaluationStatementPrompt(duties, requirements, gsGrade, jobSeries, jobTitle, positionTitle);

        // Prepare OpenAI API request with streaming
        List<Message> messages = new ArrayList<>();
        messages.add(new Message("system", 
            "You are an expert federal HR classification specialist with deep knowledge of the OPM Factor Evaluation System. " +
            "Create comprehensive, professional evaluation statements with detailed factor analysis and proper point assignments. " +
            "Always use specific values, actual analysis, and concrete justifications."
        ));
        messages.add(new Message("user", prompt));

        OpenAIRequest openaiRequest = new OpenAIRequest("gpt-4", messages, true);
        openaiRequest.setMax_tokens(4000);
        openaiRequest.setTemperature(0.2); // Lower temperature for consistency
        ...
        // Format and send the complete evaluation statement
        String formattedEvaluation = fixEvaluationFormatting(fullText.toString());
        writer.println("{\"fullEvaluation\": \"" + escapeJson(formattedEvaluation) + "\"}");
        writer.flush();
        
        System.out.println("Evaluation statement generation completed successfully");
        ...
        System.out.println("=== EVALUATION STATEMENT GENERATION COMPLETE ===");
    }
}

// Helper method to build the evaluation statement prompt
private String buildEvaluationStatementPrompt(String duties, String requirements, String gsGrade, 
                                            String jobSeries, String jobTitle, String positionTitle) {
    ...
    return String.format("""
        You are a federal HR classification specialist with expertise in the OPM Factor Evaluation System.
        
        Create a comprehensive position evaluation statement for the following position:
        ...
        **BACKGROUND:**
        Write 2-3 sentences describing the position's organizational context, primary purpose, and key responsibilities within the agency structure.
        
        **TITLE AND SERIES DETERMINATION:**
        Write 2-3 sentences explaining why this specific job series and title are appropriate based on the duties and requirements. Reference relevant classification standards.
        
        **FACTOR ANALYSIS:**
        
        For each of the 9 OPM factors, provide:
        - Factor name and assigned level with points
        - 2-3 paragraphs of detailed justification explaining:
          - Why this level is appropriate based on the specific duties
          - Why it meets this level but not the next higher level
          - Why it exceeds the lower level
        - Use specific examples from the duties to support the evaluation
        
        **Factor 1 - Knowledge Required by the Position**
        Level: [Assign appropriate level 1-1 through 1-9]
        Points: [Corresponding points from the scale]
        [Detailed justification paragraphs]
        
        **Factor 2 - Supervisory Controls**
        Level: [Assign appropriate level 2-1 through 2-5]
        Points: [Corresponding points]
        [Detailed justification paragraphs]
        
        **Factor 3 - Guidelines**
        Level: [Assign appropriate level 3-1 through 3-5]
        Points: [Corresponding points]
        [Detailed justification paragraphs]
        
        **Factor 4 - Complexity**
        Level: [Assign appropriate level 4-1 through 4-6]
        Points: [Corresponding points]
        [Detailed justification paragraphs]
        
        **Factor 5 - Scope and Effect**
        Level: [Assign appropriate level 5-1 through 5-6]
        Points: [Corresponding points]
        [Detailed justification paragraphs]
        
        **Factor 6 - Personal Contacts**
        Level: [Assign appropriate level 6-1 through 6-4]
        Points: [Corresponding points]
        [Detailed justification paragraphs]
        
        **Factor 7 - Purpose of Contacts**
        Level: [Assign appropriate level 7-1 through 7-4]
        Points: [Corresponding points]
        [Detailed justification paragraphs]
        
        **Factor 8 - Physical Demands**
        Level: [Assign appropriate level 8-1 through 8-3]
        Points: [Corresponding points]
        [Brief justification]
        
        **Factor 9 - Work Environment**
        Level: [Assign appropriate level 9-1 through 9-3]
        Points: [Corresponding points]
        [Brief justification]
        
        **TOTAL EVALUATION:**
        Total Points: [Sum all factor points]
        Grade Range: [Corresponding GS grade range]
        Final Grade: [Final GS grade determination]
        
        **CONCLUSION:**
        Brief summary confirming the recommended classification and any special considerations.
        
        Use the official OPM Factor Point values: %s
        
        Ensure all point assignments are realistic and appropriate for the target grade level. Provide thorough, professional analysis that demonstrates deep understanding of federal classification principles.
        """, 
        jobSeries, jobTitle, positionTitle, gsGrade, duties, requirements,
        jobSeries, positionTitle, gsGrade, FACTOR_POINTS.toString()
    );
}

// Helper method to format evaluation statement
private String fixEvaluationFormatting(String evaluationText) {
    if (evaluationText == null || evaluationText.trim().isEmpty()) return "";

    // Fix headers and ensure proper formatting
    evaluationText = evaluationText.replaceAll("\\*\\*([^\\*]+)\\*\\*", "**$1**\n");
    evaluationText = evaluationText.replaceAll("(Factor\\s*\\d+\\s*-[^\\n]+)", "\n\n**$1**");
    evaluationText = evaluationText.replaceAll("(Level:\\s*\\d+-\\d+)", "**$1**");
    evaluationText = evaluationText.replaceAll("(Points:\\s*\\d+)", "**$1**");
    evaluationText = evaluationText.replaceAll("(Total Points:\\s*\\d+)", "\n\n**$1**");
    evaluationText = evaluationText.replaceAll("(Final Grade:\\s*GS-\\d+)", "**$1**");
    
    // Clean up multiple line breaks
    evaluationText = evaluationText.replaceAll("\\n{3,}", "\n\n");
    evaluationText = evaluationText.replaceAll(" +\\n", "\n");
