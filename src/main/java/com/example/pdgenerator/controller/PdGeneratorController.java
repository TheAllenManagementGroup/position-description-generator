...
public ResponseEntity<String> proxyUsaJobs(@RequestParam Map<String, String> params) {
    try {
        // Validate required headers are configured
        if (usajobsApiKey == null || usajobsApiKey.isBlank() || usajobsUserAgent == null || usajobsUserAgent.isBlank()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .contentType(MediaType.APPLICATION_JSON)
                    .body("{\"error\":\"USAJOBS credentials not configured\",\"details\":\"Set usajobs.api.key and usajobs.user.agent\"}");
        }

        // Clamp ResultsPerPage to USAJOBS max (500)
        Map<String, String> safeParams = new LinkedHashMap<>(params);
        String rpp = safeParams.get("ResultsPerPage");
        if (rpp != null) {
            try {
                int v = Integer.parseInt(rpp);
                if (v > 500) safeParams.put("ResultsPerPage", "500");
                else if (v < 1) safeParams.put("ResultsPerPage", "50");
            } catch (NumberFormatException ignored) {
                safeParams.put("ResultsPerPage", "50");
            }
        }
        ...
        // Safe-dedupe: ignore case and normalize spaces, keep first occurrence
        ...
        // Return upstream status so frontend can handle .ok properly
        return ResponseEntity
...
// Helper: dedupe PositionTitle ignoring case in USAJOBS response
private String dedupeUsaJobsTitlesCaseInsensitive(String json) {
    ...
    try {
        ...
        for (JsonNode item : items) {
            JsonNode mod = item.get("MatchedObjectDescriptor");
            String title = (mod != null && mod.has("PositionTitle")) ? mod.get("PositionTitle").asText("") : "";
            String key = title.replaceAll("\\s+", " ").trim().toLowerCase(Locale.ROOT);
            if (!key.isEmpty()) seen.putIfAbsent(key, item);
        }
        ...
        // Optionally sync count if present
