...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> body) throws Exception {
    ...
    // Optimized, compact AI prompt
    StringBuilder prompt = new StringBuilder();
    prompt.append("You are an OPM HR expert. Analyze each factor content and assign correct levels and points using FES and two-grade GS system (GS-5 to GS-15). ");
    prompt.append("Levels must match factor numbers and points must match OPM charts. Consider complexity, responsibility, and scope.\n\n");

    // Compact factor points map
    prompt.append("FACTOR_POINTS={");
    prompt.append("1:{1-1:200,1-2:350,1-3:500,1-4:750,1-5:950,1-6:1250,1-7:1550,1-8:1850,1-9:2210},");
    prompt.append("2:{2-1:25,2-2:125,2-3:275,2-4:450,2-5:650},");
    prompt.append("3:{3-1:25,3-2:125,3-3:275,3-4:450,3-5:650},");
    prompt.append("4:{4-1:25,4-2:75,4-3:150,4-4:225,4-5:325,4-6:450},");
    prompt.append("5:{5-1:25,5-2:75,5-3:150,5-4:225,5-5:325,5-6:450},");
    prompt.append("6:{6-1:10,6-2:25,6-3:60,6-4:110},");
    prompt.append("7:{7-1:20,7-2:50,7-3:120,7-4:220},");
    prompt.append("8:{8-1:5,8-2:20,8-3:50},");
    prompt.append("9:{9-1:5,9-2:20,9-3:50}");
    prompt.append("}\n\n");

    prompt.append("GUIDELINES={");
    prompt.append("1:'knowledge depth/breadth, education, specialization',");
    prompt.append("2:'independence, supervision, review frequency',");
    prompt.append("3:'clarity of guidelines, interpretation',");
    prompt.append("4:'variety, analytical/decision complexity',");
    prompt.append("5:'impact, scope, organizational effect',");
    prompt.append("6:'contact level',7:'contact purpose',");
    prompt.append("8:'physical demands',9:'work environment'");
    prompt.append("}\n\n");

    prompt.append("FACTOR_CONTENTS={");
    for (int i = 1; i <= 9; i++) {
        String key = "Factor " + i;
        String content = factors.getOrDefault(key, "");
        if (!content.isEmpty()) {
            prompt.append(String.format("\"%d\":\"%s\"", i, content.replace("\"","'")));
            if (i < 9) prompt.append(",");
        }
    }
    prompt.append("}\n\n");

    prompt.append("Return JSON: {factors:{header,content,level,points,rationale}, totalPoints, gradeRange, finalGrade}. ");
    prompt.append("Only include factors with content. Enhance content only for clarity. Use FACTOR_POINTS to assign exact points. Include rationale and calculate totalPoints and finalGrade according to OPM.\n");
