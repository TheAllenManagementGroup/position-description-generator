...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, Object> payload) {
    try {
        ...
        System.out.println("=== REGENERATE FACTORS STARTED ===");
        
        // CRITICAL: Detect system from existing factor structure FIRST
        Map<String, String> factorHints = extractFactorHints(payload);
        boolean isSupervisory = detectSystemFromExistingFactors(factorHints, payload);
        
        String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory"));
        String gsGrade = String.valueOf(payload.getOrDefault("gsGrade", "GS-13"));
        
        // OVERRIDE detection if supervisoryLevel parameter is definitive
        if (supervisoryLevel != null && !supervisoryLevel.trim().isEmpty()) {
            boolean paramSupervisory = isSupervisoryPosition(supervisoryLevel);
            if (paramSupervisory != isSupervisory) {
                System.out.println("OVERRIDING detection based on supervisoryLevel parameter: " + supervisoryLevel);
                isSupervisory = paramSupervisory;
            }
        }
        
        System.out.println("System detected: " + (isSupervisory ? "GSSG SUPERVISORY (6 factors)" : "FES NON-SUPERVISORY (9 factors)"));
        System.out.println("Supervisory Level: " + supervisoryLevel);
        System.out.println("Target Grade: " + gsGrade);
        System.out.println("Existing factors: " + factorHints.keySet());

        // Convert factorHints to body format for regeneration
        Map<String, String> body = new HashMap<>();
        body.put("duties", duties);
        body.put("supervisoryLevel", supervisoryLevel);
        body.put("gsGrade", gsGrade);
        body.putAll(factorHints);

        // Route to correct regeneration with PROPER AI prompts
        if (isSupervisory) {
            return regenerateSupervisoryFactorsWithAI(body, duties, supervisoryLevel, gsGrade);
        } else {
            return regenerateNonSupervisoryFactorsWithAI(body, duties, supervisoryLevel, gsGrade);
        }
    } catch (Exception ex) {
        ex.printStackTrace();
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        System.out.println("=== UPDATE FACTOR POINTS WITH AI ===");
        
        // Extract duties and other context
        String duties = String.valueOf(payload.getOrDefault("duties", ""));
        String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory"));
        String expectedGrade = payload.get("expectedGrade") != null ? String.valueOf(payload.get("expectedGrade")).trim() : "GS-13";
        
        // Normalize incoming factor map
        Map<String, Object> rawFactors = extractRawFactorsMap(payload);
        Map<String, String> factorHints = extractFactorHintsFromRaw(rawFactors);

        // Detect which system (GSSG vs FES) with supervisoryLevel override
        Map<String, Object> detection = detectEvaluationSystemWithValidation(factorHints);
        boolean isSupervisory = (boolean) detection.getOrDefault("isSupervisory", false);

        // OVERRIDE with supervisoryLevel parameter if provided
        if (supervisoryLevel != null && !supervisoryLevel.trim().isEmpty()) {
            boolean paramSupervisory = isSupervisoryPosition(supervisoryLevel);
            if (paramSupervisory != isSupervisory) {
                System.out.println("OVERRIDING system detection based on supervisoryLevel: " + supervisoryLevel);
                isSupervisory = paramSupervisory;
            }
        }

        System.out.println("System detected: " + (isSupervisory ? "GSSG Supervisory" : "FES Non-Supervisory"));
        System.out.println("Target grade: " + expectedGrade);
        System.out.println("Duties available: " + !duties.isBlank());

        // Use AI to intelligently determine factor levels based on duties
        Map<String, Object> aiFactors;
        if (isSupervisory) {
            aiFactors = generateSupervisoryFactorsWithAI(duties, expectedGrade, supervisoryLevel);
        } else {
            aiFactors = generateNonSupervisoryFactorsWithAI(duties, expectedGrade, supervisoryLevel);
        }

        // Calculate totals
        int totalPoints = 0;
        for (Object factorData : aiFactors.values()) {
            if (factorData instanceof Map) {
                @SuppressWarnings("unchecked")
                Map<String, Object> factor = (Map<String, Object>) factorData;
                Object pts = factor.get("points");
                if (pts instanceof Number) {
                    totalPoints += ((Number) pts).intValue();
                }
            }
        }

        String finalGrade = isSupervisory ? expectedGrade : calculateFinalGrade(totalPoints);
        String gradeRange = getPointRangeForGrade(finalGrade);

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("success", true);
        result.put("factors", aiFactors);
        ...
        if (expectedGrade != null && !expectedGrade.isBlank()) {
            ...
            if (!expectedGrade.equalsIgnoreCase(finalGrade)) {
        ...
        err.put("error", "Failed to update factor points: " + ex.getMessage());
        return err;
    }
}
...
private Map<String, Object> generateSupervisoryFactorsWithAI(String duties, String targetGrade, String supervisoryLevel) throws Exception {
    ...
    String prompt = pdfContext + String.format("""
        ...
        For each factor, provide a substantive 3-4 sentence rationale that:
        - References specific supervisory duties
        - Explains scope, breadth, and organizational impact
        - Describes delegated authority and decision-making level
        - Explains why this level is appropriate and not adjacent levels
        
        DO NOT use generic placeholder text. Every rationale must be specific to the actual duties.
        
        Respond ONLY in JSON format:
        {
          "Factor 1": {"level": "1-X", "points": NNN, "rationale": "Substantive 3-4 sentences"},
          "Factor 2": {"level": "2-X", "points": NNN, "rationale": "Substantive 3-4 sentences"},
          "Factor 3": {"level": "3-X", "points": NNN, "rationale": "Substantive 3-4 sentences"},
          "Factor 4A": {"level": "4A-X", "points": NNN, "rationale": "Substantive 3-4 sentences"},
          "Factor 4B": {"level": "4B-X", "points": NNN, "rationale": "Substantive 3-4 sentences"},
          "Factor 5": {"level": "5-X", "points": NNN, "rationale": "Substantive 3-4 sentences"},
          "Factor 6": {"level": "6-X", "points": NNN, "rationale": "Substantive 3-4 sentences"}
        }
    ...
    // Validate and format factors with SUBSTANTIVE rationales
    ...
    for (String factorId : factorOrder) {
        ...
        if (factorData instanceof Map) {
            ...
            // CRITICAL: Generate substantive rationale if AI failed or provided generic text
            if (rationale == null || rationale.trim().isEmpty() || 
                rationale.contains("Default supervisory level") ||
                rationale.length() < 100) {
                System.out.println("Generating substantive rationale for supervisory " + key);
...
/**
 * FIXED: Detect evaluation system by checking factor TITLES first (corrected logic)
 */
private boolean detectSystemFromExistingFactors(Map<String, String> factorHints, Map<String, Object> payload) {
    ...
    System.out.println("  Factor hints: " + factorHints);
    
    // PRIORITY 1: Check factor TITLES/NAMES in the actual content
    Set<String> allContent = new HashSet<>();
    
    // Add all factorHints values (these contain the actual factor text)
    for (Map.Entry<String, String> entry : factorHints.entrySet()) {
        if (entry.getValue() != null && !entry.getValue().isBlank()) {
            allContent.add(entry.getValue().toLowerCase());
            System.out.println("  Checking content: " + entry.getKey() + " = " + entry.getValue().substring(0, Math.min(100, entry.getValue().length())));
        }
    }
    
    // Also check payload string values
    collectAllStringValues(payload, allContent);
    
    // GSSG-specific titles (ONLY in supervisory evaluations)
    boolean hasGssgTitle = false;
    String[] gssgTitles = {
    ...
    // FES-specific titles (ONLY in non-supervisory evaluations)
    boolean hasFesTitle = false;
    String[] fesTitles = {
    ...
    for (String content : allContent) {
        for (String gssgTitle : gssgTitles) {
            if (content.contains(gssgTitle)) {
                hasGssgTitle = true;
                System.out.println("  ✓ Found GSSG title: '" + gssgTitle + "'");
                break;
            }
        }
        
        for (String fesTitle : fesTitles) {
            if (content.contains(fesTitle)) {
                hasFesTitle = true;
                System.out.println("  ✓ Found FES title: '" + fesTitle + "'");
                break;
            }
        }
    }
    
    // If we found GSSG-specific titles, it's GSSG
    if (hasGssgTitle && !hasFesTitle) {
        System.out.println("  → GSSG SUPERVISORY (found GSSG titles)");
        return true;
    }
    
    // If we found FES-specific titles, it's FES
    if (hasFesTitle && !hasGssgTitle) {
        System.out.println("  → FES NON-SUPERVISORY (found FES titles)");
        return false;
    }
    
    // PRIORITY 2: Check for Factor 4A/4B (GSSG only) or 7/8/9 (FES only)
    boolean has4A = false;
    boolean has4B = false;
    boolean has7 = false;
    boolean has8 = false;
    ...
    allKeys.addAll(factorHints.keySet());
    collectAllKeys(payload, allKeys);
    
    for (String key : allKeys) {
        String k = key.trim().toUpperCase();
        if (k.contains("4A") || k.equals("FACTOR 4A")) has4A = true;
        if (k.contains("4B") || k.equals("FACTOR 4B")) has4B = true;
        if (k.matches(".*\\b7\\b.*") || k.equals("FACTOR 7")) has7 = true;
        if (k.matches(".*\\b8\\b.*") || k.equals("FACTOR 8")) has8 = true;
        if (k.matches(".*\\b9\\b.*") || k.equals("FACTOR 9")) has9 = true;
    }
    
    System.out.println("  Keys: 4A=" + has4A + ", 4B=" + has4B + ", 7=" + has7 + ", 8=" + has8 + ", 9=" + has9);
    
    if (has4A || has4B) {
        System.out.println("  → GSSG SUPERVISORY (found 4A/4B)");
        return true;
    }
    
    if (has7 || has8 || has9) {
        System.out.println("  → FES NON-SUPERVISORY (found 7/8/9)");
        return false;
    }
    
    // PRIORITY 3: Check point values (GSSG has different scale than FES)
    for (String content : allContent) {
        // GSSG Factor 1 unique values: 175, 775, 900
        if (content.contains("175 points") || content.contains("775 points") || content.contains("900 points")) {
            System.out.println("  → GSSG SUPERVISORY (found GSSG Factor 1 points)");
            return true;
        }
        
        // FES Factor 1 unique high values: 1250, 1550, 1850
        if (content.contains("1250 points") || content.contains("1550 points") || content.contains("1850 points")) {
            System.out.println("  → FES NON-SUPERVISORY (found FES Factor 1 points)");
            return false;
        }
    }
    
    // PRIORITY 4: Count factors
    int factorCount = 0;
    for (String key : allKeys) {
        if (key.toUpperCase().matches(".*FACTOR\\s*\\d+.*")) {
            factorCount++;
        }
    }
    ...
    if (factorCount >= 8) {
        System.out.println("  → FES NON-SUPERVISORY (8-9 factors)");
        return false;
    } else if (factorCount >= 5 && factorCount <= 7) {
        System.out.println("  → GSSG SUPERVISORY (5-7 factors)");
        return true;
    }
    
    // FALLBACK: Check supervisoryLevel parameter
    String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory"));
    boolean isSupervisory = isSupervisoryPosition(supervisoryLevel);
    System.out.println("  → FALLBACK to supervisoryLevel: " + supervisoryLevel + " = " + (isSupervisory ? "GSSG" : "FES"));
    return isSupervisory;
}

/**
 * IMPROVED: Regenerate GSSG supervisory factors with FULL substantive content
 */
private Map<String, Object> regenerateSupervisoryFactorsWithAI(Map<String, String> body, 
                                                                String duties, 
                                                                String supervisoryLevel,
                                                                String gsGrade) throws Exception {
    System.out.println("=== REGENERATING GSSG SUPERVISORY FACTORS (AI) ===");
    ...
    String prompt = pdfContext + String.format("""
        ...
        You are an expert OPM HR specialist using the GSSG (General Schedule Supervisory Guide) 6-factor system.
        
        CRITICAL INSTRUCTIONS:
        - Use ONLY these exact factor names and ranges:
        ...
        - Total points MUST be between %d and %d for %s
        - Each rationale MUST be 4-6 FULL substantive sentences that:
          * Quote or reference specific supervisory duties from the duties provided
          * Explain the scope, breadth, and organizational impact of supervision
          * Describe the level of delegated authority and decision-making
          * Explain why this level is appropriate (not the level below)
          * Explain why it does NOT meet the next higher level
          * Use professional HR classification language
        
        DO NOT write generic statements. DO NOT use placeholders.
        Every rationale must be specific to the actual duties provided.
        
        Target Grade: %s (%d-%d points required)
        Supervisory Level: %s
        ...
        Respond ONLY in JSON format:
        {
          "Factor 1": {
            "level": "1-X",
            "points": NNN,
            "rationale": "The position requires [specific knowledge]. The duties demonstrate [specific supervisory scope]. This exceeds level X-Y because [specific reason]. It does not meet level X-Z because [specific missing element]."
          },
          "Factor 2": {...},
          "Factor 3": {...},
          "Factor 4A": {...},
        ...
        Supervisory Duties:
        %s
        """, minPoints, maxPoints, gsGrade, gsGrade, minPoints, maxPoints, supervisoryLevel, duties);
    
    String response = callOpenAIWithTimeout(prompt, 90);
    String jsonResponse = extractJsonFromResponse(response);
    ...
    // Process and validate all 7 GSSG factors
    int totalPoints = 0;
    Map<String, Object> validatedFactors = new LinkedHashMap<>();
    ...
    for (String factorId : factorOrder) {
        ...
        // Fallback to intelligent defaults if AI failed
        if (level == null || level.isBlank()) {
            level = getIntelligentGssgLevel(Integer.parseInt(factorId.replaceAll("[^0-9]", "")), duties, gsGrade);
            System.out.println("  Using intelligent level for " + key + ": " + level);
        }
        
        level = normalizeGssgLevel(factorId, level);
        
        if (points == 0) {
            points = getGssgPointsForLevel(factorId, level);
        }
        
        // Ensure substantive rationale (minimum 200 chars)
        if (rationale == null || rationale.trim().isEmpty() || rationale.length() < 200) {
            System.out.println("  Generating detailed rationale for " + key);
            rationale = buildGssgRationale(
                Integer.parseInt(factorId.replaceAll("[^0-9]", "")), 
                level, duties, 
                factorId.contains("A") || factorId.contains("B") ? factorId : null
        ...
        System.out.println("  " + key + ": Level " + level + " = " + points + " pts (" + rationale.length() + " chars)");
    }
    
    // Adjust if outside grade range
    if (totalPoints < minPoints || totalPoints > maxPoints) {
        System.out.println("WARNING: Adjusting from " + totalPoints + " to fit " + minPoints + "-" + maxPoints);
    ...
    System.out.println("=== GSSG REGENERATION COMPLETE ===");
    System.out.println("Total: " + totalPoints + " points = " + gsGrade + " (" + gradeRange + ")");
...
/**
 * IMPROVED: Regenerate FES non-supervisory factors with FULL substantive content
 */
private Map<String, Object> regenerateNonSupervisoryFactorsWithAI(Map<String, String> body, 
                                                                   String duties, 
                                                                   String supervisoryLevel,
                                                                   String gsGrade) throws Exception {
    System.out.println("=== REGENERATING FES NON-SUPERVISORY FACTORS (AI) ===");
    ...
    String prompt = pdfContext + String.format("""
        ...
        You are an expert OPM HR specialist using the FES (Factor Evaluation System) 9-factor system.
        ...
        - Each rationale MUST be 4-6 FULL substantive sentences that:
          * Quote or reference specific duties from the duties provided
          * Explain the technical complexity and analytical requirements
          * Describe the level of independence and decision-making authority
          * Explain why this level is appropriate (not the level below)
          * Explain why it does NOT meet the next higher level
          * Use professional HR classification language
        ...
        Respond ONLY in JSON format:
        {
          "Factor 1": {
            "level": "1-X",
            "points": NNN,
            "rationale": "The position requires [specific knowledge]. The duties demonstrate [specific complexity]. This exceeds level X-Y because [specific reason]. It does not meet level X-Z because [specific missing element]."
          },
          "Factor 2": {...},
    ...
    // Process and validate all 9 FES factors
    int totalPoints = 0;
    Map<String, Object> validatedFactors = new LinkedHashMap<>();
    ...
    for (int i = 1; i <= 9; i++) {
        ...
        if (points == 0) {
            points = getPointsForLevel(factorId, level);
        }
    ...
    System.out.println("=== FES REGENERATION COMPLETE ===");
    System.out.println("Total: " + totalPoints + " points = " + finalGrade + " (" + gradeRange + ")");
...
/**
 * Helper: Collect all string values recursively
 */
private void collectAllStringValues(Map<String, Object> map, Set<String> values) {
    if (map == null) return;
    ...
    for (Map.Entry<String, Object> entry : map.entrySet()) {
        ...
        if (val instanceof String) {
            ...
            collectAllStringValues(nested, values);
        }
    }
}

/**
 * Helper: Recursively collect all keys from nested maps
 */
private void collectAllKeys(Map<String, Object> map, Set<String> keys) {
    if (map == null) return;
