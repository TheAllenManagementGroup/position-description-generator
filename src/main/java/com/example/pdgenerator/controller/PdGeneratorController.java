...
private boolean determineLOCKEDSystem(Map<String, String> existingFactors) {
    ...
    // GSSG supervisory indicators (check titles, not just keys)
    if (combined.contains("PROGRAM SCOPE AND EFFECT") ||
        ...
        System.out.println("LOCKED: GSSG (found supervisory titles)");
        return true;
    }
    ...
    if (combined.contains("KNOWLEDGE REQUIRED BY THE POSITION") ||
        ...
        System.out.println("LOCKED: FES (found non-supervisory titles)");
        return false;
    }
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        ...
        System.out.println("Payload keys: " + payload.keySet());
        System.out.println("Payload content: " + payload.toString());
        
        // The payload likely comes directly as factor keys, not nested
        Map<String, String> bodyAsStrings = new HashMap<>();
        payload.forEach((k, v) -> bodyAsStrings.put(k, v == null ? "" : v.toString()));
        
        Map<String, String> existingFactors = bodyAsStrings; // Use payload directly
        
        if (existingFactors.isEmpty()) {
            return Map.of("success", false, "error", "No factors found in payload");
        }
        
        boolean isSupervisory = determineLOCKEDSystem(existingFactors);
        String duties = bodyAsStrings.getOrDefault("duties", "");
        String targetGrade = bodyAsStrings.getOrDefault("expectedGrade", 
                            bodyAsStrings.getOrDefault("gsGrade", "GS-13"));
        
        // Route based on locked system
        if (isSupervisory) {
            return updateGSSGFactorPointsLocked(existingFactors, payload, duties, targetGrade, null);
        } else {
            return updateFESFactorPointsLocked(existingFactors, payload, duties, targetGrade, null);
        }
    } catch (Exception ex) {
