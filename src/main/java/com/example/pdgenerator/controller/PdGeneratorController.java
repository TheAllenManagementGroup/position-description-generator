...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> body) throws Exception {
    ...
    if (factors == null || factors.isEmpty()) return Map.of("error", "No factors provided");

    StringBuilder prompt = new StringBuilder();
    prompt.append("You are an OPM HR expert. Evaluate each factor content using FES. ");
    prompt.append("Assign correct level and points according to OPM charts and two-grade GS system (GS-5 to GS-15). ");
    prompt.append("Factor levels must match numbers (1-1…1-9, 2-1…2-5, etc.).\n");

    // Compact factor points
    prompt.append("POINTS={1:{1-1:200,1-2:350,1-3:500,1-4:750,1-5:950,1-6:1250,1-7:1550,1-8:1850,1-9:2210},");
    prompt.append("2:{2-1:25,2-2:125,2-3:275,2-4:450,2-5:650},3:{3-1:25,3-2:125,3-3:275,3-4:450,3-5:650},");
    prompt.append("4:{4-1:25,4-2:75,4-3:150,4-4:225,4-5:325,4-6:450},5:{5-1:25,5-2:75,5-3:150,5-4:225,5-5:325,5-6:450},");
    prompt.append("6:{6-1:10,6-2:25,6-3:60,6-4:110},7:{7-1:20,7-2:50,7-3:120,7-4:220},8:{8-1:5,8-2:20,8-3:50},9:{9-1:5,9-2:20,9-3:50}}\n");

    // Merge guidelines
    prompt.append("GUIDELINES={1:'knowledge',2:'supervision',3:'guidelines clarity',4:'complexity',5:'scope & impact',6:'contacts',7:'contact purpose',8:'physical',9:'environment'}\n");

    // Only include factors with content
    prompt.append("CONTENT={");
    factors.forEach((k,v) -> {
        if (!v.isBlank()) prompt.append(k.replace("Factor ","")+":\""+v.replace("\"","'")+"\",");
    });
    prompt.append("}\n");

    prompt.append("Return JSON: {factors:{header,content,level,points,rationale},totalPoints,gradeRange,finalGrade}. Only include factors with content. Use POINTS to assign exact points.");

    // Call AI
    String response = callOpenAIWithTimeout(prompt.toString(), 60); // shorter timeout
