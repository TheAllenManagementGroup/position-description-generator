...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, String> body) {
    try {
        ...
        // Use robust structure-based detection
        boolean isSupervisory = detectSupervisoryByFactorStructure(body);
        
        System.out.println("Detected evaluation type: " + (isSupervisory ? "SUPERVISORY (GSSG)" : "NON-SUPERVISORY (FES)"));
        System.out.println("Supervisory Level parameter: " + supervisoryLevel);
        
        if (isSupervisory) {
            return regenerateSupervisoryFactors(body, duties, supervisoryLevel);
        } else {
            return regenerateNonSupervisoryFactors(body, duties, supervisoryLevel);
        }
...
private int processSupervisoryFactor(Map<String, Object> factors, Map<String, Object> validatedFactors, 
    ...
    if (value instanceof Map) {
        ...
        // Get correct GSSG factor name
        String gssgFactorName = getGssgFactorName(factorNum);

        Map<String, Object> validatedFactor = new LinkedHashMap<>();
        validatedFactor.put("header", factorKey + " – " + gssgFactorName + " Level " + level + ", " + points + " Points");
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> factors) {
    ...
    // CRITICAL: Use structure-based detection, NOT supervisory level parameter
    boolean isSupervisory = detectSupervisoryByFactorStructure(factors);
    
    System.out.println("=== UPDATE FACTOR POINTS ===");
    System.out.println("Detected evaluation type: " + (isSupervisory ? "SUPERVISORY (GSSG)" : "NON-SUPERVISORY (FES)"));
    System.out.println("Supervisory Level parameter: " + supervisoryLevel);
    System.out.println("Expected Grade: " + expectedGrade);
    System.out.println("Factors received: " + factors.keySet());
...
/**
 * Detect supervisory evaluation by checking factor structure
 * This works for both Map<String, String> and Map<String, Object>
 */
private boolean detectSupervisoryByFactorStructure(Map<?, ?> factors) {
    // Check for definitive markers
    boolean has4A = factors.containsKey("Factor 4A") || factors.containsKey("Factor 4A_originalLevel");
    boolean has4B = factors.containsKey("Factor 4B") || factors.containsKey("Factor 4B_originalLevel");
    boolean has7 = factors.containsKey("Factor 7") || factors.containsKey("Factor 7_originalLevel");
    boolean has8 = factors.containsKey("Factor 8") || factors.containsKey("Factor 8_originalLevel");
    boolean has9 = factors.containsKey("Factor 9") || factors.containsKey("Factor 9_originalLevel");
    
    System.out.println("Factor presence: 4A=" + has4A + ", 4B=" + has4B + ", 7=" + has7 + ", 8=" + has8 + ", 9=" + has9);
    
    if (has4A || has4B) {
        System.out.println("DEFINITIVE: Factor 4A/4B present → GSSG (Supervisory)");
        return true;
    }
    
    if (has7 || has8 || has9) {
        System.out.println("DEFINITIVE: Factor 7/8/9 present → FES (Non-Supervisory)");
        return false;
    }
    
    // Count factor numbers
    Set<Integer> factorNumbers = new HashSet<>();
    for (Object key : factors.keySet()) {
        String keyStr = key.toString();
        if (keyStr.matches("Factor \\d+") && !keyStr.contains("_originalLevel")) {
            try {
                int num = Integer.parseInt(keyStr.replaceAll("[^0-9]", ""));
                factorNumbers.add(num);
    ...
    int maxFactorNum = factorNumbers.isEmpty() ? 0 : Collections.max(factorNumbers);
    System.out.println("Max factor number: " + maxFactorNum + ", Total factors: " + factorNumbers.size());
    
    if (maxFactorNum >= 7) {
        System.out.println("CONCLUSION: Max factor >= 7 → FES (Non-Supervisory)");
        return false;
    }
    
    if (maxFactorNum <= 6 && factorNumbers.size() <= 6) {
        System.out.println("CONCLUSION: Max factor <= 6 → GSSG (Supervisory)");
        return true;
    }
    
    System.out.println("CONCLUSION: Defaulting to FES (Non-Supervisory)");
    return false;
}
...
private Map<String, Object> updateSupervisoryFactorPoints(Map<String, Object> factors, 
    ...
    try {
        ...
        for (int i = 1; i <= 6; i++) {
            if (i == 4) {
                extractOriginalLevel(factors, "4A", originalLevels);
                extractOriginalLevel(factors, "4B", originalLevels);
            } else {
                extractOriginalLevel(factors, String.valueOf(i), originalLevels);
            }
        }

        String pdfContext = getAutoPdfContext();
        String prompt = buildSupervisoryUpdatePrompt(pdfContext, duties, gsGrade, supervisoryLevel);
        
        String response = callOpenAIWithTimeout(prompt, 25);
        String jsonResponse = extractJsonFromResponse(response);
        
        ObjectMapper mapper = new ObjectMapper();
        Map<String, Map<String, String>> aiLevels = mapper.readValue(jsonResponse, 
        ...
        for (int i = 1; i <= 6; i++) {
            if (i == 4) {
                totalPoints += processSupervisoryFactorUpdate(aiLevels, finalFactors, factors, 
                    "4A", "Factor 4A", originalLevels.get("4A"));
                totalPoints += processSupervisoryFactorUpdate(aiLevels, finalFactors, factors, 
                    "4B", "Factor 4B", originalLevels.get("4B"));
            } else {
                String factorNum = String.valueOf(i);
                totalPoints += processSupervisoryFactorUpdate(aiLevels, finalFactors, factors, 
                    factorNum, "Factor " + i, originalLevels.get(factorNum));
            }
        }
        ...
        // Grade alignment if needed
        int minPoints = getMinPointsForGrade(finalGrade);
        int maxPoints = getMaxPointsForGrade(finalGrade);
        if (totalPoints < minPoints || totalPoints > maxPoints) {
            totalPoints = adjustSupervisoryPointsForGrade(totalPoints, finalFactors, gsGrade);
...
private Map<String, Object> updateNonSupervisoryFactorPoints(Map<String, Object> factors, 
                                                             String expectedGrade, 
                                                             String supervisoryLevel) {
    // Use existing 9-factor FES logic
    ...
    try {
        ...
        String pdfContext = getAutoPdfContext();
        String prompt = buildNonSupervisoryUpdatePrompt(pdfContext, duties, expectedGrade, supervisoryLevel);
        ...
        for (int i = 1; i <= 9; i++) {
            ...
            totalPoints += processNonSupervisoryFactorUpdate(aiLevels, finalFactors, factors, 
                factorNum, factorKey, originalLevels.get(factorNum), FACTOR_POINTS);
        }
        ...
        if (totalPoints < minPoints || totalPoints > maxPoints) {
            ...
            for (int i = 1; i <= 9; i++) {
                ...
                if (f != null) {
                    eval.append("Factor ").append(i).append(" Level ").append(f.get("level"))
                        .append(", ").append(f.get("points")).append(" Points\n");
                }
            }
            String aligned = enforceExactGradeAlignment(eval.toString(), finalGrade, minPoints, maxPoints);
            ...
            while (m.find()) {
                ...
                if (f != null) {
                    f.put("level", level);
                    f.put("points", points);
                    f.put("header", "Factor " + factorNum + " – " + getFesFactorName(factorNum) + 
...
private int processSupervisoryFactorUpdate(Map<String, Map<String, String>> aiLevels, 
    ...
    if (levelData == null) {
        System.out.println("WARNING: No AI data for " + factorKey);
        return 0;
    }
    ...
    System.out.println("GSSG " + factorKey + " (" + gssgFactorName + "): Level=" + correctedLevel + ", Points=" + points);
    
    Map<String, Object> factorResult = new LinkedHashMap<>();
    factorResult.put("header", factorKey + " – " + gssgFactorName + " Level " + correctedLevel + ", " + points + " Points");
...
private int processNonSupervisoryFactorUpdate(Map<String, Map<String, String>> aiLevels,
    ...
    if (points == null) {
        System.out.println("WARNING: Invalid level " + correctedLevel + " for factor " + factorNum);
        correctedLevel = findClosestValidLevel(factorNum, correctedLevel, factorPointMap);
        points = factorPointMap.get(correctedLevel);
    }
    
    // Get correct FES factor name
    String fesFactorName = getFesFactorName(factorNum);
    
    System.out.println("FES " + factorKey + " (" + fesFactorName + "): Level=" + correctedLevel + ", Points=" + points);
    
    Map<String, Object> factorResult = new LinkedHashMap<>();
    factorResult.put("header", factorKey + " – " + fesFactorName + " Level " + correctedLevel + ", " + points + " Points");
    factorResult.put("content", factors.getOrDefault(factorKey, ""));
    factorResult.put("level", correctedLevel);
    factorResult.put("points", points);
    factorResult.put("rationale", levelData.getOrDefault("rationale", ""));
...
/**
 * Get the correct GSSG (supervisory) factor name
 */
private String getGssgFactorName(String factorNum) {
    switch (factorNum) {
        case "1":
            return "PROGRAM SCOPE AND EFFECT";
        case "2":
            return "ORGANIZATIONAL SETTING";
        case "3":
            return "SUPERVISORY & MANAGERIAL AUTHORITY EXERCISED";
        case "4":
        case "4A":
            return "PERSONAL CONTACTS (NATURE OF CONTACTS)";
        case "4B":
            return "PERSONAL CONTACTS (PURPOSE OF CONTACTS)";
        case "5":
            return "DIFFICULTY OF TYPICAL WORK DIRECTED";
        case "6":
            return "OTHER CONDITIONS";
        default:
            return "Factor " + factorNum;
    }
}

/**
 * Get the correct FES (non-supervisory) factor name
 */
private String getFesFactorName(String factorNum) {
    switch (factorNum) {
        case "1":
            return "Knowledge Required by the Position";
        case "2":
            return "Supervisory Controls";
        case "3":
            return "Guidelines";
        case "4":
            return "Complexity";
        case "5":
            return "Scope and Effect";
        case "6":
            return "Personal Contacts";
        case "7":
            return "Purpose of Contacts";
        case "8":
            return "Physical Demands";
        case "9":
