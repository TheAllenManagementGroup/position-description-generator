...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, Object> payload) {
    try {
        ...
        // CRITICAL FIX: Extract existing factor structure to determine LOCKED system
        Map<String, String> existingFactors = extractExistingFactorStructure(payload);
        
        // STEP 1: LOCK THE SYSTEM based on existing factors
        boolean isSupervisory = determineLOCKEDSystem(existingFactors);
        
        // OVERRIDE: Check if client explicitly sent systemLocked
        String clientSystemLock = (String) payload.get("systemLocked");
        if (clientSystemLock != null) {
            boolean clientIsSupervisory = "GSSG".equalsIgnoreCase(clientSystemLock);
            if (clientIsSupervisory != isSupervisory) {
                System.out.println("WARNING: Client system lock differs from detected system. Client: " + clientSystemLock + ", Detected: " + (isSupervisory ? "GSSG" : "FES"));
                isSupervisory = clientIsSupervisory; // Honor client's explicit lock
            }
        }
        ...
        // STEP 2: Get target grade
        String targetGrade = String.valueOf(payload.getOrDefault("gsGrade", 
                            payload.getOrDefault("expectedGrade", "GS-13")));
        
        // Get supervisoryLevel from payload or infer from system
        String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", 
            isSupervisory ? "Supervisor" : "Non-Supervisory"));
        
        // STEP 3: Route to correct regeneration method that PRESERVES the system
        if (isSupervisory) {
            return regenerateGSSGFactors(existingFactors, duties, targetGrade, supervisoryLevel);
        } else {
...
private String extractDutiesFromPayload(Map<String, Object> payload) {
    try {
        String d = String.valueOf(payload.getOrDefault("duties", "")).trim();
        ...
        return combineMajorDuties(payload);
    } catch (Exception e) {
        return "";
    }
}
...
private String combineMajorDuties(Map<String, Object> sections) {
    ...
    List<Map.Entry<String, Object>> numbered = sections.entrySet().stream()
        .filter(e -> e.getKey() != null && e.getValue() != null && e.getKey().trim().matches("(?i)^MAJOR DUTY\\s*\\d+$"))
        .sorted((a,b) -> {
        ...
        .toList();
    if (!numbered.isEmpty()) {
        return numbered.stream().map(e -> String.valueOf(e.getValue())).collect(Collectors.joining("\n"));
    }
    // Fallback to single "MAJOR DUTIES"
    for (Map.Entry<String, Object> e : sections.entrySet()) {
        if (e.getKey() != null && e.getValue() != null && e.getKey().trim().equalsIgnoreCase("MAJOR DUTIES")) {
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        ...
        // Respect explicit client lock if sent
        ...
        // Duties: use explicit or extract from payload (prevents "the duties provided")
...
private String getGssgGradeRange(int totalPoints) {
    // GSSG point ranges for each grade
    if (totalPoints >= 2755) return "2755+";
    if (totalPoints >= 2350) return "2350-2754";
    if (totalPoints >= 1855) return "1855-2349";
    if (totalPoints >= 1605) return "1605-1854";
    if (totalPoints >= 1350) return "1350-1604";
    if (totalPoints >= 1100) return "1100-1349";
    if (totalPoints >= 855) return "855-1099";
    return "Below 855";
}

// Update updateSummarySections calls to properly format:
