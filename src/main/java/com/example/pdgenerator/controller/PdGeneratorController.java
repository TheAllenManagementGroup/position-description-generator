...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, Object> payload) {
    try {
        ...
        if (paramSupervisory != isSupervisory) isSupervisory = paramSupervisory;
...
private boolean detectSystemFromExistingFactors(Map<String, String> factorHints, Map<String, Object> payload) {
    // Check keys and values for GSSG/FES indicators
    boolean hasGssgTitle = false, hasFesTitle = false;
    for (String val : factorHints.values()) {
        String v = val.toLowerCase();
        if (v.contains("program scope and effect") || v.contains("organizational setting") ||
            v.contains("supervisory & managerial authority") || v.contains("difficulty of typical work directed") ||
            v.contains("personal contacts (nature") || v.contains("personal contacts (purpose)")) {
            hasGssgTitle = true;
        }
        if (v.contains("knowledge required by the position") || v.contains("supervisory controls") ||
            v.contains("guidelines") || v.contains("complexity") || v.contains("scope and effect") ||
            v.contains("purpose of contacts") || v.contains("physical demands") || v.contains("work environment")) {
            hasFesTitle = true;
        }
    }
    // Factor keys
    Set<String> keys = factorHints.keySet();
    if (keys.stream().anyMatch(k -> k.contains("4A") || k.contains("4B"))) return true; // GSSG
    if (keys.stream().anyMatch(k -> k.contains("7") || k.contains("8") || k.contains("9"))) return false; // FES
    // Titles
    if (hasGssgTitle && !hasFesTitle) return true;
    if (hasFesTitle && !hasGssgTitle) return false;
    // Fallback: use supervisoryLevel
    String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory"));
    return isSupervisoryPosition(supervisoryLevel);
}
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        ...
        // Improved system detection: check factor keys/values and use param override
        boolean isSupervisory = detectSystemFromExistingFactors(factorHints, payload);
        boolean paramSupervisory = isSupervisoryPosition(supervisoryLevel);
        if (paramSupervisory != isSupervisory) isSupervisory = paramSupervisory;
        ...
        // Post-process rationales for substance
        for (Object factorObj : aiFactors.values()) {
            if (factorObj instanceof Map) {
                ...
                // Enforce minimum substantive rationale length; fallback to deterministic if too short
                if (rationale == null || rationale.trim().length() < 150) {
                    int num = 1;
                    String levelStr = (String)factor.get("level");
                    if (levelStr != null) {
                        try {
                            num = Integer.parseInt(levelStr.replaceAll("[^0-9]", ""));
                        } catch (Exception e) { num = 1; }
                    }
                    factor.put("rationale", generateSubstantiveRationale(num, levelStr, duties));
                }
            }
        }
