...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        ...
        // Extract factor structure from payload
        Map<String, String> existingFactors = extractExistingFactorStructure(payload);
        
        // CRITICAL: Determine locked system from existing factor keys
        boolean isSupervisory = determineLOCKEDSystem(existingFactors);
        
        // Check for explicit system lock signal from client
        String systemLocked = (String) payload.get("systemLocked");
        if (systemLocked != null) {
            boolean clientSystem = "GSSG".equalsIgnoreCase(systemLocked);
            if (clientSystem != isSupervisory) {
                System.out.println("WARNING: Client system (" + systemLocked + ") differs from detected system (" + 
                    (isSupervisory ? "GSSG" : "FES") + ") - using client system");
                isSupervisory = clientSystem;
            }
        }
        
        System.out.println("SYSTEM LOCKED: " + (isSupervisory ? "SUPERVISORY (GSSG)" : "NON-SUPERVISORY (FES)"));
