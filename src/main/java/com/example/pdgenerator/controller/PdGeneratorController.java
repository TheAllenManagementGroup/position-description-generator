...
/**
 * Detect which evaluation system is in use from existing factor headers.
 */
private Map<String, Object> detectEvaluationSystemWithValidation(Map<String, String> factorHints) {
    Map<String, Object> result = new LinkedHashMap<>();
    
    if (factorHints == null || factorHints.isEmpty()) {
        result.put("isSupervisory", false);
        result.put("reason", "Defaulting to FES - no factor hints provided");
        return result;
    }
    
    // Check for GSSG indicators (Factor 4A, 4B)
    boolean has4A = factorHints.keySet().stream().anyMatch(k -> k.matches("(?i)Factor\\s*4A"));
    boolean has4B = factorHints.keySet().stream().anyMatch(k -> k.matches("(?i)Factor\\s*4B"));
    
    // Check for FES indicators (Factor 7, 8, 9)
    boolean has7 = factorHints.keySet().stream().anyMatch(k -> k.matches("(?i)Factor\\s*7"));
    boolean has8 = factorHints.keySet().stream().anyMatch(k -> k.matches("(?i)Factor\\s*8"));
    boolean has9 = factorHints.keySet().stream().anyMatch(k -> k.matches("(?i)Factor\\s*9"));
    
    if (has4A || has4B) {
        result.put("isSupervisory", true);
        result.put("reason", "GSSG detected - has Factor 4A or 4B");
        return result;
    }
    
    if (has7 || has8 || has9) {
        result.put("isSupervisory", false);
        result.put("reason", "FES detected - has Factor 7, 8, or 9");
        return result;
    }
    
    // Count total factors
    long factorCount = factorHints.keySet().stream()
        .filter(k -> k.matches("(?i)Factor\\s*\\d+[A-Za-z]?"))
        .count();
    
    boolean isSupervisory = factorCount <= 7;
    result.put("isSupervisory", isSupervisory);
    result.put("reason", isSupervisory ? 
        "GSSG inferred - 7 or fewer factors" : 
        "FES inferred - 8 or more factors");
...
/**
 * Extract current factor data from payload for processing.
 */
