...
private boolean determineLockedSystemFromCurrentFactors(Map<String, Object> body) {
    // Check what factor types already exist - this is the LOCKED system
    boolean has4A = body.keySet().stream().anyMatch(k -> k.contains("4A"));
    boolean has4B = body.keySet().stream().anyMatch(k -> k.contains("4B"));
    boolean has7 = body.keySet().stream().anyMatch(k -> k.equals("Factor 7"));
    boolean has8 = body.keySet().stream().anyMatch(k -> k.equals("Factor 8"));
    ...
    // Definitive: if we have 4A/4B, we are IN supervisory mode (GSSG)
    if (has4A || has4B) {
        System.out.println("LOCKED SYSTEM: SUPERVISORY (GSSG) - detected from existing Factor 4A/4B");
        return true;
    }
    
    // Definitive: if we have 7/8/9, we are IN non-supervisory mode (FES)
    if (has7 || has8 || has9) {
        System.out.println("LOCKED SYSTEM: NON-SUPERVISORY (FES) - detected from existing Factors 7/8/9");
        return false;
    }
    
    // If we only have factors 1-6, assume supervisory (more likely for locked state)
    System.out.println("LOCKED SYSTEM: Ambiguous - defaulting to SUPERVISORY (Factors 1-6 only)");
    return true;
}
