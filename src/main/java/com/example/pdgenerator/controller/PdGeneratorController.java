...
/**
 * IMPROVED: Call this FIRST in regenerateFactors to properly detect and validate
 */
...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, String> body) {
    try {
        ...
        // STEP 1: Detect system with validation
        Map<String, Object> detection = detectEvaluationSystemWithValidation(body);
        boolean isSupervisory = (boolean) detection.get("isSupervisory");
        
        // STEP 2: Validate factor structure matches detected system
        if (!validateFactorStructure(body, isSupervisory)) {
            return Map.of(
                "error", "Factor structure does not match " + 
                        (isSupervisory ? "supervisory (GSSG)" : "non-supervisory (FES)") + " system",
                "success", false,
                "detection", detection
            );
        }
        
        // STEP 3: Extract and validate levels according to correct system
        Map<String, String> factorLevels = extractAndValidateFactorLevels(body, isSupervisory);
        
        System.out.println("Extracted and validated factor levels: " + factorLevels);
        System.out.println("Processing as: " + (isSupervisory ? "SUPERVISORY (GSSG)" : "NON-SUPERVISORY (FES)"));
        
        // STEP 4: Route to correct regeneration method
        String supervisoryLevel = body.getOrDefault("supervisoryLevel", "Non-Supervisory");
