...
private String getLockedFactorTitle(String factorId, boolean isSupervisory) {
    if (isSupervisory) {
        // GSSG Supervisory factor titles (LOCKED)
        switch (factorId) {
            case "1": return "PROGRAM SCOPE AND EFFECT";
            case "2": return "ORGANIZATIONAL SETTING";
            case "3": return "SUPERVISORY & MANAGERIAL AUTHORITY EXERCISED";
            case "4A": return "PERSONAL CONTACTS (NATURE OF CONTACTS)";
            case "4B": return "PERSONAL CONTACTS (PURPOSE OF CONTACTS)";
            case "5": return "DIFFICULTY OF TYPICAL WORK DIRECTED";
            case "6": return "OTHER CONDITIONS";
            default: return "Factor " + factorId;
        }
    } else {
        // FES Non-Supervisory factor titles (LOCKED)
        switch (factorId) {
...
/**
 * Build factor header with LOCKED title and variable level/points
 */
private String buildLockedFactorHeader(String factorId, String level, int points, boolean isSupervisory) {
    String title = getLockedFactorTitle(factorId, isSupervisory);
    return String.format("FACTOR %s - %s Level %s, %d Points", factorId, title, level, points);
}

/**
 * Update regenerateGSSGFactors to use locked titles
 */
private Map<String, Object> regenerateGSSGFactorsLocked(Map<String, String> existingFactors, 
                                                         String duties, String targetGrade) throws Exception {
    System.out.println("Regenerating GSSG factors with LOCKED titles for grade: " + targetGrade);
    
    int minPoints = getMinPointsForGrade(targetGrade);
    int maxPoints = getMaxPointsForGrade(targetGrade);
    int targetMid = (minPoints + maxPoints) / 2;
    
    // Get AI suggestions for levels only
    String pdfContext = getAutoPdfContext();
    String prompt = pdfContext + String.format("""
        You are an OPM classification expert using the GSSG 6-factor supervisory system.
        
        Target Grade: %s (%d-%d points)
        
        Analyze these supervisory duties and assign appropriate levels for ALL 7 GSSG factors:
        ...
        Respond ONLY in JSON format with levels only:
        {
        ...
        Duties: %s
        """, targetGrade, minPoints, maxPoints, duties);
    
    String aiResponse = callOpenAIWithTimeout(prompt, 45);
    ...
    // Build validated factors with LOCKED titles
    ...
    for (String factorId : factorOrder) {
        ...
        try {
            ...
            if (!numStr.isEmpty()) fidNum = Integer.parseInt(numStr);
        } catch (Exception ignored) {}
        if (level == null) {
        ...
        // USE LOCKED TITLE - only level and points change
    ...
    // Adjust to grade range if needed (titles remain locked)
    ...
    System.out.println("GSSG regeneration complete with locked titles: " + totalPoints + " pts = " + targetGrade);
...
/**
 * Update regenerateFESFactors to use locked titles
 */
private Map<String, Object> regenerateFESFactorsLocked(Map<String, String> existingFactors,
                                                       String duties, String targetGrade) throws Exception {
    System.out.println("Regenerating FES factors with LOCKED titles for grade: " + targetGrade);
    
    // Similar implementation to GSSG but for FES factors 1-9
    String pdfContext = getAutoPdfContext();
    String prompt = pdfContext + String.format("""
        You are an OPM classification expert using the FES 9-factor non-supervisory system.
        
        Target Grade: %s
        
        Analyze these non-supervisory duties and assign appropriate levels for ALL 9 FES factors.
    ...
    System.out.println("FES regeneration complete with locked titles: " + totalPoints + " pts = " + finalGrade);
...
/**
 * REPLACE existing regenerateGSSGFactors method with this locked version
 */
private Map<String, Object> regenerateGSSGFactors(Map<String, String> existingFactors, 
                                                   String duties, String targetGrade) throws Exception {
    return regenerateGSSGFactorsLocked(existingFactors, duties, targetGrade);
}

/**
 * REPLACE existing regenerateFESFactors method with this locked version
 */
private Map<String, Object> regenerateFESFactors(Map<String, String> existingFactors,
                                                  String duties, String targetGrade) throws Exception {
    return regenerateFESFactorsLocked(existingFactors, duties, targetGrade);
}
...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, Object> payload) {
    try {
        String duties = String.valueOf(payload.getOrDefault("duties", "")).trim();
        if (duties.isEmpty()) {
            return Map.of("success", false, "error", "Duties are required");
        }

        // STEP 1: LOCK THE SYSTEM - detect from existing factors
        ...
        String systemType = isSupervisory ? "GSSG (6-factor supervisory)" : "FES (9-factor non-supervisory)";
        System.out.println("=== REGENERATE FACTORS - SYSTEM LOCKED: " + systemType + " ===");
        System.out.println("Existing factor keys: " + existingFactors.keySet());

        // STEP 2: Get target grade (for both systems)
        String targetGrade = String.valueOf(payload.getOrDefault("gsGrade", 
                            payload.getOrDefault("expectedGrade", "GS-13")));
        
        // STEP 3: Route to correct regeneration (preserves system)
        if (isSupervisory) {
            return regenerateGSSGFactors(existingFactors, duties, targetGrade);
        } else {
            return regenerateFESFactors(existingFactors, duties, targetGrade);
        }
        
    } catch (Exception ex) {
        ex.printStackTrace();
        return Map.of("success", false, "error", "Failed to regenerate factors: " + ex.getMessage());
    }
}

/**
 * CRITICAL: Extract existing factor structure to determine locked system
 */
private Map<String, String> extractExistingFactorStructure(Map<String, Object> payload) {
    ...
    // Check all possible locations for factor data
    Object factorsObj = payload.get("factors");
    Object sectionsObj = payload.get("pdSections");
    
    // Priority 1: Check "factors" object
    if (factorsObj instanceof Map) {
        ...
        for (Map.Entry<String, Object> entry : factorsMap.entrySet()) {
            String key = entry.getKey();
            if (key.matches("Factor \\d+[AB]?")) {
                Object val = entry.getValue();
                if (val instanceof Map) {
                    @SuppressWarnings("unchecked")
                    Map<String, Object> factorData = (Map<String, Object>) val;
                    String header = String.valueOf(factorData.get("header"));
                    structure.put(key, header);
                } else if (val instanceof String) {
                    structure.put(key, (String) val);
                }
            }
        }
    }
    
    // Priority 2: Check "pdSections" object
    if (structure.isEmpty() && sectionsObj instanceof Map) {
        @SuppressWarnings("unchecked")
        Map<String, Object> sectionsMap = (Map<String, Object>) sectionsObj;
        
        for (Map.Entry<String, Object> entry : sectionsMap.entrySet()) {
            String key = entry.getKey();
            if (key.matches("Factor \\d+[AB]?")) {
                structure.put(key, String.valueOf(entry.getValue()));
            }
        }
    }
    
    // Priority 3: Check top-level keys
...
/**
 * DETERMINISTIC: Lock system based on existing factor structure
 */
private boolean determineLOCKEDSystem(Map<String, String> existingFactors) {
    if (existingFactors.isEmpty()) {
        System.out.println("No existing factors found - defaulting to FES");
        return false; // Default to FES (safer)
    }
    
    // DEFINITIVE INDICATORS - these override everything
    boolean has4A = existingFactors.keySet().stream().anyMatch(k -> k.contains("4A"));
    boolean has4B = existingFactors.keySet().stream().anyMatch(k -> k.contains("4B"));
    boolean has7 = existingFactors.containsKey("Factor 7");
    boolean has8 = existingFactors.containsKey("Factor 8");
    boolean has9 = existingFactors.containsKey("Factor 9");
    ...
    // Check factor names in headers
    for (Map.Entry<String, String> entry : existingFactors.entrySet()) {
        String content = entry.getValue().toLowerCase();
        
        // GSSG-specific factor names
        if (content.contains("program scope and effect") ||
            content.contains("organizational setting") ||
            content.contains("difficulty of typical work directed") ||
            content.contains("other conditions")) {
            System.out.println("LOCKED: GSSG (found supervisory factor name)");
            return true;
        }
        
        // FES-specific factor names
        if (content.contains("knowledge required by the position") ||
            content.contains("purpose of contacts") ||
            content.contains("physical demands") ||
            content.contains("work environment")) {
            System.out.println("LOCKED: FES (found non-supervisory factor name)");
            return false;
        }
    }
    
    // Count factors to determine system
    int factorCount = existingFactors.size();
    if (factorCount <= 7) {
        System.out.println("LOCKED: GSSG (factor count: " + factorCount + ")");
        return true;
    } else {
        System.out.println("LOCKED: FES (factor count: " + factorCount + ")");
        return false;
    }
}

/**
 * Helper: Extract level from AI factor response
 */
private String extractLevelFromAIFactor(Object factorObj, String defaultPrefix) {
    if (factorObj instanceof Map) {
        @SuppressWarnings("unchecked")
        Map<String, Object> factor = (Map<String, Object>) factorObj;
        Object levelObj = factor.get("level");
        if (levelObj != null) {
            return String.valueOf(levelObj);
        }
    }
    return null;
}
...
/**
 * Helper: Extract rationale from AI factor response
 */
private String extractRationaleFromAIFactor(Object factorObj) {
    if (factorObj instanceof Map) {
        @SuppressWarnings("unchecked")
        Map<String, Object> factor = (Map<String, Object>) factorObj;
        Object rationaleObj = factor.get("rationale");
        if (rationaleObj != null) {
            return String.valueOf(rationaleObj);
        }
    }
    return null;
}
...
/**
 * Adjust GSSG total to fit grade range
 */
private int adjustGSSGToGradeRange(Map<String, Object> factors, int currentTotal, 
                                   int minPoints, int maxPoints) {
    if (currentTotal >= minPoints && currentTotal <= maxPoints) {
        return currentTotal;
    }
...
/**
 * CRITICAL: Detect evaluation system from existing factor structure
 * Checks for Factor 4A/4B (GSSG only) or Factors 7/8/9 (FES only)
 */
private boolean detectSystemFromExistingFactors(Map<String, String> factorHints, Map<String, Object> payload) {
    System.out.println("[detectSystemFromExistingFactors] Starting detection...");
    System.out.println("  Factor hints keys: " + factorHints.keySet());
    System.out.println("  Payload keys: " + payload.keySet());
    
    // Check for DEFINITIVE indicators first - check ALL possible keys
    boolean has4A = false;
    boolean has4B = false;
    boolean has7 = false;
    boolean has8 = false;
    ...
    // Check factor hints
    for (String key : factorHints.keySet()) {
        String normalized = key.trim().replaceAll("\\s+", " ");
        if (normalized.contains("4A") || normalized.equals("Factor 4A")) has4A = true;
        if (normalized.contains("4B") || normalized.equals("Factor 4B")) has4B = true;
        if (normalized.equals("Factor 7")) has7 = true;
        if (normalized.equals("Factor 8")) has8 = true;
        if (normalized.equals("Factor 9")) has9 = true;
    }
    
    // Check payload keys
    for (String key : payload.keySet()) {
        String normalized = key.trim().replaceAll("\\s+", " ");
        if (normalized.contains("4A") || normalized.equals("Factor 4A") || 
            normalized.equals("Factor 4A_level") || normalized.equals("Factor 4A_originalLevel")) has4A = true;
        if (normalized.contains("4B") || normalized.equals("Factor 4B") ||
            normalized.equals("Factor 4B_level") || normalized.equals("Factor 4B_originalLevel")) has4B = true;
        if (normalized.equals("Factor 7") || normalized.equals("Factor 7_level") || 
            normalized.equals("Factor 7_originalLevel")) has7 = true;
        if (normalized.equals("Factor 8") || normalized.equals("Factor 8_level") || 
            normalized.equals("Factor 8_originalLevel")) has8 = true;
        if (normalized.equals("Factor 9") || normalized.equals("Factor 9_level") || 
            normalized.equals("Factor 9_originalLevel")) has9 = true;
    }
    
    System.out.println("  Detection results: 4A=" + has4A + ", 4B=" + has4B + ", 7=" + has7 + ", 8=" + has8 + ", 9=" + has9);
    
    if (has4A || has4B) {
        System.out.println("[detectSystemFromExistingFactors] ✓ Found Factor 4A/4B -> GSSG Supervisory");
        return true;
    }
    
    if (has7 || has8 || has9) {
        System.out.println("[detectSystemFromExistingFactors] ✓ Found Factors 7/8/9 -> FES Non-Supervisory");
        return false;
    }
    
    // Check point values in ALL payload values
    for (Map.Entry<String, Object> entry : payload.entrySet()) {
        Object val = entry.getValue();
        ...
        // GSSG-specific point values (Factors 1-6 have different scales)
        // Factor 1 GSSG: 175, 350, 550, 775, 900 (max)
        // Factor 1 FES: 50, 200, 350, 550, 750, 950, 1250, 1550, 1850 (max)
        if (valStr.contains("775") || valStr.contains("900")) { // GSSG Factor 1 unique values
            System.out.println("[detectSystemFromExistingFactors] ✓ Found GSSG point value (775/900) -> GSSG Supervisory");
            return true;
        }
        // Factor 5 GSSG: 75, 205, 340, 505, 650, 800, 930, 1030 (max)
        // Factor 5 FES: 25, 75, 150, 225, 325, 450 (max)
        if (valStr.contains("930") || valStr.contains("1030") || valStr.contains("205") || valStr.contains("505")) {
            System.out.println("[detectSystemFromExistingFactors] ✓ Found GSSG point value (930/1030/205/505) -> GSSG Supervisory");
            return true;
        }
        
        // FES-specific high point values (Factor 1 only goes up to 1850)
        if (valStr.contains("1250") || valStr.contains("1550") || valStr.contains("1850")) {
            System.out.println("[detectSystemFromExistingFactors] ✓ Found FES point value (1250/1550/1850) -> FES Non-Supervisory");
            return false;
        }
    }
    
    // Count factors present
    int factorCount = 0;
    for (String key : payload.keySet()) {
        if (key.matches("Factor \\d+[AB]?") || key.matches("Factor \\d+")) factorCount++;
    }
    
    System.out.println("  Factor count: " + factorCount);
    
    // If 6-7 factors, likely GSSG; if 8-9 factors, likely FES
    if (factorCount <= 7) {
        System.out.println("[detectSystemFromExistingFactors] Factor count " + factorCount + " <= 7 -> likely GSSG");
        return true;
    } else if (factorCount >= 8) {
        System.out.println("[detectSystemFromExistingFactors] Factor count " + factorCount + " >= 8 -> likely FES");
        return false;
    }
    
    // Final fallback: check supervisoryLevel parameter
    String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory"));
    boolean isSupervisory = isSupervisoryPosition(supervisoryLevel);
    System.out.println("[detectSystemFromExistingFactors] Using supervisoryLevel fallback: " + supervisoryLevel + " -> " + (isSupervisory ? "GSSG" : "FES"));
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        System.out.println("=== UPDATE FACTOR POINTS DEBUG ===");
        System.out.println("Received payload keys: " + payload.keySet());
        System.out.println("Full payload: " + payload);
        
        // STEP 1: LOCK THE SYSTEM
        Map<String, String> existingFactors = extractExistingFactorStructure(payload);
        System.out.println("Extracted existing factors: " + existingFactors.keySet());
        
        if (existingFactors.isEmpty()) {
            System.err.println("ERROR: No existing factors found in payload!");
            return Map.of(
                "success", false, 
                "error", "No existing factors found. Payload must include factor data.",
                "debug", Map.of(
        ...
        String systemType = isSupervisory ? "GSSG (6-factor)" : "FES (9-factor)";
        System.out.println("LOCKED SYSTEM: " + systemType);
        System.out.println("Existing factors: " + existingFactors.keySet());
        
        String duties = String.valueOf(payload.getOrDefault("duties", ""));
        if (duties.isEmpty() || duties.equals("null")) {
            System.err.println("WARNING: No duties provided in payload");
        }
        ...
        // STEP 2: Extract which specific factor was updated
        String updatedFactorKey = extractUpdatedFactorKey(payload);
        System.out.println("Updated factor: " + updatedFactorKey);
        
        // STEP 3: Route to correct update handler with LOCKED titles
        Map<String, Object> result;
        if (isSupervisory) {
            result = updateGSSGFactorPointsLocked(existingFactors, payload, duties, targetGrade, updatedFactorKey);
        } else {
            result = updateFESFactorPointsLocked(existingFactors, payload, duties, targetGrade, updatedFactorKey);
        }
        
        result.put("systemLocked", systemType);
        System.out.println("Update successful: " + result.get("totalPoints") + " points");
        return result;
        
    } catch (Exception ex) {
        System.err.println("EXCEPTION in updateFactorPoints:");
        ex.printStackTrace();
...
private Map<String, Object> updateGSSGFactorPointsLocked(Map<String, String> existingFactors,
    ...
    System.out.println("Updating GSSG factors (locked titles) for " + targetGrade);
    ...
    System.out.println("GSSG update complete: " + totalPoints + " pts = " + targetGrade);
...
/**
 * Update FES factors with LOCKED titles
 */
private Map<String, Object> updateFESFactorPointsLocked(Map<String, String> existingFactors,
                                                         Map<String, Object> payload,
                                                         String duties, String targetGrade,
                                                         String updatedFactorKey) throws Exception {
    System.out.println("Updating FES factors (locked titles) for " + targetGrade);
...
/**
 * Enhanced extraction with better null handling
 */
private Map<String, Object> extractCurrentFactorData(Map<String, Object> payload) {
    ...
    // Priority 1: factors object
    Object factorsObj = payload.get("factors");
    if (factorsObj instanceof Map) {
        @SuppressWarnings("unchecked")
        Map<String, Object> factorsMap = (Map<String, Object>) factorsObj;
        factorData.putAll(factorsMap);
        System.out.println("Extracted from 'factors': " + factorsMap.keySet());
    }
    
    // Priority 2: pdSections
    Object sectionsObj = payload.get("pdSections");
    if (sectionsObj instanceof Map) {
        @SuppressWarnings("unchecked")
        Map<String, Object> sectionsMap = (Map<String, Object>) sectionsObj;
        factorData.putAll(sectionsMap);
        System.out.println("Extracted from 'pdSections': " + sectionsMap.keySet());
    }
    
    // Priority 3: currentSections
    Object currentObj = payload.get("currentSections");
    if (currentObj instanceof Map) {
        @SuppressWarnings("unchecked")
        Map<String, Object> currentMap = (Map<String, Object>) currentObj;
        factorData.putAll(currentMap);
        System.out.println("Extracted from 'currentSections': " + currentMap.keySet());
    }
    ...
    for (Map.Entry<String, Object> entry : payload.entrySet()) {
        String key = entry.getKey();
        if (key.matches("Factor \\d+[AB]?")) {
            factorData.put(key, entry.getValue());
        }
    }
...
private Map<String, Object> extractFactorFromCurrent(Map<String, Object> currentFactors, String factorKey) {
    ...
    if (factorObj instanceof String) {
        ...
        // Extract level
        Pattern levelPattern = Pattern.compile("Level\\s+(\\d+[AB]?-\\d+)");
        ...
        // Extract points
        Pattern pointsPattern = Pattern.compile("(\\d+)\\s+Points");
...
/**
 * Update GSSG factors - LOCKED to 1,2,3,4A,4B,5,6
 */
private Map<String, Object> updateGSSGFactorPoints(Map<String, String> existingFactors,
                                                    Map<String, Object> payload,
                                                    String duties, String targetGrade,
                                                    String updatedFactorKey) throws Exception {
    System.out.println("Updating GSSG factors (locked to 7 factors)");
    ...
    // Build result with GSSG structure
    Map<String, Object> validatedFactors = new LinkedHashMap<>();
    int totalPoints = 0;
    ...
    for (String factorId : gssgOrder) {
        ...
        // CRITICAL: Generate substantive rationale, not default
        if (rationale == null || rationale.isEmpty() || rationale.contains("Default supervisory")) {
            rationale = buildGssgRationale(
                Integer.parseInt(factorId.replaceAll("[^0-9]", "")), 
                level, duties, 
        ...
        Map<String, Object> factorData = new LinkedHashMap<>();
        factorData.put("header", "Factor " + factorId + " - " + getGSSGFactorName(factorId) + 
    ...
    // Adjust to grade range
    totalPoints = adjustGSSGToGradeRange(validatedFactors, totalPoints, minPoints, maxPoints);
...
/**
 * Update FES factors - LOCKED to 1-9
 */
private Map<String, Object> updateFESFactorPoints(Map<String, String> existingFactors,
                                                   Map<String, Object> payload,
                                                   String duties, String targetGrade,
                                                   String updatedFactorKey) throws Exception {
    System.out.println("Updating FES factors (locked to 9 factors)");
    
    // Extract current factor data from payload
    Map<String, Object> currentFactors = extractCurrentFactorData(payload);
    
    // Build result with FES structure
    Map<String, Object> validatedFactors = new LinkedHashMap<>();
    int totalPoints = 0;
    ...
    for (int i = 1; i <= 9; i++) {
        ...
        // CRITICAL: Generate substantive rationale
        if (rationale == null || rationale.isEmpty()) {
            rationale = generateSubstantiveRationale(i, level, duties);
        }
        
        Map<String, Object> factorData = new LinkedHashMap<>();
        factorData.put("header", "Factor " + i + " - " + getFactorName(i) + 
                                " Level " + level + ", " + points + " Points");
        factorData.put("level", level);
        factorData.put("points", points);
        factorData.put("rationale", rationale);
...
/**
 * Extract which factor was updated (if any)
 */
private String extractUpdatedFactorKey(Map<String, Object> payload) {
    // Check for explicit indicator
    Object updatedKey = payload.get("updatedFactor");
    if (updatedKey != null) {
        return String.valueOf(updatedKey);
    }
    
    // Check for factor with _level suffix (indicates update)
    for (String key : payload.keySet()) {
        if (key.matches("Factor \\d+[AB]?_level")) {
...
/**
 * IMPROVED: Get default supervisory factors with SUBSTANTIVE rationales
 */
private Map<String, Object> getDefaultSupervisoryFactors(String gsGrade) {
    ...
    // Use empty duties for now - will be replaced by substantive rationales
    String placeholderDuties = "supervisory duties involving program management and oversight";
    
    for (int i = 1; i <= 6; i++) {
        if (i == 4) {
            String level4A = defaultLevels.get("4A");
            int points4A = getGssgPointsForLevel("4A", level4A);
            String rationale4A = buildGssgRationale(4, level4A, placeholderDuties, "4A");
            factors.put("Factor 4A", Map.of(
                "level", level4A, 
                "points", points4A, 
                "rationale", rationale4A
            ));
                
            String level4B = defaultLevels.get("4B");
            int points4B = getGssgPointsForLevel("4B", level4B);
            String rationale4B = buildGssgRationale(4, level4B, placeholderDuties, "4B");
            factors.put("Factor 4B", Map.of(
                "level", level4B, 
                "points", points4B, 
                "rationale", rationale4B
            ));
        } else {
            String factorNum = String.valueOf(i);
            String level = defaultLevels.get(factorNum);
            int points = getGssgPointsForLevel(factorNum, level);
            String rationale = buildGssgRationale(i, level, placeholderDuties, null);
...
/**
 * Generate supervisory factors using AI analysis of duties
 */
private Map<String, Object> generateSupervisoryFactorsWithAI(String duties, String targetGrade, String supervisoryLevel) throws Exception {
    String pdfContext = getAutoPdfContext();
    ...
    String prompt = pdfContext + String.format("""
        You are an OPM HR expert using the GSSG 6-factor supervisory system.
        
        CRITICAL: Analyze duties and assign appropriate levels for ALL 7 factors:
        - Factor 1 - Program Scope and Effect (1-1 to 1-5)
        - Factor 2 - Organizational Setting (2-1 to 2-3)
        - Factor 3 - Supervisory & Managerial Authority (3-1 to 3-4)
        - Factor 4A - Personal Contacts (Nature) (4A-1 to 4A-4)
        ...
        {
          "Factor 1": {"level": "1-X", "points": NNN, "rationale": "2-3 sentences"},
          "Factor 2": {"level": "2-X", "points": NNN, "rationale": "2-3 sentences"},
          "Factor 3": {"level": "3-X", "points": NNN, "rationale": "2-3 sentences"},
          "Factor 4A": {"level": "4A-X", "points": NNN, "rationale": "2-3 sentences"},
          "Factor 4B": {"level": "4B-X", "points": NNN, "rationale": "2-3 sentences"},
          "Factor 5": {"level": "5-X", "points": NNN, "rationale": "2-3 sentences"},
          "Factor 6": {"level": "6-X", "points": NNN, "rationale": "2-3 sentences"}
        }
    ...
    try {
        ...
        System.err.println("Failed to parse AI response, using defaults: " + e.getMessage());
        factors = getDefaultSupervisoryFactors(targetGrade);
    }
    
    // Validate and format factors
    Map<String, Object> validatedFactors = new LinkedHashMap<>();
    String[] factorOrder = {"1", "2", "3", "4A", "4B", "5", "6"};
    ...
    for (String factorId : factorOrder) {
        ...
        if (factorData instanceof Map) {
            @SuppressWarnings("unchecked")
            Map<String, Object> factor = (Map<String, Object>) factorData;
            ...
            if (rationale == null || rationale.trim().isEmpty()) {
                rationale = "Supervisory analysis for " + getGSSGFactorName(factorId);
            }
            
            Map<String, Object> validated = new LinkedHashMap<>();
            validated.put("header", key + " - " + getGSSGFactorName(factorId) + 
                         " Level " + level + ", " + points + " Points");
            validated.put("level", level);
            validated.put("points", points);
            validated.put("rationale", rationale);
            
            validatedFactors.put(key, validated);
        }
    }
...
/**
 * Generate non-supervisory factors using AI analysis of duties
 */
private Map<String, Object> generateNonSupervisoryFactorsWithAI(String duties, String targetGrade, String supervisoryLevel) throws Exception {
    String pdfContext = getAutoPdfContext();
    ...
    String prompt = pdfContext + String.format("""
        You are an OPM HR expert using the FES 9-factor non-supervisory system.
        
        CRITICAL: Analyze duties and assign appropriate levels for ALL 9 factors:
    ...
    try {
        ...
        System.err.println("Failed to parse AI response, using intelligent defaults: " + e.getMessage());
        return generateIntelligentDefaultFactors(duties, targetGrade);
    }
    ...
    for (int i = 1; i <= 9; i++) {
        ...
        if (factorData instanceof Map) {
            ...
            if (level == null) level = getIntelligentLevel(i, duties);
            level = normalizeAndValidateFesLevel(factorId, level);
            ...
            Map<String, Object> validated = new LinkedHashMap<>();
            validated.put("header", key + " - " + getFactorName(i) + 
                         " Level " + level + ", " + points + " Points");
            validated.put("level", level);
            validated.put("points", points);
            validated.put("rationale", rationale);
...
/**
 * Generate intelligent default factors based on duties analysis (fallback)
 */
private Map<String, Object> generateIntelligentDefaultFactors(String duties, String targetGrade) {
    Map<String, Object> factors = new LinkedHashMap<>();
    ...
    for (int i = 1; i <= 9; i++) {
        ...
        String level = getIntelligentLevel(i, duties);
        int points = getPointsForLevel(factorId, level);
        String rationale = generateSubstantiveRationale(i, level, duties);
        
        Map<String, Object> factor = new LinkedHashMap<>();
        factor.put("header", key + " - " + getFactorName(i) + 
                   " Level " + level + ", " + points + " Points");
        factor.put("level", level);
        factor.put("points", points);
        factor.put("rationale", rationale);
...
@SuppressWarnings("unchecked")
private Map<String, String> extractFactorHints(Map<String, Object> payload) {
    Map<String, String> out = new LinkedHashMap<>();

    // 1) nested "factors"
    Object fObj = payload.get("factors");
    if (fObj instanceof Map<?, ?> nested) {
        out.putAll(extractFactorHintsFromRaw((Map<String, Object>) nested));
    }

    // 2) nested "pdSections" / "currentSections" / "existingFactors"
    for (String k : List.of("pdSections", "currentSections", "existingFactors")) {
        Object sec = payload.get(k);
        if (sec instanceof Map<?, ?> m) {
            out.putAll(extractFactorHintsFromRaw((Map<String, Object>) m));
        }
    }

    // 3) top-level factor keys
    out.putAll(extractFactorHintsFromRaw(payload));

    return out;
}

@SuppressWarnings("unchecked")
private Map<String, Object> extractRawFactorsMap(Map<String, Object> payload) {
    Object fObj = payload.get("factors");
    if (fObj instanceof Map<?, ?> nested) {
        return (Map<String, Object>) nested;
    }
    // fallback: use payload itself
    return payload;
}

/**
 * From a raw map (possibly containing Factor 4A / 4B keys with string or object values), pull out
 * a simple FactorId -> content/level string map for detection and processing.
 */
private Map<String, String> extractFactorHintsFromRaw(Map<String, Object> raw) {
    Map<String, String> out = new LinkedHashMap<>();
    ...
    raw.forEach((k, v) -> {
        if (k == null) return;
        String key = k.trim();
        // Match Factor 1..9 or Factor 4A/4B (allow variants like "Factor4A" with no space)
        if (key.matches("(?i)^Factor\\s*\\d+[A-B]?$") || key.matches("(?i)^Factor\\s*\\d+")) {
            if (v instanceof String s) {
                if (!s.isBlank()) out.put(key.replaceAll("\\s+", " "), s.trim());
            } else if (v instanceof Map<?, ?> m) {
                Object content = m.get("content");
                Object rationale = m.get("rationale");
                Object level = m.get("level");
                // Prefer content/rationale if present, otherwise serialize level
                if (content instanceof String cs && !cs.isBlank()) {
                    out.put(key.replaceAll("\\s+", " "), cs.trim());
                } else if (rationale instanceof String rs && !rs.isBlank()) {
                    out.put(key.replaceAll("\\s+", " "), rs.trim());
                } else if (level instanceof String ls && !ls.isBlank()) {
                    out.put(key.replaceAll("\\s+", " "), ls.trim());
                }
            }
        }
        // Also carry through any explicit originalLevel hints
        if (key.matches("(?i)^Factor\\s*\\d+[A-B]?_originalLevel$") && v != null) {
            out.put(key, String.valueOf(v));
        }
    });

    // Normalize compact headers like "Factor4A" -> "Factor 4A"
    Map<String, String> normalized = new LinkedHashMap<>();
    out.forEach((k, v) -> {
        String nk = k.replaceAll("(?i)Factor\\s*(\\d{1,2})\\s*([AB])", "Factor $1$2")
                     .replaceAll("(?i)Factor\\s*(\\d{1,2})", "Factor $1")
                     .trim();
        normalized.put(nk, v);
    });
    return normalized;
...
/**
 * Merge any discovered factor sections into a copy of the incoming body so downstream helpers can
 * read original factor headers/levels if present (preserve original keys like "Factor 4A").
 */
private Map<String, String> mergeFactorsIntoBody(Map<String, String> original, Map<String, String> candidateSections) {
    Map<String, String> merged = new HashMap<>(original);
    if (candidateSections == null || candidateSections.isEmpty()) return merged;
    for (Map.Entry<String, String> e : candidateSections.entrySet()) {
        // only merge if not already present (preserve explicit body values first)
...
/**
 * Scan all string values in the provided body for lines that look like factor headers.
 * Returns a map of detected canonical Factor keys -> extracted value (the whole factor paragraph).
 */
private Map<String, String> scanValuesForFactorHeaders(Map<String, String> body) {
    Map<String, String> found = new LinkedHashMap<>();
    if (body == null || body.isEmpty()) return found;

    // Pattern to find factor header lines and capture the header and following rationale block (up to next Factor or summary)
    Pattern headerPattern = Pattern.compile("(?im)(Factor\\s*(\\d{1,2})([A-Za-z])?\\b[\\s\\S]{0,200}?Level\\s*\\d+-[\\dA-Za-z]+,\\s*\\d+\\s*Points)"); 
    for (Map.Entry<String, String> e : body.entrySet()) {
        String value = e.getValue();
        if (value == null || value.isBlank()) continue;
        Matcher m = headerPattern.matcher(value);
        while (m.find()) {
            String headerBlock = m.group(1).trim();
            // normalize header key
            String keyMatch = m.group(1);
            Matcher k = Pattern.compile("(?i)Factor\\s*(\\d{1,2})([A-Za-z])?").matcher(keyMatch);
            if (k.find()) {
                String num = k.group(1);
                String letter = k.group(2) != null ? k.group(2).toUpperCase() : "";
                String canonical = "Factor " + num + letter;
                // if a longer paragraph follows, extract up to next blank line or 400 chars
                int start = m.start(1);
                int tailEnd = Math.min(value.length(), m.end(1) + 400);
                String tail = value.substring(m.start(1), tailEnd).trim();
                // store only first occurrence
                if (!found.containsKey(canonical)) found.put(canonical, tail);
            }
        }
    }

    // Also attempt to detect "Factor4A", "Factor4 B", "4A Level ..." run-together tokens
    if (found.isEmpty()) {
        Pattern compact = Pattern.compile("(?i)(Factor\\s*4\\s*A|Factor4A|4A\\b).*?Level\\s*4-\\d+,\\s*\\d+\\s*Points");
        for (String v : body.values()) {
            if (v == null) continue;
            Matcher m = compact.matcher(v);
            if (m.find()) {
                String canonical = "Factor 4A";
                if (!found.containsKey(canonical)) found.put(canonical, m.group(0));
            }
        }
    }
