...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, Object> body) {
    try {
        String duties = (String) body.getOrDefault("duties", "");
        String supervisoryLevel = (String) body.getOrDefault("supervisoryLevel", "Non-Supervisory");
        
        if (duties == null || duties.trim().isEmpty()) {
            return Map.of("error", "Duties are required", "success", false);
        }

        System.out.println("=== REGENERATE FACTORS ===");
        
        // STEP 1: READ the current factors to lock the system
        Map<String, Object> currentAnalysis = analyzeCurrentFactorSystem(body);
        boolean isSupervisory = (boolean) currentAnalysis.get("isSupervisory");
        
        System.out.println("Current system detected: " + (isSupervisory ? "GSSG SUPERVISORY" : "FES NON-SUPERVISORY"));
        System.out.println("Reason: " + currentAnalysis.get("reason"));
        
        // STEP 2: Extract current factor headers to preserve exact names
        Map<String, String> currentHeaders = extractCurrentFactorHeaders(body, isSupervisory);
        System.out.println("Preserved factor headers: " + currentHeaders.keySet());
        
        // STEP 3: Regenerate using correct system
        if (isSupervisory) {
            return regenerateSupervisoryAfterDutiesUpdate(body, duties, supervisoryLevel, currentHeaders);
        } else {
            return regenerateNonSupervisoryAfterDutiesUpdate(body, duties, supervisoryLevel, currentHeaders);
        }

    } catch (Exception e) {
        System.err.println("Exception in regenerateFactorsAfterDutiesUpdate: " + e.getMessage());
        e.printStackTrace();
...
/**
 * Extract current factor headers from body to preserve exact names
 */
private Map<String, String> extractCurrentFactorHeaders(Map<String, Object> body, boolean isSupervisory) {
    Map<String, String> headers = new HashMap<>();
    
    if (isSupervisory) {
        // Extract GSSG factor headers
        for (String key : new String[]{"Factor 1", "Factor 2", "Factor 3", "Factor 4A", "Factor 4B", "Factor 5", "Factor 6"}) {
            Object value = body.get(key);
            if (value != null) {
                String header = extractHeaderFromFactorContent(value.toString());
...
/**
 * Extract just the factor name/title from content
 * e.g., "Factor 1 - Program Scope and Effect Level 1-3, 550 Points\n..."
 * -> "Program Scope and Effect"
 */
private String extractHeaderFromFactorContent(String content) {
    // Pattern: "Factor X - NAME Level"
    Pattern p = Pattern.compile("Factor\\s+\\d+[A-Z]*\\s*-\\s*([^\\n]+?)\\s+Level", Pattern.CASE_INSENSITIVE);
...
/**
 * Regenerate supervisory after duties update
 * Uses locked GSSG system with preserved headers
 */
private Map<String, Object> regenerateSupervisoryAfterDutiesUpdate(Map<String, Object> body,
                                                                    String duties,
                                                                    String supervisoryLevel,
                                                                    Map<String, String> currentHeaders) throws Exception {
    String gsGrade = (String) body.getOrDefault("gsGrade", "GS-13");
    int minPoints = getMinPointsForGrade(gsGrade);
    int maxPoints = getMaxPointsForGrade(gsGrade);
    
    System.out.println("Regenerating GSSG with target " + gsGrade + " (" + minPoints + "-" + maxPoints + ")");
    
    String pdfContext = getAutoPdfContext();
    
    String prompt = pdfContext + String.format("""
        You are an OPM HR expert. Re-evaluate these supervisory duties against GSSG 6-factor system ONLY.
        
        IMPORTANT: When duties change, reassess the factor levels but KEEP THE SAME GSSG SYSTEM.
        Use ONLY these exact GSSG supervisory factor names:
        - Factor 1 - Program Scope and Effect (levels 1-1 to 1-5)
        - Factor 2 - Organizational Setting (levels 2-1 to 2-3)
        - Factor 3 - Supervisory & Managerial Authority (levels 3-1 to 3-4)
        - Factor 4A - Personal Contacts (Nature) (levels 4A-1 to 4A-4)
        - Factor 4B - Personal Contacts (Purpose) (levels 4B-1 to 4B-4)
        - Factor 5 - Difficulty of Typical Work Directed (levels 5-1 to 5-8)
        - Factor 6 - Other Conditions (levels 6-1 to 6-6)
        
        Target Grade: %s (%d-%d points)
        Supervisory Level: %s
        
        Based on UPDATED duties, re-evaluate all 7 GSSG factors.
        Total points MUST be between %d and %d.
        
        Respond in JSON only:
        {
          "Factor 1": {"level": "1-X", "points": NNN},
          "Factor 2": {"level": "2-X", "points": NNN},
          "Factor 3": {"level": "3-X", "points": NNN},
          "Factor 4A": {"level": "4A-X", "points": NNN},
          "Factor 4B": {"level": "4B-X", "points": NNN},
          "Factor 5": {"level": "5-X", "points": NNN},
          "Factor 6": {"level": "6-X", "points": NNN}
        }
    ...
    // Process all 7 GSSG factors with preserved headers
    for (String factorId : java.util.Arrays.asList("1", "2", "3", "4A", "4B", "5", "6")) {
        String key = "Factor " + factorId;
        
        // Get AI's suggested level
        Object aiData = aiFactors.get(key);
        String aiLevel = null;
        if (aiData instanceof Map) {
            @SuppressWarnings("unchecked")
            Map<String, Object> m = (Map<String, Object>) aiData;
            aiLevel = (String) m.get("level");
        }
        
        if (aiLevel == null) aiLevel = factorId + "-1";
        
        // Validate and normalize GSSG level
        aiLevel = normalizeGssgLevel(factorId, aiLevel);
        int points = getGssgPointsForLevel(factorId, aiLevel);
        
        // Get preserved header or use default
        String headerName = currentHeaders.getOrDefault(key, getGSSGFactorName(factorId));
        
        Map<String, Object> result = new LinkedHashMap<>();
        result.put("header", "Factor " + factorId + " - " + headerName + " Level " + aiLevel + ", " + points + " Points");
        result.put("level", aiLevel);
        result.put("points", points);
        result.put("factorName", headerName);
        
        validatedFactors.put(key, result);
        totalPoints += points;
        
        System.out.println("  Factor " + factorId + " (" + headerName + "): " + aiLevel + " = " + points);
    }
    
    // Ensure total is within range
    if (totalPoints < minPoints || totalPoints > maxPoints) {
        System.out.println("WARNING: Total " + totalPoints + " outside [" + minPoints + "-" + maxPoints + "]");
    ...
    System.out.println("Result: " + totalPoints + " points = " + gsGrade + " (" + gradeRange + ")");
    ...
    return Map.of(
        ...
        "systemLocked", "GSSG (preserved from existing factors)"
    );
}

/**
 * Regenerate non-supervisory after duties update
 * Uses locked FES system with preserved headers
 */
private Map<String, Object> regenerateNonSupervisoryAfterDutiesUpdate(Map<String, Object> body,
                                                                       String duties,
                                                                       String supervisoryLevel,
                                                                       Map<String, String> currentHeaders) throws Exception {
    System.out.println("Regenerating FES with preserved headers");
    
    String pdfContext = getAutoPdfContext();
    
    String prompt = pdfContext + String.format("""
        You are an OPM HR expert. Re-evaluate these duties against FES 9-factor NON-SUPERVISORY system ONLY.
        
        IMPORTANT: When duties change, reassess the factor levels but KEEP THE SAME FES SYSTEM.
        Use ONLY these exact FES factor names:
        ...
        Based on UPDATED duties, re-evaluate all 9 FES factors.
    ...
    // Process all 9 FES factors with preserved headers
    for (int i = 1; i <= 9; i++) {
        ...
        // Validate and normalize FES level
        aiLevel = normalizeAndValidateFesLevel(factorId, aiLevel);
        int points = getPointsForLevel(factorId, aiLevel);
        
        // Get preserved header or use default
        String headerName = currentHeaders.getOrDefault(key, getFactorName(i));
        ...
        System.out.println("  Factor " + i + " (" + headerName + "): " + aiLevel + " = " + points);
    }
    ...
    System.out.println("Result: " + totalPoints + " points = " + finalGrade + " (" + gradeRange + ")");
    ...
    return Map.of(
        ...
        "systemLocked", "FES (preserved from existing factors)"
    );
}
