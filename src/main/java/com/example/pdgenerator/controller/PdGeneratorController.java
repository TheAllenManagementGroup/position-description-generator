...
public Map<String, String> generateEvaluationStatement(@RequestBody Map<String, String> body) throws Exception {
    ...
    String introPrompt = String.format("""
        ...
        Write:
        1. Recommended job series and position title (top of the document)
        2. 2–3 sentence background of the position
        3. 2–3 sentence title and series determination
        """, jobSeries, jobTitle, positionTitle, gsGrade, duties, requirements);

    String intro = callOpenAIWithTimeout(introPrompt, 60);
    fullEvaluation.append(intro).append("\n\n");
    ...
    for (String factorName : factors) {
        String factorPrompt = String.format("""
            You are a federal HR classification specialist.

            Analyze the "%s" factor for this position.
            Job series: %s
            Assigned grade: %s
            Duties: %s
            Requirements: %s

            For this factor, do the following:
            - Assign a realistic point value from the official OPM FES scale for this series and grade.
            - Write 3–5 sentences explaining the duties/requirements that justify this level.
            - Explicitly state:
              • Why this factor level fits the assigned grade.
              • Why it does NOT reach the level expected for the next higher grade.
              • Why it EXCEEDS the level expected for the next lower grade.

            Return only this factor’s evaluation.
            """, factorName, jobSeries, gsGrade, duties, requirements);

        String factorEval = callOpenAIWithTimeout(factorPrompt, 60);
        fullEvaluation.append("**").append(factorName).append("**\n");
        fullEvaluation.append(factorEval).append("\n\n");
    }
    ...
    String summaryPrompt = String.format("""
        ...
        - Explains how the total points fit within the official OPM point range for the assigned grade.
        - Explains why the duties/requirements do NOT meet the next higher grade level.
        - Explains why they clearly EXCEED the lower grade level.
        """, jobSeries, gsGrade, duties, requirements);

    String summary = callOpenAIWithTimeout(summaryPrompt, 60);
    fullEvaluation.append("**Grade Determination Summary**\n");
    fullEvaluation.append(summary);

    return Map.of("evaluationStatement", fullEvaluation.toString().trim());
}
