...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> factors) throws Exception {
    ...
    // Use a safe TypeReference for Java versions prior to 17
    Map<String, Map<String,String>> aiLevels = mapper.readValue(
        jsonResponse,
    ...
    for (Map.Entry<String, Map<String,String>> entry : aiLevels.entrySet()) {
        ...
        int points = 0;
        if (FACTOR_POINTS.containsKey(factorNum) && FACTOR_POINTS.get(factorNum).containsKey(level)) {
            points = FACTOR_POINTS.get(factorNum).get(level);
        }
        ...
        finalFactors.put("Factor " + factorNum, Map.of(
            ...
            "level", level,
            "points", points,
            "rationale", data.getOrDefault("rationale","")
        ));
    }
...
private String calculateGradeRange(int totalPoints) {
    if (totalPoints <= 1100) return "0855-1100"; // GS-05
    if (totalPoints <= 1350) return "1105-1350"; // GS-07
    if (totalPoints <= 1600) return "1355-1600"; // GS-09
    if (totalPoints <= 1850) return "1605-1850"; // GS-11
    if (totalPoints <= 2100) return "1855-2100"; // GS-12 low
    if (totalPoints <= 2350) return "2105-2350"; // GS-12 high
    if (totalPoints <= 2750) return "2355-2750"; // GS-13
    if (totalPoints <= 3150) return "2755-3150"; // GS-14
    if (totalPoints <= 4050) return "3605-4050"; // GS-15
    return "4051+"; // above GS-15
}

private String calculateFinalGrade(int totalPoints) {
    if (totalPoints <= 1100) return "GS-05";
    if (totalPoints <= 1350) return "GS-07";
    if (totalPoints <= 1600) return "GS-09";
    if (totalPoints <= 1850) return "GS-11";
    if (totalPoints <= 2100) return "GS-12";
    if (totalPoints <= 2350) return "GS-12";
    if (totalPoints <= 2750) return "GS-13";
    if (totalPoints <= 3150) return "GS-14";
    if (totalPoints <= 4050) return "GS-15";
    return "GS-15+";
}
