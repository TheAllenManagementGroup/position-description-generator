...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, String> body) {
    try {
        ...
        // STEP 0: Try to recover existing PD/factor headers from the request body (robust)
        // Accept multiple shapes:
        // - direct factor keys in body ("Factor 4A" -> "...")
        // - a JSON string under keys like "currentSections" / "pdSections" / "existingFactors"
        // - raw PD text in other fields: scan values for Factor headers (e.g., pasted PD)
        Map<String, String> candidateSections = new HashMap<>();
        // 1) If the caller provided an explicit sections/factors JSON as a string, try to parse it
        String[] possibleJsonKeys = new String[] { "currentSections", "pdSections", "existingFactors", "sectionsJson" };
        for (String k : possibleJsonKeys) {
            if (body.containsKey(k) && body.get(k) != null && !body.get(k).isBlank()) {
                String json = body.get(k).trim();
                try {
                    ObjectMapper mapper = new ObjectMapper();
                    // try Map<String,String> first
                    Map<String, String> parsed = mapper.readValue(json, new TypeReference<Map<String,String>>() {});
                    candidateSections.putAll(parsed);
                    System.out.println("Found explicit sections JSON under key: " + k);
                    break;
                } catch (Exception ex) {
                    // not parseable as JSON - fall through to scanning
                    System.out.println("Could not parse JSON in key " + k + ": " + ex.getMessage());
                }
            }
        }

        // 2) If no explicit JSON, collect any direct Factor keys present in body
        if (candidateSections.isEmpty()) {
            for (Map.Entry<String, String> e : body.entrySet()) {
                String key = e.getKey();
                String val = e.getValue();
                if (key != null && key.matches("(?i)^Factor\\s*\\d+[A-Za-z]?$") && val != null && !val.isBlank()) {
                    candidateSections.put(key.trim(), val);
                }
            }
        }

        // 3) If still empty, scan body values (duties or pasted PD) for factor header lines and extract them
        if (candidateSections.isEmpty()) {
            Map<String, String> scanned = scanValuesForFactorHeaders(body);
            if (!scanned.isEmpty()) {
                candidateSections.putAll(scanned);
                System.out.println("Detected factor headers by scanning body values.");
            }
        }

        // STEP 1: Analyze what system is CURRENTLY in use (prefer explicit candidateSections)
        Map<String, Object> currentAnalysis;
        if (!candidateSections.isEmpty()) {
            currentAnalysis = analyzeCurrentFactorSystem(new HashMap<>(candidateSections.entrySet()
                .stream().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue))));
        } else {
            // fallback: analyze original request body (may contain factor keys or factor-like values)
            currentAnalysis = analyzeCurrentFactorSystem(new HashMap<>(body));
        }

        boolean isSupervisory = (boolean) currentAnalysis.getOrDefault("isSupervisory", false);
        
        System.out.println("Regenerating for: " + (isSupervisory ? "GSSG SUPERVISORY" : "FES NON-SUPERVISORY"));
        
        if (isSupervisory) {
            // pass the candidateSections map (if any) into the regeneration helper so original factor headers/levels are preserved
            return regenerateSupervisoryFactorsWithLock(mergeFactorsIntoBody(body, candidateSections), duties, supervisoryLevel);
        } else {
            return regenerateNonSupervisoryFactorsWithLock(mergeFactorsIntoBody(body, candidateSections), duties, supervisoryLevel);
        }
