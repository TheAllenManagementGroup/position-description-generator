...
/**
 * IMPROVED: Regenerate GSSG supervisory factors with substantive AI-generated rationales
 */
private Map<String, Object> regenerateSupervisoryFactorsWithAI(Map<String, String> body, 
    ...
    String prompt = pdfContext + String.format("""
        ...
        - Each rationale MUST be 3-5 substantive sentences that:
          * Reference specific supervisory duties
          * Explain the scope and impact of supervision
          * Describe the level of authority exercised
          * Justify why this level (not higher or lower)
        ...
        Respond ONLY in JSON format:
        {
          "Factor 1": {
            "level": "1-X",
            "points": NNN,
            "rationale": "Substantive 3-5 sentence explanation referencing duties..."
          },
          "Factor 2": {...},
          "Factor 3": {...},
          "Factor 4A": {...},
    ...
    for (String factorId : factorOrder) {
        ...
        if (factorData instanceof Map) {
            ...
            // Ensure substantive rationale
            if (rationale == null || rationale.trim().isEmpty() || rationale.length() < 100) {
            ...
            System.out.println("  " + key + ": Level " + level + " = " + points + " pts");
        }
    }
...
/**
 * IMPROVED: Regenerate FES non-supervisory factors with substantive AI-generated rationales
 */
private Map<String, Object> regenerateNonSupervisoryFactorsWithAI(Map<String, String> body, 
    ...
    String prompt = pdfContext + String.format("""
        ...
        - Each rationale MUST be 3-5 substantive sentences that:
          * Reference specific non-supervisory duties
          * Explain the technical complexity and independence
          * Describe the analytical and problem-solving requirements
          * Justify why this level (not higher or lower)
    ...
    try {
        ...
        System.err.println("Failed to parse AI response, using intelligent defaults: " + e.getMessage());
        factors = generateIntelligentDefaultFactors(duties, gsGrade);
    }
...
/**
 * CRITICAL: Detect evaluation system by checking factor TITLES first, then structure
 */
private boolean detectSystemFromExistingFactors(Map<String, String> factorHints, Map<String, Object> payload) {
    ...
    // PRIORITY 1: Check factor TITLES/NAMES (most definitive)
    int gssgTitleMatches = 0;
    int fesTitleMatches = 0;
    
    // GSSG factor titles (supervisory)
    String[] gssgTitles = {
        ...
        "other conditions",
        "personal contacts (nature",
        "personal contacts (purpose",
        "nature of contacts",
        "purpose of contacts"
    };
    
    // FES factor titles (non-supervisory)
    String[] fesTitles = {
    ...
    // Check all values in factorHints and payload
    Set<String> allValues = new HashSet<>();
    allValues.addAll(factorHints.values());
    
    // Collect all string values from payload recursively
    collectAllStringValues(payload, allValues);
    ...
    System.out.println("  Title matches - GSSG: " + gssgTitleMatches + ", FES: " + fesTitleMatches);
    
    // If we have clear title matches, use those (most reliable)
    if (gssgTitleMatches > 0 && gssgTitleMatches > fesTitleMatches) {
        System.out.println("  ✓ GSSG Supervisory (based on factor titles)");
        return true;
    }
    if (fesTitleMatches > 0 && fesTitleMatches > gssgTitleMatches) {
        System.out.println("  ✓ FES Non-Supervisory (based on factor titles)");
        return false;
    }
    
    // PRIORITY 2: Check for Factor 4A/4B or 7/8/9 keys (structural indicators)
    boolean has4A = false;
    boolean has4B = false;
    boolean has7 = false;
    boolean has8 = false;
    ...
    // Check factor hints keys
    for (String key : factorHints.keySet()) {
        String normalized = key.trim().replaceAll("\\s+", " ");
        if (normalized.matches("(?i)Factor\\s*4A") || normalized.equals("Factor 4A")) has4A = true;
        if (normalized.matches("(?i)Factor\\s*4B") || normalized.equals("Factor 4B")) has4B = true;
        if (normalized.matches("(?i)Factor\\s*7\\b")) has7 = true;
        if (normalized.matches("(?i)Factor\\s*8\\b")) has8 = true;
        if (normalized.matches("(?i)Factor\\s*9\\b")) has9 = true;
    }
    
    // Check payload keys recursively
    Set<String> allKeys = new HashSet<>();
    collectAllKeys(payload, allKeys);
    
    for (String key : allKeys) {
        String normalized = key.trim().replaceAll("\\s+", " ");
        if (normalized.matches("(?i)Factor\\s*4A") || normalized.contains("4A")) has4A = true;
        if (normalized.matches("(?i)Factor\\s*4B") || normalized.contains("4B")) has4B = true;
        if (normalized.matches("(?i)Factor\\s*7\\b") || normalized.endsWith("7_level") || normalized.equals("7")) has7 = true;
        if (normalized.matches("(?i)Factor\\s*8\\b") || normalized.endsWith("8_level") || normalized.equals("8")) has8 = true;
        if (normalized.matches("(?i)Factor\\s*9\\b") || normalized.endsWith("9_level") || normalized.equals("9")) has9 = true;
    }
    
    System.out.println("  Structure detection: 4A=" + has4A + ", 4B=" + has4B + ", 7=" + has7 + ", 8=" + has8 + ", 9=" + has9);
    
    // DEFINITIVE: Factor 4A or 4B = GSSG Supervisory
    if (has4A || has4B) {
        System.out.println("  ✓ GSSG Supervisory (found 4A/4B keys)");
        return true;
    }
    
    // DEFINITIVE: Factors 7, 8, or 9 = FES Non-Supervisory
    if (has7 || has8 || has9) {
        System.out.println("  ✓ FES Non-Supervisory (found 7/8/9 keys)");
        return false;
    }
    
    // PRIORITY 3: Check point values for GSSG vs FES scales
    for (Map.Entry<String, Object> entry : payload.entrySet()) {
        ...
        // GSSG-specific point values
        // Factor 1 GSSG: 175, 350, 550, 775, 900 (only goes to level 1-5)
        // Factor 5 GSSG: 205, 505, 650, 800, 930, 1030
        if (valStr.contains("775") || valStr.contains("900") || 
            valStr.contains("205") || valStr.contains("505") || 
            valStr.contains("930") || valStr.contains("1030")) {
            System.out.println("  ✓ GSSG Supervisory (found GSSG-specific point value)");
            return true;
        }
        
        // FES-specific high values
        // Factor 1 FES: 1250, 1550, 1850 (goes up to level 1-9)
        if (valStr.contains("1250") || valStr.contains("1550") || valStr.contains("1850")) {
            System.out.println("  ✓ FES Non-Supervisory (found FES-specific point value)");
            return false;
        }
    }
    
    // PRIORITY 4: Count total factors (6-7 = GSSG, 8-9 = FES)
    int factorCount = 0;
    for (String key : allKeys) {
        if (key.matches("(?i)Factor\\s*\\d+[AB]?")) factorCount++;
    }
    
    System.out.println("  Factor count: " + factorCount);
    
    if (factorCount <= 7) {
        System.out.println("  → GSSG Supervisory (6-7 factors)");
        return true;
    ...
    // FINAL FALLBACK: Use supervisoryLevel parameter
    String supervisoryLevel = String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory"));
...
/**
 * Helper: Recursively collect all string values from nested maps
 */
