...
/**
 * FIX #3: Improved regenerateFactors with better detection and error handling
 */
...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, String> body) {
    try {
        ...
        System.out.println("Duties length: " + duties.length());
        
        // Determine system from content
        boolean isSupervisory = determineSystemFromContent(body);
        System.out.println("Detected system: " + (isSupervisory ? "SUPERVISORY (GSSG)" : "NON-SUPERVISORY (FES)"));
        
        if (isSupervisory) {
            return regenerateSupervisoryFactorsImproved(body, duties, supervisoryLevel);
        } else {
            return regenerateNonSupervisoryFactorsImproved(body, duties, supervisoryLevel);
        }
...
/**
 * FIX #4: Individual factor/duty update - detect system and use correct prompt
 */
...
public Map<String, String> updateMajorDuties(@RequestBody Map<String, String> body) throws Exception {
    ...
    String factorName = body.getOrDefault("factorName", "");
    String supervisoryLevel = body.getOrDefault("supervisoryLevel", "Non-Supervisory");
    
    // DETECT SYSTEM from factor name
    boolean isSupervisory = isSupervisoryFactorName(factorName);
    
    System.out.println("Update major duties request");
    System.out.println("  Factor name: " + factorName);
    System.out.println("  System detected: " + (isSupervisory ? "SUPERVISORY" : "NON-SUPERVISORY"));
    System.out.println("  Current content length: " + currentContent.length());
    ...
    try {
        ...
        if (isSupervisory) {
            ...
            prompt = String.format("""
                You are a federal HR specialist using GSSG supervisory classification.
                
                Rewrite the following supervisory factor content to be more professional and clear.
                This is for the GSSG factor: %s
                Supervisory Level: %s
                
                Keep the same supervisory meaning and responsibility level.
                Return ONLY the improved content, no explanation.
                
                %s
                """, factorName, supervisoryLevel, currentContent);
        } else {
            // FES prompt
            prompt = String.format("""
                You are a federal HR specialist using FES non-supervisory classification.
                
                Rewrite the following factor content to be more professional and clear.
                This is for the FES factor: %s
                
                Keep the same analytical/technical meaning and complexity level.
                Return ONLY the improved content, no explanation.
                
                %s
                """, factorName, currentContent);
        }
