...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        ...
        // STEP 3: Route to correct update handler
        Map<String, Object> result;
        if (isSupervisory) {
            result = updateGSSGFactorPoints(existingFactors, payload, duties, targetGrade, updatedFactorKey);
        } else {
            result = updateFESFactorPoints(existingFactors, payload, duties, targetGrade, updatedFactorKey);
        }
        ...
        ex.printStackTrace();
        return Map.of("success", false, "error", "Update failed: " + ex.getMessage());
    }
}
...
/**
 * Extract specific factor from current data
 */
