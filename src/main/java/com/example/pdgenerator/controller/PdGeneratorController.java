...
private boolean detectSystemFromExistingFactors(Map<String, String> factorHints, Map<String, Object> payload) {
    ...
    // GSSG indicators: Factor 4A or 4B
    boolean has4A = factorHints.containsKey("Factor 4A") || 
                    payload.containsKey("Factor 4A") ||
                    payload.containsKey("Factor 4A_level") ||
                    payload.containsKey("Factor 4A_originalLevel");
    boolean has4B = factorHints.containsKey("Factor 4B") || 
                    payload.containsKey("Factor 4B") ||
                    payload.containsKey("Factor 4B_level") ||
                    payload.containsKey("Factor 4B_originalLevel");
    
    // FES indicators: Factors 7, 8, or 9
    boolean has7 = factorHints.containsKey("Factor 7") || 
                   payload.containsKey("Factor 7") ||
                   payload.containsKey("Factor 7_level") ||
                   payload.containsKey("Factor 7_originalLevel");
    boolean has8 = factorHints.containsKey("Factor 8") || 
                   payload.containsKey("Factor 8") ||
                   payload.containsKey("Factor 8_level") ||
                   payload.containsKey("Factor 8_originalLevel");
    boolean has9 = factorHints.containsKey("Factor 9") || 
                   payload.containsKey("Factor 9") ||
                   payload.containsKey("Factor 9_level") ||
                   payload.containsKey("Factor 9_originalLevel");
    ...
    // Check point ranges to differentiate
    // GSSG uses different point scale (e.g., Factor 1 goes to 900, Factor 5 to 1030)
    // FES uses different point scale (e.g., Factor 1 goes to 1850, Factor 5 to 450)
    for (Map.Entry<String, Object> entry : payload.entrySet()) {
        if (entry.getKey().matches("Factor \\d+") && entry.getValue() != null) {
            String val = entry.getValue().toString();
            // Look for GSSG-specific point values
            if (val.contains("550") || val.contains("775") || val.contains("900") || // Factor 1 GSSG
                val.contains("650") || val.contains("800") || val.contains("930") || val.contains("1030")) { // Factor 5 GSSG
                System.out.println("[detectSystemFromExistingFactors] Found GSSG point values -> GSSG Supervisory");
                return true;
            }
            // Look for FES-specific high point values
            if (val.contains("1250") || val.contains("1550") || val.contains("1850")) { // Factor 1 FES only
                System.out.println("[detectSystemFromExistingFactors] Found FES point values -> FES Non-Supervisory");
                return false;
            }
        }
    }
    ...
    System.out.println("[detectSystemFromExistingFactors] No definitive indicators, using supervisoryLevel: " + supervisoryLevel + " -> " + (isSupervisory ? "GSSG" : "FES"));
    return isSupervisory;
}

/**
 * Update points for a single or multiple factors. Detects the correct scale (GSSG vs FES)
 * from the provided Factor headers BEFORE computing points, so it won’t switch systems.
 */
@PostMapping("/update-factor-points")
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        // Normalize incoming factor map (supports payloads where factors are top-level or nested under "factors")
        ...
        // Detect which system the caller is using (GSSG vs FES)
        ...
        // Expected grade (optional)
        String expectedGrade = payload.get("expectedGrade") != null ? String.valueOf(payload.get("expectedGrade")).trim() : null;

        // Use LinkedHashMap to preserve order in response
        Map<String, Object> finalFactors = new LinkedHashMap<>();
        List<String> warnings = new ArrayList<>();
        int totalPoints = 0;

        if (isSupervisory) {
            // GSSG: Factors 1,2,3,4A,4B,5,6
            Map<String, String> levels = extractAndValidateFactorLevels(factorHints, true);
            String[] order = {"1","2","3","4A","4B","5","6"};

            for (String id : order) {
                String key = "Factor " + id;
                String level = levels.getOrDefault(id, id + "-1");
                // Normalize and ensure correct ranges for GSSG
                level = normalizeAndValidateGssgLevel(id, level);

                int points = getGssgPointsForLevel(id, level);
                if (points <= 0) {
                    warnings.add("Unrecognized level for " + key + ": " + level + " -> defaulting");
                    level = id + "-1";
                    points = getGssgPointsForLevel(id, level);
                }

                Map<String, Object> f = new LinkedHashMap<>();
                f.put("header", key + " Level " + level + ", " + points + " Points");
                f.put("level", level);
                f.put("points", points);

                String content = factorHints.getOrDefault(id, factorHints.getOrDefault(key, ""));
                f.put("content", content);

                // Build rationale using existing helper
                int factorNum = id.startsWith("4") ? 4 : Integer.parseInt(id.replaceAll("[^0-9]",""));
                f.put("rationale", buildGssgRationale(factorNum, level, content, id.matches("4[A|B]") ? id : null));

                finalFactors.put(key, f);
                totalPoints += points;
            }
        } else {
            // FES: Factors 1..9
            Map<String, String> levels = extractAndValidateFactorLevels(factorHints, false);
            for (int i = 1; i <= 9; i++) {
                String id = String.valueOf(i);
                String key = "Factor " + id;

                String level = levels.getOrDefault(id, id + "-1");
                level = normalizeAndValidateFesLevel(id, level);

                int points = getPointsForLevel(id, level);
                if (points <= 0) {
                    warnings.add("Unrecognized level for " + key + ": " + level + " -> defaulting");
                    level = id + "-1";
                    points = getPointsForLevel(id, level);
                }
                ...
                f.put("rationale", generateSubstantiveRationale(Integer.parseInt(id), level, content));
