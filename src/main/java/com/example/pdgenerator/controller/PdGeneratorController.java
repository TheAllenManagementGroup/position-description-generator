...
public class PdGeneratorController {
    ...
    @GetMapping("/")
    public String home() {
    return "Position Description Generator is running! Use /api/generate to POST requests.";
}
    ...
    public void rewriteDutiesStreaming(@RequestBody Map<String, String> body, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            openaiRequest.setMax_tokens(1500); // <-- update method name
...
public Map<String, String> generateEvaluationStatement(@RequestBody Map<String, String> body) throws Exception {
    ...
    String occupationalHandbookUrl = "http://localhost:8080/Occupational_Handbook.pdf";
    String prompt = String.format("""
    Use the following links as guidelines and references: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/
    Use this as a reference and guide: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/
    Use this as a reference and guide: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/factor-evaluation-system/
    ...
    Based on the following job duties, requirements, assigned grade level (%s), job series (%s), job title (%s), and position title (%s),
    generate a comprehensive and substantial evaluation statement for a federal position classification.
    The statement must include:
    - At the top, list the top recommended job series and position title
    - Background of the position (at least 2-3 sentences)
    - Title and series determination (explain why this title and series were chosen, at least 2-3 sentences)
    - All 9 classification factors (Knowledge Required, Supervisory Controls, Guidelines, Complexity, Scope and Effect, Personal Contacts, Purpose of Contacts, Physical Demands, Work Environment, plus any other relevant factor)
    - For each factor, provide the specific points assigned and a detailed justification (at least 3-5 sentences per factor, with examples and context)
    - Assign realistic point values for all factors, including factors 6-9, based on the official OPM Factor Evaluation System (FES) for the assigned grade and series. Use the actual point ranges for each factor as published by OPM for the series and grade (e.g., Knowledge Required for GS-13 Engineer is typically 1550 points, Supervisory Controls 450 points, Guidelines 450 points, Complexity 225 points, Scope and Effect 225 points, Personal Contacts 60 points, Purpose of Contacts 220 points, Physical Demands 20 points, Work Environment 20 points).
    - Grade range must be the official OPM points range for the assigned GS grade (e.g., 2355-2750 for GS-12). Do not use GS labels or text ranges.
    - Clearly explain the reasoning behind the assigned grade, referencing duties and additional requirements, with a thorough summary (at least 3-5 sentences)
    - Explain specifically why the position does NOT meet the requirements for the next higher grade level, and why it EXCEEDS the requirements for the previous lower grade level.
    Duties:
    %s
    Additional Requirements:
    %s
    IMPORTANT: Use the official OPM Factor Evaluation System (FES) point values for each factor, matching the assigned grade and series. Do not use placeholder values like "3 points" or "low points". If you do not know the exact point value, estimate based on typical OPM standards for this series and grade. For example, for GS-13 Engineer, use values like 1550, 450, 450, 225, 225, 60, 220, 20, 20.
    Return only the evaluation statement, no commentary or extra text.
    """, gsGrade, jobSeries, jobTitle, positionTitle, duties, requirements, occupationalHandbookUrl);
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> body) throws Exception {
    @SuppressWarnings("unchecked")
    Map<String, String> factors = (Map<String, String>) body.get("factors");
    if (factors == null || factors.isEmpty()) {
        return Map.of("error", "No factors provided");
    }

    // Build a comprehensive prompt for the AI
    String handbookUrl = "http://localhost:8080/Occupational_Handbook.pdf";
    StringBuilder prompt = new StringBuilder();
    prompt.append("You are a federal HR specialist expert in OPM's Factor Evaluation System (FES). Analyze each factor's content and assign appropriate levels and points according to OPM standards.\n\n");
    prompt.append("Reference the official OPM Occupational Handbook here: " + handbookUrl + "\n\n");
    prompt.append("Make sure to use the following links as references: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/factor-evaluation-system/\n");
    prompt.append("Use this as a reference and guide: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/\n");
    prompt.append("Use this as a reference and guide: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/\n");

    prompt.append("Reference the official OPM two-grade interval system for professional and administrative positions: https://www.opm.gov/policy-data-oversight/classification-qualifications/classifying-general-schedule-positions/positionclassificationintro.pdf\n");
    prompt.append("When assigning the final grade, use only the two-grade interval grades (GS-5, GS-7, GS-9, GS-11, GS-12, GS-13, GS-14, GS-15).\n");

    prompt.append("CRITICAL REQUIREMENTS:\n");
    prompt.append("1. Factor levels MUST match the factor number (e.g., Factor 1 uses levels 1-1, 1-2, 1-3, 1-4, 1-5, etc.)\n");
    prompt.append("2. Factor 2 uses levels 2-1, 2-2, 2-3, 2-4, 2-5\n");
    prompt.append("3. Factor 3 uses levels 3-1, 3-2, 3-3, 3-4, 3-5\n");
    prompt.append("4. And so on for all 9 factors\n");
    prompt.append("5. Points must correspond exactly to OPM point charts for each level\n");
    prompt.append("6. Analyze content thoroughly - even long content needs proper evaluation\n");
    prompt.append("7. Always make meaningful assessments based on complexity, responsibility, and scope described\n\n");

    prompt.append("OPM FES POINT REFERENCE:\n");
    prompt.append("Factor 1 (Knowledge): 1-1=200, 1-2=350, 1-3=500, 1-4=750, 1-5=950, 1-6=1250, 1-7=1550, 1-8=1850, 1-9=2210\n");
    prompt.append("Factor 2 (Supervisory Controls): 2-1=25, 2-2=125, 2-3=275, 2-4=450, 2-5=650\n");
    prompt.append("Factor 3 (Guidelines): 3-1=25, 3-2=125, 3-3=275, 3-4=450, 3-5=650\n");
    prompt.append("Factor 4 (Complexity): 4-1=25, 4-2=75, 4-3=150, 4-4=225, 4-5=325, 4-6=450\n");
    prompt.append("Factor 5 (Scope and Effect): 5-1=25, 5-2=75, 5-3=150, 5-4=225, 5-5=325, 5-6=450\n");
    prompt.append("Factor 6 (Personal Contacts): 6-1=10, 6-2=25, 6-3=60, 6-4=110\n");
    prompt.append("Factor 7 (Purpose of Contacts): 7-1=20, 7-2=50, 7-3=120, 7-4=220\n");
    prompt.append("Factor 8 (Physical Demands): 8-1=5, 8-2=20, 8-3=50\n");
    prompt.append("Factor 9 (Work Environment): 9-1=5, 9-2=20, 9-3=50\n\n");

    prompt.append("FACTOR ANALYSIS GUIDELINES:\n");
    prompt.append("- Factor 1: Assess depth/breadth of knowledge, education requirements, specialization\n");
    prompt.append("- Factor 2: Evaluate independence, supervision received, review frequency\n");
    prompt.append("- Factor 3: Analyze clarity of procedures, interpretation needed, precedent availability\n");
    prompt.append("- Factor 4: Consider variety of work, analytical requirements, decision-making complexity\n");
    prompt.append("- Factor 5: Evaluate work impact, geographic scope, organizational effect\n");
    prompt.append("- Factors 6-7: Assess contact levels and purposes (work together as a pair)\n");
    prompt.append("- Factors 8-9: Physical and environmental conditions\n\n");

    prompt.append("Factor Contents to Analyze:\n");
    for (int i = 1; i <= 9; i++) {
        String key = "Factor " + i;
        String content = factors.getOrDefault(key, "");
        if (!content.isEmpty()) {
            prompt.append(String.format("FACTOR %d CONTENT:\n%s\n\n", i, content));
        }
    }
    ...
    prompt.append("    \"Factor 1\": {\n");
    prompt.append("      \"header\": \"Factor 1 - Knowledge Required by the Position Level 1-7, 1550 Points\",\n");
    prompt.append("      \"content\": \"[enhanced content if needed, or original]\",\n");
    prompt.append("      \"level\": \"1-7\",\n");
    prompt.append("      \"points\": 1550,\n");
    prompt.append("      \"rationale\": \"Brief explanation of level assignment\"\n");
    prompt.append("    }\n");
    prompt.append("    // Continue for all factors with content\n");
    prompt.append("  },\n");
    prompt.append("  \"totalPoints\": [sum of all factor points],\n");
    prompt.append("  \"gradeRange\": \"[appropriate GS grade range]\",\n");
    prompt.append("  \"finalGrade\": \"GS-XX\"\n");
    prompt.append("}\n\n");

    prompt.append("INSTRUCTIONS:\n");
    prompt.append("1. Analyze each factor's content thoroughly, regardless of length\n");
    prompt.append("2. Assign appropriate level (must match factor number pattern)\n");
    prompt.append("3. Use exact points from the reference chart above\n");
    prompt.append("4. Create proper header with factor name, level, and points\n");
    prompt.append("5. Provide rationale for level assignment\n");
    prompt.append("6. Calculate accurate total points and grade\n");
    prompt.append("7. Only include factors that have actual content\n");
    prompt.append("8. Enhance content only if it improves clarity - preserve original meaning\n");
    prompt.append("IMPORTANT: For grade range, return ONLY the official OPM points range for the assigned GS grade (e.g., 2355-2750 for GS-12). Do NOT use GS labels or text ranges.\n");

    String response = callOpenAIWithTimeout(prompt.toString(), 60); // Increased timeout for complex analysis
    ...
    try {
        ...
        ObjectMapper mapper = new ObjectMapper();
        Map<String, Object> result = mapper.readValue(jsonResponse, new com.fasterxml.jackson.core.type.TypeReference<Map<String, Object>>() {});

        // Validate the response structure
        if (!result.containsKey("factors") || !result.containsKey("totalPoints")) {
            System.err.println("Invalid AI response structure: " + response);
            return Map.of("error", "Invalid response from AI - missing required fields");
        }

        // Additional validation of factor levels
        @SuppressWarnings("unchecked")
        Map<String, Object> resultFactors = (Map<String, Object>) result.get("factors");
        Map<String, Object> correctedFactors = new LinkedHashMap<>();
        for (String factorKey : resultFactors.keySet()) {
            @SuppressWarnings("unchecked")
            Map<String, Object> factorData = (Map<String, Object>) resultFactors.get(factorKey);

            // Fix key: ensure "Factor X" format
            String correctedKey = factorKey.replaceAll("Factor\\s*(\\d+)", "Factor $1");

            // Fix header: ensure "Factor X - ..." and "Level Y-Z, NNNN Points" spacing
            if (factorData.containsKey("header")) {
                String header = (String) factorData.get("header");
                // Ensure "Factor X" format
                header = header.replaceAll("Factor\\s*(\\d+)", "Factor $1");
                // Ensure "LevelY-Z" becomes "Level Y-Z"
                header = header.replaceAll("Level\\s*(\\d+-\\d+)", "Level $1");
                // Ensure ",NNNN Points" becomes ", NNNN Points"
                header = header.replaceAll(",\\s*(\\d+\\s*Points)", ", $1");
                factorData.put("header", header.trim());
            }

            correctedFactors.put(correctedKey, factorData);
        }
        result.put("factors", correctedFactors);

        // Add this after parsing the AI response in updateFactorPoints
        Map<String, Map<String, String>> levelPointMap = Map.of(
            "1", Map.ofEntries(
                Map.entry("200", "1-1"),
                Map.entry("350", "1-2"),
                Map.entry("500", "1-3"),
        ...
        for (String factorKey : resultFactors.keySet()) {
            @SuppressWarnings("unchecked")
            Map<String, Object> factorData = (Map<String, Object>) resultFactors.get(factorKey);
            String factorNum = factorKey.replaceAll("[^0-9]", "");
            String pointsStr = String.valueOf(factorData.get("points"));
            Map<String, String> map = levelPointMap.get(factorNum);
            if (map != null && map.containsKey(pointsStr)) {
                String correctLevel = map.get(pointsStr);
                factorData.put("level", correctLevel);
                // Update header if present
                if (factorData.containsKey("header")) {
                    String header = (String) factorData.get("header");
                    header = header.replaceAll("Level [0-9]+-[0-9]+", "Level " + correctLevel);
                    factorData.put("header", header);
                }
            }
        }
        ...
        System.err.println("Error parsing AI response: " + response);
        e.printStackTrace();
