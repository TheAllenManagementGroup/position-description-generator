...
public Map<String, Object> regenerateFactors(@RequestBody Map<String, Object> payload) {
    try {
        ...
        // CRITICAL: Detect current system from existing factor LEVELS/POINTS before regeneration
        ...
        System.out.println("[regenerate-factors] Detected system: " + (isSupervisory ? "GSSG (supervisory)" : "FES (non-supervisory)"));
        System.out.println("[regenerate-factors] Factor hints: " + factorHints.keySet());

        // Route to the correct regeneration flow
        if (isSupervisory) {
            // Expect/produce GSSG factors: 1,2,3,4A,4B,5,6
            return regenerateSupervisoryFactorsWithLock(factorHints, duties, String.valueOf(payload.getOrDefault("supervisoryLevel", "Supervisory")));
        } else {
            // Expect/produce FES factors: 1..9
            return regenerateNonSupervisoryFactorsWithLock(factorHints, duties, String.valueOf(payload.getOrDefault("supervisoryLevel", "Non-Supervisory")));
        }
    } catch (Exception ex) {
        ex.printStackTrace();
