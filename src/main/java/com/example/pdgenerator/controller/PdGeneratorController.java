    ...
    try {
        ...
        if (clientSystemLock != null) {
            ...
            if (clientIsSupervisory != isSupervisory) {
                System.out.println("WARNING: Client system lock differs. Client: " + clientSystemLock + ", Detected: " + (isSupervisory ? "GSSG" : "FES"));
...
public Map<String, Object> updateFactorPoints(@RequestBody Map<String, Object> payload) {
    try {
        ...
        Map<String, Object> out;
        if (isSupervisory) {
            out = updateGSSGFactorPointsLocked(existingFactors, payload, duties, targetGrade, null);
        } else {
            out = updateFESFactorPointsLocked(existingFactors, payload, duties, targetGrade, null);
        }
