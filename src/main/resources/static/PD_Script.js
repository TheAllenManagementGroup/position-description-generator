...
async function saveSectionEdit(title) {
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // Always update factor sections with header in section title, not in content
            if (data.factors && typeof data.factors === 'object') {
                for (let i = 1; i <= 9; i++) {
                    ...
                    if (data.factors[key]) {
                        ...
                        // Store rationale/content only, header will be used as section title
                        pdSections[header] = (factor.rationale || '') + (factor.content ? `\n${factor.content}` : '');
                        // Remove any old Factor section with different header
                        Object.keys(pdSections).forEach(k => {
                            if (/^Factor\s*\d+/i.test(k) && k !== header) {
                                delete pdSections[k];
                            }
                        });
                    }
