...
window.formatGeneratedPD = function(pdText) {
    ...
    text = text.replace(/U\.S\.\s*Department\s*of\s*Justice([A-Z][a-z]+\s*[A-Z][a-z]+)/g, 'U.S. Department of Justice\n$1');
    text = text.replace(/Criminal\s*Division\s*Criminal\s*Division/g, 'Criminal Division');
    text = text.replace(/(Criminal\s*Division)\s*(GS-\d+)/g, '$1\n$2');
    text = text.replace(/(GS-\d+)\s*(Organizational\s*Title:)/g, '$1\n$2');
    ...
    majorHeaders.forEach(header => {
        // Remove any existing asterisks and ensure proper bold formatting
        const cleanHeader = header.replace(/\*/g, '');
        
        // Handle cases where header is concatenated with previous content
        const regex1 = new RegExp(`([a-z\\.])\\s*\\*{0,2}${escapeRegex(cleanHeader)}\\*{0,2}`, 'gi');
        text = text.replace(regex1, `$1\n\n**${cleanHeader}**`);
        
        // Handle standalone headers
        const regex2 = new RegExp(`^\\s*\\*{0,2}${escapeRegex(cleanHeader)}\\*{0,2}\\s*$`, 'gmi');
        text = text.replace(regex2, `**${cleanHeader}**`);
        
        // Ensure there's always a line break after the header
        const regex3 = new RegExp(`\\*\\*${escapeRegex(cleanHeader)}\\*\\*\\s*([A-Z])`, 'g');
        text = text.replace(regex3, `**${cleanHeader}**\n\n$1`);
    });
    
    // Step 4: Fix Factor headers - they need special handling
    text = text.replace(/\*\*Factor(\d+)\s*-\s*([^*]+?)\s*Level(\d+)-(\d+),\s*(\d+)\s*Points\*\*/g, 
        '\n**Factor $1 - $2 Level $3-$4, $5 Points**');
    
    // Handle Factor headers that might not be bolded yet
    text = text.replace(/(^|\n)\s*Factor\s*(\d+)\s*-\s*([^L]+?)\s*Level\s*(\d+)-(\d+),?\s*(\d+)\s*Points/gm, 
    ...
    text = text.replace(/\*\*EVALUATION SUMMARY:\*\*\s*\*\*Total Points:\s*(\d+)\*\*\s*\*\*Final Grade:\s*(GS-\d+)\*\*\s*\*\*Grade Range:\s*([\d-]+)\*\*/g,
        '**EVALUATION SUMMARY:**\n\n**Total Points:** $1\n**Final Grade:** $2\n**Grade Range:** $3');
    
    // Handle cases where summary items might be formatted differently
    text = text.replace(/\*\*Total Points:\*\*(\d+)/g, '**Total Points:** $1');
    text = text.replace(/\*\*Final Grade:\*\*(GS-\d+)/g, '**Final Grade:** $1');
    text = text.replace(/\*\*Grade Range:\*\*([\d-]+)/g, '**Grade Range:** $1');
    
    // Step 7: Fix CONDITIONS OF EMPLOYMENT bullet points
    text = text.replace(/(\*\*CONDITIONS OF EMPLOYMENT:\*\*)\s*-\s*Must/g, '$1\n\n- Must');
    text = text.replace(/-\s*Must([^-]+?)-\s*Must/g, '- Must$1\n- Must');
    text = text.replace(/-\s*Must([^-\n]*?)([A-Z][a-z]+\s+travel)/g, '- Must$1\n- $2');
    
    // Ensure all condition bullets are properly formatted
    const conditionPattern = /(\*\*CONDITIONS OF EMPLOYMENT:\*\*[^*]*?)([^-\n])(Must [^-]*?)(?=Must |$|\*\*)/g;
    text = text.replace(conditionPattern, (match, prefix, separator, condition) => {
        return prefix + '\n- ' + condition.trim();
    });
    ...
    text = text.replace(/[ \t]+$/gm, ''); // Remove trailing spaces on lines
    
    // Step 11: Final pass to ensure sections are properly separated
    majorHeaders.forEach(header => {
        const cleanHeader = header.replace(/\*/g, '');
        text = text.replace(new RegExp(`([a-z])\\s*(\\*\\*${escapeRegex(cleanHeader)}\\*\\*)`, 'g'), 
            '$1\n\n$2');
    });
