...
function splitPDSections(text) {
    ...
    // Normalize various dash characters and special characters
    text = text.replace(/â€"/g, '-');  // HTML entity for en-dash
    text = text.replace(/[–—−]/g, '-');
    text = text.replace(/['']/g, "'").replace(/[""]/g, '"');
    
    // Fix escaped asterisks and backslashes
    text = text.replace(/\\\*/g, '*');
    text = text.replace(/\\\\/g, '');
    
    // Insert line breaks before run-together sections
    text = text.replace(/(Factor\s*\d+\s*[-]\s*[^\n]+?Level\s*\d+-\d+,\s*\d+\s*Points?)/gi, '\n$1');
    text = text.replace(/(Total Points[:\s]*\d+)/gi, '\n$1');
    text = text.replace(/(Final Grade[:\s]*GS-?\d+)/gi, '\n$1');
    text = text.replace(/(Grade Range[:\s]*\d+-\d+)/gi, '\n$1');
    ...
    const sectionPatterns = [
        ...
        /\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*/,
        // Factor headers (with various formats)
        /Factor\s*(\d+)\s*[-]\s*([^\n]+?)\s*Level\s*(\d+-\d+),\s*(\d+)\s*Points?/i,
        // Summary lines
        /Total Points[:\s]*(\d+)/i,
        /Final Grade[:\s]*(GS-?\d+)/i,
        /Grade Range[:\s]*(\d+-\d+)/i,
        // Plain text section headers (at start of line)
        /^(Introduction|Major Duties|Conditions of Employment|Title and Series Determination|Fair Labor Standards Act Determination|Performs other duties as assigned)$/im
    ];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Check markdown header (allow for more flexible patterns)
        let match = line.match(/^\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*$/);
        if (match) {
            let title = match[1].trim();
            // Skip FACTOR EVALUATION and EVALUATION SUMMARY
            if (!/^FACTOR EVALUATION/i.test(title) && 
                !/^EVALUATION SUMMARY$/i.test(title) && 
                !/COMPLETE ANALYSIS/i.test(title)) {
                matches.push({
                    title: title,
                    lineIndex: i,
                    type: 'markdown'
        ...
        // Check Factor header with flexible spacing and dash characters
        match = line.match(/^\*?\*?Factor\s*(\d+)\s*[-â€"–]\s*(.+?)\s*Level\s*(\d+-\d+),\s*(\d+)\s*Points?\*?\*?/i);
        if (match) {
            let title = `Factor ${match[1]} - ${match[2].trim()} Level ${match[3]}, ${match[4]} Points`;
            matches.push({
                title: title,
                lineIndex: i,
                type: 'factor',
        ...
        // Check summary lines (with or without **)
        match = line.match(/^\*?\*?Total Points[:\s]*\*?\*?\s*(\d+)/i);
        if (match) {
            matches.push({
                title: `Total Points: ${match[1]}`,
                lineIndex: i,
                type: 'summary'
            });
            continue;
        ...
        match = line.match(/^\*?\*?Final Grade[:\s]*\*?\*?\s*(GS-?\d+)/i);
        if (match) {
            matches.push({
                title: `Final Grade: ${match[1]}`,
                lineIndex: i,
                type: 'summary'
            });
            continue;
        ...
        match = line.match(/^\*?\*?Grade Range[:\s]*\*?\*?\s*(\d+-\d+)/i);
        if (match) {
            matches.push({
                title: `Grade Range: ${match[1]}`,
                lineIndex: i,
                type: 'summary'
            });
            continue;
        ...
        // Check plain text headers
        if (/^(Introduction|Major Duties|Conditions of Employment|Title and Series Determination|Fair Labor Standards Act Determination|Performs other duties as assigned)$/i.test(line)) {
            matches.push({
                title: line.toUpperCase().trim().replace(/:$/, ''),
                lineIndex: i,
                type: 'plain'
            });
            continue;
    ...
    if (matches.length > 0) {
        let headerLines = lines.slice(0, matches[0].lineIndex);
        let headerContent = headerLines.join('\n').trim();
        // Remove markdown asterisks from header content
        headerContent = headerContent.replace(/^\*\*|\*\*$/gm, '');
        if (headerContent) {
    ...
    for (let i = 0; i < matches.length; i++) {
        let match = matches[i];
        let startLine = match.lineIndex + 1;
        let endLine = (i < matches.length - 1) ? matches[i + 1].lineIndex : lines.length;
        ...
        // For factor sections, remove the level/points line if it's repeated
        if (match.type === 'factor') {
            content = content.replace(/^Factor\s*\d+\s*[-â€"–]\s*.+?Level\s*\d+-\d+,\s*\d+\s*Points?\s*/i, '').trim();
        }
        
        // For summary sections, store empty content
        if (match.type === 'summary') {
            sections[match.title] = '';
        } else if (content) {
            sections[match.title] = content;
        }
    }
    
    // If no HEADER found, extract from first few lines
    if (!sections['HEADER'] && lines.length > 0) {
        let headerLines = lines.slice(0, Math.min(10, lines.length));
        let headerContent = headerLines.filter(l => l.trim()).slice(0, 8).join('\n').trim();
        headerContent = headerContent.replace(/^\*\*|\*\*$/gm, '');
        sections['HEADER'] = headerContent;
    }
