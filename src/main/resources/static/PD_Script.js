...
function renderDutyInputs(dutiesArr) {
    ...
    secondary.forEach((duty, i) => {
        ...
        dutyHtml.innerHTML = `
            ...
            <button type="button" class="remove-duty" onclick="removeDuty(this)" title="Remove duty" style="align-self:flex-start;">×</button>
        `;
        list.appendChild(dutyHtml);
    });

    // Controls (Add Duty kept as before)
    container.innerHTML += `
        <div style="display:flex;gap:0.75rem;align-items:center;margin-top:0.75rem;">
            <button class="btn btn-primary" type="button" onclick="addDuty()"><i class="fa fa-plus"></i> Add Duty</button>
...
/* Add secondary duty (keeps primary as-is) */
window.addDuty = function() {
    ...
    // Wire auto-resize and input sync
...
/* Remove duty button handler (removes secondary duty group). Primary has no remove button. */
window.removeDuty = function(button) {
    const group = button.closest('.duty-input-group');
    if (group) {
        const parentList = group.parentElement;
        group.remove();
        // Re-number secondary duty numbers
...
function updateDutyCounter() {
    ...
    const primary = container.querySelector('.primary-duty-input');
    const secondaryInputs = container.querySelectorAll('.secondary-duty-input');
    const filledSecondary = Array.from(secondaryInputs).filter(t => t.value && t.value.trim().length > 0).length;
    const primaryFilled = primary && primary.value && primary.value.trim().length > 0;

    const totalEntered = (primaryFilled ? 1 : 0) + filledSecondary;
    const minRequired = 1 + 5; // primary + five secondary
    if (primaryFilled && (filledSecondary >= 5)) {
        counter.textContent = `Primary + ${filledSecondary} secondary duties entered (minimum met ✓)`;
        counter.style.color = '#4caf50';
    } else {
        const need = (primaryFilled ? 0 : 1) + Math.max(0, 5 - filledSecondary);
        counter.textContent = `${totalEntered} duties entered (need ${need} more to meet minimum: 1 primary + 5 secondary)`;
        counter.style.color = '#ff6b6b';
    }
}

// Sync current DOM inputs back into formState.duties array: [primary, ...secondary]
function syncFormStateFromInputs() {
    const container = document.getElementById('dutiesContainer');
    if (!container) return;
    const primaryEl = container.querySelector('.primary-duty-input');
    const secondaryEls = container.querySelectorAll('.secondary-duty-input');

    const primaryVal = primaryEl ? primaryEl.value : '';
    const secondaryVals = Array.from(secondaryEls).map(t => t.value || '');

    formState.duties = [primaryVal, ...secondaryVals];
    // Also update unknownDuties if user was in unknown-series flow
    if (Array.isArray(formState.unknownDuties)) {
        formState.unknownDuties = [...formState.duties];
    }
}
...
function splitPDSections(text) {
    ...
    const sectionHeaderRegex = /(?:\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*|^Factor\s*\d+\s*[–-]\s*[^\n]+?Level\s*\d+-[\w]+,\s*\d+\s*Points?|^Total Points:\s*\d+|^Final Grade:\s*GS-?\d+|^Grade Range:\s*\d+-\d+)/gm;
