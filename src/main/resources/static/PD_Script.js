...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- HEADER FIXES ---
    // Split after "U.S. Department of Justice" with proper spacing
    text = text.replace(/(U\.S\. Department of Justice)\s*([A-Z])/g, '$1\n$2');
    // Split after "Criminal Division" (or any Division) with proper spacing
    text = text.replace(/([A-Za-z &]+ Division)\s*(GS-\d+)/g, '$1\n$2');
    // Split after GS grade with proper spacing
    text = text.replace(/(GS-\d+)\s*(Organizational Title:)/g, '$1\n$2');
