...
function renderGradeAnalysisResultStyled(data) {
    ...
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        ...
        try {
            ...
            // Remove any streaming/generation text or duties above the evaluation statement
            let cleaned = fullText.replace(/^(Generating evaluation statement\.\.\.\s*)?/i, '');
            // Remove any lines that look like duties or are not part of the evaluation statement
            cleaned = cleaned.replace(/^(?:\s*[\d\.\-\*]+\s+.*\n)+/gm, '');
            // Remove any text before the first "**Factor" or "**Total Points"
            cleaned = cleaned.replace(/^[\s\S]*?(?=(\*\*Factor|\*\*Total Points|\*\*Final Grade|\*\*Grade Range))/m, '');
