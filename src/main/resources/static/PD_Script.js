...
window.formatGeneratedPD = function(pdText) {
    ...
    // Step 2: Fix major section headers - ensure they're properly separated and bolded
    const majorSections = [
        'HEADER:',
        'INTRODUCTION:',
        'MAJOR DUTIES:',
    ...
    majorSections.forEach(section => {
        // Remove existing asterisks and ensure proper formatting
        const cleanSection = section.replace(/\*/g, '');
        
        // Handle cases where section might be concatenated with previous text
        const regex1 = new RegExp(`([a-z\\.])\\s*\\*?\\*?${escapeRegex(cleanSection)}\\*?\\*?`, 'gi');
        text = text.replace(regex1, `$1\n\n**${cleanSection}**\n`);
        
        // Handle standalone sections
        const regex2 = new RegExp(`^\\s*\\*?\\*?${escapeRegex(cleanSection)}\\*?\\*?\\s*$`, 'gmi');
        text = text.replace(regex2, `\n\n**${cleanSection}**\n`);
    });

    // Step 3: Fix Factor headers with proper formatting
    text = text.replace(/(Factor\s*)(\d+)\s*[-–]\s*([^L]+?)\s*(Level\s*)(\d+)[-–](\d+)\s*,?\s*(\d+)\s*Points?/gi, 
    ...
    // Step 7: Ensure proper spacing after colons in headers
    text = text.replace(/(\*\*[^*]+\*\*)\n*([A-Z])/g, '$1\n\n$2');
