...
function formatUpdatedPD(sectionsObject) {
    ...
    // CRITICAL FIX: Add summary sections with more flexible matching
    const summaryMatchers = [
        { pattern: /^Total\s*Points/i, fallback: 'Total Points' },
        { pattern: /^Final\s*Grade/i, fallback: 'Final Grade' },
        { pattern: /^Grade\s*Range/i, fallback: 'Grade Range' }
    ];
    
    summaryMatchers.forEach(({ pattern, fallback }) => {
        ...
        if (!summaryKey) {
            summaryKey = workingSections[fallback] ? fallback : null;
        }
        ...
        if (summaryKey && workingSections[summaryKey] && workingSections[summaryKey].trim()) {
            ...
            console.log('Summary section not found or empty:', fallback);
        }
    });
    ...
    // Add any remaining sections that weren't included
...
// Also export helper functions that might be needed
...
function saveSectionEdit(title) {
    ...
    if (!currentKey) {
        console.error('No current key found for:', title);
        return;
    }
    
    // Prevent editing summary sections (but allow saving)
    const isSummarySection = /^(Total Points|Final Grade|Grade Range)/i.test(currentKey);
    
    const textareaId = `textarea-${sectionKeyToId(currentKey)}`;
    ...
    const displayId = `display-${sectionKeyToId(currentKey)}`;
...
function updateSectionDisplayAndTextarea(sectionKey, content) {
    ...
    const displayId = `display-${sectionKeyToId(actualKey)}`;
    const textareaId = `textarea-${sectionKeyToId(actualKey)}`;
    
    const displayDiv = document.getElementById(displayId) || document.getElementById(`display-${sectionKeyToId(sectionKey)}`);
    const textareaDiv = document.getElementById(textareaId) || document.getElementById(`textarea-${sectionKeyToId(sectionKey)}`);
...
function undoSectionEdit(title) {
    ...
    const currentKey = getCurrentSectionKey(title);
    console.log('Current key found:', currentKey);
    ...
    const textareaId = `textarea-${sectionKeyToId(currentKey)}`;
    const textarea = document.getElementById(textareaId);
    console.log('Looking for textarea:', textareaId, 'Found:', !!textarea);
    ...
    if (!window.sectionEditStacks[currentKey]) {
        window.sectionEditStacks[currentKey] = [pdSections[currentKey] || ''];
    }
...
function resetSectionEdit(title) {
    ...
    const originalValue = pdSections[currentKey] || '';
...
function getCurrentSectionKey(title) {
    ...
    // Try exact match first
    if (pdSections[title] !== undefined) return title;
    ...
    // As last resort, return the original title
    return title;
}
...
window.formatUpdatedPD = function(sectionsObject) {
    const processedSections = ensureBasicSections(sectionsObject);

    // 1. HEADER and INTRODUCTION
    const sectionOrder = [];
    ['HEADER', 'INTRODUCTION'].forEach(sectionKey => {
        const key = findSectionKeyFlexible(processedSections, sectionKey);
        if (key) sectionOrder.push(key);
    });

    // 2. MAJOR DUTIES (main section, if present)
    const majorDutiesKey = findSectionKeyFlexible(processedSections, 'MAJOR DUTIES');
    if (majorDutiesKey) sectionOrder.push(majorDutiesKey);

    // 3. All "MAJOR DUTY X" sections in order
    const majorDutyKeys = Object.keys(processedSections)
        .filter(k => /^MAJOR DUTY \d+$/i.test(k))
        .sort((a, b) => {
            const aNum = parseInt(a.match(/\d+/)[0], 10);
            const bNum = parseInt(b.match(/\d+/)[0], 10);
            return aNum - bNum;
        });
    majorDutyKeys.forEach(key => sectionOrder.push(key));

    // 4. All "major duty" sections with percent in parentheses
    const percentMajorDutyKeys = Object.keys(processedSections)
        .filter(k => /\(\d+%\)$/.test(k) && !/^MAJOR DUTY \d+$/i.test(k));
    percentMajorDutyKeys.forEach(key => sectionOrder.push(key));

    // 5. FACTOR EVALUATION - COMPLETE ANALYSIS (if present)
    const factorEvalKey = findSectionKeyFlexible(processedSections, 'FACTOR EVALUATION - COMPLETE ANALYSIS');
    if (factorEvalKey) sectionOrder.push(factorEvalKey);

    // 6. All Factor sections in order (use actual keys, not generic)
    const factorKeys = Object.keys(processedSections)
        .filter(k => /^Factor\s*\d+/i.test(k))
        .sort((a, b) => {
            const aNum = parseInt(a.match(/^Factor\s*(\d+)/i)?.[1] || '0', 10);
            const bNum = parseInt(b.match(/^Factor\s*(\d+)/i)?.[1] || '0', 10);
    ...
    // 9. Any remaining sections not already rendered
    const alreadyRendered = new Set(sectionOrder);
    Object.keys(processedSections).forEach(key => {
        if (!alreadyRendered.has(key) && processedSections[key] && processedSections[key].trim()) {
            sectionOrder.push(key);
        }
    });
    ...
    let formatted = '';
    sectionOrder.forEach(section => {
        const content = processedSections[section];
        if (content && content.trim()) {
            formatted += `**${section}**\n${content.trim()}\n\n`;
        }
    });
