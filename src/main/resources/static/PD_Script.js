...
function setupPositionTitleDropdown(subSeries) {
    subSeries = Array.isArray(subSeries) ? subSeries : [];
    ...
    input.addEventListener('input', function() {
        showList();
        // Update formState as user types (for 0301 or manual entry)
    ...
    function showList() {
        ...
        let filtered = subSeries.filter(t => t.toLowerCase().includes(term));
...
async function fetchPositionTitlesFromAPI(jobSeries) {
    ...
    if (positionTitleCache[jobSeries]) {
        console.log(`Using cached position titles for ${jobSeries}:`, positionTitleCache[jobSeries].length, 'titles');
    ...
    try {
        const url = `/api/proxy/usajobs?ResultsPerPage=500&JobCategoryCode=${jobSeries}`;
        ...
        if (data.SearchResult && data.SearchResult.SearchResultItems) {
            console.log('Found', data.SearchResult.SearchResultItems.length, 'job items');
            ...
            console.warn('No SearchResult or SearchResultItems in API response');
        }

        // Convert Set to Array and sort
        const titlesArray = Array.from(titles).sort();
        
        console.log('Extracted', titlesArray.length, 'unique titles');
        ...
        console.log(`Successfully fetched and cached ${titlesArray.length} position titles for ${jobSeries}`);
        return titlesArray;
...
// Updated renderPositionTitleField function with API integration
async function renderPositionTitleField() {
    ...
    if (formState.jobSeries) {
        ...
        try {
            ...
            setupPositionTitleDropdown(titles);
            ...
            const subSeries = (jobSeriesData[formState.jobSeries] && Array.isArray(jobSeriesData[formState.jobSeries].subSeries))
                ? jobSeriesData[formState.jobSeries].subSeries
                : [jobSeriesData[formState.jobSeries]?.title || 'Position Title'];
            ...
            group.innerHTML = `
                ...
                <div style="color:#ef4444; font-size:0.9em; margin-top:0.5rem;">
                    <i class="fas fa-exclamation-triangle"></i> Unable to fetch titles from API. Using cached data.
                </div>
            `;
            
            setupPositionTitleDropdown(subSeries);
    ...
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        ...
        evalResultDiv.innerHTML = `
            <div class="eval-scrollbox">
                ...
                <div id="evalStatementText" style="margin-top:1em; white-space:pre-line; color:#374151;"></div>
            </div>
        `;
        let fullText = '';
        try {
...
function setupPositionTitleDropdown(subSeries) {
    if (!Array.isArray(subSeries) || subSeries.length === 0) {
        console.warn('setupPositionTitleDropdown: No titles provided', subSeries);
        subSeries = [];
    }
    
    console.log('Setting up position title dropdown with', subSeries.length, 'titles');
    
    const input = document.getElementById('positionTitleInput');
    const list = document.getElementById('positionTitleDropdownList');
    if (!input || !list) {
        console.error('Position title input or list not found');
        return;
    }

    // Clear previous event listeners by cloning and replacing the elements
    ...
    function showList() {
        const term = freshInput.value.trim().toLowerCase();
        console.log('Searching for:', term, 'in', subSeries.length, 'titles');
        ...
        if (filtered.length > 0) {
            freshList.innerHTML = filtered.map(t => 
                `<div class="dropdown-item" data-value="${t}">${t}</div>`
            ).join('');
        } else if (subSeries.length === 0) {
            freshList.innerHTML = `<div class="dropdown-item disabled">No titles available</div>`;
        } else {
            freshList.innerHTML = `<div class="dropdown-item disabled">No results found</div>`;
        }
        
        freshList.style.display = 'block';
    }

    freshInput.addEventListener('focus', showList);
    freshInput.addEventListener('input', function() {
        showList();
        // Update formState as user types
        formState.positionTitle = this.value;
        if (typeof window.checkComplete === 'function') {
            window.checkComplete();
        }
    });
    freshInput.addEventListener('blur', () => setTimeout(() => freshList.style.display = 'none', 200));
