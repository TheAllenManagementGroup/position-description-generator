...
function updateDutyNumbers() {
    ...
    groups.forEach((group, idx) => {
        ...
        const textarea = group.querySelector('.duty-input');
        if (numberSpan) numberSpan.textContent = `${idx + 1}.`;
        if (textarea) textarea.placeholder = `Duty/Responsibility ${idx + 1}...`;
    });
}
...
function renderDutyInputs(dutiesArr) {
    ...
    while (duties.length < 6) duties.push('');
    duties.forEach((duty, i) => {
        // Use <textarea> for vertical expansion, set styles for no horizontal scroll
        const group = document.createElement('div');
        group.className = 'duty-input-group';
        group.style.display = 'flex';
        group.style.alignItems = 'flex-start';
        group.style.gap = '0.5em';

        const number = document.createElement('span');
        number.className = 'duty-number';
        number.style.width = '2em';
        number.style.textAlign = 'right';
        number.style.fontWeight = '600';
        number.style.color = '#2563eb';
        number.style.marginTop = '0.5em';
        number.textContent = `${i + 1}.`;

        const textarea = document.createElement('textarea');
        textarea.className = 'duty-input';
        textarea.placeholder = `Duty/Responsibility ${i + 1}...`;
        textarea.value = duty;
        ...
        removeBtn.className = 'remove-duty';
    ...
    // Add Clear Duties button (only one)
    let clearBtn = document.getElementById('clearDutiesBtn');
    if (clearBtn) clearBtn.remove();
    clearBtn = document.createElement('button');
    clearBtn.className = 'btn btn-outline';
    clearBtn.id = 'clearDutiesBtn';
    clearBtn.style.marginTop = '1em';
    clearBtn.innerHTML = `<i class="fas fa-eraser"></i> Clear Duties`;
