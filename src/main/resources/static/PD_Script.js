...
async function updateFactorSectionsAfterEdit(factorTitle, factorContent, sectionDiv) {
    try {
        ...
        if (data.factors) {
            // Remove all old factor keys
            Object.keys(pdSections).forEach(key => {
                if (/^Factor\s*\d+/i.test(key)) delete pdSections[key];
            });
            // Add updated factors
...
function formatUpdatedPD(sectionsObject) {
    // Clean up duplicates and get a fresh copy
    const cleanedSections = cleanupMajorDutyDuplicates({ ...sectionsObject });

    // Helper to flexibly find a section key (case-insensitive, ignores extra spaces)
    function findSectionKeyFlexible(obj, label) {
        const norm = s => s.replace(/\s+/g, '').toUpperCase();
        const target = norm(label);
        return Object.keys(obj).find(k => norm(k).startsWith(target));
    }
    ...
    // 2. Major Duties (summary section)
    ...
    // 3. Major Duty sections, sorted numerically
    const majorDutyKeys = Object.keys(cleanedSections)
    ...
    sectionOrder.push(...majorDutyKeys);

    // 4. Factor Evaluation (summary section)
    ...
    // 5. Factor sections, sorted numerically
    ...
    sectionOrder.push(...factorKeys);

    // 6. Summary sections (Total Points, Final Grade, Grade Range)
    ['Total Points', 'Final Grade', 'Grade Range'].forEach(sectionType => {
        const key = Object.keys(cleanedSections).find(k => k.startsWith(sectionType));
        if (key) sectionOrder.push(key);
    });

    // 7. Other common sections
    [
    ...
    // 8. Add any remaining sections not already included
