...
function splitPDSections(text) {
    ...
    // Normalize other factor spacing
    text = text.replace(/Factor(\d+)\s*-/g, 'Factor $1 -');

    // CRITICAL: Ensure factor headers start on new lines
    // Match "Factor X" at the start of a line or preceded by a period + space
    text = text.replace(/([.]\s+)(Factor\s+\d+[AB]?\s+[-–—])/g, '$1\n$2');
    text = text.replace(/(\n)(\s*)(Factor\s+\d+[AB]?\s+[-–—])/g, '$1$3');
    ...
    // Now split by looking for Factor headers that span multiple lines
    // Factor headers can be: "Factor 3 - TITLE\nLevel X-Y, Z Points"
    const factorHeaderPattern = /^Factor\s+(\d+[AB]?)\s+[-–—]\s+([A-Z][^\n]*(?:\n[A-Z\s()]+)*?)\s*Level\s+(\d+-\d+),\s*(\d+)\s+Points/gm;
    
    let factorMatches = [];
    let match;
    
    // First, find all factor headers
    while ((match = factorHeaderPattern.exec(text)) !== null) {
        const factorNum = match[1];
        const factorTitle = match[2].replace(/\n/g, ' ').trim();
        const level = match[3];
        const points = match[4];
        const fullHeader = `Factor ${factorNum} - ${factorTitle} Level ${level}, ${points} Points`;
    ...
    // Also look for bolded section headers
    const boldHeaderPattern = /\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*/g;
    let boldMatches = [];
    while ((match = boldHeaderPattern.exec(text)) !== null) {
        const header = match[1].trim();
        
        // Skip if this is a factor evaluation header
    ...
    // Also look for summary items
    const summaryPattern = /^((?:Total Points|Final Grade|Grade Range):\s*[\w\d\-+]+)/gm;
    let summaryMatches = [];
    ...
    let allMatches = [...boldMatches, ...factorMatches, ...summaryMatches];
    allMatches.sort((a, b) => a.index - b.index);
    allMatches.push({ title: 'END', index: text.length, endIndex: text.length, raw: '' });
    ...
    for (let i = 0; i < allMatches.length - 1; i++) {
        ...
        const start = currentMatch.endIndex;
