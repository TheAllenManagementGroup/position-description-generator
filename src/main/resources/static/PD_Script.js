...
async function saveSectionEdit(title) {
    console.log('=== SAVE SECTION EDIT START ===');
    console.log('Saving section:', title);
    
    // Don't allow editing summary sections
    if (/^Total Points:|^Final Grade:|^Grade Range:/i.test(title)) {
        console.log('Attempted to edit summary section, skipping');
        return;
    }

    const textarea = document.getElementById(`textarea-${sectionKeyToId(title)}`);
    if (!textarea) {
        console.warn('Textarea not found for:', title);
        return;
    }
    ...
    function collectDuties() {
        // Collect all MAJOR DUTY sections
        const dutyKeys = Object.keys(pdSections)
        ...
        if (dutyKeys.length > 0) {
            return dutyKeys.map(k => String(pdSections[k] || '').trim()).filter(Boolean).join('\n');
        }

        // Fallback to single MAJOR DUTIES section
        const mdKey = Object.keys(pdSections).find(k => /^MAJOR DUTIES$/i.test(k.trim()));
        return mdKey ? String(pdSections[mdKey] || '').trim() : '';
    }
    ...
    function insertFactorsInOrder(factorsObj) {
        ...
        console.log('Inserting factors in order:', order);

        order.forEach(id => {
            const factorKey = `Factor ${id}`;
            const srcKey = Object.keys(factorsObj).find(k =>
                k.toLowerCase() === factorKey.toLowerCase() || k.toLowerCase() === id.toLowerCase()
            );
            if (!srcKey) return;
            
            const factor = factorsObj[srcKey];
            ...
            console.log(`Inserted factor: ${header}`);
        });
    }

    // Factor path
    if (/^Factor\s*\d+/i.test(newTitle)) {
        console.log('Detected factor edit, updating factor points...');
        try {
            ...
            console.log('Factor contents collected:', Object.keys(factorContents));
            ...
            console.log('System detected:', isSupervisory ? 'GSSG' : 'FES');
            ...
            console.log('Factor update response received');

            // CRITICAL: Preserve MAJOR DUTIES before removing factors
            const majorDutiesBackup = {};
            Object.keys(pdSections).forEach(key => {
                if (/^MAJOR DUT(Y|IES)/i.test(key)) {
                    majorDutiesBackup[key] = pdSections[key];
                }
            });

            removeExistingFactorKeys();

            // Restore MAJOR DUTIES
            Object.assign(pdSections, majorDutiesBackup);
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        console.log('Detected MAJOR DUTIES edit, regenerating factors...');
        try {
            const loadingDiv = document.createElement('div');
            ...
            // Update MAJOR DUTIES content
            pdSections['MAJOR DUTIES'] = newContent;

            // Remove other MAJOR DUTY variations but keep MAJOR DUTIES
            Object.keys(pdSections).forEach(key => {
            ...
            console.log('System locked as:', isSupervisory ? 'GSSG' : 'FES');
            ...
            // CRITICAL: Preserve MAJOR DUTIES before removing factors
            const majorDutiesContent = pdSections['MAJOR DUTIES'];
            
            removeExistingFactorKeys();

            // Restore MAJOR DUTIES
            pdSections['MAJOR DUTIES'] = majorDutiesContent;
    ...
    console.log('=== SAVE SECTION EDIT END ===');
}
window.saveSectionEdit = saveSectionEdit;
...
async function updateSummarySections(data) {
    ...
    // Add ALL THREE summary sections - ALWAYS show even if empty
    ...
    if (data.gradeRange !== undefined && data.gradeRange !== null && data.gradeRange !== 'Unknown') {
...
function renderEditableSections() {
    ...
    console.log('pdSections keys:', Object.keys(pdSections));
    ...
    ['HEADER', 'INTRODUCTION'].forEach(sectionKey => {
        ...
        if (key && !renderedSections.has(key)) {
            console.log(`Rendering ${sectionKey} with key: ${key}`);
            container.appendChild(createSectionDiv(key, pdSections[key]));
            renderedSections.add(key);
        }
    });

    // 2. MAJOR DUTIES section - PRESERVE original keys
    const majorDutyKeys = Object.keys(pdSections).filter(k =>
        /^MAJOR DUT(Y|IES)\s*\d*/i.test(k.trim())
    );
    console.log('Major duty keys found:', majorDutyKeys);
    
    if (majorDutyKeys.length > 0) {
        // Render each major duty section separately to preserve them
        majorDutyKeys
            .sort((a, b) => {
            ...
            .forEach(key => {
                if (!renderedSections.has(key)) {
                    console.log(`Rendering major duty: ${key}`);
                    container.appendChild(createSectionDiv(key, pdSections[key]));
                    renderedSections.add(key);
                }
            });
    } else if (pdSections['MAJOR DUTIES']) {
        console.log('Rendering single MAJOR DUTIES section');
        container.appendChild(createSectionDiv('MAJOR DUTIES', pdSections['MAJOR DUTIES']));
        renderedSections.add('MAJOR DUTIES');
    }

    // 3. FACTOR sections - Handle both FES (1-9) and GSSG (1-6 with 4A/4B)
    const factorKeys = Object.keys(pdSections)
        .filter(k => /^Factor\s*\d+[AB]?/i.test(k))
        .sort((a, b) => {
            // Extract factor number and letter
            const aMatch = a.match(/Factor\s*(\d+)([AB])?/i);
            const bMatch = b.match(/Factor\s*(\d+)([AB])?/i);
            ...
            // If same number, sort by letter (4A before 4B)
            const aLetter = aMatch[2] || '';
            const bLetter = bMatch[2] || '';
            return aLetter.localeCompare(bLetter);
        });

    console.log('Factor keys found (sorted):', factorKeys);

    factorKeys.forEach(key => {
        if (!renderedSections.has(key)) {
            console.log(`Rendering factor: ${key}`);
            container.appendChild(createSectionDiv(key, pdSections[key]));
            renderedSections.add(key);
        }
    });

    // 4. Summary sections - place after all factors
    ['Total Points', 'Final Grade', 'Grade Range'].forEach(sectionKey => {
        const key = findSectionKeyFlexible(pdSections, sectionKey);
        if (key && !renderedSections.has(key)) {
            console.log(`Rendering summary: ${key}`);
            container.appendChild(createSectionDiv(key, pdSections[key]));
            renderedSections.add(key);
        }
    });
    ...
    ['CONDITIONS OF EMPLOYMENT', 'TITLE AND SERIES DETERMINATION', 'FAIR LABOR STANDARDS ACT DETERMINATION'].forEach(sectionKey => {
        const key = findSectionKeyFlexible(pdSections, sectionKey);
        if (key && !renderedSections.has(key)) {
            console.log(`Rendering other section: ${key}`);
            container.appendChild(createSectionDiv(key, pdSections[key]));
            renderedSections.add(key);
        }
    });

    // 6. Render any remaining sections
    Object.keys(pdSections).forEach(key => {
        if (!renderedSections.has(key)) {
