...
function renderJobClassificationStep(content) {
    ...
    // --- FIX: Always set up the position title input for 0301 ---
    ...
    if (is0301Series) {
        // Remove dropdown and all dropdown event listeners
        if (positionTitleDropdownList) positionTitleDropdownList.style.display = 'none';
        // Replace the input node to remove all previous event listeners
        const newInput = positionTitleInput.cloneNode(true);
        positionTitleInput.parentNode.replaceChild(newInput, positionTitleInput);
        newInput.value = formState.positionTitle || '';
        newInput.disabled = false;
        newInput.placeholder = "Type position title...";
    ...
    function checkComplete() {
        ...
        const positionTitleFilled = document.getElementById('positionTitleInput').value.trim() !== '';
    ...
    document.getElementById('positionTitleInput').addEventListener('input', function() {
        formState.positionTitle = this.value;
        checkComplete();
    });
    if (orgTitleInput) {
...
function setupPositionTitleDropdown() {
    ...
    // If 0301, do NOT show a dropdown, just allow free text entry
    if (formState.jobSeries === '0301') {
        if (list) list.style.display = 'none';
        input.removeEventListener('focus', showList);
        input.removeEventListener('input', showList);
        input.removeEventListener('blur', hideList);
    ...
    // Otherwise, setup dropdown as usual
    input.addEventListener('focus', showList);
    input.addEventListener('input', showList);
