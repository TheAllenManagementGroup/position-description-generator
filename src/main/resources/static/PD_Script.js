...
function extractSeriesTitle(job, seriesCode, positionTitle) {
    ...
    return seriesTitle
        .replace(/GS-\d{4}\s_i, '') REPLCE _ WITH * /
        .replace(/Series$/i, '')
        .trim();
}
...
// DEMO: Hard-coded group series, job series, and position titles
const demoGroupSeriesOptions = [
    { code: "0300", name: "General Administrative, Clerical, and Office Services" },
    { code: "0500", name: "Accounting and Budget" },
    { code: "1100", name: "Business and Industry" }
];

const demoJobSeriesData = {
    "0343": {
        title: "Management and Program Analyst",
        subSeries: ["Management Analyst", "Program Analyst", "Operations Analyst"],
        keywords: ["analysis", "program", "management", "operations", "evaluation"],
        locations: ["Washington, DC", "New York, NY", "San Francisco, CA"],
        departments: ["Department of Justice", "Department of Homeland Security"],
        groupCode: "0300",
        groupName: "General Administrative, Clerical, and Office Services",
        groups: [{ code: "0300", name: "General Administrative, Clerical, and Office Services" }]
    },
    "0560": {
        title: "Budget Analyst",
        subSeries: ["Budget Analyst", "Financial Analyst"],
        keywords: ["budget", "finance", "analysis", "reporting"],
        locations: ["Washington, DC", "Chicago, IL"],
        departments: ["Department of Treasury"],
        groupCode: "0500",
        groupName: "Accounting and Budget",
        groups: [{ code: "0500", name: "Accounting and Budget" }]
    },
    "1102": {
        title: "Contract Specialist",
        subSeries: ["Contract Specialist", "Procurement Analyst"],
        keywords: ["contract", "procurement", "acquisition", "negotiation"],
        locations: ["Washington, DC", "Dallas, TX"],
        departments: ["Department of Defense"],
        groupCode: "1100",
        groupName: "Business and Industry",
        groups: [{ code: "1100", name: "Business and Industry" }]
    }
};

// Use demo data for job series
let jobSeriesData = demoJobSeriesData;
...
//REMOVE THIS AFTER API BACK UP
function getGroupSeriesOptionsFromAPI() {
    // Use demo group series options
    return demoGroupSeriesOptions;
}
...
async function showTitleSeriesAboveGradeAnalysis(duties, gsGrade, jobSeries, jobTitle, supervisoryLevel) {
    ...
    try {
        const response = await fetch('/api/generate-title-series', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ duties, gsGrade, jobSeries, jobTitle, supervisoryLevel })
        });
        const data = await response.json();
        container.innerHTML = `<div class="card" style="margin-bottom:1.5em;"><strong>Title and Series Determination</strong><br>${data.titleSeriesSection}</div>`;
    } catch (err) {
...
function renderJobClassificationStep(content) {
    // COMMENT OUT API LOADING AND USE DEMO DATA
    // if (!jobSeriesData || Object.keys(jobSeriesData).length === 0) {
    //     content.innerHTML = `
    //         <div class="card">
    //             <div class="card-header">
    ...
    // Use hardcoded demo data for group series options
    const groupSeriesOptions = getGroupSeriesOptionsFromAPI();
...
// Your render function (this works fine too)
...
function setupDutiesValidation() {
    ...
    gradeBtn.onclick = async function() {
        ...
        try {
            ...
            // Setup accept grade button logic to generate Title and Series Determination only after grade is accepted
            setTimeout(() => {
                const acceptGradeBtn = document.getElementById('acceptGradeBtn');
                const gradeAcceptedMsg = document.getElementById('gradeAcceptedMsg');
                if (acceptGradeBtn) {
                    acceptGradeBtn.onclick = function() {
                        formState.gsGrade = safeData.gsGrade || formState.gsGrade || '';
                        acceptGradeBtn.disabled = true;
                        acceptGradeBtn.style.opacity = '0.6';
                        if (gradeAcceptedMsg) gradeAcceptedMsg.style.display = '';
                        // Now generate Title and Series Determination
                        ...
                        const continueDutiesBtn = document.getElementById('continueDutiesBtn');
                        if (continueDutiesBtn) continueDutiesBtn.style.display = '';
                    };
                }
            }, 100);
...
function renderDutiesGradeAnalysis(data) {
    ...
    if (acceptGradeBtn) {
        acceptGradeBtn.onclick = function() {
            ...
            // Generate Title and Series Determination only after grade is accepted
            showTitleSeriesAboveGradeAnalysis(
                (formState.duties || []).join('\n'),
                formState.gsGrade,
                formState.jobSeries,
