...
function renderJobClassificationStep(content) {
                ...
                <div class="form-group">
                    ...
                    <div class="dropdown-wrapper">
                        <input class="form-input" id="positionTitleInput" autocomplete="off"
                            placeholder="${is0301Series ? 'Select "Choose your own title" below' : 'Search or type position title...'}"
                    ...
                    ${is0301Series ? `
                        <div style="margin-top:0.5em;">
                            <select id="positionTitle0301Select" class="form-input">
                                <option value="">Select an option</option>
                                <option value="chooseOwn">Choose your own organizational title</option>
                            </select>
                        </div>
                    ` : ''}
                </div>
    ...
    if (!is0301Series) setupPositionTitleDropdown();
    ...
    // For 0301 series, handle the select and require organizational title
    let requireOrgTitle = false;
    if (is0301Series) {
        const positionTitleInput = document.getElementById('positionTitleInput');
        const positionTitleSelect = document.getElementById('positionTitle0301Select');
        if (positionTitleSelect) {
            positionTitleSelect.onchange = function() {
                if (this.value === 'chooseOwn') {
                    requireOrgTitle = true;
                    positionTitleInput.value = '';
                    formState.positionTitle = '';
                    setupClassificationValidation();
                } else {
                    requireOrgTitle = false;
                }
            };
        }
    }
    ...
    function checkComplete() {
        ...
        if (formState.jobSeries === '0301') {
            // For 0301, require organizational title if "choose your own title" is selected
            const positionTitleSelect = document.getElementById('positionTitle0301Select');
            if (positionTitleSelect && positionTitleSelect.value === 'chooseOwn') {
                positionTitleFilled = orgTitleFilled;
            }
        }
    ...
    continueBtn.onclick = function() {
        if (!continueBtn.disabled) {
            // For 0301, set positionTitle to organizationalTitle if "choose your own title" is selected
            if (formState.jobSeries === '0301') {
                const positionTitleSelect = document.getElementById('positionTitle0301Select');
                if (positionTitleSelect && positionTitleSelect.value === 'chooseOwn') {
