...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- SECTION HEADER FORMATTING (must come before factor formatting) ---
    ...
    // --- FACTOR HEADER FIXES ---
    // Handle both with and without asterisks, and fix spacing issues
    text = text.replace(
    ...
    // Add space before numbered duty sections
    text = text.replace(/(\d+)\.\s+\*\*([^*]+?)\*\*\s*\((\d+%)\)/g, '\n$1. **$2 ($3)**');
    text = text.replace(/(\d+)\.\s+([A-Z][^(\n]+?)\s*\((\d+%)\)/g, '\n$1. **$2 ($3)**');
    
    // Handle bullet points under duties
    text = text.replace(/\n\s*-\s+/g, '\n   - ');
    ...
    text = text.replace(
        /\*\*Final Grade:\s*(GS-\d+)\*\*/gi,
        '\n**Final Grade:** $1'
    );
    text = text.replace(
        /\*\*Grade Range:\s*(\d+[-–]\d+)\*\*/gi,
        '\n**Grade Range:** $1'
    );
    
    // Handle non-bolded versions
...
function renderGeneratedPDStep(content) {
    content.innerHTML = `
        <div class="card">
            ...
            <div class="card-content">
                <div class="pd-display" id="finalPD" style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; padding: 1.5rem; border-radius: 0.5rem; background-color: #f9fafb; line-height: 1.8; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;"></div>
    ...
    if (generatedText) {
        ...
        const paragraphs = finalPDElement.querySelectorAll('p');
        ...
        strongElements.forEach(el => {
            // Check if it's a main section header (all caps with colon)
            if (el.textContent.match(/^[A-Z\s\-]+:$/)) {
                el.style.cssText = `
                    color: #1e40af;
                    font-weight: 700;
                    font-size: 1.1em;
