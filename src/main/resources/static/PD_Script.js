...
function splitPDSections(text) {
    ...
    // normalize common run-together tokens and invisible whitespace
    text = text.replace(/\u00A0/g, ' ').replace(/\u200B/g, ' ');

    // normalize various dash characters to simple hyphen
    text = text.replace(/[–—―]/g, '-');

    // Ensure "Factor4A"/"Factor4B" variants get a space
    text = text.replace(/Factor\s*4A/gi, 'Factor 4A');
    text = text.replace(/Factor\s*4B/gi, 'Factor 4B');
    text = text.replace(/Factor4A/gi, 'Factor 4A');
    text = text.replace(/Factor4B/gi, 'Factor 4B');

    // Fix common run-together Level tokens like "Level4-4,100" -> "Level 4-4, 100"
    text = text.replace(/Level\s*([0-9]+)-([0-9]+)\s*,?\s*([0-9]{2,6})/gi, 'Level $1-$2, $3');

    // If "Level" appears on a line by itself after a Factor header, merge them onto one line
    // e.g. "Factor 3 – TITLE\nLevel 3-4, 900 Points" => "Factor 3 – TITLE Level 3-4, 900 Points"
    text = text.replace(/(Factor\s*\d+[A-Za-z]?[^\n]*?)\n\s*(Level\s*\d+-\d+,\s*\d+\s*Points?)/gi, '$1 $2');

    // Insert a newline before Factor headers that might be run together with previous text
    // Handles bolded or plain Factor headers and 4A/4B forms
    text = text.replace(/([^\n])\s*(\*\*?Factor\s*\d+[A-Za-z]?[^\n]*?Level\s*\d+-\d+,\s*\d+\s*Points\**)/gi, '\n$2');

    // Also ensure bolded plain headers are on their own lines
    text = text.replace(/([^\n])\s*(\*\*[A-Z0-9 \-()\/&,:]+\*\*)/g, '\n$2');

    // Normalize common summary tokens onto their own lines
    ...
    // Build section header regex - capture bold headers OR Factor headers (including 4A/4B) with Level+Points OR summary lines
    const sectionHeaderRegex = /(?:\*\*\s*([A-Z0-9 \-:&(),%\/']+?)\s*\*\*|^Factor\s*\d+[A-Za-z]?(?:\s*[–-]\s*[^\n]*?)?\s*Level\s*\d+-\d+,\s*\d+\s*Points|^Total Points:\s*\d+|^Final Grade:\s*GS-?\d+|^Grade Range:\s*[\d\-]+)/gim;

    // Find all headers and their indices
    let matches = [];
    let m;
    while ((m = sectionHeaderRegex.exec(text)) !== null) {
        const raw = m[0];
        // If bold capture group present, use that as title, else use raw
        const title = (m[1] && m[1].trim()) ? m[1].trim() : raw.trim();
        matches.push({ raw, title: title.replace(/\*\*/g, '').trim(), index: m.index });
    }
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        // Strip surrounding asterisks from content only
        let cleanContent = rawContent.replace(/^\*{1,2}/gm, '').replace(/\*{1,2}$/gm, '').trim();

        // Skip "Factor Evaluation" and "Evaluation Summary" as standalone headers (they wrap other content)
        if (/^FACTOR EVALUATION/i.test(headerTitle) || /^EVALUATION SUMMARY$/i.test(headerTitle)) {
            continue;
        }

        // Treat summary headers as header-only keys with empty content
        if (/^Total Points:/i.test(headerTitle) || /^Final Grade:/i.test(headerTitle) || /^Grade Range:/i.test(headerTitle)) {
            sections[headerTitle] = '';
            continue;
        }

        // If this is a Factor header but content begins with a duplicate header line, try to remove it
        if (/^Factor\s*\d+/i.test(headerTitle)) {
            // If the content itself starts with "Factor X" (duplicated header), drop that leading line
            cleanContent = cleanContent.replace(/^(?:\**\s*Factor\s*\d+[A-Za-z]?[^\n]*\n+)/i, '').trim();
        }

        // Store header -> content (if empty string, still store)
        sections[headerTitle] = cleanContent || '';
    }

    // Fallback splitting if too few sections detected: look for plain headers including Factor 4A/4B
    if (Object.keys(sections).length < 5) {
        const plainHeaders = [
            'HEADER', 'INTRODUCTION', 'MAJOR DUTIES',
            'Factor 1','Factor 2','Factor 3','Factor 4','Factor 5',
            'Factor 6','Factor 7','Factor 8','Factor 9','Factor 4A','Factor 4B',
            'Total Points','Final Grade','Grade Range',
            'CONDITIONS OF EMPLOYMENT','TITLE AND SERIES DETERMINATION','FAIR LABOR STANDARDS ACT DETERMINATION'
        ];
        ...
        while ((m = plainHeaderRegex.exec(text)) !== null) {
            plainMatches.push({ title: m[2], index: m.index + (m[1] ? m[1].length : 0) });
        }
        plainMatches.push({ title: 'END', index: text.length });

        if (plainMatches.length > 1) {
            // HEADER content before first detected header
    ...
    // Ensure summary values present if detected in body but not captured as headers
    if (!Object.keys(sections).some(k => /^Total Points:/i.test(k))) {
        const tp = text.match(/Total Points[:\s]*([0-9]{2,5})/i);
