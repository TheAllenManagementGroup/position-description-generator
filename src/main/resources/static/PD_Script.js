...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- flexible header splitting (handles different Division names) ---
    text = text.replace(/(U\.S\. Department of Justice)(?=[A-Z0-9])/g, '$1\n');
    text = text.replace(/([A-Za-z &]+ Division)(?=\s*[A-Za-z0-9])/g, '$1\n');
    text = text.replace(/(GS-\d+)/g, '\n$1');
    text = text.replace(/(Organizational Title:)/g, '\n$1');
    text = text.replace(/^\s*\*\*HEADER:\*\*/m, '**HEADER:**\n');

    // --- normalize main section headers ---
    ...
    // --- robust Factor formatting ---
    // Matches variants like:
    // Factor1 - Title Level1-7,1250 Points
    // **Factor2 - Title Level2-5,650 Points**
    // Level6-3,60 Points  (odd comma placement handled)
    text = text.replace(/(\*{0,2})\s*Factor\s*(\d+)\s*[-–—]?\s*([\s\S]*?)\s*Level\s*([0-9]+)\s*[-–—]?\s*([0-9]+)\s*,?\s*([0-9,]+)\s*Points(\*{0,2})/gi,
        function(_, asterisks, num, rawTitle, lv1, lv2, pts) {
            // clean title & points
            let title = (rawTitle || '').replace(/[\*\-:]+$/g, '').replace(/\s+/g, ' ').trim();
            let points = (pts || '').replace(/\s+/g, '').trim();
            return `\n\n**Factor ${num} – ${title} (Level ${lv1}-${lv2}, ${points} Points)**\n`;
        }
    );

    // If any Factor lines remain (simpler formats), at least make them bold + spaced
    text = text.replace(/(\*{0,2})\s*Factor\s*(\d+)\s*[-–—]?\s*-\s*([^\n\*]+)/gi,
        function(_, a, num, t) {
    ...
    // --- Major Duties formatting (consistent numbering + bold titles) ---
    text = text.replace(/(\d+)\.\s*([A-Z][^:\n]+?)\s*:/g, '\n\n$1. **$2:**');

    // --- Evaluation summary splitting (if one-liner) ---
    text = text.replace(
        /(\*\*EVALUATION SUMMARY:\*\*)\s*Total Points:?\s*([0-9,]+)\s*Final Grade:?\s*(GS-\d+)\s*Grade Range:?\s*([0-9,-]+)/gi,
        '\n\n$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4'
    );

    // bullets newline-safe
    text = text.replace(/(^|\n)\s*-\s*/g, '\n- ');
...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- Step 1: Normalize excessive blank lines ---
    text = text.replace(/\n{3,}/g, '\n\n');

    // --- Step 2: Extract and separate HEADER info ---
    text = text.replace(/(U\.S\. Department of Justice)([A-Za-z0-9])/g, '$1\n$2');
    text = text.replace(/([A-Za-z &]+ Division)(?=\s*[A-Za-z0-9])/g, '$1\n');
    text = text.replace(/(GS-\d+)/g, '\n$1');
    text = text.replace(/(Organizational Title:)/g, '\n$1');

    // --- Step 3: Major sections formatting ---
    const sections = [
        'HEADER:',
        'INTRODUCTION:',
        'MAJOR DUTIES:',
    ...
    sections.forEach(section => {
        const re = new RegExp(`\\*{0,2}${section.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\*{0,2}`, 'gi');
        text = text.replace(re, `\n\n**${section}**\n`);
    });

    // --- Step 4: Separate HEADER from INTRODUCTION ---
    text = text.replace(/\*\*HEADER:\*\*\s*\n*/, '**HEADER:**\n\n');
    text = text.replace(/\*\*INTRODUCTION:\*\*\s*\n*/, '**INTRODUCTION:**\n\n');

    // --- Step 5: Format Major Duties: number + bold title ---
    text = text.replace(/(\d+)\.\s*([A-Z][^:\n]+?)\s*:/g, '\n\n$1. **$2:**');

    // --- Step 6: Format Factors ---
    // Full Factor line with Level and Points
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*([^\nL]+?)\s*Level\s*([0-9]+)-([0-9]+)\s*,?\s*([\d,]+)\s*Points/gi,
        '\n\n**Factor $1 – $2 (Level $3-$4, $5 Points)**');

    // Simple Factor line fallback
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*-\s*([^\n\*]+)/gi,
        '\n\n**Factor $1 – $2**');
    ...
    // --- Step 8: Conditions & bullets ---
    text = text.replace(/(^|\n)\s*-\s*/g, '\n- ');

    // --- Step 9: Clean spacing & trim each line ---
    text = text.split('\n').map(line => line.trim()).join('\n');
    text = text.replace(/\n{2,}/g, '\n\n');

    // --- Step 10: Optional: add table-style formatting ---
    // e.g., indent factors and duties for readability
    text = text.replace(/(\*\*Factor \d+)/g, '\n$1'); // extra line before factors
    text = text.replace(/(\d+\.\s\*\*[^*]+\*\*:)/g, '\n$1'); // extra line before duties
