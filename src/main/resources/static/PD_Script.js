...
function splitPDSections(text) {
    ...
    // ✅ Add normalization for Factor 3 spacing issues
    text = text.replace(/Factor3/g, 'Factor 3');

    // Insert line breaks before section headers that are run together
    text = text.replace(/(Factor\s*\d+[AB]?\s*[–-]\s*[^\n]+?Level\s*\d+-[\w]+,\s*\d+\s*Points?)/g, '\n$1');

    // CRITICAL FIX: Match Factor headers that span multiple lines
    text = text.replace(/(\n\s*)(Factor\s*\d+[AB]?\s*[–-]\s*[A-Z][^\n]+)\n(Level\s*\d+-\d+,\s*\d+\s*Points)/g, '\n$2 $3');

    // ✅ Add forced line breaks before Factor 3, 4A, and 4B if missing
    text = text.replace(/(?<!\n)(\*\*Factor\s*3\s*[–-])/g, '\n$1');
    text = text.replace(/(?<!\n)(\*\*Factor\s*4A\s*[–-])/g, '\n$1');
    text = text.replace(/(?<!\n)(\*\*Factor\s*4B\s*[–-])/g, '\n$1');
    ...
    // Also insert line breaks before plain section headers
    text = text.replace(/(^|\n)(Introduction|Major Duties|Conditions of Employment|Title and Series Determination|Fair Labor Standards Act Determination)(\n|$)/gi, '\n$2\n');

    text = text.replace(/\n{3,}/g, '\n\n');

    // Enhanced section header regex - now includes Factor 3, 4A, and 4B correctly
