...
function formatUpdatedPD(sectionsObject) {
    ...
    console.log('CRITICAL: Summary sections in cleanedSections:', summaryKeys);
    summaryKeys.forEach(key => {
        console.log(`  CRITICAL: Summary section "${key}"`);
        console.log(`    - content: "${cleanedSections[key]}"`);
        console.log(`    - content length: ${(cleanedSections[key] || '').length}`);
    });
    ...
    sectionOrder.forEach(sectionKey => {
        ...
        if (sectionKey === 'Total Points' || sectionKey === 'Final Grade' || sectionKey === 'Grade Range') {
            console.log(`CRITICAL: Processing summary section: ${sectionKey}`);
            console.log(`  - found key: "${key}"`);
            console.log(`  - content: "${content}"`);
        ...
        // Special handling for summary sections - ALWAYS show them
        if (key && (sectionKey === 'Total Points' || sectionKey === 'Final Grade' || sectionKey === 'Grade Range')) {
            console.log(`  CRITICAL: ADDING summary section: "${key}"`);
            formatted += `\n\n**${key}**\n\n`;
            summaryAdded.push(key);
        } else if (key && content && content.trim()) {
            // HEADER: each variable on its own line
            if (sectionKey === 'HEADER') {
                content = content
    ...
    console.log('CRITICAL: Summary sections added to formatted:', summaryAdded);
    console.log('After ordered sections, checking remaining...');
    ...
    Object.keys(cleanedSections).forEach(key => {
        ...
        if (isSummarySection) {
            console.log(`CRITICAL: Remaining check - Summary: "${key}", already rendered: ${alreadyRendered}`);
        }
        
        if (!alreadyRendered) {
            if (isSummarySection) {
                console.log(`CRITICAL: Adding remaining summary section: "${key}"`);
                formatted += `\n\n**${key}**\n\n`;
                summaryAdded.push(key);
            } else if (cleanedSections[key] && cleanedSections[key].trim()) {
                formatted += `\n\n**${key}**\n\n${cleanedSections[key].trim()}\n`;
    ...
    console.log('CRITICAL: Final summary sections added:', summaryAdded);
    formatted = formatted.replace(/\n{3,}/g, '\n\n').trim();
    
    console.log('Final formatted length:', formatted.length);
    console.log('CRITICAL: Checking if formatted contains "Total Points":', formatted.includes('Total Points'));
    console.log('CRITICAL: Checking if formatted contains "Final Grade":', formatted.includes('Final Grade'));
    console.log('CRITICAL: Checking if formatted contains "Grade Range":', formatted.includes('Grade Range'));
...
function splitPDSections(text) {
    ...
    // This handles DOCX extraction issues where factors are concatenated
    ...
    // Extract summary values from EVALUATION SUMMARY section if not already found
