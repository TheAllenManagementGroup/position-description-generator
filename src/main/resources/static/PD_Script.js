...
function renderEditSectionsStep(content) {
    content.innerHTML = `
        <div style="margin-bottom: 1.5rem;">
            ...
            <!-- Removed left indent/padding -->
            <div id="editableSections" class="rich-editor-wrapper" style="padding:0; margin:0;">
...
function createSectionDiv(title, content) {
    ...
    if (isSummarySection) {
        ...
        if (/^Total Points:/i.test(safeTitle)) {
            ...
            sectionDiv.innerHTML = `<div class="section-header" style="${noIndent}"><span class="section-title-label" style="font-weight:700; font-size:1.1em;">Total Points: ${value}</span></div>`;
        } else if (/^Final Grade:/i.test(safeTitle)) {
            value = safeTitle.replace(/^Final Grade:\s*/i, '');
            sectionDiv.innerHTML = `<div class="section-header" style="${noIndent}"><span class="section-title-label" style="font-weight:700; font-size:1.1em;">Final Grade: ${value}</span></div>`;
        } else if (/^Grade Range:/i.test(safeTitle)) {
            value = safeTitle.replace(/^Grade Range:\s*/i, '');
            sectionDiv.innerHTML = `<div class="section-header" style="${noIndent}"><span class="section-title-label" style="font-weight:700; font-size:1.1em;">Grade Range: ${value}</span></div>`;
        }
        return sectionDiv;
    }
    ...
    if (isNonEditable) {
        sectionDiv.innerHTML = `
            <div class="section-header" style="${noIndent}">
                <span class="section-title-label" style="font-weight:700; font-size:1.1em;">${safeTitle}</span>
            </div>
            <div class="section-content" style="${noIndent}">
                <div class="content-display" id="display-${sectionKeyToId(safeTitle)}" style="${noIndent}">
    ...
    sectionDiv.innerHTML = `
        <div class="section-header" style="${noIndent}">
            <div class="section-header-content" style="display:flex;align-items:center;gap:.75rem;${noIndent}">
        ...
        <div class="section-content" style="${noIndent}">
            <div class="content-display" onclick="toggleSectionEdit('${safeTitle}')" id="display-${sectionKeyToId(safeTitle)}" style="${noIndent}">
                ${cleanContent(content || '') || '<span style="color: #9ca3af; font-style: italic;">Click to add content...</span>'}
            </div>
            <div class="content-editor" style="display: none; ${noIndent}" id="editor-${sectionKeyToId(safeTitle)}">
                <textarea class="editor-textarea" id="textarea-${sectionKeyToId(safeTitle)}" style="width:100%;min-height:120px;margin-left:0;padding-left:0;">${cleanContent(content || '')}</textarea>
                <div class="editor-actions" style="${noIndent}">
