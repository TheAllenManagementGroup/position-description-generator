...
function setupJobSeriesDropdown() {
    ...
    list.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
            ...
            // If 0301 is selected (by code or by full title), instantly re-render the step
            const selectedText = input.value.toLowerCase();
            if (
                formState.jobSeries === '0301' ||
                selectedText.includes('0301 - miscellaneous administration and program')
            ) {
