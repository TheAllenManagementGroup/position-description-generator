...
function renderJobClassificationStep(content) {
    ...
    const is0301Series = formState.jobSeries === '0301';

    // If 0301, clear position titles and set only one option
    if (is0301Series) {
    ...
    content.innerHTML = `
        <div class="card">
            ...
            <div class="card-content">
                ...
                <div class="form-group">
                    ...
                    <div class="dropdown-wrapper">
                        <input class="form-input" id="positionTitleInput" autocomplete="off"
                            placeholder="${is0301Series ? 'Select "Choose your own organizational title"' : 'Search or type position title...'}"
                            value="${formState.positionTitle || ''}"
                            ${!formState.jobSeries ? 'disabled' : ''}
                            ${is0301Series ? 'readonly' : ''}
                        >
                        <div class="dropdown-list" id="positionTitleDropdownList" style="display:none;">
                            ${is0301Series ? `
                                <div class="dropdown-item" data-value="chooseOwn">Choose your own organizational title</div>
                            ` : ''}
                        </div>
                    </div>
                ...
                <div class="form-group">
                    <label class="form-label" for="organizationalTitleInput">Organizational Title <span style="color:#64748b; font-weight:400;">${is0301Series ? '(required)' : '(optional)'}</span></label>
    ...
    // Custom position title dropdown logic for 0301
    const positionTitleInput = document.getElementById('positionTitleInput');
    const positionTitleDropdownList = document.getElementById('positionTitleDropdownList');
    if (is0301Series) {
        positionTitleInput.value = '';
        formState.positionTitle = '';
        positionTitleInput.addEventListener('focus', () => {
            positionTitleDropdownList.style.display = 'block';
        });
        positionTitleInput.addEventListener('blur', () => setTimeout(() => positionTitleDropdownList.style.display = 'none', 150));
        positionTitleDropdownList.addEventListener('mousedown', function(e) {
            if (e.target.classList.contains('dropdown-item') && e.target.dataset.value === 'chooseOwn') {
                positionTitleInput.value = 'Choose your own organizational title';
                formState.positionTitle = '';
                positionTitleDropdownList.style.display = 'none';
                checkComplete();
            }
        });
    } else {
    ...
    // Save organizational title to formState.organizationalTitle
    ...
    // Make position title required for continue (organizational title for 0301 + chooseOwn)
    ...
    function checkComplete() {
        ...
        let positionTitleFilled = positionTitleInput.value.trim() !== '';
        let orgTitleFilled = orgTitleInput.value.trim() !== '';

        if (is0301Series) {
            // Only allow continue if user picked "choose your own organizational title" and filled org title
            positionTitleFilled = positionTitleInput.value === 'Choose your own organizational title' && orgTitleFilled;
        }
    ...
    jobSeriesInput.addEventListener('input', function() {
        formState.jobSeries = this.value;
        // If user selects 0301, clear position title immediately
        if (this.value === '0301') {
            formState.positionTitle = '';
            formState.positionTitles = [];
            if (positionTitleInput) positionTitleInput.value = '';
        }
        checkComplete();
        renderJobClassificationStep(content); // Re-render to update for 0301 logic
    });
    ...
    if (orgTitleInput) {
        orgTitleInput.addEventListener('input', checkComplete);
    }
    checkComplete();

    continueBtn.onclick = function() {
        if (!continueBtn.disabled) {
            // For 0301, set positionTitle to organizationalTitle if "choose your own organizational title" is selected
            if (is0301Series && positionTitleInput.value === 'Choose your own organizational title') {
                formState.positionTitle = formState.organizationalTitle;
            }
    ...
    <div class="card">
        ...
        <div class="card-content">
            ...
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                <button class="btn btn-primary" style="margin-bottom:0;" onclick="addDuty()"><i class="fa fa-plus"></i> Add Duty</button>
...
function renderDutiesStep(content) {
    ...
    <div class="card">
        ...
        <div class="card-content">
            ...
            <div id="dutiesContainer"></div>
            <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                <button class="btn btn-primary" style="margin-bottom:0;" onclick="addDuty()">
                    <i class="fa fa-plus"></i> Add Duty
                </button>
                <button class="btn btn-outline" id="clearDutiesBtn" style="margin-bottom:0;">
                    <i class="fas fa-eraser"></i> Clear Duties
                </button>
                <div style="flex:1"></div>
    ...
    document.getElementById('clearDutiesBtn').onclick = function() {
        formState.duties = [];
        renderDutyInputs(Array(6).fill(''));
        updateDutyCounter();
    };
    ...
    window.renderDutyInputs = function(dutiesArr) {
        const container = document.getElementById('dutiesContainer');
        container.innerHTML = '';
        const duties = Array.isArray(dutiesArr) ? dutiesArr.slice(0, 20) : [];
        while (duties.length < 6) duties.push('');
        duties.forEach((duty, i) => {
            container.innerHTML += `
                <div class="duty-input-group" style="display:flex;align-items:center;">
                    <span class="duty-number" style="width:2em;text-align:right;font-weight:600;color:#2563eb;">${i + 1}.</span>
                    <input type="text" class="duty-input" value="${duty}" placeholder="Duty/Responsibility ${i + 1}...">
    ...
    // Enhanced AI rewrite button logic to prevent horizontal scroll/bleed
    document.getElementById('aiRewriteBtn').onclick = async function() {
        ...
        try {
            ...
            // Remove numbers, bullets, and dashes from the start of each line
            rewritten = rewritten
                .split('\n')
            ...
            resultDiv.innerHTML = `
                <div style="margin-bottom:1em; max-width:100%; word-break:break-word; white-space:pre-line; background:#f3f4f6; border-radius:0.5em; padding:1em;">
                    ...
                    <div style="margin-top:0.5em;">
                        ${rewritten.map(duty => `<div style="margin-bottom:0.5em; word-break:break-word; white-space:pre-line;">${duty}</div>`).join('')}
                    </div>
                </div>
            ...
            document.getElementById('useRewritesBtn').onclick = function() {
                formState.unknownDuties = rewritten;
...
function fixPDFormatting(pdText) {
    ...
    // Ensure each variable in HEADER is on its own line
    text = text.replace(/Job Series:\s*([^\n]+?)Position Title:/gi, 'Job Series: $1\nPosition Title:');
    text = text.replace(/Position Title:\s*([^\n]+?)Agency:/gi, 'Position Title: $1\nAgency:');
    text = text.replace(/Agency:\s*([^\n]+?)Organization:/gi, 'Agency: $1\nOrganization:');
    text = text.replace(/Organization:\s*([^\n]+?)Supervisory Level:/gi, 'Organization: $1\nSupervisory Level:');
    text = text.replace(/Supervisory Level:\s*([^\n]+?)Duties\/Responsibilities Context:/gi, 'Supervisory Level: $1\nDuties/Responsibilities Context:');
    // Add blank line after last header variable before **INTRODUCTION**
    text = text.replace(/(Duties\/Responsibilities Context:[^\n]+)\s*\*\*INTRODUCTION\*\*/gi, '$1\n\n\n**INTRODUCTION**');

    // --- Ensure section headers are bold and on their own line with spacing ---
    ...
    sectionHeaders.forEach(header => {
        // Remove extra asterisks and colons, then bold and add blank lines
        const regex = new RegExp(`\\*{0,2}\\s*${header}\\s*:?\s*\\*{0,2}`, 'gi');
        text = text.replace(regex, `\n\n**${header}**\n\n`);
    });

    // --- Factor header spacing fix ---
    // Fix "Factor1 – Knowledge..." to "Factor 1 – Knowledge..." and "Level1-7,1250 Points" to "Level 1-7, 1250 Points"
    text = text.replace(/Factor\s*([0-9])([–-])\s*/gi, 'Factor $1 $2 ');
    text = text.replace(/Level\s*([0-9]+)-([0-9]+),\s*([0-9]+)\s*Points/gi, 'Level $1-$2, $3 Points');
    // Also fix missing spaces after commas in factor headers
    text = text.replace(/(Level\s*[0-9]+-[0-9]+),([0-9]+\s*Points)/gi, '$1, $2');

    // --- Ensure factor headers are bold and spaced ---
    text = text.replace(
        /\*\*\s*Factor\s*([0-9])\s*[–-]\s*([^\n]+?)\s*Level\s*([0-9]+)-([0-9]+),\s*([0-9]+)\s*Points\s*\*\*/gi,
        '\n\n**Factor $1 – $2 Level $3-$4, $5 Points**\n\n'
    );
    text = text.replace(
        /Factor\s*([0-9])\s*[–-]\s*([^\n]+?)\s*Level\s*([0-9]+)-([0-9]+),\s*([0-9]+)\s*Points/gi,
        '\n\n**Factor $1 – $2 Level $3-$4, $5 Points**\n\n'
    );

    // --- Ensure evaluation summary is spaced and bold ---
    text = text.replace(/Total Points:\s*(\d+)/gi, '\n\n**Total Points: $1**\n');
    text = text.replace(/Final Grade:\s*(GS-\d+)/gi, '\n\n**Final Grade: $1**\n');
    text = text.replace(/Grade Range:\s*([0-9\-+]+)/gi, '\n\n**Grade Range: $1**\n');

    // --- Add blank lines before bullets and after bullets ---
    text = text.replace(/\n\s*-\s*/g, '\n\n- ');
    text = text.replace(/([^\n])(- )/g, '$1\n\n$2');

    // --- Add blank lines between paragraphs (after periods) ---
    ...
    text = text.replace(/(\*\*HEADER\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*INTRODUCTION\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*MAJOR DUTIES\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*FACTOR EVALUATION - COMPLETE ANALYSIS\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*EVALUATION SUMMARY\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*CONDITIONS OF EMPLOYMENT\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*TITLE AND SERIES DETERMINATION\*\*)\s*([^\n]+)/, '$1\n$2');
    text = text.replace(/(\*\*FAIR LABOR STANDARDS ACT DETERMINATION\*\*)\s*([^\n]+)/, '$1\n$2');
...
window.formatGeneratedPD = function(pdText) {
    ...
    // ===== AGGRESSIVE HEADER SECTION FIXES =====
    // Fix run-together header fields (most common issue)
    text = text.replace(/Job Series:\s*GS-(\d+)Position Title:/gi, 'Job Series: GS-$1\n\nPosition Title:');
    text = text.replace(/Position Title:\s*([^\n]+?)Agency:/gi, 'Position Title: $1\n\nAgency:');
    text = text.replace(/Agency:\s*([^\n]+?)Organization:/gi, 'Agency: $1\n\nOrganization:');
    text = text.replace(/Agency:\s*([^\n]+?)(?=[A-Z][a-z]+ [A-Z])/gi, 'Agency: $1\n\n');
    text = text.replace(/Organization:\s*([^\n]+?)Supervisory Level:/gi, 'Organization: $1\n\nSupervisory Level:');
    text = text.replace(/([A-Z][a-z]+ Division)([A-Z][a-z]+ division)/gi, '$1\n\n$2');
    
    // Force separation before any **SECTION** after Supervisory Level
    text = text.replace(/(Supervisory Level:\s*[^\n]+?)(\*\*[A-Z])/gi, '$1\n\n\n$2');
    
    // ===== FIX SECTION HEADERS =====
    // Ensure **HEADER** is properly separated
    text = text.replace(/([^\n])\*\*HEADER\*\*/gi, '$1\n\n**HEADER**\n\n');
    
    // Ensure **INTRODUCTION** is on its own line with spacing
    text = text.replace(/([^\n])\*\*INTRODUCTION\*\*/gi, '$1\n\n**INTRODUCTION**\n\n');
    text = text.replace(/\*\*INTRODUCTION\*\*([A-Z])/gi, '**INTRODUCTION**\n\n$1');
    
    // Fix all major section headers
    const sectionHeaders = [
        'HEADER',
        'INTRODUCTION',
        'MAJOR DUTIES',
    ...
    sectionHeaders.forEach(header => {
        ...
        // Fix if header runs into content
        const contentPattern = new RegExp(`\\*\\*${header}\\*\\*([A-Z])`, 'gi');
        text = text.replace(contentPattern, `**${header}**\n\n$1`);
    });
    
    // ===== AGGRESSIVE FACTOR HEADER FIXES =====
    // Fix Factor1 -> Factor 1, Level1-7 -> Level 1-7
    text = text.replace(/Factor(\d+)\s*[–-]/gi, 'Factor $1 –');
    text = text.replace(/Level(\d+)-(\d+)/gi, 'Level $1-$2');
    
    // Remove commas from points numbers in factors (1,250 -> 1250 for processing)
    text = text.replace(/(\d+),(\d+)\s*Points/gi, '$1$2 Points');
    
    // Format complete factor headers with aggressive spacing
    text = text.replace(
        /([^\n])?(\*{0,2})\s*Factor\s+(\d+)\s+[–-]\s+([^\n]+?)\s+Level\s+(\d+)-(\d+),?\s*(\d+)\s*Points\s*(\*{0,2})/gi,
        '\n\n**Factor $3 – $4 Level $5-$6, $7 Points**\n\n'
    );
    
    // ===== EVALUATION SUMMARY FIXES =====
    text = text.replace(/Total Points:(\d+)/gi, '\n\n**Total Points:** $1\n\n');
    text = text.replace(/Final Grade:\s*(GS-\d+)/gi, '**Final Grade:** $1\n\n');
    text = text.replace(/Grade Range:\s*([\d\-+]+)/gi, '**Grade Range:** $1\n\n');
    
    // ===== MAJOR DUTIES FORMATTING =====
    // Fix duty headers with percentages
            ...
            <div class="card-header">
                <div class="card-title" style="display: flex; align-items: center; gap: 6px;">
                    ...
                    <span class="step-info" style="position:relative;">
                        ...
                        <div class="step-tooltip">
                            Select your federal agency, sub-organization, and lowest level office/division. All three are required to continue.
                        </div>
                    </span>
                </div>
                <div class="card-description">Select the federal agency and sub-organization</div>
            </div>
...
function setupAgencyDropdowns(agencyData) {
    ...
    // Sort agencies and subOrgs alphabetically
    agencyData.sort((a, b) => a.name.localeCompare(b.name));
    agencyData.forEach(agency => {
        agency.subOrgs.sort((a, b) => a.localeCompare(b));
    });

    // Replace select with input for agency search/autocomplete
    let agencyInput = document.getElementById('agencyInput');
    let agencyList = document.getElementById('agencyDropdownList');
    if (!agencyInput) {
        agencyInput = document.createElement('input');
        agencyInput.type = 'text';
        agencyInput.className = 'form-input';
        agencyInput.id = 'agencyInput';
        agencyInput.placeholder = 'Search or select federal agency...';
        federalAgencyDropdown.parentElement.replaceChild(agencyInput, federalAgencyDropdown);
    }
    if (!agencyList) {
        agencyList = document.createElement('div');
        agencyList.id = 'agencyDropdownList';
        agencyList.className = 'dropdown-list';
        agencyList.style.display = 'none';
        agencyInput.parentElement.appendChild(agencyList);
    }

    // Restore agency input value from formState if present
    if (formState.federalAgency) {
        agencyInput.value = formState.federalAgency;
    }

    // Replace select with input for sub-org search/autocomplete
    let subOrgInput = document.getElementById('subOrgInput');
    let subOrgList = document.getElementById('subOrgDropdownList');
    if (!subOrgInput) {
        subOrgInput = document.createElement('input');
        subOrgInput.type = 'text';
        subOrgInput.className = 'form-input';
        subOrgInput.id = 'subOrgInput';
        subOrgInput.placeholder = 'Search or select sub-organization...';
        subOrgDropdown.parentElement.replaceChild(subOrgInput, subOrgDropdown);
    }
    if (!subOrgList) {
        subOrgList = document.createElement('div');
        subOrgList.id = 'subOrgDropdownList';
        subOrgList.className = 'dropdown-list';
        subOrgList.style.display = 'none';
        subOrgInput.parentElement.appendChild(subOrgList);
    }

    // Restore sub-org input value from formState if present
    if (formState.subOrg) {
        subOrgInput.value = formState.subOrg;
        subOrgInput.disabled = false;
    }

    // Render agency options with filter
    function showAgencyList() {
        const term = agencyInput.value.trim().toLowerCase();
        let results = agencyData.filter(agency => agency.name.toLowerCase().includes(term));
        agencyList.innerHTML = results.length
            ? results.map(agency => `<div class="dropdown-item" data-value="${agency.name}">${agency.name}</div>`).join('')
            : `<div class="dropdown-item disabled">No results found</div>`;
        agencyList.style.display = 'block';
    }

    agencyInput.addEventListener('focus', showAgencyList);
    agencyInput.addEventListener('input', showAgencyList);
    agencyInput.addEventListener('blur', () => setTimeout(() => agencyList.style.display = 'none', 150));

    agencyList.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
            agencyInput.value = e.target.textContent;
    ...
    // Render sub-org options with filter
    function showSubOrgList() {
        const selectedAgency = formState.federalAgency;
        const term = subOrgInput.value.trim().toLowerCase();
        const agency = agencyData.find(a => a.name === selectedAgency);
        let results = [];
        if (agency) {
            results = agency.subOrgs.filter(subOrg => subOrg.toLowerCase().includes(term));
        }
        subOrgList.innerHTML = results.length
            ? results.map(subOrg => `<div class="dropdown-item" data-value="${subOrg}">${subOrg}</div>`).join('')
            : `<div class="dropdown-item disabled">No results found</div>`;
    ...
    subOrgList.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
            subOrgInput.value = e.target.textContent;
    ...
    // Lowest org input with auto-detection
    let inputTimeout;
    ...
    // Auto-detection function for agency and sub-organization (unchanged)
            ...
            if (searchInput.includes(keyword)) {
                ...
                if (agencyMatch) {
                    ...
                    agencyMatch.subOrgs.forEach(subOrg => {
                        ...
                        let score = 0;
                        if (searchInput.includes(subOrgLower.split('(')[0].trim())) score += 100;
                        const inputWords = searchInput.split(/\s+/);
                        const subOrgWords = subOrgLower.split(/\s+/);
                        inputWords.forEach(inputWord => {
                            subOrgWords.forEach(subOrgWord => {
                                if (inputWord.length > 2 && subOrgWord.includes(inputWord)) score += 10;
                                if (subOrgWord.length > 2 && inputWord.includes(subOrgWord)) score += 10;
                            });
                        });
                        const acronymMatch = subOrg.match(/\(([^)]+)\)/);
                        if (acronymMatch) {
                            const acronym = acronymMatch[1].toLowerCase();
                            if (searchInput.includes(acronym)) score += 50;
                        }
                    ...
                    if (bestSubOrg && bestSubScore > 0) {
                        ...
                        formState.federalAgency = agency;
                        showSubOrgList();
                        subOrgInput.value = bestSubOrg;
                        formState.subOrg = bestSubOrg;
                        subOrgInput.disabled = false;
                        subOrgList.style.display = 'none';
                        updateContinueButton();
                        return;
    ...
    function updateContinueButton() {
        const lowestOrgFilled = lowestOrgInput.value.trim() !== '';
        const agencyFilled = agencyInput.value && agencyInput.value !== '';
        const subOrgFilled = subOrgInput.value && subOrgInput.value !== '';
        const canContinue = lowestOrgFilled && agencyFilled && subOrgFilled;
...
function setupJobSeriesDropdown() {
    ...
    list.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
            ...
            // If 0301 - Miscellaneous Administration And Program is selected, instantly re-render the step
            if (
                formState.jobSeries === '0301' ||
                input.value.toLowerCase().includes('0301 - miscellaneous administration and program')
            ) {
                renderStep(3); // Instantly refresh UI for 0301 logic
                return;
            }
...
function setupClassificationValidation() {
    ...
    function checkComplete() {
        const groupSeriesFilled = groupSeriesInput.value.trim() !== '';
        const jobSeriesFilled = jobSeriesInput.value.trim() !== '';
        let positionTitleFilled = positionTitleInput && positionTitleInput.value.trim() !== '';
        let orgTitleFilled = orgTitleInput && orgTitleInput.value.trim() !== '';

        // For 0301, require "Choose your own organizational title" AND organizational title filled
    ...
    continueBtn.onclick = function() {
        if (!continueBtn.disabled) {
            if (formState.jobSeries === '0301' && positionTitleInput.value === 'Choose your own organizational title') {
                formState.positionTitle = formState.organizationalTitle;
            }
            if (formState.usedAIRecommendation) {
                renderStep(5);
...
function setupGenerateButton() {
    ...
    btn.onclick = async function() {
        ...
        try {
            await tryNonStreamingGeneration(payload); // <-- Use non-streaming
        } catch (err) {
            aiResult.innerHTML = `
...
function formatUpdatedPD(sectionsObject) {
    ...
    sectionOrder.forEach(sectionKey => {
        ...
        if (content && content.trim()) {
            ...
            // Bold section header and add blank lines before/after
            formatted += `\n\n**${sectionKey}**\n\n${content}\n`;
        }
    });
    ...
    Object.keys(cleanedSections).forEach(key => {
        if (!sectionOrder.some(section => findSectionKeyFlexible(cleanedSections, section) === key) && cleanedSections[key] && cleanedSections[key].trim()) {
...
function createSectionDiv(title, content) {
    ...
    // Sanitize title for selector (remove newlines and quotes)
    const safeTitle = title.replace(/[\n\r"]/g, ' ').replace(/\s+/g, ' ').trim();

    // Check if this section already exists in the DOM
    const existingSection = document.querySelector(`[data-section-title="${safeTitle}"]`);
    if (existingSection) {
        console.log(`Section "${safeTitle}" already exists in DOM, removing old version`);
    ...
    if (!window.sectionEditStacks[safeTitle]) window.sectionEditStacks[safeTitle] = [cleanedContent];

    const sectionDiv = document.createElement('div');
    sectionDiv.className = 'editor-section default';
    sectionDiv.setAttribute('data-section-title', safeTitle); // Use sanitized title

    if (safeTitle.startsWith('MAJOR DUTY ')) {
        sectionDiv.classList.add('major-duty-item');
    }
    if (safeTitle.startsWith('Factor')) {
    ...
    sectionDiv.innerHTML = `
        ...
        <div class="section-content">
            ...
            <div class="content-editor" style="display: none;" id="editor-${sectionKeyToId(safeTitle)}">
                <textarea class="editor-textarea" id="textarea-${sectionKeyToId(safeTitle)}" style="width:100%;min-height:120px;">${cleanedContent}</textarea>
    ...
    if (!isSummarySection) {
        setTimeout(() => {
            const textarea = sectionDiv.querySelector(`#textarea-${sectionKeyToId(safeTitle)}`);
            if (textarea) {
                textarea.disabled = false;
                textarea.readOnly = false;
                textarea.oninput = function() {
                    const stack = window.sectionEditStacks[safeTitle];
...
function renderDutyInputs(dutiesArr) {
    ...
    // Add Clear Duties button
    container.innerHTML += `
        <button class="btn btn-outline" id="clearDutiesBtn" style="margin-top:1em;">
            <i class="fas fa-eraser"></i> Clear Duties
        </button>
    `;
    updateDutyCounter();

    // Setup clear duties handler
    document.getElementById('clearDutiesBtn').onclick = function() {
        formState.duties = [];
        renderDutyInputs(Array(6).fill(''));
        updateDutyCounter();
...
function setupAIRewriteButton(textareaId, resultId, buttonId) {
    ...
    btn.onclick = async function() {
        ...
        // Ensure rewritten duties are visually contained in the card and wrap lines
        resultDiv.innerHTML = `
        ...
        try {
            ...
            // Display rewritten duties, each on its own line, contained and wrapped
            resultDiv.innerHTML = `
                <div style="margin-bottom:1em; background:#f3f4f6; border-radius:0.5em; padding:1em; max-width:100%; word-break:break-word; white-space:pre-line;">
                    <strong>Rewritten Duties:</strong>
                    <div style="margin-top:0.5em;">
            ...
            // When user clicks "Use Rewrites", fill the duties boxes (without numbers/bullets)
            document.getElementById('useRewritesBtn').onclick = function() {
                formState.unknownDuties = rewritten;
                renderDutyInputs(formState.unknownDuties);
...
function splitPDSections(text) {
    ...
    // Section header regex (matches bolded and plain headers, including Factor headers with colon)
    const sectionHeaderRegex = /(?:\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*|Factor\s*\d+\s*[–-]\s*[^:\n]+Level\s*\d+-\d+,\s*\d+\s*Points:|Factor\s*\d+\s*[–-]\s*[^:\n]+Level\s*\d+-\d+,\s*\d+\s*Points|Total Points:\s*\d+|Final Grade:\s*GS-?\d+|Grade Range:\s*\d+-\d+|MAJOR DUTIES)/g;

    // Find all section header matches and their indices
    let matches = [];
    let match;
    while ((match = sectionHeaderRegex.exec(text)) !== null) {
        let header = match[1] || match[0];
        // Remove bold markers but keep colons in header
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        // Remove leading/trailing asterisks ONLY (do not add or remove colons or other characters)
        const cleanContent = rawContent.replace(/^\*{1,2}/gm, '').replace(/\*{1,2}$/gm, '');
        const header = matches[i].title;
        // Only add section if content is not empty
        if (!sections[header] && cleanContent) {
            sections[header] = cleanContent;
        }
    }

    // --- Ensure summary sections are always present and saved ---
    // Extract summary values from the original text if not already present
    const totalPointsMatch = text.match(/Total Points\s*:\s*(\d+)/i);
    if (totalPointsMatch && !Object.keys(sections).some(k => /^Total Points:/i.test(k))) {
        sections[`Total Points: ${totalPointsMatch[1]}`] = `Total Points: ${totalPointsMatch[1]}`;
    }
    const finalGradeMatch = text.match(/Final Grade\s*:\s*(GS-?\d+)/i);
    if (finalGradeMatch && !Object.keys(sections).some(k => /^Final Grade:/i.test(k))) {
        sections[`Final Grade: ${finalGradeMatch[1]}`] = `Final Grade: ${finalGradeMatch[1]}`;
    }
    const gradeRangeMatch = text.match(/Grade Range\s*:\s*([\d\-]+)/i);
    if (gradeRangeMatch && !Object.keys(sections).some(k => /^Grade Range:/i.test(k))) {
        sections[`Grade Range: ${gradeRangeMatch[1]}`] = `Grade Range: ${gradeRangeMatch[1]}`;
    }
...
function setupPositionTitleDropdown() {
    ...
    function showList() {
        // Only show "Choose Your Own Organizational Title" for 0301
        if (formState.jobSeries === '0301') {
            list.innerHTML = `<div class="dropdown-item" data-value="chooseOwn">Choose Your Own Organizational Title</div>`;
            list.style.display = 'block';
            return;
        }

        // Otherwise, show normal position titles
        let titles = [];
    ...
    list.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
            input.value = e.target.dataset.value === 'chooseOwn'
                ? 'Choose your own organizational title'
                : e.target.dataset.value;
            formState.positionTitle = e.target.dataset.value === 'chooseOwn'
                ? ''
                : e.target.dataset.value;
            list.style.display = 'none';
            setupClassificationValidation();
