...
function splitPDSections(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (match) {
            ...
            if (!/^FACTOR EVALUATION/i.test(title) && !/^EVALUATION SUMMARY$/i.test(title) && !/COMPLETE ANALYSIS/i.test(title)) {
                matches.push({
                    title: title,
                    lineIndex: i,
                    type: 'markdown'
        ...
        // Check Factor header (can span multiple formats)
        match = line.match(/^Factor\s*(\d+)\s*[-]\s*(.+?)\s*Level\s*(\d+-\d+),\s*(\d+)\s*Points?/i);
        if (match) {
        ...
        // Check summary lines
        match = line.match(/^Total Points[:\s]*(\d+)/i);
        if (match) {
            matches.push({
        ...
        match = line.match(/^Final Grade[:\s]*(GS-?\d+)/i);
        if (match) {
            matches.push({
        ...
        match = line.match(/^Grade Range[:\s]*(\d+-\d+)/i);
        if (match) {
            matches.push({
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        if (match.type === 'factor') {
            content = content.replace(/^Factor\s*\d+\s*[-]\s*.+?Level\s*\d+-\d+,\s*\d+\s*Points?\s*/i, '').trim();
        }
