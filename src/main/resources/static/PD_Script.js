...
async function saveSectionEdit(title) {
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // Use the global detectSystemFromHeaders function
...
async function updateFactorSections(factors) {
    for (const [factorKey, factorData] of Object.entries(factors)) {
        ...
        // **FIX**: Always build header as: FactorX - Name LevelY-Z, NNNN Points
        ...
        const content = factorData.rationale || factorData.content || pdSections[currentKey] || '';

        // **FIX**: Update pdSections with new header
        if (currentKey !== newHeader) {
            pdSections[newHeader] = content;
            delete pdSections[currentKey];
            
            // **FIX**: Update the DOM element's data-section-key attribute
            updateSectionKeyInDOM(currentKey, newHeader);
        } else {
            pdSections[currentKey] = content;
        }

        // **FIX**: Update both display and textarea
...
async function updateSummarySections(data) {
    ...
    for (const update of updates) {
        if (update.value !== undefined && update.value !== null) {
            ...
            // **FIX**: Remove all old keys for this summary type
            Object.keys(pdSections).forEach(k => {
            ...
            // **FIX**: Add new summary section
            pdSections[headerWithValue] = '';
            updateSectionDisplayAndTextarea(headerWithValue, '');
        }
    }
    
    // **FIX**: Force re-render of sections to show updated summaries
    if (typeof renderEditableSections === 'function') {
        renderEditableSections();
    }
}
