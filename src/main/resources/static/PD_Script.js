...
function renderJobClassificationStep(content) {
    ...
    document.getElementById('previousStepBtn').onclick = function() {
        renderStep(2); // Go back to Agency Setup step
    };
...
function formatUpdatedPD(sectionsObject) {
    ...
    sectionOrder.forEach(sectionKey => {
        ...
        if (content && content.trim()) {
            // HEADER: ensure each variable is on its own line
            ...
            if (sectionKey === 'MAJOR DUTIES') {
                // Split on numbered duties (e.g., 1. ... 2. ...)
                content = content.replace(/(\d+\.\s)/g, '\n$1').replace(/\n{2,}/g, '\n').trim();
            }

            // FACTOR sections: ensure header/content separation
            if (/^Factor \d+$/i.test(sectionKey)) {
                // If the header is not bolded, add bold and blank lines
                content = content.replace(/^\*\*(.*?)\*\*/g, '').trim();
            }

            // EVALUATION SUMMARY: ensure summary sections are present
            if (sectionKey === 'EVALUATION SUMMARY') {
                // Add summary sections if not present
                ['Total Points', 'Final Grade', 'Grade Range'].forEach(sumKey => {
                    const sumSection = findSectionKeyFlexible(cleanedSections, sumKey);
                    if (sumSection && cleanedSections[sumSection]) {
                        content += `\n\n${cleanedSections[sumSection]}`;
                    }
                });
            }
...
function splitPDSections(text) {
    ...
    if (totalPointsMatch && !Object.keys(sections).some(k => /^Total Points:/i.test(k))) {
        sections[`Total Points: ${totalPointsMatch[1]}`] = '';
    }
...
async function generateDOCX(content, filename) {
    ...
    blocks = blocks.flatMap(block => {
        if (block.startsWith('MAJOR DUTIES')) {
            // Split numbered duties (e.g., 1. ... 2. ...)
            return block.split(/(?=\d+\.\s)/g).map(duty => duty.trim()).filter(Boolean);
        }
        return [block];
    });
