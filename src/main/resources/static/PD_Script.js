...
function renderSidebar(currentStep) {
    ...
    steps.forEach((step, idx) => {
        const stepNum = idx + 1;
        const clickable = stepNum <= window.maxStepCompleted;
        ...
        if (isCompleted) {
            ...
            iconHtml = `<div class="step-icon current" style="background:${darkTeal};color:${green};border:2px solid ${green};">
                <i class="fas ${step.icon}" style="color:${green};"></i>
            </div>`;
        } else {
            iconHtml = `<div class="step-icon upcoming" style="background:#fff;color:#9ca3af;border:2px solid #d1d5db;">
        ...
        container.innerHTML += `
            <div class="${stepClass}" ${clickable ? `style="cursor:pointer;${highlightStyle}" onclick="renderStep(${step.id})"` : `style="${highlightStyle}"`}>
                ${iconHtml}
    ...
    if (searchInput.includes(keyword)) {
        ...
        if (agencyMatch) {
            ...
            agencyMatch.subOrgs.forEach(subOrg => {
                ...
                // Direct inclusion match (ignore acronym in parentheses)
                const subOrgMain = subOrgLower.split('(')[0].trim();
                if (searchInput.includes(subOrgMain) && subOrgMain.length > 2) {
                    score += 100;
                }
...
agencyData.forEach(agency => {
    agency.subOrgs.forEach(subOrg => {
        ...
        // Check if input contains significant parts of sub-org name (ignore acronym)
        const subOrgMain = subOrgLower.split('(')[0].trim();
