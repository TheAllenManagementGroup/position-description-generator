...
window.formatGeneratedPD = function(pdText) {
    ...
    // 1. Fix header section: add newlines between department, division, grade, and title
    text = text.replace(
        /(U\.S\. Department of Justice)(Criminal Division)/g,
        '$1\n$2'
    );
    text = text.replace(
        /(Criminal Division)(Criminal Division)/g,
        '$1'
    );
    text = text.replace(
        /(Criminal Division)(GS-\d+)/g,
        '$1\n$2'
    );
    text = text.replace(
        /(GS-\d+)(Organizational Title:)/g,
        '$1\n$2'
    );
    // Add newline before header if missing
    text = text.replace(/^\*\*HEADER:\*\*/gm, '\n**HEADER:**\n');

    // 2. Ensure introduction section starts on a new line after header
    text = text.replace(/\*\*INTRODUCTION:\*\*/g, '\n\n**INTRODUCTION:**\n');

    // 3. Major section headers
    [
        'MAJOR DUTIES:', 'FACTOR EVALUATION - COMPLETE ANALYSIS:',
        'CONDITIONS OF EMPLOYMENT:', 'TITLE AND SERIES DETERMINATION:', 'FAIR LABOR STANDARDS ACT DETERMINATION:'
    ].forEach(section => {
        text = text.replace(new RegExp(`\\*\\*${section.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}\\*\\*`, 'g'), `\n\n**${section}**\n`);
    });

    // 4. Factor headers: ensure each starts on a new line, is bold, and spaced
    text = text.replace(
        /Factor\s*(\d+)\s*-\s*([^\n]+?)\s*Level\s*(\d+)-(\d+),\s*(\d+)\s*Points/g,
        '\n\n**Factor $1 – $2 Level $3-$4, $5 Points**\n'
    );

    // 5. Numbered major duties: ensure spacing and bolding
    text = text.replace(/(\d+)\.\s*([A-Z][^:]+?):/g, '\n\n$1. **$2:**');

    // 6. Evaluation summary: split summary line into separate bold lines
