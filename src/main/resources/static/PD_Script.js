...
async function saveSectionEdit(title) {
    ...
    // Build factor payload from current sections (preserve existing text)
        ...
        keys.forEach(k => {
            ...
            if (m) {
                ...
                if (content && String(content).trim()) {
                    factorContents[reqKey] = String(content).trim(); // send existing factor text
                }
            }
        });
    ...
    // Combine duties for backend AI
    function collectDuties() {
        // Prefer numbered duties if present
        const numbered = Object.keys(pdSections)
            ...
            .map(k => String(pdSections[k] || '').trim())
            .filter(Boolean);
    ...
    // Remove only factor keys (preserve MAJOR DUTIES and everything else)
    ...
    // Insert factors (headers only); do not touch MAJOR DUTIES
    function insertFactorsInOrder(factorsObj) {
        ...
        order.forEach(id => {
            ...
            // Use server-provided rationale/content if any, otherwise keep existing text we already sent
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            const payload = {
                ...
                duties: collectDuties() // send duties to backend so AI can use them
            };
            ...
            // Only replace factor sections; keep MAJOR DUTIES and other sections intact
