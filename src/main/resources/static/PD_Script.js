...
function formatUpdatedPD(sectionsObject) {
    ...
    // Deduplicate all "MAJOR DUTY X" and "MAJOR DUTIESX"
    const majorDutyMap = {};
    Object.keys(cleanedSections).forEach(key => {
    ...
    // Always include summary sections if present
...
function saveSectionEdit(title) {
    // Prevent editing summary sections
    ...
    if (sectionDiv) {
        ...
        if (headerLabel && headerLabel.textContent !== title) {
            newTitle = headerLabel.textContent;
            // Move stacks/history to new key
            window.sectionEditStacks[newTitle] = window.sectionEditStacks[title] || [];
            window.sectionEditHistory[newTitle] = window.sectionEditHistory[title] || [];
            delete window.sectionEditStacks[title];
            delete window.sectionEditHistory[title];
        }
    }
...
// Helper to get the current section key (handles title changes)
