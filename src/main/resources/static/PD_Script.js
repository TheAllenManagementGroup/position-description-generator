...
async function saveSectionEdit(title) {
    ...
    // Save edited content to history / stacks
    let newTitle = title;
    ...
    // Update pdSections and UI immediately
    ...
    // Normalize various factor key variants into canonical "Factor N" or "Factor NA" (e.g., "Factor 4A")
    ...
    // Helper to collect factor content with SYSTEM DETECTION
    function collectFactorContents() {
        ...
        keys.forEach(k => {
            ...
            if (m) {
                ...
                const reqKey = canonical.replace(/\s+/g, ' ');
                const content = pdSections[k];
                if (content && String(content).trim()) {
                    factorContents[reqKey] = String(content).trim();
                }
            }
        });

        // Include originalLevel hints if present
        Object.keys(pdSections).forEach(k => {
            const m = k.match(/^(Factor\s*\d+[A-Za-z]?)_originalLevel$/i);
            if (m) {
                const reqKey = m[1].replace(/\s+/g, ' ');
                factorContents[reqKey + "_originalLevel"] = pdSections[k];
            }
        });
    ...
    // Remove any pdSections keys that are factor-related (robustly)
    ...
    // Insert factors in correct order; ensures 4A then 4B (when supervisory)
    function insertFactorsInOrder(factorsObj) {
        const has4A = Object.keys(factorsObj).some(k => /^Factor\s*4A$/i.test(k) || /^4A$/i.test(k));
        const has4B = Object.keys(factorsObj).some(k => /^Factor\s*4B$/i.test(k) || /^4B$/i.test(k));
        ...
        function nameFor(id) {
            const n = getFactorName ? getFactorName(id.replace(/[A-Za-z]/g, '')) : null;
            if (id.toUpperCase() === '4A') return 'Personal Contacts (Nature of Contacts)';
            if (id.toUpperCase() === '4B') return 'Personal Contacts (Purpose of Contacts)';
            return n || `Factor ${id}`;
        }
        ...
        Object.keys(factorsObj || {}).forEach(k => {
            ...
            else canonMap[k] = factorsObj[k];
        });

        order.forEach(id => {
            const factorKey = `Factor ${id}`;
            const responseKey = Object.keys(canonMap).find(k =>
                k.toLowerCase() === factorKey.toLowerCase() ||
                k.toLowerCase() === id.toLowerCase() ||
                k.toLowerCase() === (`Factor ${id}`).toLowerCase()
            );
            if (!responseKey) return;
            const factor = canonMap[responseKey];
            if (!factor) return;
            ...
            if (typeof factor === 'string') {
                ...
                level = factor.level || factor.Level || '';
                points = factor.points || factor.Points || '';
                rationale = factor.rationale || factor.content || factor.Rationale || '';
            }

            const factorName = nameFor(id);
            const header = `Factor ${id} - ${factorName}` + (level ? ` Level ${level}` : '') + (points ? `, ${points} Points` : '');
            pdSections[header] = (rationale || '').toString().trim();
        });
    }

    // If this is a Factor section
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // CRITICAL: Detect system from existing factor structure
            ...
            if (has4A || has4B) {
                ...
                // Count how many factors present
                const factorCount = Object.keys(factorContents).filter(k => /^Factor\s*\d+/i.test(k)).length;
                isSupervisory = factorCount <= 7; // GSSG has max 7 factors (1,2,3,4A,4B,5,6)
            }
            
            console.log('SYSTEM LOCKED:', isSupervisory ? 'SUPERVISORY (GSSG)' : 'NON-SUPERVISORY (FES)');
            console.log('Factor keys being sent:', Object.keys(factorContents));
            ...
            const payload = {
                ...
                systemLocked: isSupervisory ? 'GSSG' : 'FES' // NEW: explicit system signal
            };
            ...
            // Remove existing Factor keys robustly
            removeExistingFactorKeys();

            // Prefer centralized helper updateFactorSections if available
            if (data.factors && typeof updateFactorSections === 'function') {
                try {
                    updateFactorSections(data.factors);
                } catch (err) {
                    console.warn('updateFactorSections failed, falling back to manual insert:', err);
            ...
            // Update summary sections if provided
            if (typeof updateSummarySections === 'function') {
                ...
                if (data.totalPoints !== undefined) pdSections[`Total Points: ${data.totalPoints}`] = `Total Points: ${data.totalPoints}`;
                if (data.finalGrade) pdSections[`Final Grade: ${data.finalGrade}`] = `Final Grade: ${data.finalGrade}`;
                if (data.gradeRange) pdSections[`Grade Range: ${data.gradeRange}`] = `Grade Range: ${data.gradeRange}`;
            }
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // CRITICAL: Collect CURRENT factor structure to preserve system
            ...
            if (has4A || has4B) {
                ...
                const factorCount = Object.keys(currentFactors).filter(k => /^Factor\s*\d+/i.test(k)).length;
                isSupervisory = factorCount <= 7;
            }

            console.log('MAJOR DUTIES UPDATE - SYSTEM LOCKED:', isSupervisory ? 'SUPERVISORY (GSSG)' : 'NON-SUPERVISORY (FES)');
            ...
            const payload = {
                ...
                currentSections: JSON.stringify(currentFactors), // Send existing factors to backend
