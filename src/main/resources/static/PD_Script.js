...
function renderUnknownSeriesStep(content) {
    ...
    <div class="card">
        ...
        <div class="form-group">
            <label class="form-label-strong" for="supervisoryLevelSelect">Supervisory Level</label>
            <div class="dropdown-wrapper">
        ...
        <div class="step-container">
            ...
            <div class="step-content">
                ...
                <div class="form-section">
                    <div class="form-group">
                        <label class="form-label-strong" for="lowestOrgInput">Lowest Level Organization/Office</label>
                        <input 
                            class="form-input" 
                            id="lowestOrgInput" 
                    ...
                    <div class="form-row">
                        <div class="form-group form-group-half">
                            <label class="form-label-strong" for="federalAgencyDropdown">Federal Agency</label>
                            <div class="dropdown-wrapper">
                        ...
                        <div class="form-group form-group-half">
                            <label class="form-label-strong" for="subOrgDropdown">Sub-Organization</label>
                            <div class="dropdown-wrapper">
...
window.formatGeneratedPD = function(pdText) {
    ...
    // Create a structured parser that identifies sections by content patterns
    const sections = {
        header: '',
        introduction: '',
        duties: [],
    ...
    const headerMatch = text.match(/HEADER:\s*(.*?)(?=INTRODUCTION:|$)/s);
    if (headerMatch) {
        let headerContent = headerMatch[1].trim();
        // Fix the concatenated department info
        headerContent = headerContent
            .replace(/U\.S\.\s*Department\s*of\s*Justice([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/g, 'U.S. Department of Justice\n$1')
            .replace(/Criminal\s*Division\s*Criminal\s*Division/g, 'Criminal Division')
            .replace(/(Criminal\s*Division)\s*(GS-\d+)/g, '$1\n$2')
            .replace(/(GS-\d+)\s*(Organizational\s*Title:)/g, '$1\n$2');
        sections.header = headerContent;
    }
    
    // Parse INTRODUCTION section
    const introMatch = text.match(/INTRODUCTION:\s*(.*?)(?=MAJOR DUTIES:|$)/s);
    if (introMatch) {
        sections.introduction = introMatch[1].trim();
    }
    
    // Parse MAJOR DUTIES section
    const dutiesMatch = text.match(/MAJOR DUTIES:\s*(.*?)(?=FACTOR EVALUATION|$)/s);
    if (dutiesMatch) {
        let dutiesText = dutiesMatch[1].trim();
        // Split on numbered items
        const dutyMatches = dutiesText.match(/(\d+)\.\s*([^:]+?)(\s*\(\d+%\))?\s*:\s*([^0-9]+?)(?=\d+\.|$)/g);
        if (dutyMatches) {
            sections.duties = dutyMatches.map(duty => {
                const dutyParts = duty.match(/(\d+)\.\s*([^:]+?)(\s*\(\d+%\))?\s*:\s*(.*)/s);
                if (dutyParts) {
                    return {
                        number: dutyParts[1],
                        title: dutyParts[2].trim(),
                        percentage: dutyParts[3] || '',
                        description: dutyParts[4].trim()
                    };
                }
                return { number: '', title: duty, percentage: '', description: '' };
            });
        }
    }
    
    // Parse FACTOR sections
    const factorMatches = text.match(/Factor\s*(\d+)\s*-\s*([^L]+?)\s*Level\s*(\d+)-(\d+),?\s*(\d+)\s*Points?\s*(.*?)(?=Factor\s*\d+|EVALUATION SUMMARY|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES|FAIR LABOR STANDARDS|$)/gs);
    if (factorMatches) {
        sections.factors = factorMatches.map(factor => {
            const factorParts = factor.match(/Factor\s*(\d+)\s*-\s*([^L]+?)\s*Level\s*(\d+)-(\d+),?\s*(\d+)\s*Points?\s*(.*)/s);
            if (factorParts) {
                return {
                    number: factorParts[1],
                    title: factorParts[2].trim(),
                    level: `${factorParts[3]}-${factorParts[4]}`,
                    points: factorParts[5],
                    description: factorParts[6].trim()
                };
            }
            return null;
        }).filter(Boolean);
    ...
    const summaryMatch = text.match(/EVALUATION SUMMARY:\s*(.*?)(?=CONDITIONS OF EMPLOYMENT|TITLE AND SERIES|FAIR LABOR STANDARDS|$)/s);
    if (summaryMatch) {
        const summaryText = summaryMatch[1];
        const totalMatch = summaryText.match(/Total Points:\s*(\d+)/);
        const gradeMatch = summaryText.match(/Final Grade:\s*(GS-\d+)/);
        const rangeMatch = summaryText.match(/Grade Range:\s*([\d-]+)/);
        
        if (totalMatch) sections.evaluationSummary.totalPoints = totalMatch[1];
        if (gradeMatch) sections.evaluationSummary.finalGrade = gradeMatch[1];
        if (rangeMatch) sections.evaluationSummary.gradeRange = rangeMatch[1];
    }
    
    // Parse CONDITIONS OF EMPLOYMENT
    const conditionsMatch = text.match(/CONDITIONS OF EMPLOYMENT:\s*(.*?)(?=TITLE AND SERIES|FAIR LABOR STANDARDS|$)/s);
    if (conditionsMatch) {
        let conditionsText = conditionsMatch[1].trim();
        // Split on "Must" to create bullet points
        const mustItems = conditionsText.split(/(?=Must)/g).filter(item => item.trim());
        sections.conditions = mustItems.map(item => item.trim());
    }
    
    // Parse TITLE AND SERIES DETERMINATION
    const titleMatch = text.match(/TITLE AND SERIES DETERMINATION:\s*(.*?)(?=FAIR LABOR STANDARDS|$)/s);
    if (titleMatch) {
        sections.titleSeries = titleMatch[1].trim();
    }
    
    // Parse FAIR LABOR STANDARDS ACT DETERMINATION
    const flsaMatch = text.match(/FAIR LABOR STANDARDS ACT DETERMINATION:\s*(.*?)$/s);
    if (flsaMatch) {
        sections.flsa = flsaMatch[1].trim();
    }
    
    // Now rebuild the document with proper formatting
    let result = [];
    
    // HEADER
    if (sections.header) {
        result.push('**HEADER:**\n');
        result.push(sections.header + '\n');
    }
    
    // INTRODUCTION
    if (sections.introduction) {
        result.push('\n**INTRODUCTION:**\n');
        result.push(sections.introduction + '\n');
    }
    
    // MAJOR DUTIES
    if (sections.duties.length > 0) {
        result.push('\n**MAJOR DUTIES:**\n');
        sections.duties.forEach(duty => {
            result.push(`\n${duty.number}. **${duty.title}**${duty.percentage}:\n`);
            result.push(duty.description + '\n');
        });
    }
    
    // FACTOR EVALUATION
    if (sections.factors.length > 0) {
        result.push('\n**FACTOR EVALUATION - COMPLETE ANALYSIS:**\n');
        sections.factors.forEach(factor => {
            result.push(`\n**Factor ${factor.number} - ${factor.title} Level ${factor.level}, ${factor.points} Points**\n`);
            result.push(factor.description + '\n');
        });
    }
    
    // EVALUATION SUMMARY
    if (sections.evaluationSummary.totalPoints) {
        result.push('\n**EVALUATION SUMMARY:**\n');
        result.push(`\n**Total Points:** ${sections.evaluationSummary.totalPoints}\n`);
        result.push(`**Final Grade:** ${sections.evaluationSummary.finalGrade}\n`);
        result.push(`**Grade Range:** ${sections.evaluationSummary.gradeRange}\n`);
    }
    ...
    if (sections.titleSeries) {
        result.push('\n**TITLE AND SERIES DETERMINATION:**\n');
        result.push(sections.titleSeries + '\n');
    }
    
    // FAIR LABOR STANDARDS ACT DETERMINATION
    if (sections.flsa) {
        result.push('\n**FAIR LABOR STANDARDS ACT DETERMINATION:**\n');
        result.push(sections.flsa + '\n');
    }
