...
async function saveSectionEdit(title) {
    ...
    function collectFactorContents() {
        ...
        keys.forEach(k => {
            ...
            if (m) {
                ...
                if (content && String(content).trim()) factorContents[canonical] = String(content).trim();
            }
        });
        // originalLevel hints
        Object.keys(pdSections).forEach(k => {
            const m = k.match(/^(Factor\s*\d+[A-Za-z]?)_originalLevel$/i);
            if (m) factorContents[m[1].replace(/\s+/g,' ')+ "_originalLevel"] = pdSections[k];
        });
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            const tp = data.totalPoints !== undefined ? data.totalPoints : safeInt(data.totalPoints);
            ...
            pdSections[`Final Grade: ${fg || calculateFinalGrade(tp) || 'Unknown'}`] = '';
            pdSections[`Grade Range: ${gr || getPointRangeForGrade(fg) || 'Unknown'}`] = '';
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            const fg = data.finalGrade || calculateFinalGrade(tp) || 'Unknown';
            const gr = (data.gradeRange !== undefined && data.gradeRange !== null) ? data.gradeRange : getPointRangeForGrade(fg) || 'Unknown';
