...
window.formatGeneratedPD = function(pdText) {
    ...
    // 1. HEADER block formatting
    text = text.replace(
        /(U\.S\. Department of Justice)(Executive Office for U\.S\. Attorneys)/,
        '$1\n$2'
    );
    text = text.replace(
    ...
    text = text.replace(
        /(Criminal Division)(GS-\d+)/,
        '$1\n$2'
    );
    text = text.replace(
        /(GS-\d+)(Organizational Title:)/,
        '$1\n$2'
    );
    text = text.replace(/^\*\*HEADER:\*\*/gm, '**HEADER:**\n');

    // 2. Major section headers: ensure blank lines before/after
    ...
    sectionHeaders.forEach(section => {
        const re = new RegExp(`\\*\\*${escapeRegex(section)}\\*\\*`, 'g');
        text = text.replace(re, `\n\n**${section}**\n`);
    });

    // 3. Factor headers — spacing + bold cleanup
    text = text.replace(
        /Factor\s*(\d+)\s*-\s*([^\n]+?)\s*Level\s*(\d+)-(\d+),\s*(\d+)\s*Points/g,
        '\n\n**Factor $1 – $2 (Level $3-$4, $5 Points)**\n'
    );

    // 4. Major Duties numbering — bold duty titles + spacing
    text = text.replace(
        /(\d+)\.\s*\*\*([^*]+?)\*\*:?/g,
        '\n\n$1. **$2**'
    );

    // 5. Evaluation Summary split into clean lines
    text = text.replace(
        /(\*\*EVALUATION SUMMARY:\*\*)\s*\**Total Points:\s*(\d+)\**\s*\**Final Grade:\s*(GS-\d+)\**\s*\**Grade Range:\s*([\d-]+)\**/gi,
        '\n\n$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4'
    );

    // 6. Bullet points — ensure one per line
    text = text.replace(/- /g, '\n- ');

    // 7. Clean up whitespace + normalize newlines
    text = text.replace(/[ \t]+\n/g, '\n')
            .replace(/\n[ \t]+/g, '\n')
            .replace(/\n{3,}/g, '\n\n');
