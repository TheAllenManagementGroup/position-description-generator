...
async function saveSectionEdit(title) {
    ...
    // Helper to collect factor content keys (includes 4A/4B and lettered factors)
    function collectFactorContents() {
        const factorContents = {};
        const keys = Object.keys(pdSections || {});
        keys.forEach(k => {
            // Match "Factor X", "Factor X - ..." or exact "Factor 4A" etc.
            const m = k.match(/^(Factor\s*\d+[A-Za-z]?)/i);
            if (m) {
                const canonical = m[1].replace(/\s+/g, ' ').trim();
                // Use canonical key like "Factor 4A" as request expects
        ...
        // Also include any explicit originalLevel entries present in pdSections or global stacks
    ...
    // If this is a Factor section (any Factor X or Factor 4A/4B)
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // Remove existing Factor* keys (clean slate)
            Object.keys(pdSections).forEach(key => {
                if (/^Factor\s*\d+[A-Za-z]?\b/i.test(key) || /^Factor\s*\d+[A-Za-z]?\s*-.+Level/i.test(key)) {
                    delete pdSections[key];
                }
            });

            // Prefer centralized helper updateFactorSections if available (it enforces ordering including 4A/4B)
            if (data.factors && typeof updateFactorSections === 'function') {
                try {
                    updateFactorSections(data.factors);
                } catch (err) {
                    // Fallback to manual insertion below
                    console.warn('updateFactorSections failed, falling back to manual insert:', err);
                }
            }

            // Fallback/manual insertion: ensure factors added back (handles both GSSG and FES)
            if ((!data.factors || Object.keys(data.factors).length === 0) && data.factors !== undefined) {
                // nothing to add
            } else if (data.factors && Object.keys(data.factors).length > 0) {
                // If updateFactorSections already added them, skip manual add if any Factor keys exist
                const existingFactorKey = Object.keys(pdSections).find(k => /^Factor\s*\d+/i.test(k));
                if (!existingFactorKey) {
                    // Build ordering: check for 4A/4B to decide supervisory order
                    const has4A = Object.keys(data.factors).some(k => /^Factor\s*4A/i.test(k) || /^4A$/i.test(k));
                    const has4B = Object.keys(data.factors).some(k => /^Factor\s*4B/i.test(k) || /^4B$/i.test(k));
                    ...
                    order.forEach(id => {
                        ...
                        // allow variations: "Factor 4A" or "4A"
                        const responseKey = Object.keys(data.factors).find(k => k.toLowerCase() === factorKey.toLowerCase() || k.toLowerCase() === id.toLowerCase());
                        if (!responseKey) return;
                        const factor = data.factors[responseKey];
                        if (!factor) return;
                        const level = factor.level || factor.Level || '';
                        const points = factor.points || factor.Points || '';
                        const rationale = factor.rationale || factor.content || '';
                        const factorName = getFactorName(id.replace(/[A-Za-z]/g, '')) || `Factor ${id}`;
                        const header = `Factor ${id} - ${factorName}` + (level ? ` Level ${level}` : '') + (points ? `, ${points} Points` : '');
                        pdSections[header] = rationale || '';
                    });
                }
            }
            ...
            if (typeof updateSummarySections === 'function') {
                ...
                // Fallback: directly set summary keys
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Replace old major duties with new content (already done above)
            ...
            // Prepare payload (include supervisoryLevel & expected grade if available)
            const payload = {
                duties: newContent,
                supervisoryLevel: (typeof formState !== 'undefined' && formState.supervisoryLevel) ? formState.supervisoryLevel : '',
            ...
            Object.keys(pdSections).forEach(key => {
                if (/^Factor\s*\d+[A-Za-z]?\b/i.test(key)) {
                    delete pdSections[key];
                }
            });

            // Reuse updateFactorSections if available
            if (data.factors && typeof updateFactorSections === 'function') {
                try {
                    updateFactorSections(data.factors);
                } catch (err) {
            ...
            // Manual fallback insertion if updateFactorSections not used
            if (data.factors && Object.keys(data.factors).length > 0) {
                ...
                const anyFactorPresent = Object.keys(pdSections).some(k => /^Factor\s*\d+/i.test(k));
                if (!anyFactorPresent) {
                    order.forEach(id => {
                        const factorKey = `Factor ${id}`;
                        const responseKey = Object.keys(data.factors).find(k => k.toLowerCase() === factorKey.toLowerCase() || k.toLowerCase() === id.toLowerCase());
...
async function updateFactorSections(factors) {
    console.log('Updating factor sections (enhanced ordering):', factors);

    // Determine if supervisory (GSSG) by presence of 4A/4B keys
    const has4A = Object.prototype.hasOwnProperty.call(factors, 'Factor 4A') || Object.keys(factors).some(k => /^Factor\s*4A/i.test(k));
    const has4B = Object.prototype.hasOwnProperty.call(factors, 'Factor 4B') || Object.keys(factors).some(k => /^Factor\s*4B/i.test(k));
    const isSupervisory = has4A || has4B;

    // Define ordering and friendly names for both systems
    ...
    const names = isSupervisory ? gssgNames : fesNames;

    // Remove any existing Factor X keys that might conflict (clean slate)
    Object.keys(pdSections).forEach(k => {
        if (/^Factor\s*\d+[A-Za-z]?\b/i.test(k)) {
            // keep other text but remove factor headers (we'll re-add in correct order)
            delete pdSections[k];
        }
    });

    // Add factors in the required order when present in the response
    order.forEach(id => {
        const factorKey = `Factor ${id}`;
        // Accept either exact key or case-insensitive variants
        const responseKey = Object.keys(factors).find(k => k.toLowerCase() === factorKey.toLowerCase());
        if (!responseKey) return;

        const factor = factors[responseKey];
        if (!factor) return;

        // Friendly name lookup (fallback to generic)
        const factorName = names[id] || `Factor ${id}`;

        // Normalize level & points fields if nested under different property names
        const level = factor.level || factor.Level || (factor.header && (factor.header.match(/Level\s*([0-9A-Z\-]+)/i) || [])[1]) || '';
        const points = factor.points || factor.Points || factor.pointsValue || factor.pointsValue === 0 ? factor.points : factor.points;

        // Build header
        const headerLevel = level ? ` Level ${level}` : '';
        const headerPoints = (typeof points === 'number' && !isNaN(points)) ? `, ${points} Points` : '';

        const newSectionKey = `Factor ${id} - ${factorName}${headerLevel}${headerPoints}`;

        // Store only the rationale/content (do not duplicate header into content)
        const rationale = (factor.rationale || factor.Rationale || factor.content || '').toString().trim();
        pdSections[newSectionKey] = rationale;

        // Ensure we didn't accidentally leave older factor keys - remove any other Factor id keys (robust cleanup)
        Object.keys(pdSections).forEach(existingKey => {
            if (existingKey === newSectionKey) return;
            const m = existingKey.match(/^Factor\s*(\d+[A-Za-z]?)/i);
            if (m && m[1].toString().toUpperCase() === id.toUpperCase()) {
                if (existingKey !== newSectionKey) delete pdSections[existingKey];
            }
        });
    });
    ...
    if (typeof renderEditableSections === 'function') {
        ...
        console.log('renderEditableSections not available to refresh UI.');
    }
}
