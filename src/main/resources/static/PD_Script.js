...
window.formatGeneratedPD = function(pdText) {
    ...
    // Initial cleanup - normalize line breaks and excessive spacing
    let text = pdText
        .replace(/\r\n/g, '\n')
        .replace(/\n{3,}/g, '\n\n')
        .trim();
    
    // Fix header spacing issues - handle department and division separately
    text = text.replace(/(U\.S\. Department of Justice)([A-Z])/g, '$1\n$2');
    text = text.replace(/([A-Za-z &]+ Division)([A-Z][^G])/g, '$1\n$2');
    text = text.replace(/(Criminal Division)(Criminal Division)/g, '$1\n$2');
    text = text.replace(/(GS-\d+)(Organizational Title:)/g, '$1\n\n$2');
    
    // Main section headers with proper line breaks
    ...
    // Format section headers - ensure they're on their own lines with proper spacing
    sectionHeaders.forEach(section => {
        const escapedSection = section.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        
        // Handle sections that might be directly attached to previous content
        if (section === 'INTRODUCTION:') {
            text = text.replace(new RegExp(`(\\*\\*${escapedSection}\\*\\*)([A-Z])`, 'gi'), '$1\n\n$2');
            text = text.replace(new RegExp(`([^\\n])\\s*\\*\\*${escapedSection}\\*\\*`, 'gi'), '$1\n\n**' + section + '**');
            text = text.replace(new RegExp(`([^\\n])\\s*${escapedSection}`, 'gi'), '$1\n\n**' + section + '**');
        } else {
            // Handle both with and without asterisks for other sections
            text = text.replace(new RegExp(`\\*\\*${escapedSection}\\*\\*`, 'gi'), `\n\n**${section}**`);
            text = text.replace(new RegExp(`(?<!\\*)${escapedSection}(?!\\*)`, 'gi'), `\n\n**${section}**`);
        }
    });
    
    // Fix Factor headers with proper spacing around numbers
    // Handle patterns like "Factor1 - Knowledge" or "Factor 1 - Knowledge"
    text = text.replace(/(Factor\s*)(\d+)(\s*[-–]\s*)([^\n]+?)(Level\s*)(\d+)(\s*[-–]\s*)(\d+)\s*[,\s]*(\d+)\s*(Points)/gi, 
        '\n\n**Factor $2 - $4 (Level $6-$8, $9 $10)**\n');
    
    // Alternative pattern for factors that might be formatted differently
    text = text.replace(/(Factor)(\d+)(\s*-\s*)([^\n]+?Level\d+-\d+,\s*\d+\s*Points)/gi, 
        '\n\n**Factor $2 - $4**\n');
    
    // Major duties formatting with better number detection
    text = text.replace(/^\s*(\d+)\.\s*\*\*([^*]+?)\*\*:\s*/gmi, '\n\n$1. **$2:**\n');
    text = text.replace(/^\s*(\d+)\.\s*([A-Z][^:\n]{5,}?):\s*/gmi, '\n\n$1. **$2:**\n');
    
    // Handle duties that might have percentage in parentheses
    text = text.replace(/(\d+)\.\s*\*\*([^*]+?)\s*\((\d+%)\)\*\*:/g, '\n\n$1. **$2 ($3):**\n');
    
    // Enhanced evaluation summary formatting
    text = text.replace(
        /(\*\*EVALUATION SUMMARY:\*\*)\s*\*\*Total Points:\s*(\d+)\*\*\s*\*\*Final Grade:\s*(GS-\d+)\*\*\s*\*\*Grade Range:\s*(\d+[-–]\d+)\*\*/gi,
        '\n\n$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4\n'
    );
    
    // Handle case where evaluation summary items are not already bolded
    ...
    // Clean up specific spacing issues
    text = text
        ...
        .replace(/(\*\*[^*]+\*\*)\n{3,}/g, '$1\n\n')
        // Fix spacing before section headers
        .replace(/([^\n])\n(\*\*[A-Z][^*]*:\*\*)/g, '$1\n\n$2')
...
U.S. Department of JusticeCriminal DivisionCriminal DivisionGS-13Organizational Title: Criminal Investigator Supervisor**INTRODUCTION:**
The Criminal Investigator Supervisor is a critical role...
...
// Enhanced version that shows before/after for debugging
window.formatGeneratedPDWithDebug = function(pdText, showDebug = false) {
    if (showDebug) {
        console.log('=== BEFORE FORMATTING ===');
        console.log(pdText.substring(0, 500) + '...');
        console.log('\n=== HEADER ISSUES ===');
        const headerMatches = pdText.match(/U\.S\. Department of Justice[A-Z]/);
        if (headerMatches) console.log('Found header issue:', headerMatches[0]);
        
        console.log('\n=== FACTOR ISSUES ===');
        const factorMatches = pdText.match(/Factor\d+\s*-[^L]*Level\d+-\d+,\s*\d+\s*Points/g);
        if (factorMatches) console.log('Found factor issues:', factorMatches);
    }
    
    const formatted = window.formatGeneratedPD(pdText);
