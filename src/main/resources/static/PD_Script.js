...
async function saveSectionEdit(title) {
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Call backend to regenerate factors and summary
            const response = await fetch('/api/regenerate-factors', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    duties: newDuties // Only send the updated major duties
                })
            });
            ...
            // Update factor sections and summary in UI
            if (data.factors) {
                await updateFactorSections(data.factors);
            }
...
async function regenerateFactorsAndSummary(newDuties, gsGrade, supervisoryLevel, jobSeries, jobTitle) {
    const response = await fetch('/api/regenerate-factors', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    // Update factor sections and summary in UI
    if (data.factors) {
        await updateFactorSectionsFromEvaluationStatement(data.factors);
    }
    if (data.totalPoints && data.finalGrade && data.gradeRange) {
        await updateSummarySections({
            totalPoints: data.totalPoints,
...
async function updateFactorSectionsFromEvaluationStatement(evaluationStatement) {
    // Split the evaluation statement into sections
    const factorRegex = /\*\*Factor\s*(\d+)\s*[-–]\s*([^\n]+?)\s*Level\s*(\d+-\d+),\s*(\d+)\s*Points\*\*([\s\S]*?)(?=\*\*Factor\s*\d+|$)/g;
    let match;
    let foundFactors = 0;

    while ((match = factorRegex.exec(evaluationStatement)) !== null) {
        ...
        const header = `Factor ${factorNum} - ${factorName} Level ${level}, ${points} Points`;
        pdSections[`Factor ${factorNum}`] = `**${header}**\n${content}`;
        foundFactors++;
    }

    // If no factors found, fallback to splitting by "**Factor"
    if (foundFactors === 0) {
        const parts = evaluationStatement.split(/\*\*Factor\s*/).slice(1);
        parts.forEach(part => {
            const numMatch = part.match(/^(\d+)/);
            if (numMatch) {
                const factorNum = numMatch[1];
                pdSections[`Factor ${factorNum}`] = `**Factor ${part.trim()}`;
            }
        });
    }

    // Re-render the editable sections to reflect updates
    renderEditableSections();
}
...
function renderEditableSections() {
    ...
    // 2. Combine all "MAJOR DUTY X" and "MAJOR DUTIESX" into one editable "MAJOR DUTIES" section
    ...
    // 3. Render all other sections except individual major duty sections
    Object.keys(pdSections).forEach(key => {
        if (
