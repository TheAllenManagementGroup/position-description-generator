...
function getAdjacentGSGrades(recommendedGradeNum) {
    ...
    let neighbors = [];
    if (idx !== -1) {
        // GS-5: only GS-5 and GS-7
        if (recommendedGradeNum === 5) {
            neighbors = ['GS-5', 'GS-7'];
        }
        // GS-15: only GS-14 and GS-15
        else if (recommendedGradeNum === 15) {
            neighbors = ['GS-14', 'GS-15'];
        }
        // All others: previous, current, next
        else {
...
function setupRefineGradeAnalysisUI() {
    ...
    // Add duty button
    const addBtn = document.getElementById('addRefineDutyBtn');
    if (addBtn) {
...
function renderDutiesStep(content) {
    ...
    // Force validation check after a brief delay to ensure DOM is ready
    setTimeout(() => {
        if (typeof setupDutiesValidation === 'function') {
            setupDutiesValidation();
        }
    }, 100);
}
            ...
            <div class="card" style="max-width: 48rem; margin: 0 auto;">
                <div class="card-header">
                    <div class="card-title">
                        ...
                        <span class="step-info" tabindex="0" style="position:relative;">
                            <i class="fas fa-info-circle" style="color:#2563eb; margin-left:8px; cursor:help;"></i>
                            <span class="step-tooltip" style="display:none; position:absolute; left:24px; top:50%; transform:translateY(-50%); background:#fff; color:#111827; border:1px solid #e5e7eb; border-radius:6px; padding:8px 12px; font-size:13px; box-shadow:0 2px 8px rgba(0,0,0,0.08); z-index:10; min-width:180px;">
...
const infoSpan = content.querySelector('.card-title .step-info');
if (infoSpan) {
    const tooltip = infoSpan.querySelector('.step-tooltip');
    infoSpan.addEventListener('mouseenter', () => { tooltip.style.display = 'block'; });
    infoSpan.addEventListener('mouseleave', () => { tooltip.style.display = 'none'; });
    infoSpan.addEventListener('focus', () => { tooltip.style.display = 'block'; });
    infoSpan.addEventListener('blur', () => { tooltip.style.display = 'none'; });
}
...
function renderGradeAnalysisResultStyled(data) {
    ...
    // Only valid GS grades
    const validGrades = [5, 7, 9, 11, 12, 13, 14, 15];
    function getAdjacentGSGrades(num) {
        const idx = validGrades.indexOf(num);
        if (idx === -1) return [`GS-${num}`];
        if (num === 5) return ['GS-5', 'GS-7'];
        if (num === 15) return ['GS-14', 'GS-15'];
        return [`GS-${validGrades[idx - 1]}`, `GS-${validGrades[idx]}`, `GS-${validGrades[idx + 1]}`];
    }
    ...
    if (Array.isArray(data.gradeRelevancy) && data.gradeRelevancy.length > 0) {
        // Map by grade for fast lookup
        const gradeMap = {};
        data.gradeRelevancy.forEach(item => {
            let g = String(item.grade).replace(/[^0-9]/g, '');
            if (validGrades.includes(Number(g))) {
                gradeMap[`GS-${g}`] = Math.round(Number(item.percentage) || 0);
            }
        });
        adjacentGrades.forEach(g => {
            gradeData.push({
                grade: g,
                percentage: gradeMap[g] !== undefined ? gradeMap[g] : 0
            });
        });
    }
    // Fallback: always show 60/30/10 or 60/40 for 2 grades, ensuring no 0% values
    if (gradeData.length === 0 || gradeData.every(g => g.percentage === 0)) {
        if (adjacentGrades.length === 2) {
            gradeData = [
                { grade: adjacentGrades[0], percentage: 40 },
                { grade: adjacentGrades[1], percentage: 60 }
            ];
        } else {
            gradeData = [
                { grade: adjacentGrades[0], percentage: 10 },
                { grade: adjacentGrades[1], percentage: 60 },
                { grade: adjacentGrades[2], percentage: 30 }
            ];
        }
    }
    
    // Ensure no grade has 0% - redistribute if needed
    const totalPercentage = gradeData.reduce((sum, item) => sum + item.percentage, 0);
    if (totalPercentage === 0) {
        // All zeros - use default distribution
        if (gradeData.length === 2) {
            gradeData[0].percentage = 40;
            gradeData[1].percentage = 60;
        } else if (gradeData.length === 3) {
            gradeData[0].percentage = 10;
            gradeData[1].percentage = 60;
            gradeData[2].percentage = 30;
        }
    } else {
        // Replace any 0% with minimum 5% and adjust others proportionally
        const zerosCount = gradeData.filter(g => g.percentage === 0).length;
        if (zerosCount > 0) {
            const minPercentage = 5;
            const totalToRedistribute = zerosCount * minPercentage;
            const nonZeroGrades = gradeData.filter(g => g.percentage > 0);
            
            // Set zeros to minimum
            ...
            // Reduce non-zero grades proportionally
            if (nonZeroGrades.length > 0) {
                const reductionFactor = (100 - totalToRedistribute) / (100 - (zerosCount * 0));
                gradeData.forEach(item => {
                    if (item.percentage > minPercentage) {
                        item.percentage = Math.max(minPercentage, Math.round(item.percentage * reductionFactor));
                    }
                });
            }
        }
    ...
    const sorted = [...gradeData].sort((a, b) => b.percentage - a.percentage);
    const colorMap = {};
    if (sorted[0]) colorMap[sorted[0].grade] = '#00780aff';
    if (sorted[1]) colorMap[sorted[1].grade] = '#50c23cff';
    if (sorted[2]) colorMap[sorted[2].grade] = '#facc15';
        ...
        <div style="max-width: 900px; margin: 0 auto; padding: 1.5rem; background: #f9fafb;">
            <div style="margin-bottom: 1.5rem;">
                <p style="font-size: 0.875rem; color: #6b7280;">
                    ...
                    <span style="font-weight: 700;">${data.gsGrade || 'N/A'}</span>
                </p>
            </div>
            <div style="background: white; border-radius: 0.5rem; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); border: 1px solid #e5e7eb; padding: 2rem;">
                ...
                <div style="text-align: center; margin-bottom: 2.5rem;">
                    <div style="font-size: 3rem; font-weight: 700; color: #111827; margin-bottom: 0.5rem;">${data.gsGrade || 'N/A'}</div>
                            ...
                            <div style="display: flex; align-items: center;">
                                ...
                                <div style="flex: 1; margin: 0 1.5rem;">
                                    <div style="width: 100%; background: #e5e7eb; border-radius: 9999px; height: 0.75rem;">
                                        <div style="
                                            background: ${colorMap[item.grade] || '#e5e7eb'};
                                            height: 0.75rem;
                ...
                <div>
                    ...
                    <div id="evalExportBtns" style="margin-top:1em; display:none;">
    ...
    // Setup refine UI and button handlers
    ...
    if (acceptGradeBtn) {
        acceptGradeBtn.onclick = function() {
            formState.gsGrade = data.gsGrade;
    ...
    regenerateBtn.onclick = async function() {
        ...
        const oldDuties = Array.isArray(formState.unknownDuties) ? formState.unknownDuties.filter(d => d.trim()) : [];
    ...
    // Evaluation Statement button
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        // Always use the latest grade from the most recent analysis
        const latestGrade = formState.gsGrade || data.gsGrade || '';
        const oldDuties = Array.isArray(formState.unknownDuties) ? formState.unknownDuties.filter(d => d.trim()) : [];
        const newDuties = (formState.refineDuties || []).filter(d => d.trim());
        const allDuties = [...oldDuties, ...newDuties].join('\n');
        ...
        evalResultDiv.innerHTML = `
            <div class="pd-display" style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; padding: 1rem; border-radius: 0.5rem; background-color: #f9fafb; line-height: 1.6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                <div style="text-align:center; margin:2em 0;">
                    <span class="spinner" style="width:24px; height:24px; border-width:3px;"></span>
                    <span style="margin-left:0.5em; color:#2563eb; font-weight:600;">Generating evaluation statement...</span>
                </div>
        ...
        try {
            ...
            if (!response.body) {
                evalResultDiv.innerHTML = '<div style="color:red;">No response from server.</div>';
                return;
            }
            ...
            while (true) {
                ...
                for (let line of lines) {
                    if (line.trim().startsWith('data:')) {
                        let json = line.trim().slice(5);
                        try {
                            let parsed = JSON.parse(json);
                            if (parsed.response) {
                                fullText += parsed.response;
                            }
                            if (parsed.evaluationStatement) {
                                fullText += parsed.evaluationStatement;
                            }
                        } catch (e) {
                            // Ignore parse errors
                        }
            ...
            // Clean up any repeated or malformed factor sections
            let cleaned = fullText.replace(/^(Generating evaluation statement\.\.\.\s*)?/i, '');
            cleaned = cleaned.replace(/(\*\*Factor\s*\d+\s*[–-][^\n]+(\n.*?))+?(?=\n|$)/gis, '');
            cleaned = cleaned.replace(/\*{2,}/g, '**'); // Normalize bold markers

            const formatted = window.formatGeneratedPD(cleaned);
            document.getElementById('evalStatementText').innerHTML = window.markdownToHTML(formatted);

            // Show export buttons and set up handlers
            document.getElementById('evalExportBtns').style.display = '';
            setupEvalStatementExportButtons(formatted);

            // Remove spinner
            const spinner = evalResultDiv.querySelector('.spinner');
            if (spinner) spinner.parentElement.remove();

        } catch (err) {
            evalResultDiv.innerHTML = `<div style="color:red;">Error: ${err.message}</div>`;
        }
    };
...
function renderDutiesGradeAnalysis(data) {
    ...
    // Only valid GS grades
    const validGrades = [5, 7, 9, 11, 12, 13, 14, 15];
    let gradeNum = parseInt(recommendedGrade.replace(/[^0-9]/g, ''), 10);
    if (!validGrades.includes(gradeNum)) gradeNum = 12;

    // Get adjacent grades for display
    function getAdjacentGSGrades(num) {
        const idx = validGrades.indexOf(num);
        if (idx === -1) return [`GS-${num}`];
        if (num === 5) return ['GS-5', 'GS-7'];
    ...
    // Build gradeData: ensure every adjacent grade is present, fill missing with 0%
    let gradeData = [];
    if (Array.isArray(data.gradeRelevancy) && data.gradeRelevancy.length > 0) {
        // Map by grade for fast lookup
        const gradeMap = {};
        data.gradeRelevancy.forEach(item => {
            let g = String(item.grade).replace(/[^0-9]/g, '');
            if (validGrades.includes(Number(g))) {
                // Boost percentages for visual impact
                let pct = Math.round(Number(item.percentage) || 0);
                if (pct > 0 && pct < 20) pct += 10;
                if (pct > 70) pct += 10;
                gradeMap[`GS-${g}`] = pct;
            }
        });
        adjacentGrades.forEach(g => {
            gradeData.push({
                grade: g,
                percentage: gradeMap[g] !== undefined ? gradeMap[g] : 10
            });
        });
    }
    // Fallback: always show 60/30/10 or 60/40 for 2 grades
    if (gradeData.length === 0 || gradeData.every(g => g.percentage === 0)) {
        if (adjacentGrades.length === 2) {
            gradeData = [
                { grade: adjacentGrades[0], percentage: 40 },
                { grade: adjacentGrades[1], percentage: 70 }
            ];
        } else {
            gradeData = [
                { grade: adjacentGrades[0], percentage: 20 },
                { grade: adjacentGrades[1], percentage: 70 },
                { grade: adjacentGrades[2], percentage: 30 }
            ];
        }
    ...
    gradeAnalysisDiv.innerHTML = `
        <div style="max-width: 900px; margin: 0 auto; padding: 1.5rem; background: #f9fafb;">
            ...
            <div style="background: white; border-radius: 0.5rem; box-shadow: 0 1px 2px 0 rgba(0, 0, 0, 0.05); border: 1px solid #e5e7eb; padding: 2rem;">
                ...
                <div style="margin: 2rem 0 1rem 0; border-bottom: 1px solid #e5e7eb;"></div>
                <div id="titleSeriesSection" style="margin-bottom:2em;"></div>
                <h3 style="font-size: 1.1rem; color: #1e293b; font-weight: 600; margin-bottom: 0.5em;">
                    Refine Grade Analysis <span style="color: #64748b; font-weight: 400; font-size: 0.95em;">(Optional)</span>
                </h3>
                <div>
    ...
    // Show Title & Series Determination section
...
function splitPDSections(text) {
    ...
    // Section header regex for both formats (markdown and plain)
    const sectionHeaderRegex = /(?:\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*|^Factor\s*\d+\s*[–-]\s*[^\n]+?Level\s*\d+-\d+,\s*\d+\s*Points?|^Total Points:\s*\d+|^Final Grade:\s*GS-?\d+|^Grade Range:\s*\d+-\d+)/gm;
