...
function renderDutiesStep(content) {
    ...
    document.getElementById('aiRewriteBtn').onclick = async function() {
        ...
        try {
            ...
            // Display rewritten duties, each on its own line, wrapping text (no horizontal scroll)
            resultDiv.innerHTML = `
...
function setupAgencyDropdowns(agencyData) {
    ...
    // Add search input for agency
    let agencySearchInput = document.getElementById('agencySearchInput');
    if (!agencySearchInput) {
        agencySearchInput = document.createElement('input');
        agencySearchInput.type = 'text';
        agencySearchInput.className = 'form-input';
        agencySearchInput.id = 'agencySearchInput';
        agencySearchInput.placeholder = 'Type to filter agencies...';
        federalAgencyDropdown.parentElement.insertBefore(agencySearchInput, federalAgencyDropdown);
    }

    // Add search input for sub-org
    let subOrgSearchInput = document.getElementById('subOrgSearchInput');
    if (!subOrgSearchInput) {
        subOrgSearchInput = document.createElement('input');
        subOrgSearchInput.type = 'text';
        subOrgSearchInput.className = 'form-input';
        subOrgSearchInput.id = 'subOrgSearchInput';
        subOrgSearchInput.placeholder = 'Type to filter sub-organizations...';
        subOrgDropdown.parentElement.insertBefore(subOrgSearchInput, subOrgDropdown);
    }

    // Remove any extra menu option under the text box
    const extraMenus = federalAgencyDropdown.parentElement.querySelectorAll('.dropdown-list, .dropdown-menu');
    extraMenus.forEach(menu => menu.remove());
    const extraMenusSub = subOrgDropdown.parentElement.querySelectorAll('.dropdown-list, .dropdown-menu');
    extraMenusSub.forEach(menu => menu.remove());

    // Render agency options with filter
    function renderAgencyOptions(filter = '') {
        federalAgencyDropdown.innerHTML = '<option value="">Select federal agency</option>';
        agencyData.forEach(agency => {
            if (!filter || agency.name.toLowerCase().includes(filter.toLowerCase())) {
                federalAgencyDropdown.innerHTML += `<option value="${agency.name}">${agency.name}</option>`;
            }
        });
    }

    // Render sub-org options with filter
    function renderSubOrgOptions(selectedAgency, filter = '') {
        ...
        if (agency) {
            agency.subOrgs.forEach(subOrg => {
                if (!filter || subOrg.toLowerCase().includes(filter.toLowerCase())) {
                    subOrgDropdown.innerHTML += `<option value="${subOrg}">${subOrg}</option>`;
                }
            });
    ...
    // Initial render
    renderAgencyOptions();
    renderSubOrgOptions(federalAgencyDropdown.value);

    // Agency search filter
    agencySearchInput.addEventListener('input', function() {
        renderAgencyOptions(this.value);
        renderSubOrgOptions(federalAgencyDropdown.value, subOrgSearchInput.value);
        updateContinueButton();
    });

    // Sub-org search filter
    subOrgSearchInput.addEventListener('input', function() {
        renderSubOrgOptions(federalAgencyDropdown.value, this.value);
        updateContinueButton();
    });

    // Agency selection
    federalAgencyDropdown.addEventListener('change', function() {
        renderSubOrgOptions(this.value, subOrgSearchInput.value);
    ...
    if (formState.federalAgency) {
        renderSubOrgOptions(formState.federalAgency, subOrgSearchInput.value);
        subOrgDropdown.value = formState.subOrg || '';
    }
