...
async function saveSectionEdit(title) {
    ...
    // Helpers
    function canonicalFactorKey(rawKey) {
        if (!rawKey || typeof rawKey !== 'string') return null;
        const k = rawKey.replace(/\*/g, '').trim();
        const m = k.match(/(?:Factor\s*)?(\d{1,2})\s*[-\s]*([AaBb])?$/i);
        if (!m) return null;
        const num = m[1];
        const letter = m[2] ? m[2].toUpperCase() : '';
        return `Factor ${num}${letter}`;
    }
    ...
    function collectFactorContents() {
        ...
        keys.forEach(k => {
            // Match factor headers like "Factor 1 - Knowledge Required Level 1-7, 1250 Points"
            const factorMatch = k.match(/^Factor\s*(\d+[A-Za-z]?)\s*-\s*(.+?)\s*Level\s*(\d+-\d+),\s*(\d+)\s*Points/i);
            
            if (factorMatch) {
                const factorId = factorMatch[1]; // "1" or "4A"
                const factorName = factorMatch[2]; // "Knowledge Required by the Position"
                const level = factorMatch[3]; // "1-7"
                const points = parseInt(factorMatch[4]); // 1250
                const content = pdSections[k] || '';
                
                // Send as a structured object that backend can parse
                const factorKey = `Factor ${factorId}`;
                factorContents[factorKey] = {
                    level: level,
                    points: points,
                    rationale: String(content).trim(),
                    header: k // Include full header for reference
                };
            } else {
                // Fallback: if header doesn't match pattern, try simple factor match
        ...
        Object.keys(pdSections).forEach(k => {
            const m = k.match(/^(Factor\s*\d+[A-Za-z]?)_originalLevel$/i);
            if (m) {
                const factorKey = m[1].replace(/\s+/g,' ');
                factorContents[factorKey + "_originalLevel"] = pdSections[k];
            }
        });
    ...
    function collectDutiesCombined() {
        // Prefer single MAJOR DUTIES if present, otherwise combine numbered MAJOR DUTY N entries
    ...
    function removeExistingFactorKeys() {
        Object.keys(pdSections).forEach(key => {
            if (canonicalFactorKey(key)) {
    ...
    function insertFactorsInOrder(factorsObj) {
        ...
        const canonMap = {};
        Object.keys(factorsObj || {}).forEach(k => {
            const ck = canonicalFactorKey(k) || (typeof k === 'string' ? k.trim() : null);
            if (ck) canonMap[ck] = factorsObj[k];
        });

        order.forEach(id => {
            const factorKey = `Factor ${id}`;
            const srcKey = Object.keys(canonMap).find(k =>
                k.toLowerCase() === factorKey.toLowerCase() || k.toLowerCase() === id.toLowerCase()
            );
            if (!srcKey) return;
            const factor = canonMap[srcKey];
            const level = factor.level || factor.Level || '';
            const points = factor.points || factor.Points || '';
            const factorName = getFactorName(String(id).replace(/[A-Za-z]/g, '')) || `Factor ${id}`;
            const header = `Factor ${id} - ${factorName}` + (level ? ` Level ${level}` : '') + (points ? `, ${points} Points` : '');
            const text = (factor.rationale || factor.content || pdSections[srcKey] || '').toString().trim();
            pdSections[header] = text;
        });
    }

    // SHARED: Detect system from factor header TITLES
    function detectSystemFromHeaders() {
        // GSSG supervisory factor names
        const gssgIndicators = [
        ...
        // Check all pdSections keys (which contain the full headers)
        Object.keys(pdSections).forEach(key => {
            const lowerKey = key.toLowerCase();
            if (gssgIndicators.some(ind => lowerKey.includes(ind))) gssgCount++;
            if (fesIndicators.some(ind => lowerKey.includes(ind))) fesCount++;
        });
        
        // Also check for definitive key patterns
        const has4A = Object.keys(pdSections).some(k => /Factor\s*4A/i.test(k));
        const has4B = Object.keys(pdSections).some(k => /Factor\s*4B/i.test(k));
        const has7 = Object.keys(pdSections).some(k => /Factor\s*7[^A-Z]/i.test(k) || /Factor\s*7\s*-/i.test(k));
        const has8 = Object.keys(pdSections).some(k => /Factor\s*8[^A-Z]/i.test(k) || /Factor\s*8\s*-/i.test(k));
        const has9 = Object.keys(pdSections).some(k => /Factor\s*9[^A-Z]/i.test(k) || /Factor\s*9\s*-/i.test(k));
        ...
        console.log('System detection from headers:', {
            ...
            has8,
            has9,
            sampleKeys: Object.keys(pdSections).filter(k => /^Factor/i.test(k)).slice(0, 3)
        });
        
        if (has4A || has4B) return true;  // Definitely GSSG
        if (has7 || has8 || has9) return false; // Definitely FES
        if (gssgCount > fesCount && gssgCount > 0) return true;
        if (fesCount > gssgCount && fesCount > 0) return false;
        
        // Fallback: count factors
        return Object.keys(pdSections).filter(k => /^Factor\s*\d+/i.test(k)).length <= 7;
    }

    // ACTION: if editing a Factor section -> recalc points/grade
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // Validate we have duties
            if (!dutiesPayload || dutiesPayload.trim().length === 0) {
                console.warn('No duties found, using factor content as fallback');
            }

            // Detect system from FACTOR HEADER TITLES
            let isSupervisory = detectSystemFromHeaders();

            // Client explicit override (if provided earlier)
            if (typeof formState !== 'undefined' && formState.systemLocked) {
                isSupervisory = String(formState.systemLocked).toUpperCase() === 'GSSG';
            }

            console.log('Factor update payload:', {
                system: isSupervisory ? 'GSSG' : 'FES',
                detectionMethod: 'Factor header titles in pdSections',
                factorCount: Object.keys(factorContents).length,
                hasDuties: !!dutiesPayload,
                sampleHeaders: Object.keys(pdSections).filter(k => /^Factor/i.test(k)).slice(0, 3)
            });
            
            console.log('Full factor contents being sent:', factorContents);

            const payload = {
                factors: factorContents,
                expectedGrade: (typeof formState !== 'undefined' && formState.gsGrade) ? formState.gsGrade : '',
                supervisoryLevel: (typeof formState !== 'undefined' && formState.supervisoryLevel) ? formState.supervisoryLevel : 'Non-Supervisory',
            ...
            if (!response.ok) {
                // try to extract helpful diagnostics from response
                let txt = '';
                try { 
            ...
            // Preserve MAJOR DUTIES (combine into single section) before removing factor keys
            const majorDutyKeys = Object.keys(pdSections).filter(k => /^MAJOR DUT(Y|IES)\s*\d*/i.test(k.trim()));
            const combinedBackup = majorDutyKeys.length > 0
                ? majorDutyKeys.sort((a,b)=> (parseInt(a.match(/\d+/)?.[0]||'0',10)-parseInt(b.match(/\d+/)?.[0]||'0',10)))
                    .map(k => pdSections[k]).filter(Boolean).join('\n\n')
                : (pdSections['MAJOR DUTIES'] || '');

            // Remove only factor keys
            removeExistingFactorKeys();

            // Restore single MAJOR DUTIES entry
            ...
            // Insert/Update returned factor sections (preserve locked titles when updateFactorSections is available)
            if (data.factors && typeof updateFactorSections === 'function') {
                try {
                    updateFactorSections(data.factors);
                } catch (err) {
                    console.warn('updateFactorSections failed, fallback to manual insert:', err);
            ...
            // ALWAYS ensure summary sections exist and include Final Grade & Grade Range
            const tp = data.totalPoints !== undefined ? data.totalPoints : (typeof safeInt === 'function' ? safeInt(data.totalPoints) : 0);
            const fg = data.finalGrade || data.finalGrade === '' ? data.finalGrade : (data.finalGrade || '');
            const gr = (data.gradeRange !== undefined && data.gradeRange !== null) ? data.gradeRange : (data.gradeRange || 'Unknown');

            // Remove old summary keys first
            Object.keys(pdSections).forEach(k => {
            ...
            // Add/update summary headers explicitly (always add Final Grade and Grade Range even if 'Unknown')
            pdSections[`Total Points: ${tp || 0}`] = '';
            pdSections[`Final Grade: ${fg || (typeof calculateFinalGrade === 'function' ? calculateFinalGrade(tp) : 'Unknown')}`] = '';
            pdSections[`Grade Range: ${gr || (typeof getPointRangeForGrade === 'function' ? getPointRangeForGrade(fg) : 'Unknown')}`] = '';
            ...
            // show error to user and keep MAJOR DUTIES intact
    ...
    // MAJOR DUTIES path: FIRST detect system from existing headers, THEN regenerate
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Save MAJOR DUTIES into single key
            pdSections['MAJOR DUTIES'] = newContent;

            // Remove numbered MAJOR DUTY N keys to avoid duplication on server; keep single MAJOR DUTIES for payload
            Object.keys(pdSections).forEach(key => {
            ...
            // CRITICAL: Detect system from EXISTING factor header titles BEFORE regenerating
            let isSupervisory = detectSystemFromHeaders();
            ...
            // Collect current factors to send structure info to server
            const currentFactors = collectFactorContents();

            const payload = {
                duties: newContent,
                supervisoryLevel: isSupervisory ? 'Supervisory' : 'Non-Supervisory',
            ...
            // Remove existing factor keys (safe)
            removeExistingFactorKeys();

            // Restore MAJOR DUTIES (we already set it to newContent)
            pdSections['MAJOR DUTIES'] = newContent;

            // Insert returned factors
            if (data.factors && typeof updateFactorSections === 'function') {
                try {
                    updateFactorSections(data.factors);
                } catch (err) {
            ...
            // Always ensure summary fields are present
            const tp = data.totalPoints !== undefined ? data.totalPoints : 0;
            const fg = data.finalGrade || (typeof calculateFinalGrade === 'function' ? calculateFinalGrade(tp) : 'Unknown');
            const gr = (data.gradeRange !== undefined && data.gradeRange !== null) ? data.gradeRange : (typeof getPointRangeForGrade === 'function' ? getPointRangeForGrade(fg) : 'Unknown');
...
async function updateSummarySections(data) {
    ...
    // Add new summary sections with updated values - ALWAYS show even if empty
    if (data.totalPoints !== undefined && data.totalPoints !== null) {
        const key = `Total Points: ${data.totalPoints}`;
        pdSections[key] = ''; // Empty content, header only
        console.log('Added Total Points section:', key);
    }
    
    if (data.finalGrade) {
        const key = `Final Grade: ${data.finalGrade}`;
        pdSections[key] = ''; // Empty content, header only
        console.log('Added Final Grade section:', key);
    }
    
    if (data.gradeRange && data.gradeRange !== 'Unknown') {
        const key = `Grade Range: ${data.gradeRange}`;
        pdSections[key] = ''; // Empty content, header only
        console.log('Added Grade Range section:', key);
    }
    ...
    // CRITICAL: Re-render to show updated sections
...
async function updateFactorSections(factors) {
    console.log('Updating factor sections (LOCKED TITLES - only level/points change):', factors);

    // Determine system from EXISTING pdSections keys (CRITICAL)
    ...
    console.log('Existing factor keys:', existingKeys.filter(k => /^Factor\s*\d+/i.test(k)));

    const order = isSupervisory ? ['1','2','3','4A','4B','5','6'] : ['1','2','3','4','5','6','7','8','9'];

    order.forEach(id => {
        const factorKey = `Factor ${id}`;
        const responseKey = Object.keys(factors).find(k => 
            k.toLowerCase() === factorKey.toLowerCase() || 
            k.toLowerCase() === id.toLowerCase()
        );
        
        if (!responseKey) return;
        
        const factor = factors[responseKey];
        if (!factor) return;

        const level = factor.level || factor.Level || '';
        const points = factor.points || factor.Points || '';
        const rationale = (factor.rationale || factor.content || '').toString().trim();
        
        // CRITICAL: Find existing key to preserve EXACT title
        const existingKey = existingKeys.find(k => {
            const m = k.match(/^Factor\s*(\d+[A-Za-z]?)\s*[-–]\s*(.+?)\s*Level/i);
            if (!m) return false;
            return m[1].replace(/\s+/g, '').toUpperCase() === id.replace(/\s+/g, '').toUpperCase();
        });

        if (existingKey) {
            // Extract the LOCKED title from existing key
            const titleMatch = existingKey.match(/^(Factor\s*\d+[A-Za-z]?\s*[-–]\s*.+?)\s*Level/i);
            const lockedTitle = titleMatch ? titleMatch[1].trim() : `Factor ${id}`;
            
            // Build new key with LOCKED title, only updating level/points
            const newKey = `${lockedTitle} Level ${level}, ${points} Points`;
            
            console.log(`PRESERVING: "${existingKey}" → "${newKey}"`);
            
            // Update with new key if changed
            if (existingKey !== newKey) {
                pdSections[newKey] = rationale;
                delete pdSections[existingKey];
            } else {
                pdSections[newKey] = rationale;
            }
        } else {
            // No existing key - should not happen, but create with server's title as fallback
            console.warn(`No existing key found for Factor ${id} - using server title`);
            const serverTitle = factor.header || `Factor ${id}`;
            pdSections[serverTitle] = rationale;
        }
    });
