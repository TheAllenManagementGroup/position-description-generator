...
const steps = [
    ...
    { id: 3, name: "Position Classification", description: "Define job series and title", icon: "fa-users" },
...
window.formatGeneratedPD = function(pdText) {
    ...
    // 1. Ensure major section headers are bold and start on new lines
    [
        ...
        const re = new RegExp(`\\*\\*${section.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}\\*\\*`, 'g');
        text = text.replace(re, `\n\n**${section}**\n`);
        text = text.replace(new RegExp(section, 'g'), `\n\n**${section}**\n`);
    });

    // 2. Factor headers: ensure each starts on a new line and is bold
    text = text.replace(/(Factor\d+\s*-[^\n]+?Level\d+-\d+,\s*\d+\s*Points)/g, '\n\n**$1**\n');
    text = text.replace(/(Factor\d+\s*-[^\n]+?Level\d+,\s*\d+\s*Points)/g, '\n\n**$1**\n');

    // 3. Numbered major duties: ensure spacing and bolding
    text = text.replace(/(\d+)\.\s*([A-Z][^:]+?):/g, '\n\n$1. **$2:**');

    // 4. Evaluation summary: split summary line into separate bold lines
    text = text.replace(/(\*\*EVALUATION SUMMARY:\*\*)\s*Total Points:(\d+)\s*Final Grade:\s*(GS-\d+)\s*Grade Range:(\d+-\d+)/gi,
        '\n\n$1\n**Total Points: $2**\n**Final Grade: $3**\n**Grade Range: $4**\n');

    // 5. If summary is not bolded, fix it
    text = text.replace(/EVALUATION SUMMARY:/gi, '\n\n**EVALUATION SUMMARY:**\n');

    // 6. Ensure each conditions, title/series, and FLSA section starts on a new line
    [
        'CONDITIONS OF EMPLOYMENT:', 'TITLE AND SERIES DETERMINATION:', 'FAIR LABOR STANDARDS ACT DETERMINATION:'
    ].forEach(section => {
        text = text.replace(new RegExp(`\\*\\*${section}\\*\\*`, 'g'), `\n\n**${section}**\n`);
    ...
    // 7. Remove excessive spaces and normalize line breaks
    text = text.replace(/[ \t]+\n/g, '\n').replace(/\n[ \t]+/g, '\n').replace(/\n{3,}/g, '\n\n');
