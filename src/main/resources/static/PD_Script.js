...
async function fetchJobSeriesFromAPI(page = 1, allJobs = []) {
    try {
        ...
        while (pagesFetched < maxPages) {
            console.log(`Fetching job series from backend proxy... Page ${currentPage}`);
            const url = `/api/proxy/usajobs?ResultsPerPage=${resultsPerPage}&Page=${currentPage}`;
            ...
            if (items.length) {
                ...
                const seen = new Set(aggregated.map(i => i?.MatchedObjectId).filter(Boolean));
                items.forEach(it => {
            ...
            totalJobs = totalJobs ?? (data?.SearchResult?.SearchResultCount || items.length);
            const fetchedSoFar = currentPage * resultsPerPage;
            pagesFetched++;

            if (fetchedSoFar >= totalJobs) break;
            currentPage++;
        }
...
// Update renderDutiesStep to restore duties from formState
function renderDutiesStep(content) {
    // CRITICAL: Ensure formState.duties exists and has values BEFORE rendering
    ...
    // CRITICAL FIX: Call renderDutyInputs IMMEDIATELY with formState.duties
    console.log('Rendering duties with formState.duties:', formState.duties);
    renderDutyInputs(formState.duties);
    ...
    if (prevBtn) {
        prevBtn.addEventListener('click', () => {
            syncFormStateFromInputs();
            console.log('Saved duties before going back:', formState.duties);
            renderStep(3);
        });
    }
...
function renderReviewGenerateStep(content) {
    ...
    document.getElementById('previousReviewBtn').onclick = function() {
        syncFormStateFromInputs(); // Save duties before going back
        renderStep(4);
    };
...
function renderDutyInputs(dutiesArr) {
    ...
    // PRIMARY DUTY section (now numbered "1.")
    container.innerHTML += `
    ...
    // SECONDARY DUTIES section (start numbering at 2.)
    container.innerHTML += `
    ...
    // Auto-resize and input listeners with sync
    container.querySelectorAll('.duty-input').forEach(textarea => {
        textarea.addEventListener('input', function() {
            this.style.height = 'auto';
            this.style.height = (this.scrollHeight + 2) + 'px';
            syncFormStateFromInputs(); // Sync on every input
            updateDutyCounter();
        });
