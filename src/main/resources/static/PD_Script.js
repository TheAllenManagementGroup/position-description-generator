...
function splitPDSections(text) {
    ...
    const sectionHeaderRegex = new RegExp(
        [
            // Bolded headers (with or without colon)
            '\\*\\*([A-Z][A-Z0-9 \\-:&(),%/]+)\\*\\*\\s*:?',
            // Plain headers (DOE style)
            '(HEADER|INTRODUCTION|MAJOR DUTIES|FACTOR EVALUATION - COMPLETE ANALYSIS|EVALUATION SUMMARY|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES DETERMINATION|FAIR LABOR STANDARDS ACT DETERMINATION)',
            // Factor headers (plain or bolded)
            'Factor\\s*\\d+\\s*[–-]\\s*[^\\n]+',
            // Factor headers with no dash (for PDF weirdness)
            'Factor\\s*\\d+\\s*[^\\n]{5,40}Level\\d+-\\d+,\\d+\\s*Points',
            // Total Points, Final Grade, Grade Range (with or without spaces)
            'Total\\s*Points\\s*:\\s*\\d+',
            'Final\\s*Grade\\s*:\\s*GS-?\\d+',
            'Grade\\s*Range\\s*:\\s*\\d+-\\d+'
        ].join('|'),
        'gim'
    );
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        // If header runs into content, split after header
        ...
        if (!rawContent && matches[i + 1].index - headerEnd < 100) {
            // Try to split at first period or bullet
