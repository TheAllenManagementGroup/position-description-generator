...
async function handleAIRecommendation() {
    ...
    try {
        const response = await fetch('/api/recommend-series', {
            ...
            body: JSON.stringify({ 
                duties: duties,
                supervisoryLevel: formState.supervisoryLevel || 'Non-Supervisory'
            })
        });
        ...
        if (!data.recommendations || data.recommendations.length === 0) {
            aiSeriesResult.innerHTML = `
                <div class="alert alert-warning">
                    <strong>No High-Confidence Matches Found</strong><br>
                    The AI could not find any job series with 65% or higher confidence for your duties.
                    <br><br>
                    <strong>Suggestions:</strong>
                    <ul>
                        <li>Add more specific and detailed duties</li>
                        <li>Include technical skills and specialized knowledge</li>
                        <li>Mention specific software, tools, or methodologies you use</li>
                        <li>Describe the level of decision-making authority</li>
                    </ul>
                </div>
            `;
            return;
...
function displayAIRecommendations(data, container) {
    // Get recommendations from AI response (already filtered for >= 65% confidence on backend)
    const recs = (data.recommendations || []).slice(0, 3);

    // Dynamic colors based on confidence ranges
    const getConfidenceColor = (confidence) => {
        if (confidence >= 85) return { color: '#00780aff', bg: '#d1fae5' }; // Deep green
        if (confidence >= 75) return { color: '#50c23cff', bg: '#ecfdf5' }; // Light green  
        if (confidence >= 65) return { color: '#facc15', bg: '#fef9c3' };   // Yellow
        return { color: '#ef4444', bg: '#fecaca' }; // Red for low confidence
    };
    ...
    if (recs.length > 0) {
        ...
        recs.forEach((rec, index) => {
            ...
            html += `
                <button class="recommendation-btn"
                    ...
                    data-explanation="${rec.explanation || ''}"
        ...
        html += `<div class="alert alert-warning">
            No job series found with confidence >= 65%. 
            Please review your duties and try again, or consider using more specific descriptions.
        </div>`;
    }
    ...
    container.querySelectorAll('.recommendation-btn').forEach(btn => {
        btn.onclick = function() {
            ...
            formState.aiConfidence = confidence;
            formState.aiExplanation = explanation;
...
window.formatGeneratedPD = function(pdText) {
    ...
    // Add a line break before each "**" header if missing
    text = text.replace(/([^\n])(\*\*[A-Z])/g, '$1\n$2');

    // Fix points formatting
    text = text.replace(/(\d+)Points/g, '$1 Points');
