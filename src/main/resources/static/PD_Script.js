...
function splitPDSections(text) {
    ...
    // Normalize spacing issues in Factor 4A/4B headers
    ...
    // Insert line breaks before Factor headers (handle both with and without spaces in "Factor4A")
    text = text.replace(/(Factor\s*\d+[AB]?\s*[–-]\s*[^\n]+?Level\s*\d+-[\w]+,\s*\d+\s*Points?)/g, '\n$1');
    
    // Insert line breaks before summary items
    text = text.replace(/(Total Points:\s*\d+)/g, '\n$1');
    text = text.replace(/(Final Grade:\s*GS-?\d+)/g, '\n$1');
    text = text.replace(/(Grade Range:\s*[\d\-+]+)/g, '\n$1');

    // Clean up multiple consecutive newlines
    text = text.replace(/\n{3,}/g, '\n\n');

    // Enhanced section header regex to capture both formats
    const sectionHeaderRegex = /(?:\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*|^(Factor\s*\d+[AB]?\s*[–-]\s*[^\n]+?Level\s*\d+-[\w]+,\s*\d+\s*Points?)|^(Total Points:\s*\d+)|^(Final Grade:\s*GS-?\d+)|^(Grade Range:\s*[\d\-+]+))/gm;

    // Find all section headers
    let matches = [];
    let match;
    while ((match = sectionHeaderRegex.exec(text)) !== null) {
        // Extract the matched header from any of the capture groups
        let header = match[1] || match[2] || match[3] || match[4] || match[5] || match[0];
    ...
    matches.push({ title: 'END', index: text.length, raw: '' });
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        const headerEnd = currentMatch.index + currentMatch.raw.length;
        const start = headerEnd;
        const end = nextMatch.index;
        
        let rawContent = text.substring(start, end).trim();
        
        // Remove leading/trailing asterisks and extra whitespace
        const cleanContent = rawContent
            .replace(/^\*{1,2}/gm, '')
            .replace(/\*{1,2}$/gm, '')
            .trim();
        
        const header = currentMatch.title;

        // Skip the generic "FACTOR EVALUATION" or "FACTOR EVALUATION COMPLETE ANALYSIS" header
        if (/^FACTOR EVALUATION/i.test(header) || /^EVALUATION SUMMARY$/i.test(header)) {
            continue;
        }

        // Special handling for summary sections - store the full header as key with empty content
        if (/^Total Points:/i.test(header) || /^Final Grade:/i.test(header) || /^Grade Range:/i.test(header)) {
            sections[header] = '';
            continue;
        }

        // For factor sections, handle potential run-together content
        if (/^Factor\s*\d+[AB]?/i.test(header)) {
            // Check if content starts with the header text repeated (run-together case)
            const factorMatch = header.match(/^(Factor\s*\d+[AB]?\s*[–-].*?Points)/i);
            if (factorMatch && cleanContent.startsWith(factorMatch[1])) {
                // Remove the repeated header from content
                const contentWithoutHeader = cleanContent.substring(factorMatch[1].length).trim();
                sections[header] = contentWithoutHeader;
            } else {
    ...
    // Fallback: If too few sections, try plain text splitting
    if (Object.keys(sections).length < 5) {
        const plainHeaders = [
            ...
            'Factor 1', 'Factor 2', 'Factor 3', 'Factor 4', 'Factor 5',
            'Factor 6', 'Factor 7', 'Factor 8', 'Factor 9',
            'Factor 4A', 'Factor 4B', // Add explicit 4A/4B
            'Total Points', 'Final Grade', 'Grade Range',
            'Conditions of Employment',
            'Title and Series Determination',
            'Fair Labor Standards Act Determination'
        ...
        // Build regex to match headers (case-insensitive, with optional colon)
        const plainHeaderRegex = new RegExp(
            `(?:^|\\n)\\s*(${plainHeaders.map(h => h.replace(/ /g, '\\s*')).join('|')})\\s*:?(?=\\s|\\n|$)`,
            'gim'
        );
        ...
        plainMatches.push({ title: 'END', index: text.length, length: 0 });

        if (plainMatches.length > 1) {
            // Extract HEADER: everything before the first recognized header
            if (plainMatches[0].index > 0) {
                sections['HEADER'] = text.substring(0, plainMatches[0].index).trim();
            }
            
            for (let i = 0; i < plainMatches.length - 1; i++) {
                const header = plainMatches[i].title.toUpperCase();
                const start = plainMatches[i].index + plainMatches[i].length;
    ...
    // Extract summary values from text if not found as headers
    ...
    // If no HEADER section found, extract from first lines
    if (!sections['HEADER']) {
        const lines = text.split('\n');
        let headerLines = [];
        
        // Look for lines that contain header-like information
        for (let i = 0; i < Math.min(15, lines.length); i++) {
            const line = lines[i].trim();
            if (!line) continue;
            
            // Stop if we hit a section header
            if (/^(Introduction|Major Duties|Factor)/i.test(line)) break;
    ...
    // Normalize Factor 4A and 4B keys (handle spacing variations)
    const normalizedSections = {};
    for (const [key, value] of Object.entries(sections)) {
        // Normalize factor keys to consistent format
        let normalizedKey = key;
        
        // Handle "Factor4A" -> "Factor 4A"
        normalizedKey = normalizedKey.replace(/Factor\s*4\s*A/i, 'Factor 4A');
        normalizedKey = normalizedKey.replace(/Factor\s*4\s*B/i, 'Factor 4B');
        
        // Handle other factors
        normalizedKey = normalizedKey.replace(/Factor\s*(\d+)\s/i, 'Factor $1 ');
        
        normalizedSections[normalizedKey] = value;
    }
