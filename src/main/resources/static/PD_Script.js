...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- HEADER FIXES ---
    // Fix Justice Department + Bureau/Agency merging (like "U.S. Department of JusticeFederal Bureau of Prisons")
    text = text.replace(/(U\.S\. Department of Justice)([A-Z][a-zA-Z ]+(?:Bureau|Division|Office|Administration|Service)[^\\n]*)/g, '$1\n$2');
    
    // Fix Position/Title + GS grade merging (like "Research and Analysis SupervisorGS-11")
    text = text.replace(/(Position: [^G\n]+|[A-Z][a-zA-Z ]+Supervisor|[A-Z][a-zA-Z ]+Manager|[A-Z][a-zA-Z ]+Analyst)(GS-\d+)/g, '$1\n$2');
    
    // Split after GS grade with proper spacing
    text = text.replace(/(GS-\d+)\s*(Organizational Title:|Position:|Title:)/g, '$1\n$2');
    
    // Ensure double line break before "**INTRODUCTION:**"
    text = text.replace(/(Organizational Title:[^\n]+)\s*(\*\*INTRODUCTION:\*\*)/g, '$1\n\n$2');

    // --- FACTOR HEADER FIXES ---
    // Handle malformed factor headers like "Factor1 - Knowledge Required by the Position Level1-7,1250 Points:"
    text = text.replace(
        /Factor(\d+) - ([^L]+?)Level(\d+)-(\d+),(\d+) Points:?/gi,
        '\n\n**Factor $1 - $2 Level $3-$4, $5 Points**\n'
    );
    
    // Handle factor headers with better spacing - match various formats
    text = text.replace(
    ...
    sectionHeaders.forEach(section => {
        ...
        // Handle already bold headers
        const boldPattern = new RegExp(`\\*\\*\\s*${escapedSection}\\s*\\*\\*`, 'gi');
        text = text.replace(boldPattern, `\n\n**${section}**\n`);
        
        // Handle plain headers (not bold)
        const plainPattern = new RegExp(`(?<!\\*)\\s*${escapedSection}\\s*(?!\\*)`, 'gi');
        text = text.replace(plainPattern, `\n\n**${section}**\n`);
    });

    // --- MAJOR DUTIES FORMATTING ---
    // Handle numbered duties with percentages
    text = text.replace(/(\d+)\.\s*([^:]+?)(\s*\(\d+%\))?:\s*/gm, '\n$1. **$2$3:**\n');
    
    // Handle Research & Analysis, Technical & Program Support, etc.
    text = text.replace(/(\d+)\.\s*\*\*([^*]+?)\*\*\s*\((\d+%)\):\s*/gm, '\n$1. **$2 ($3):**\n');

    // --- EVALUATION SUMMARY FORMATTING ---
    // Handle merged evaluation summary like "Total Points:2540Final Grade: GS-11Grade Range:2356-2855"
    text = text.replace(
        /Total Points:(\d+)Final Grade: (GS-\d+)Grade Range:(\d+-\d+)/gi,
        '\n\n**Total Points:** $1\n**Final Grade:** $2\n**Grade Range:** $3'
    );
    
    // Handle other merged variations
    text = text.replace(
        /(\*\*EVALUATION SUMMARY:\*\*)\s*\*\*Total Points:\s*(\d+)\*\*\s*\*\*Final Grade:\s*(GS-\d+)\*\*\s*\*\*Grade Range:\s*(\d+[-–]\d+)\*\*/gi,
        '$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4'
    );
    
    // Handle individual evaluation summary items
    text = text.replace(/\*\*Total Points:\s*(\d+)\*\*/gi, '\n**Total Points:** $1');
    text = text.replace(/\*\*Final Grade:\s*(GS-\d+)\*\*/gi, '\n**Final Grade:** $1');
    text = text.replace(/\*\*Grade Range:\s*(\d+[-–]\d+)\*\*/gi, '\n**Grade Range:** $1');
    
    // Handle unbolded versions
    text = text.replace(/Total Points:\s*(\d+)/gi, '\n**Total Points:** $1');
    text = text.replace(/Final Grade:\s*(GS-\d+)/gi, '\n**Final Grade:** $1');
    text = text.replace(/Grade Range:\s*(\d+[-–]\d+)/gi, '\n**Grade Range:** $1');

    // --- CONDITIONS OF EMPLOYMENT FORMATTING ---
    ...
    text = text
        ...
        .replace(/\n[ \t]+/g, '\n')
        // Normalize multiple line breaks to double breaks
        .replace(/\n{3,}/g, '\n\n')
        ...
        .replace(/(\*\*[A-Z][^*]*:\*\*)\n{3,}/g, '$1\n\n')
        // Ensure line break before section headers
        .replace(/([^\n])\n(\*\*[A-Z][^*]*:\*\*)/g, '$1\n\n$2')
...
// Your markdownToHTML function (keeping as is since it works)
...
// Your render function (keeping as is since it works)
...
async function streamPDGeneration(payload) {
        ...
        for (let line of lines) {
            if (line.startsWith('data: ')) {
                ...
                try {
                    ...
                    if (parsed.response) {
                        // DON'T FORMAT DURING STREAMING - just show raw content
                        aiResult.textContent += parsed.response;
                        fullPD += parsed.response;
                    }
                    if (parsed.fullPD) {
                        // DON'T FORMAT DURING STREAMING - just accumulate
                        aiResult.textContent = parsed.fullPD;
                        fullPD = parsed.fullPD; // Use the complete PD
                    }
                } catch (e) {
    ...
    // ONLY FORMAT AFTER STREAMING IS COMPLETELY DONE
    if (fullPD.trim()) {
        // Store the raw PD
        window.generatedPD = fullPD;
        
        // Show formatted preview during streaming
        const formattedPreview = window.formatGeneratedPD(fullPD);
        aiResult.innerHTML = formattedPreview.replace(/\n/g, '<br>').replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
