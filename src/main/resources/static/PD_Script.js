...
function renderJobClassificationStep(content) {
    ...
    // --- Render the Position Title field (dropdown or text input) ---
    ...
    // --- Helper to render the Position Title field ---
    function renderPositionTitleField() {
        ...
        if (formState.jobSeries === '0301') {
            ...
            const subSeries = (formState.jobSeries && jobSeriesData[formState.jobSeries] && Array.isArray(jobSeriesData[formState.jobSeries].subSeries))
                ? jobSeriesData[formState.jobSeries].subSeries
                : [];
            const options = subSeries.length > 0 ? subSeries : [jobSeriesData[formState.jobSeries]?.title || 'Position Title'];
            group.innerHTML = `
                <label class="form-label" for="positionTitleInput">Position Title <span style="color:#ef4444;">*</span></label>
                <div class="dropdown-wrapper">
                    <input class="form-input" id="positionTitleInput" autocomplete="off"
            ...
            `;
            setupPositionTitleDropdown(options);
        }
    }

    // --- Listen for job series changes and update Position Title field immediately ---
    document.getElementById('jobSeriesDropdown').addEventListener('input', function() {
        formState.jobSeries = this.value;
        formState.positionTitle = '';
        // Also clear the input field if present
    ...
    // --- Listen for group series changes and update job series and position title fields ---
    document.getElementById('groupSeriesDropdown').addEventListener('input', function() {
        formState.groupSeries = this.value;
        formState.jobSeries = '';
        formState.positionTitle = '';
        const jobSeriesInput = document.getElementById('jobSeriesDropdown');
        if (jobSeriesInput) jobSeriesInput.value = '';
        const positionTitleInput = document.getElementById('positionTitleInput');
        if (positionTitleInput) positionTitleInput.value = '';
        setupJobSeriesDropdown();
        renderPositionTitleField();
