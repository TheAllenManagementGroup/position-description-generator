...
window.formatGeneratedPD = function(pdText) {
    ...
    // Ensure section headers are bold and spaced
    [
    ...
    // Major duties: ensure each starts on a new line
    text = text.replace(/(\d+)\.\s*([A-Z][^:]+?):/g, '\n\n$1. **$2:**');

    // Fix factor headers: add spaces and proper formatting
    // Factor1 - Knowledge Required by the Position Level1-7,1250 Points
    // becomes: Factor 1 - Knowledge Required by the Position Level 1-7, 1250 Points
    text = text.replace(/(Factor)(\d+)\s*(-\s*[^L]+?)\s*(Level)(\d+)(-\d+),(\d+)\s*(Points)/g, 
        '\n\n**$1 $2 $3 $4 $5$6, $7 $8**\n');

    // Evaluation summary: split into separate bold lines
