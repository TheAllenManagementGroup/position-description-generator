...
function splitPDSections(text) {
    ...
    // Fallback: If too few sections, try to split plain PDs by common headers
    if (Object.keys(sections).length < 5) {
        ...
        // Build a regex to match all headers at line start
        const plainHeaderRegex = new RegExp(
            `^(${plainHeaders.map(h => h.replace(/ /g, '[ \\-]')).join('|')}):?`,
            'gim'
        );
