...
function splitPDSections(text) {
    ...
    // Pre-process text to insert line breaks before section headers that are run together
    ...
    // Enhanced section header regex to catch both formats
    const sectionHeaderRegex = /(?:\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*|Factor\s*\d+\s*[–-]\s*[^\n]+?Level\s*\d+-\d+,\s*\d+\s*Points|Total Points:\s*\d+|Final Grade:\s*GS-?\d+|Grade Range:\s*[\d\-]+|MAJOR DUTIES|Major Duties|Introduction|INTRODUCTION|Conditions of Employment|CONDITIONS OF EMPLOYMENT|Title and Series Determination|TITLE AND SERIES DETERMINATION|Fair Labor Standards Act Determination|FAIR LABOR STANDARDS ACT DETERMINATION)/g;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^\*\*[A-Z][A-Z0-9 \-:&(),%/]+\*\*$/.test(line)) {
            ...
            const startIdx = text.indexOf(line, currentLine);
            matches.push({
                raw: line,
                title: header,
                index: startIdx
            });
            currentLine = startIdx + line.length;
        }
        // Check for Factor headers
        else if (/^Factor\s*\d+\s*[–-]/.test(line)) {
            const startIdx = text.indexOf(line, currentLine);
            matches.push({
                raw: line,
                title: line.trim(),
        ...
        else if (/^Total Points:\s*\d+/.test(line)) {
            const startIdx = text.indexOf(line, currentLine);
            matches.push({
                raw: line,
                title: line.trim(),
        ...
        else if (/^Final Grade:\s*GS-?\d+/.test(line)) {
            const startIdx = text.indexOf(line, currentLine);
            matches.push({
                raw: line,
                title: line.trim(),
        ...
        else if (/^Grade Range:\s*[\d\-]+/.test(line)) {
            const startIdx = text.indexOf(line, currentLine);
            matches.push({
                raw: line,
                title: line.trim(),
        ...
        // Check for non-markdown section headers (like "Major Duties", "Introduction")
        else if (/^(Major Duties|Introduction|Conditions of Employment|Title and Series Determination|Fair Labor Standards Act Determination)$/i.test(line)) {
            const startIdx = text.indexOf(line, currentLine);
            matches.push({
                raw: line,
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        // Skip FACTOR EVALUATION sections
        ...
        if (/^Total Points:/i.test(header) || /^Final Grade:/i.test(header) || /^Grade Range:/i.test(header)) {
            ...
            if (/^Factor\s*\d+/i.test(header) && cleanContent) {
                // Check if content starts with another factor header
                const nextFactorMatch = cleanContent.match(/^(.+?)(Factor\s*\d+\s*[–-])/s);
                if (nextFactorMatch) {
                    sections[header] = nextFactorMatch[1].trim();
                } else {
    ...
    // If no summary sections found in headers, calculate from factors
    const hasTotalPoints = Object.keys(sections).some(k => /^Total Points:/i.test(k));
    const hasFinalGrade = Object.keys(sections).some(k => /^Final Grade:/i.test(k));
    const hasGradeRange = Object.keys(sections).some(k => /^Grade Range:/i.test(k));
    
    if (!hasTotalPoints || !hasFinalGrade || !hasGradeRange) {
        console.log('Summary sections not found in headers, calculating from factors...');
        
        // Extract points from all factor sections
        let totalPoints = 0;
        Object.keys(sections).forEach(key => {
            if (/^Factor\s*\d+/i.test(key)) {
                const pointsMatch = key.match(/(\d+)\s*Points/i);
                if (pointsMatch) {
                    const points = parseInt(pointsMatch[1], 10);
                    console.log(`Found ${points} points in: ${key}`);
                    totalPoints += points;
                }
            }
        });
        ...
        if (totalPoints > 0) {
            ...
            if (totalPoints >= 5000) { grade = 'GS-15'; gradeRange = '15'; }
            else if (totalPoints >= 4480) { grade = 'GS-14'; gradeRange = '14-15'; }
            else if (totalPoints >= 3855) { grade = 'GS-13'; gradeRange = '13-14'; }
            else if (totalPoints >= 3150) { grade = 'GS-12'; gradeRange = '12-13'; }
            else if (totalPoints >= 2755) { grade = 'GS-11'; gradeRange = '11-12'; }
            else if (totalPoints >= 2355) { grade = 'GS-10'; gradeRange = '10-11'; }
            else if (totalPoints >= 1855) { grade = 'GS-9'; gradeRange = '9-10'; }
            else if (totalPoints >= 1605) { grade = 'GS-8'; gradeRange = '8-9'; }
            else if (totalPoints >= 1355) { grade = 'GS-7'; gradeRange = '7-8'; }
            else if (totalPoints >= 1105) { grade = 'GS-6'; gradeRange = '6-7'; }
            else if (totalPoints >= 855) { grade = 'GS-5'; gradeRange = '5-6'; }
            
            console.log(`Calculated summary: ${totalPoints} points = ${grade}, range ${gradeRange}`);
            
            if (!hasTotalPoints) sections[`Total Points: ${totalPoints}`] = '';
            if (!hasFinalGrade) sections[`Final Grade: ${grade}`] = '';
            if (!hasGradeRange) sections[`Grade Range: ${gradeRange}`] = '';
        }
    }
