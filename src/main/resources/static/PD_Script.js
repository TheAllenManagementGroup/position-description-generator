...
function renderGeneratedPDStep(content) {
    content.innerHTML = `
        <div class="card">
            ...
            <div class="card-content">
                <div class="pd-display" id="finalPD" style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; padding: 1rem; border-radius: 0.5rem; background-color: #f9fafb;"></div>
    ...
    if (generatedText) {
        // Use the enhanced HTML formatter for best results
        finalPDElement.innerHTML = window.formatGeneratedPDToHTML(generatedText);
    } else {
...
window.formatGeneratedPD = function(pdText) {
    ...
    // Fix the main header spacing issues
    text = text.replace(/(U\.S\. Department of Justice)([A-Z])/g, '$1\n$2');
    text = text.replace(/([A-Za-z &]+ Division)([A-Z][^G])/g, '$1\n$2');
    text = text.replace(/(Criminal Division)(Criminal Division)/g, '$1'); // Remove duplicate
    text = text.replace(/(GS-\d+)(Organizational Title:)/g, '$1\n\n$2');
    
    // Ensure HEADER and INTRODUCTION are properly separated
    text = text.replace(/(\*\*HEADER:\*\*[^*]*?)(\*\*INTRODUCTION:\*\*)/g, '$1\n\n$2');
    ...
    sectionHeaders.forEach(section => {
        ...
        // Handle existing bold formatting
        text = text.replace(new RegExp(`\\*\\*${escapedSection}\\*\\*`, 'gi'), `\n\n**${section}**\n`);
        // Handle plain text headers
        text = text.replace(new RegExp(`(?<!\\*)${escapedSection}(?!\\*)`, 'gi'), `\n\n**${section}**\n`);
    });
    
    // === FACTOR FORMATTING FIXES ===
    // This is the key fix for factor spacing issues
    // Pattern: **Factor1 - Knowledge Required by the Position Level1-7,1250 Points**
    text = text.replace(
        /\*\*Factor\s*(\d+)\s*-\s*([^L]*?)\s*Level\s*(\d+)-(\d+),\s*(\d+)\s*Points\*\*/gi,
        '\n**Factor $1 - $2(Level $3-$4, $5 Points)**\n'
    );
    
    // Handle factors without existing bold formatting
    text = text.replace(
        /Factor\s*(\d+)\s*-\s*([^L]*?)\s*Level\s*(\d+)-(\d+),\s*(\d+)\s*Points/gi,
        '\n**Factor $1 - $2(Level $3-$4, $5 Points)**\n'
    );
    ...
    // Fix numbered duties - ensure proper line breaks
    text = text.replace(/(\d+)\.\s*([^:]+?):\s*([^0-9]+?)(?=\d+\.|\*\*|$)/g, 
        '\n$1. **$2:**\n$3\n');
    
    // Handle duties with percentages in the title
    text = text.replace(/(\d+)\.\s*([^:]+?)\s*\((\d+%)\):\s*/g, '\n$1. **$2 ($3):**\n');
    
    // === EVALUATION SUMMARY FIXES ===
    // Handle the compact evaluation summary format
    text = text.replace(
        /\*\*Total Points:\s*(\d+)\*\*\s*\*\*Final Grade:\s*(GS-\d+)\*\*\s*\*\*Grade Range:\s*(\d+[-–]\d+)\*\*/gi,
        '\n**Total Points:** $1\n**Final Grade:** $2\n**Grade Range:** $3\n'
    );
    
    // Handle cases without existing bold formatting
    ...
    text = text
        ...
        .replace(/\n[ \t]+/g, '\n')
        // Fix cases where section headers are immediately followed by content
        .replace(/(\*\*[^*]+:\*\*)\n([A-Z][^*\n]+)/g, '$1\n\n$2')
        // Ensure factors have proper spacing
        .replace(/(\*\*Factor \d+[^*]+\*\*)\n([A-Z])/g, '$1\n\n$2')
        // Clean up multiple consecutive line breaks
        .replace(/\n{3,}/g, '\n\n')
        // Remove stray asterisks
        .replace(/^\*\*\s*$/gm, '')
        // Fix double closing asterisks
        .replace(/(\*\*[^*]+:\*\*)\*\*/g, '$1')
...
// Enhanced HTML formatter with better styling
window.formatGeneratedPDToHTML = function(pdText) {
    if (!pdText) return '';
    
    // First apply the text formatting
    let formattedText = window.formatGeneratedPD(pdText);
    
    // Convert to HTML with proper styling
    let html = formattedText
        // Convert section headers
        .replace(/\*\*(HEADER|INTRODUCTION|MAJOR DUTIES|FACTOR EVALUATION[^*]*|EVALUATION SUMMARY|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES DETERMINATION|FAIR LABOR STANDARDS ACT DETERMINATION):\*\*/g,
            '<div class="section-header">$1:</div>')
        // Convert factor headers
        .replace(/\*\*(Factor \d+ - [^*]+)\*\*/g, '<div class="factor-header">$1</div>')
        // Convert numbered duties
        .replace(/(\d+)\.\s*\*\*([^*]+):\*\*/g, '<div class="duty-header">$1. $2:</div>')
        // Convert other bold text
        .replace(/\*\*([^*]+):\*\*/g, '<div class="bold-label">$1:</div>')
        .replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>')
        // Convert line breaks to HTML
        .replace(/\n\n/g, '</p><p>')
        .replace(/\n/g, '<br>')
        // Wrap in container
        .replace(/^(.*)$/s, '<div class="pd-content"><p>$1</p></div>')
        // Clean up empty paragraphs
        .replace(/<p><\/p>/g, '')
        .replace(/<p><br><\/p>/g, '');
...
// Test function specifically for the provided document
window.testCurrentDocument = function() {
...
The position requires a comprehensive knowledge...`;
    
    console.log('=== TESTING CURRENT DOCUMENT ISSUES ===');
    console.log('Original text snippet:');
    ...
    console.log('\n=== HTML VERSION ===');
    const html = window.formatGeneratedPDToHTML(testText);
    console.log(html);
