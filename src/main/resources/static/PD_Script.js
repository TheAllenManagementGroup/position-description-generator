...
function renderStep(step) {
    ...
    switch (step) {
        ...
        case 'unknownSeries':
            ...
            break;
        case 4:
            await renderDutiesStepWithPrefill(content);
            break;
        case 5:
    ...
    duties.forEach((duty, i) => {
        container.innerHTML += `
            <div class="duty-input-group" style="display:flex;align-items:flex-start;gap:0.5em;">
                <span class="duty-number" style="width:2em;text-align:right;font-weight:600;color:#2563eb;margin-top:0.5em;">${i + 1}.</span>
                <textarea class="duty-input" style="flex:1;min-height:38px;resize:vertical;overflow-x:hidden;overflow-y:auto;box-sizing:border-box;" placeholder="Duty/Responsibility ${i + 1}...">${duty}</textarea>
                <button type="button" class="remove-duty" onclick="removeDuty(this)">×</button>
            </div>
        `;
    });
    // Add Clear Duties button
    if (!container.querySelector('#clearDutiesBtn')) {
