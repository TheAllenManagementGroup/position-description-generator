...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- Step 2: Ensure HEADER and INTRODUCTION are separated ---
    text = text.replace(/(U\.S\. Department of Justice)([A-Z])/g, '$1\n$2');
    text = text.replace(/(GS-\d+)/g, '\n$1');
    text = text.replace(/(Organizational Title:)/g, '\n$1');

    text = text.replace(/\*\*HEADER:\*\*\s*/i, '**HEADER:**\n\n');
    text = text.replace(/\*\*INTRODUCTION:\*\*\s*/i, '**INTRODUCTION:**\n\n');

    // --- Step 3: Format Major Duties ---
    text = text.replace(/(\d+)\.\s*([A-Z][^:\n]+?)\s*:/g, '\n\n$1. **$2:**');

    // --- Step 4: Format Factor lines ---
    // Full Factor with Level and Points
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*([^L]+?)\s*Level\s*([0-9]+)-([0-9]+)\s*,?\s*([\d,]+)\s*Points/gi,
        '\n\n**Factor $1 – $2(Level $3-$4, $5 Points)**');

    // Fallback for simple Factor lines
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*-\s*([^\n\*]+)/gi,
        '\n\n**Factor $1 – $2**');

    // --- Step 5: Format Evaluation Summary ---
    text = text.replace(/(\*\*EVALUATION SUMMARY:\*\*)\s*Total Points:?\s*([\d,]+)\s*Final Grade:?\s*(GS-\d+)\s*Grade Range:?\s*([\d,-]+)/gi,
        '\n\n$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4');

    // --- Step 6: Ensure bullets in Conditions of Employment ---
    text = text.replace(/(^|\n)\s*-\s*/g, '\n- ');
    ...
    // --- Step 8: Add spacing before numbers and Factors ---
    text = text.replace(/(\*\*Factor \d+)/g, '\n$1'); // line before Factors
    text = text.replace(/(\d+\.\s\*\*[^*]+\*\*:)/g, '\n$1'); // line before Major Duties
