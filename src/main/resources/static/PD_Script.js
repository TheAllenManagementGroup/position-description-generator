...
window.formatGeneratedPD = function(pdText) {
    ...
    // --- Step 2: Ensure HEADER and INTRODUCTION separation ---
    ...
    text = text.replace(/\*\*INTRODUCTION:\*\*/i, '\n**INTRODUCTION:**\n\n');

    // --- Step 3: Normalize main section headers ---
    const sections = [
    ...
    sections.forEach(section => {
        const re = new RegExp(`\\*\\*${section.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&')}\\*\\*`, 'gi');
        text = text.replace(re, `\n\n**${section}**\n`);
    });

    // --- Step 4: Format Major Duties ---
    text = text.replace(/(\d+)\.\s*([A-Z][^:\n]+?)\s*:/g, '\n\n$1. **$2:**');

    // --- Step 5: Format Factors ---
    // Full Factor with Level and Points
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*([\s\S]*?)\s*Level\s*(\d+)-(\d+)\s*,?\s*([\d,]+)\s*Points/gi,
        '\n\n**Factor $1 – $2(Level $3-$4, $5 Points)**');

    // Simple Factors fallback
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*-\s*([^\n\*]+)/gi,
        '\n\n**Factor $1 – $2**');

    // --- Step 6: Format Evaluation Summary ---
    ...
    // --- Step 8: Ensure spacing before numbers & Factors ---
    text = text.replace(/(\*\*Factor \d+)/g, '\n$1');
    text = text.replace(/(\d+\.\s\*\*[^*]+\*\*:)/g, '\n$1');

    // --- Step 9: Cleanup remaining whitespace ---
    text = text.replace(/[ \t]+\n/g, '\n').replace(/\n[ \t]+/g, '\n').replace(/\n{3,}/g, '\n\n');
