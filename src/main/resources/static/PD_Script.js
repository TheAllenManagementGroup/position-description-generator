...
function saveSectionEdit(title, rerender = false) {
    if (!title) return;
    ...
    // Find the current key (handles title changes)
    const currentKey = getCurrentSectionKey(title);

    // If the section title changed, move stacks/history to new key
    let newTitle = currentKey;
    const sectionDiv = textarea.closest('.editor-section');
    if (sectionDiv) {
        ...
        if (headerLabel && headerLabel.textContent !== currentKey) {
            ...
            // Remove old key from pdSections if title changed
            if (pdSections[currentKey] !== undefined && currentKey !== newTitle) {
    ...
    if (!window.sectionEditHistory[newTitle]) window.sectionEditHistory[newTitle] = [];
    window.sectionEditHistory[newTitle].push({ content: newContent, header: newTitle, ts: Date.now() });

    // Sync the live stack to only the saved content
    window.sectionEditStacks[newTitle] = [newContent];
    pdSections[newTitle] = newContent; // Always update pdSections

    // Update display
    const display = findDisplayElement(newTitle);
    if (display) {
    ...
    if (sectionDiv) sectionDiv.classList.remove('modified');
    updateSaveStatus();
    ...
    // Optionally rerender all editable sections
    if (rerender) {
        renderEditableSections();
    }
}
...
async function updateFactorSectionsAfterEdit(factorTitle, factorContent, sectionDiv) {
    try {
        // Gather all factor sections
        const allFactors = {};
        Object.keys(pdSections).forEach(key => {
            if (/^Factor\s*\d+/i.test(key)) {
                allFactors[key] = pdSections[key];
            }
        });
        // Include the just-edited factor
        allFactors[factorTitle] = factorContent;

        const response = await fetch('/api/update-factor-points', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(allFactors)
        });

        if (!response.ok) throw new Error('Server error');
        const data = await response.json();

        // Update factor sections with new headers/content
        if (data.factors) {
            Object.entries(data.factors).forEach(([factorKey, factorData]) => {
                // Remove old factor key if header changed
                const oldKey = Object.keys(pdSections).find(k => k.startsWith(factorKey));
                if (oldKey && oldKey !== factorKey) delete pdSections[oldKey];
                pdSections[factorKey] = factorData.content || '';
            });
        }
        // Update summary sections
        if (data.totalPoints || data.finalGrade || data.gradeRange) {
...
window.updateFactorSectionsAfterEdit = updateFactorSectionsAfterEdit;
