...
async function saveSectionEdit(title) {
    ...
    // Helper to detect system from existing factor headers in pdSections
    function detectSystemFromHeaders() {
        ...
        console.log('System detection from headers:', {
            ...
            sampleKeys: existingKeys.filter(k => /^Factor/i.test(k)).slice(0, 3)
        });

        if (has4A || has4B) return true;  // GSSG
        if (has7 || has8 || has9) return false; // FES
        
        // Fallback: count factors
        return existingKeys.filter(k => /^Factor\s*\d+/i.test(k)).length <= 7;
    }
    ...
    function collectFactorContents() {
        ...
        keys.forEach(k => {
            ...
            if (factorMatch) {
                ...
                const points = parseInt(factorMatch[4], 10); // 1250
                ...
                // Fallback: simple factor match
                const m = k.match(/^(Factor\s*\d+[A-Za-z]?)/i);
                if (m) {
                    const canonical = m[1].replace(/\s+/g, ' ').trim();
                    const content = pdSections[k];
                    // For factors without proper header format, send as simple string
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // CRITICAL FIX: Set supervisoryLevel to match the detected system
            ...
            const payload = {
                ...
                supervisoryLevel: supervisoryLevel, // Use the matched supervisory level
            ...
            // Preserve MAJOR DUTIES before removing factor keys
            ...
            // Insert/Update returned factor sections
            ...
            // ALWAYS ensure summary sections exist
            ...
            // Add/update summary headers explicitly
            pdSections[`Total Points: ${tp || 0}`] = '';
            pdSections[`Final Grade: ${fg || 'Unknown'}`] = '';
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Remove numbered MAJOR DUTY N keys
            Object.keys(pdSections).forEach(key => {
            ...
            // Detect system from EXISTING factor headers
            let isSupervisory = detectSystemFromHeaders();
            ...
            // Always ensure summary fields
            const tp = data.totalPoints !== undefined ? data.totalPoints : 0;
            const fg = data.finalGrade || 'Unknown';
            const gr = data.gradeRange || 'Unknown';

            // Remove old summary keys
            Object.keys(pdSections).forEach(k => {
                if (/^Total Points:/i.test(k) || /^Final Grade:/i.test(k) || /^Grade Range:/i.test(k)) {
                    delete pdSections[k];
                }
...
function updateFactorSections(updatedFactors) {
    ...
    const systemType = isGSSG ? "SUPERVISORY (GSSG)" : "NON-SUPERVISORY (FES)";
    
    console.log("LOCKED SYSTEM DETECTED:", systemType);
    console.log("Existing factor keys:", Object.keys(pdSections).filter(k => k.startsWith('Factor')));
    
    // Create normalized lookup for existing factors (case-insensitive, ignore whitespace/punctuation)
    const normalizeKey = (str) => str.toUpperCase().replace(/[^A-Z0-9]/g, '');
    
    const existingFactorMap = {};
    Object.keys(pdSections).forEach(key => {
        if (key.startsWith('Factor')) {
            // Extract factor number (e.g., "1", "2", "4A", "4B")
            const match = key.match(/Factor\s*(\d+[AB]?)/i);
            if (match) {
                const factorNum = match[1];
                existingFactorMap[factorNum] = key;
            }
        }
    });
    
    console.log("Existing factor map:", existingFactorMap);
    ...
    Object.keys(updatedFactors).forEach(factorKey => {
        ...
        const factorNum = factorKey.replace('Factor ', '').trim(); // "1", "2", "4A", etc.
        
        console.log(`Processing ${factorKey} (num: ${factorNum})`);
        
        // Find existing section key
        let existingKey = existingFactorMap[factorNum];
        
        if (existingKey) {
            console.log(`✓ Found existing key for Factor ${factorNum}: "${existingKey}"`);
            
            // Parse existing header to get original title format
            const existingHeader = pdSections[existingKey];
            const titleMatch = existingHeader.match(/Factor\s*\d+[AB]?\s*[-–]\s*(.+?)\s*Level/i);
            const originalTitle = titleMatch ? titleMatch[1].trim() : factor.name;
            
            // Build new header with ORIGINAL title but updated level/points
            const newHeader = `Factor ${factorNum} - ${originalTitle} Level ${factor.level}, ${factor.points} Points`;
            
            // Update the section with new header
            const oldKey = existingKey;
            delete pdSections[oldKey];
            pdSections[newHeader] = factor.rationale;
            
            console.log(`  Updated: "${oldKey}" → "${newHeader}"`);
            
        } else {
            console.log(`  ⚠ No existing key found for Factor ${factorNum} - using server title`);
            
            // Fallback: create new section with server's title
            const newHeader = `Factor ${factorNum} - ${factor.name} Level ${factor.level}, ${factor.points} Points`;
            pdSections[newHeader] = factor.rationale;
        }
    ...
    const gradeInfo = calculateGradeFromPoints(totalPoints);
    
    // Update grade sections
    pdSections['Total Points'] = `${totalPoints}`;
    pdSections['Final Grade'] = gradeInfo.grade;
    pdSections['Grade Range'] = gradeInfo.range;
    
    console.log(`Updated totals: ${totalPoints} points, ${gradeInfo.grade}, Range: ${gradeInfo.range}`);
