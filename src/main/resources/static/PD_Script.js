...
function renderGradeAnalysisResultStyled(data) {
    ...
    if (Array.isArray(data.gradeRelevancy) && data.gradeRelevancy.length > 0) {
        ...
        data.gradeRelevancy.forEach(item => {
            ...
            if (validGrades.includes(Number(g))) {
                // Boost percentages for visual impact - increased boost
                let pct = Math.round(Number(item.percentage) || 0);
                if (pct > 0 && pct < 25) pct += 15;
                else if (pct >= 25 && pct < 50) pct += 10;
                else if (pct >= 70) pct += 12;
                gradeMap[`GS-${g}`] = Math.min(pct, 95); // Cap at 95%
            }
        });
    ...
    // Fallback: always show boosted percentages
    if (gradeData.length === 0 || gradeData.every(g => g.percentage === 0)) {
        if (adjacentGrades.length === 2) {
            gradeData = [
                { grade: adjacentGrades[0], percentage: 50 },
                { grade: adjacentGrades[1], percentage: 85 }
            ];
        } else {
            gradeData = [
                { grade: adjacentGrades[0], percentage: 35 },
                { grade: adjacentGrades[1], percentage: 85 },
                { grade: adjacentGrades[2], percentage: 45 }
            ];
        }
    }
    ...
    // Regenerate button with grade update
    ...
    regenerateBtn.onclick = async function() {
        ...
        try {
            ...
            // Update formState with new grade BEFORE rendering
            if (newData.gsGrade) {
                formState.gsGrade = newData.gsGrade;
            }
    ...
    // Evaluation Statement button with live streaming
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        ...
        evalResultDiv.innerHTML = `
            <div class="pd-display" style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; padding: 1rem; border-radius: 0.5rem; background-color: #f9fafb; line-height: 1.6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:120px;">
                    <span class="spinner" style="width:32px; height:32px; border-width:4px; margin-bottom:1em;"></span>
                    <span style="color:#2563eb; font-weight:600; font-size:1.1em;">Generating Evaluation Statement...</span>
                </div>
        ...
        try {
            ...
            // Stream content with live updates
            while (true) {
                    ...
                    if (line.trim().startsWith('data:')) {
                        ...
                        try {
                            ...
                            if (parsed.response) {
                                ...
                                evalTextDiv.innerHTML = window.markdownToHTML(fullText);
                            }
            ...
            cleaned = cleaned.replace(/^(Generating evaluation statement\.\.\.\s*)?/i, '');
            ...
            const spinnerDiv = evalResultDiv.querySelector('div[style*="align-items:center"]');
...
function renderDutiesGradeAnalysis(data) {
    ...
    // Regenerate Grade Analysis button with grade update
    const regenerateBtn = document.getElementById('regenerateGradeAnalysisBtn');
    function updateRegenBtnState() {
        const hasAdditionalDuties = Array.isArray(formState.refineDuties) && formState.refineDuties.some(d => d.trim().length > 0);
        regenerateBtn.disabled = !hasAdditionalDuties;
    ...
    // Generate Evaluation Statement button with live streaming
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
