...
function renderGradeAnalysisResultStyled(data) {
    ...
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        ...
        try {
            ...
            if (fullText.trim()) {
                const formatted = window.formatGeneratedPD(fullText);
                const evalTextDiv = document.getElementById('evalStatementText');
                evalTextDiv.innerHTML = window.markdownToHTML(formatted);
                
                // Remove the spinner
                evalResultDiv.querySelector('.spinner')?.remove();

                document.getElementById('evalExportBtns').style.display = '';
                setupEvalStatementExportButtons(formatted);
            } else {
                document.getElementById('evalStatementText').innerHTML = '<div style="color:red;">No evaluation statement generated.</div>';
            }
...
function renderDutiesGradeAnalysis(data) {
    ...
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        ...
        try {
            ...
            // After streaming is done, display the full evaluation statement
            if (fullText.trim()) {
                const formatted = window.formatGeneratedPD(fullText);
                const evalTextDiv = document.getElementById('evalStatementText');
                evalTextDiv.innerHTML = window.markdownToHTML(formatted);
                
                // Remove the loading spinner and parent container
                const spinnerParent = evalResultDiv.querySelector('.eval-scrollbox');
                if (spinnerParent) {
                    const spinner = spinnerParent.querySelector('.spinner');
