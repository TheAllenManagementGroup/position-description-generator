...
function formatUpdatedPD(sectionsObject) {
    ...
    // More robust major duty deduplication
    const majorDutyMap = new Map();
    ...
    Object.keys(cleanedSections).forEach(key => {
        ...
        if (match) {
            const dutyNumber = match[2];
            const base = `MAJORDUTY${dutyNumber}`;
            
            if (!majorDutyMap.has(base)) {
                majorDutyMap.set(base, key);
            } else {
                // Mark duplicate for removal, but merge content if different
                const existing = majorDutyMap.get(base);
                const existingContent = cleanedSections[existing] || '';
                const currentContent = cleanedSections[key] || '';
                
                if (currentContent.trim() && currentContent.trim() !== existingContent.trim()) {
                    // Merge content if both have meaningful content
                    cleanedSections[existing] = existingContent + '\n\n' + currentContent;
                }
    ...
    // Remove duplicates
    keysToRemove.forEach(key => delete cleanedSections[key]);
    ...
    // Add individual major duty sections
    Array.from(majorDutyMap.values())
        .sort((a, b) => {
    ...
    // FIXED: Always include summary sections - check for all variations
    ...
    summaryPatterns.forEach(pattern => {
        const key = Object.keys(cleanedSections).find(k => pattern.test(k));
        if (key && cleanedSections[key] && cleanedSections[key].trim()) {
...
// Helper function to clean content of unwanted asterisks and formatting
...
function findSectionKeyFlexible(sections, targetSection) {
    ...
    // Special handling for Factor sections - match any key that starts with "Factor X"
    ...
    // Special handling for MAJOR DUTY sections
    ...
    // Match without asterisks, colons, and extra whitespace
    ...
    // Broader partial match as fallback
...
function saveSectionEdit(title) {
    ...
    const textarea = document.getElementById(`textarea-${sectionKeyToId(currentKey)}`);
    if (!textarea) return;

    const display = document.getElementById(`display-${sectionKeyToId(currentKey)}`);
    let newContent = textarea.value.trim();
    newContent = cleanContent(newContent);

    // Detect if the section title has changed (e.g., after AI update)
    let newTitle = currentKey; // Use currentKey instead of title
    ...
    if (sectionDiv) {
        ...
        if (headerLabel && headerLabel.textContent !== currentKey) {
            ...
            // Move stacks/history to new key
            if (currentKey !== newTitle) {
                window.sectionEditStacks[newTitle] = window.sectionEditStacks[currentKey] || [];
    ...
    // Update display
    const newDisplay = document.getElementById(`display-${sectionKeyToId(newTitle)}`);
    if (newDisplay) {
        newDisplay.innerHTML = newContent || '<span style="color: #9ca3af; font-style: italic;">Click to add content...</span>';
        newDisplay.style.whiteSpace = 'pre-line';
    }
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        if (typeof updateFactorSectionsAfterEdit === "function") {
...
function updateSectionDisplayAndTextarea(sectionKey, content) {
    ...
    const displayDiv = document.getElementById(`display-${sectionKeyToId(actualKey)}`) || document.getElementById(`display-${sectionKeyToId(sectionKey)}`);
    const textareaDiv = document.getElementById(`textarea-${sectionKeyToId(actualKey)}`) || document.getElementById(`textarea-${sectionKeyToId(sectionKey)}`);
...
function undoSectionEdit(title) {
    // Always use the current key (in case the title changed)
    const currentKey = getCurrentSectionKey(title);
    const textarea = document.getElementById(`textarea-${sectionKeyToId(currentKey)}`);
    if (!textarea) return; // Add safety check
...
function getCurrentSectionKey(title) {
    ...
    // Try flexible match - look for all possible variations
    const keys = Object.keys(pdSections);
    
    // First try ID-based matching
    const match = keys.find(k => sectionKeyToId(k) === sectionKeyToId(title));
    if (match) return match;
    
    // Try case-insensitive matching
    const lowerTitle = title.toLowerCase();
    const caseMatch = keys.find(k => k.toLowerCase() === lowerTitle);
    if (caseMatch) return caseMatch;
    
    // Try partial matching for factor sections
    if (/^Factor\s*\d+/i.test(title)) {
        const factorNum = title.match(/^Factor\s*(\d+)/i)?.[1];
        if (factorNum) {
            const factorMatch = keys.find(k => k.match(/^Factor\s*(\d+)/i)?.[1] === factorNum);
            if (factorMatch) return factorMatch;
        }
    }
...
function cleanupMajorDutyDuplicates(sections) {
    // Normalize all keys and group by "MAJORDUTY" + number
    const keys = Object.keys(sections);
    ...
    keys.forEach(key => {
        ...
        let normalized = key.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
        // Match MAJORDUTY1, MAJORDUTIES1, etc.
        const match = normalized.match(/^MAJORDUT(Y|IES)(\d+)$/);
        if (match) {
            const number = match[2];
            const groupKey = `MAJORDUTY${number}`;
            if (!duplicateGroups[groupKey]) duplicateGroups[groupKey] = [];
            duplicateGroups[groupKey].push({
                originalKey: key,
                normalized: normalized,
                hasSpaces: (key.match(/\s/g) || []).length,
                length: key.length,
                isDuty: key.toUpperCase().includes('DUTY '),
                isDuties: key.toUpperCase().includes('DUTIES')
            });
        }
    });

    // For each group, keep the best key and mark others for deletion
    Object.entries(duplicateGroups).forEach(([groupKey, variants]) => {
        if (variants.length > 1) {
            // Prefer more spaces, then "DUTY", then shorter length
            variants.sort((a, b) => {
                if (a.hasSpaces !== b.hasSpaces) return b.hasSpaces - a.hasSpaces;
                if (a.isDuty && b.isDuties) return -1;
                if (b.isDuty && a.isDuties) return 1;
                return a.length - b.length;
            });
            const keepKey = variants[0].originalKey;
            const deleteKeys = variants.slice(1).map(v => v.originalKey);
            toDelete.push(...deleteKeys);
        }
    });
