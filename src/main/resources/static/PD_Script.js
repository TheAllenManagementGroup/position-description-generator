...
window.formatGeneratedPD = function(pdText) {
    ...
    sectionHeaders.forEach(section => {
        ...
        // Handle sections with existing bold formatting
    ...
    // Fix Factor headers - this is the key improvement
    ...
    // Major duties formatting - fix the missing line break after duty 1
    text = text.replace(/(\d+)\.\s*\*\*([^*]+?)\*\*:\s*([^0-9]+?)(\d+\.)/g, '$1. **$2:**\n$3\n\n$4.');
    
    // General major duties pattern
    ...
    // Handle evaluation summary without existing bold formatting
    ...
    // Clean up specific spacing and formatting issues
    text = text
        ...
        .replace(/\n{3,}/g, '\n\n')
        // Remove stray asterisks that appear alone on lines
        .replace(/^\*\*\s*$/gm, '')
        // Fix cases where we have **SECTION:**** (double closing)
        .replace(/(\*\*[^*]+:\*\*)\*\*/g, '$1')
        // Clean up any remaining double asterisks at line starts
        .replace(/^\*\*(\s*\*\*)/gm, '')
