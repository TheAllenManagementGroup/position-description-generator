...
function getAdjacentGSGrades(recommendedGradeNum) {
    ...
    gradeData = gradeData.filter(item => validGrades.includes(parseInt(item.grade.replace(/[^0-9]/g, ''), 10)));
...
function renderUnknownSeriesStep(content) {
    ...
    <div class="card">
        ...
        <div class="form-group">
            <label class="form-label" for="supervisoryLevelSelect">Supervisory Level</label>
            <div class="dropdown-wrapper">
...
function renderGeneratedPDStep(content) {
    ...
    // Get the generated PD text and display it, formatted
    const generatedText = window.generatedPD || '';
    const finalPDElement = document.getElementById('finalPD');
    
    if (generatedText) {
        // Use the formatter for better readability
        finalPDElement.textContent = window.formatGeneratedPD(generatedText);
    } else {
...
function formatGeneratedPD(pdText) {
    ...
    // Fix concatenated sections by adding line breaks before major headers
    const majorSections = [
        'HEADER:', 'INTRODUCTION:', 'MAJOR DUTIES:', 
        'FACTOR EVALUATION', 'CONDITIONS OF EMPLOYMENT:', 
        'TITLE AND SERIES', 'FAIR LABOR STANDARDS'
    ];
    
    majorSections.forEach(section => {
        const regex = new RegExp(`([a-z])\\s*(${section.replace(':', '\\s*:?').replace(/\s/g, '\\s+')})`, 'gi');
        text = text.replace(regex, '$1\n\n**$2**');
    });

    // Fix Factor headers with proper spacing and formatting
    // Handle both "Factor1" and "Factor 1" patterns
    text = text.replace(/Factor\s*(\d+)\s*[-–]\s*([^L]*?)Level\s*(\d+-\d+)\s*,?\s*(\d+)\s*Points/gi, 
        '\n\n**Factor $1 – $2Level $3, $4 Points**');

    // Fix other section headers
    text = text.replace(
        /^(HEADER|INTRODUCTION|MAJOR DUTIES|FACTOR EVALUATION[^:]*|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES[^:]*|FAIR LABOR STANDARDS[^:]*)\s*:?/gmi,
        '\n\n**$1:**'
    );

    // Fix evaluation summary formatting
    text = text.replace(/\*\*EVALUATION SUMMARY:\*\*/gi, '\n\n**EVALUATION SUMMARY:**');
    text = text.replace(/\*\*Total Points:\s*(\d+)\*\*/gi, '\n**Total Points: $1**');
    text = text.replace(/\*\*Final Grade:\s*(GS-\d+)\*\*/gi, '\n**Final Grade: $1**');
    text = text.replace(/\*\*Grade Range:\s*([\d-]+)\*\*/gi, '\n**Grade Range: $1**');

    // Fix major duties numbering and spacing
    text = text.replace(/(\d+)\.([A-Z][^:]+?):\s*/g, '\n\n$1. **$2**: ');

    // Clean up spacing around percentages
    text = text.replace(/\(\s*(\d+%)\s*\)/g, '($1)');

    // Fix department/agency formatting in header
    text = text.replace(/U\.S\.\s*Department\s*of\s*([^A-Z]+)([A-Z])/gi, 'U.S. Department of $1\n$2');
    ...
    // Ensure headers start on new lines with proper spacing
    text = text.replace(/([a-z])\*\*([A-Z])/g, '$1\n\n**$2');

    // Fix any remaining concatenated text patterns
    text = text.replace(/([a-z])([A-Z][A-Z\s]{5,}:)/g, '$1\n\n**$2**');
...
// Alternative comprehensive formatter that handles the specific issues in your document
function formatGeneratedPDComprehensive(pdText) {
    if (!pdText) return '';

    let text = pdText;

    // Step 1: Add line breaks before major sections that are concatenated
    const sections = [
    ...
    sections.forEach(section => {
        // Find concatenated sections (lowercase letter followed by section)
        const pattern = new RegExp(`([a-z])\\s*(${section.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
        text = text.replace(pattern, '$1\n\n**$2**');
    });

    // Step 2: Fix Factor headers specifically
    text = text.replace(/Factor\s*(\d+)\s*-\s*([^L]+?)Level\s*(\d+)-(\d+)\s*,?\s*(\d+)\s*Points/gi,
        '\n\n**Factor $1 – $2Level $3-$4, $5 Points**');

    // Step 3: Fix evaluation summary
    text = text.replace(/\*\*EVALUATION SUMMARY:\*\*\s*\*\*Total Points:\s*(\d+)\*\*\s*\*\*Final Grade:\s*(GS-\d+)\*\*\s*\*\*Grade Range:\s*([\d-]+)\*\*/gi,
        '\n\n**EVALUATION SUMMARY:**\n**Total Points: $1**\n**Final Grade: $2**\n**Grade Range: $3**');

    // Step 4: Fix numbered duties
    text = text.replace(/(\d+)\.\s*([^:]+?):\s*/g, '\n\n$1. **$2**: ');

    // Step 5: Clean up spacing and line breaks
    text = text.replace(/\n{3,}/g, '\n\n')
            .replace(/[ \t]{2,}/g, ' ')
            .trim();
...
window.formatGeneratedPD = function(pdText) {
    if (!pdText) return '';
    let text = pdText.replace(/\r\n/g, '\n').replace(/\n{3,}/g, '\n\n');
    // Ensure headers and sections are bold and spaced
    text = text.replace(/(\*\*HEADER:\*\*)/g, '\n\n$1\n');
    text = text.replace(/(\*\*INTRODUCTION:\*\*)/g, '\n\n$1\n');
    text = text.replace(/(\*\*MAJOR DUTIES:\*\*)/g, '\n\n$1\n');
    text = text.replace(/(\*\*FACTOR EVALUATION[^\n]*\*\*)/g, '\n\n$1\n');
    text = text.replace(/(\*\*CONDITIONS OF EMPLOYMENT:\*\*)/g, '\n\n$1\n');
    text = text.replace(/(\*\*TITLE AND SERIES DETERMINATION:\*\*)/g, '\n\n$1\n');
    text = text.replace(/(\*\*FAIR LABOR STANDARDS ACT DETERMINATION:\*\*)/g, '\n\n$1\n');
    // Fix factor headers
    text = text.replace(/(\*\*Factor\d+\s*-[^\*]+?\*\*)/g, '\n\n$1\n');
    // Remove excessive spaces
    text = text.replace(/ +\n/g, '\n').replace(/\n +/g, '\n');
    // Ensure a blank line between HEADER and INTRODUCTION
