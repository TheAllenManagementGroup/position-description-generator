...
function splitPDSections(text) {
    ...
    // Ensure Fair Labor Standards Act Determination is on its own line
    text = text.replace(/(Fair Labor Standards Act Determination)/g, '\n$1');
    
    // Also handle Factor headers without Level/Points (markdown format like **FACTOR 1 – KNOWLEDGE REQUIRED BY THE POSITION**)
    text = text.replace(/(\*\*FACTOR\s*\d+\s*[–-]\s*[^\n*]+\*\*)/gi, '\n$1');

    // Clean up multiple consecutive newlines
    text = text.replace(/\n{3,}/g, '\n\n');

    // Enhanced section header regex that captures both formats
    const sectionHeaderRegex = /(?:^\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*$|^\*\*(FACTOR\s*\d+\s*[–-]\s*[^*\n]+)\*\*$|^(Introduction)$|^(Major Duties)$|^(Conditions of Employment)$|^(Title and Series Determination)$|^(Fair Labor Standards Act Determination)$|^(Factor\s*\d+\s*[–-]\s*[^\n]+?Level\s*\d+-\d+,\s*\d+\s*Points?)$|^(Total Points:\s*\d+)$|^(Final Grade:\s*GS-?\d+)$|^(Grade Range:\s*\d+-\d+)$)/gmi;

    // Find all section headers
    let matches = [];
    let match;
    while ((match = sectionHeaderRegex.exec(text)) !== null) {
        // Get the actual header text from whichever capture group matched
        let header = match[1] || match[2] || match[3] || match[4] || match[5] || match[6] || match[7] || match[8] || match[9] || match[10] || match[11];
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        // Special handling for summary sections - store the value, not empty
        if (/^Total Points:/i.test(header)) {
            const pointsMatch = header.match(/Total Points:\s*(\d+)/i);
            sections[header] = pointsMatch ? pointsMatch[1] : '';
        } else if (/^Final Grade:/i.test(header)) {
            const gradeMatch = header.match(/Final Grade:\s*(GS-?\d+)/i);
            sections[header] = gradeMatch ? gradeMatch[1] : '';
        } else if (/^Grade Range:/i.test(header)) {
            const rangeMatch = header.match(/Grade Range:\s*([\d\-]+)/i);
            sections[header] = rangeMatch ? rangeMatch[1] : '';
        } else if (!sections[header] && cleanContent) {
            // For factor sections, clean up if header/content are run together
            if (/^Factor\s*\d+/i.test(header) && cleanContent) {
                const match = cleanContent.match(/^(.+?Points\.?)\s+([\s\S]*)$/s);
    ...
    if (!Object.keys(sections).some(k => /^Total Points:/i.test(k))) {
        ...
        if (totalPointsMatch) sections[`Total Points: ${totalPointsMatch[1]}`] = totalPointsMatch[1];
    }
    if (!Object.keys(sections).some(k => /^Final Grade:/i.test(k))) {
        const finalGradeMatch = text.match(/Final Grade[:\s*]+(GS-?\d+)/i);
        if (finalGradeMatch) sections[`Final Grade: ${finalGradeMatch[1]}`] = finalGradeMatch[1];
    }
    if (!Object.keys(sections).some(k => /^Grade Range:/i.test(k))) {
        const gradeRangeMatch = text.match(/Grade Range[:\s*]+([\d\-]+)/i);
        if (gradeRangeMatch) sections[`Grade Range: ${gradeRangeMatch[1]}`] = gradeRangeMatch[1];
    }

    // If no HEADER section, extract from the first lines before the first recognized section
    if (!sections['HEADER']) {
        const firstSectionIndex = matches.length > 0 ? matches[0].index : text.length;
        const headerText = text.substring(0, firstSectionIndex).trim();
        if (headerText) {
