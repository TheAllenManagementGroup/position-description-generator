...
formState.usedAIRecommendation = false;
...
function showFactorUpdateSuccess() {
    // You can customize this to show a toast, modal, or just log to console
...
function renderAgencySetupStep(content) {
    const agencyData = [
        {
            ...
            subOrgs: ["FBI", "DEA", "ATF", "US Marshals"]
        },
        {
            name: "Department of Homeland Security",
            subOrgs: ["FEMA", "TSA", "USCIS", "CBP"]
        },
        {
            name: "Department of Defense",
            subOrgs: ["Army", "Navy", "Air Force", "Marines"]
        }
    ];
    ...
    content.innerHTML = `
        <div class="card">
            <div class="card-header">
                <div class="card-title"><i class="fas fa-building"></i> Agency Setup
                    <span class="step-info">
                        <i class="fas fa-info-circle" style="color:#2563eb; margin-left:6px; cursor:help;"></i>
                        <div class="step-tooltip">
                            Select your agency and organization.
                        </div>
                    </span>
                </div>
                <div class="card-description">Select your agency and organization</div>
            </div>
            <div class="card-content">
                <div class="form-group">
                    <label class="form-label" for="lowestOrgInput">Lowest Level Organization/Office</label>
                    <input class="form-input" id="lowestOrgInput" placeholder="Enter lowest org..." value="${formState.lowestOrg || ''}">
                </div>
                <div class="form-group">
                    <label class="form-label" for="federalAgencyDropdown">Federal Agency</label>
                    <div class="dropdown-wrapper">
                        <input class="form-input" id="federalAgencyDropdown" autocomplete="off" placeholder="Search or select agency..." value="${formState.federalAgency || ''}">
                        <div class="dropdown-list" id="federalAgencyDropdownList" style="display:none;"></div>
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="subOrgDropdown">Sub Organization</label>
                    <div class="dropdown-wrapper">
                        <input class="form-input" id="subOrgDropdown" autocomplete="off" placeholder="Search or select sub-org..." value="${formState.subOrg || ''}">
                        <div class="dropdown-list" id="subOrgDropdownList" style="display:none;"></div>
                    </div>
                </div>
                <button class="btn btn-primary" id="continueAgencyBtn">Next: Job Classification</button>
            </div>
        </div>
    `;
...
function renderUnknownSeriesStep(content) {
    ...
    <div class="card">
        ...
        <div class="card-content">
            ...
            <button class="btn btn-outline" onclick="addDuty()">Add Duty</button>
            ...
            <div class="card-content">
                <div class="review-summary" style="margin-bottom:2rem;">
                    ...
                    <ul style="list-style:none; padding-left:0;">
                        ...
                        <li><strong>Position Title(s):</strong> ${
...
function renderGeneratedPDStep(content) {
    content.innerHTML = `
        <div class="card">
            ...
            <div class="card-content">
                <div class="pd-display" id="finalPD" style="white-space: pre-line; max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; padding: 1rem; border-radius: 0.5rem; background-color: #f9fafb;"></div>
    ...
    // Get the generated PD text and display it
    const generatedText = window.generatedPD || '';
    const finalPDElement = document.getElementById('finalPD');
    
    if (generatedText) {
        // Use textContent to safely display the text while preserving formatting with CSS
        finalPDElement.textContent = generatedText;
    } else {
        // Show a message if no PD was generated
...
function setupAgencyDropdowns(agencyData) {
    ...
    // Setup agency dropdown
    const agencyInput = document.getElementById('federalAgencyDropdown');
    const agencyList = document.getElementById('federalAgencyDropdownList');
    
    function showAgencyDropdown() {
        const term = agencyInput.value.trim().toLowerCase();
        const filtered = agencyData.filter(a => a.name.toLowerCase().includes(term));
        agencyList.innerHTML = filtered.length
    ...
    agencyInput.addEventListener('focus', showAgencyDropdown);
    agencyInput.addEventListener('input', showAgencyDropdown);
    ...
    agencyList.addEventListener('mousedown', function(e) {
        if (e.target.classList.contains('dropdown-item') && !e.target.classList.contains('disabled')) {
            ...
            updateSubOrgDropdown();
            checkAgencyComplete();
        }
    });

    // Setup sub-org dropdown
    const subOrgInput = document.getElementById('subOrgDropdown');
    const subOrgList = document.getElementById('subOrgDropdownList');

    function updateSubOrgDropdown() {
        const agencyName = agencyInput.value.trim();
        const agency = agencyData.find(a => a.name === agencyName);
        if (!agency) {
            subOrgList.innerHTML = `<div class="dropdown-item disabled">Select agency first</div>`;
            return;
        }
        const term = subOrgInput.value.trim().toLowerCase();
        const filtered = agency.subOrgs.filter(s => s.toLowerCase().includes(term));
    ...
    subOrgInput.addEventListener('focus', updateSubOrgDropdown);
    subOrgInput.addEventListener('input', updateSubOrgDropdown);
    ...
    // Lowest org input handling
    const lowestOrgInput = document.getElementById('lowestOrgInput');
    lowestOrgInput.addEventListener('input', function() {
        formState.lowestOrg = this.value.trim();
        checkAgencyComplete();
    });

    function checkAgencyComplete() {
        const lowestOrg = document.getElementById('lowestOrgInput').value.trim();
        const federalAgency = document.getElementById('federalAgencyDropdown').value.trim();
        const subOrg = document.getElementById('subOrgDropdown').value.trim();
        
        if (lowestOrg && federalAgency && subOrg) {
            continueBtn.disabled = false;
            continueBtn.style.opacity = '1';
            continueBtn.style.cursor = 'pointer';
        } else {
    ...
    continueBtn.onclick = function() {
        if (!continueBtn.disabled) renderStep(3);
    };
}
            ...
            html += `
                <button class="recommendation-btn"
                    style="
                        ...
                        overflow: hidden;
                    "
                    onmouseover="this.style.borderColor='#2563eb';this.style.boxShadow='0 2px 8px 0 rgba(37,99,235,0.08)'"
                    ...
                    <span class="badge" style="margin-right:0.5em; background:#2563eb; color:#fff; font-weight:600; font-size:1em; padding:0.5em 1em;">${rec.code}</span>
                    <span style="font-weight:600; color:#1e293b;">${rec.title}</span>
                    <span style="margin-left:auto; color:#2563eb; font-weight:500;">
                        ${confidence}% match
                    </span>
...
function renderGradeAnalysisResultStyled(data) {
    ...
    regenerateBtn.onclick = async function() {
        ...
        const payload = { duties: allDuties };
        const gradeAnalysisDiv = document.getElementById('gradeAnalysisResult');
    ...
    if (value) {
        ...
        chunk.split('\n').forEach(line => {
            if (line.trim().startsWith('data: ')) {
                try {
                    const obj = JSON.parse(line.trim().substring(6));
                    if (obj.response) {
                        fullText += obj.response;
                        // Show the PD as it is being generated
                        aiResult.innerHTML = `<pre style="white-space: pre-wrap; word-wrap: break-word;">${fullText}</pre>`;
                    } else if (obj.error) {
                        aiResult.innerHTML = `<div class="alert alert-warning">Error: ${obj.error}</div>`;
                    }
                } catch (e) {
...
function formatGeneratedPD(pdText) {
    if (!pdText) return '';

    // Normalize line breaks and whitespace
    let text = pdText.replace(/\r\n/g, '\n').replace(/\n{3,}/g, '\n\n');

    // Bold likely section headers (e.g., HEADER, INTRODUCTION, MAJOR DUTIES, Factor 1, etc.)
    text = text.replace(
        /^(HEADER|INTRODUCTION|MAJOR DUTIES|FACTOR EVALUATION - COMPLETE ANALYSIS|Factor \d+|Total Points|Final Grade|Grade Range|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES DETERMINATION|FAIR LABOR STANDARDS ACT DETERMINATION)\s*:?/gim,
        '**$1**'
    );

    // Add extra spacing after section headers
    text = text.replace(/\*\*[A-Z][A-Z\s\d\-]+?\*\*/g, match => `${match}\n`);
...
function formatUpdatedPD(sectionsObject) {
    console.log('formatUpdatedPD called with sections:', Object.keys(sectionsObject || {}));
    
    if (!sectionsObject) return '';
    
    // First, clean up major duty duplicates more aggressively
    const workingSections = { ...sectionsObject };
    
    // Find all major duty sections and deduplicate
    const majorDutyRegex = /^major\s*dut(?:y|ies)\s*(\d+)/i;
    const majorDutySections = {};
    const keysToDelete = [];
    
    Object.keys(workingSections).forEach(key => {
        const match = key.match(majorDutyRegex);
        if (match) {
            const dutyNumber = match[1];
            if (!majorDutySections[dutyNumber]) {
                majorDutySections[dutyNumber] = key;
            } else {
                // This is a duplicate - merge content if needed and mark for deletion
                const existingKey = majorDutySections[dutyNumber];
                const existingContent = workingSections[existingKey] || '';
                const duplicateContent = workingSections[key] || '';
                
                if (duplicateContent.trim() && duplicateContent.trim() !== existingContent.trim()) {
                    workingSections[existingKey] = (existingContent + '\n\n' + duplicateContent).trim();
                }
    ...
    keysToDelete.forEach(key => {
        console.log('Removing duplicate major duty section:', key);
        delete workingSections[key];
    });
    ...
    ['HEADER', 'INTRODUCTION'].forEach(sectionKey => {
        const key = findSectionKeyFlexible(workingSections, sectionKey);
        if (key && workingSections[key] && workingSections[key].trim()) {
            sectionOrder.push(key);
        }
    });

    const majorDutiesKey = findSectionKeyFlexible(workingSections, 'MAJOR DUTIES');
    if (majorDutiesKey && workingSections[majorDutiesKey] && workingSections[majorDutiesKey].trim()) {
        sectionOrder.push(majorDutiesKey);
    }

    // Add individual major duty sections in numerical order
    Object.keys(majorDutySections)
        .sort((a, b) => parseInt(a) - parseInt(b))
        .forEach(num => {
            const key = majorDutySections[num];
            if (workingSections[key] && workingSections[key].trim()) {
                sectionOrder.push(key);
            }
        });

    const factorEvalKey = findSectionKeyFlexible(workingSections, 'FACTOR EVALUATION - COMPLETE ANALYSIS');
    if (factorEvalKey && workingSections[factorEvalKey] && workingSections[factorEvalKey].trim()) {
        sectionOrder.push(factorEvalKey);
    }

    // Add factor sections in numerical order
    const factorKeys = Object.keys(workingSections)
    ...
    // Add summary sections - check all possible patterns and exact matches
    const summaryChecks = [
        ['Total Points', /^Total\s*Points/i],
        ['Final Grade', /^Final\s*Grade/i],
        ['Grade Range', /^Grade\s*Range/i]
    ];
    
    summaryChecks.forEach(([fallback, pattern]) => {
        // Try pattern match first
        let summaryKey = Object.keys(workingSections).find(k => pattern.test(k));
        
        // If not found, try exact fallback
        if (!summaryKey && workingSections[fallback]) {
            summaryKey = fallback;
        }
        
        if (summaryKey && workingSections[summaryKey] && workingSections[summaryKey].trim()) {
            console.log('Adding summary section:', summaryKey, 'Content:', workingSections[summaryKey].substring(0, 50));
            sectionOrder.push(summaryKey);
        } else {
            console.log('Summary section not found or empty for pattern:', pattern, 'fallback:', fallback);
        }
    });
    ...
    const alreadyIncluded = new Set(sectionOrder);
    Object.keys(workingSections).forEach(key => {
        if (!alreadyIncluded.has(key) && workingSections[key] && workingSections[key].trim()) {
            console.log('Adding remaining section:', key);
            sectionOrder.push(key);
        }
    });
    ...
    sectionOrder.forEach(section => {
        const content = workingSections[section];
        if (content && content.trim()) {
            formatted += `**${section}**\n${content.trim()}\n\n`;
        }
    });

    console.log('Final section order:', sectionOrder);
...
function saveSectionEdit(title) {
    console.log('Save called for:', title);
    
    if (!title) {
        console.error('No title provided to saveSectionEdit');
        return;
    }
    
    const textarea = findTextareaElement(title);
    if (!textarea) {
        console.error('No textarea found for title:', title);
        return;
    }
    
    const currentKey = getCurrentSectionKey(title);
    const display = findDisplayElement(title);
    
    let newContent = textarea.value.trim();
    newContent = cleanContent(newContent);

    // Detect if the section title has changed
    let newTitle = currentKey;
    const sectionDiv = display ? display.closest('.editor-section') : null;
    if (sectionDiv) {
        const headerLabel = sectionDiv.querySelector('.section-title-label');
        if (headerLabel && headerLabel.textContent !== currentKey) {
            newTitle = headerLabel.textContent;
            // Move stacks/history to new key if title changed
            if (currentKey !== newTitle) {
                window.sectionEditStacks[newTitle] = window.sectionEditStacks[currentKey] || [newContent];
                window.sectionEditHistory[newTitle] = window.sectionEditHistory[currentKey] || [];
                delete window.sectionEditStacks[currentKey];
                delete window.sectionEditHistory[currentKey];
            }
        }
    }

    // Track history with timestamp
    if (!window.sectionEditHistory[newTitle]) {
        window.sectionEditHistory[newTitle] = [];
    }
    window.sectionEditHistory[newTitle].push({ 
        content: newContent, 
        header: newTitle, 
        ts: Date.now() 
    });
    ...
    // Update display
    if (display) {
        const content = newContent || '<span style="color: #9ca3af; font-style: italic;">Click to add content...</span>';
        display.innerHTML = content;
        display.style.whiteSpace = 'pre-line';
    }

    // Toggle edit mode
    if (typeof toggleSectionEdit === 'function') {
        toggleSectionEdit(newTitle);
    }
    
    if (sectionDiv) {
        sectionDiv.classList.remove('modified');
    }
    ...
    if (/^Factor\s*\d+/i.test(newTitle) && typeof updateFactorSectionsAfterEdit === "function") {
        updateFactorSectionsAfterEdit(newTitle, newContent, sectionDiv);
    }
    
    console.log('Saved section:', newTitle, 'Content length:', newContent.length);
}
...
function undoSectionEdit(title) {
    console.log('Undo called for:', title);
    ...
    const possibleIds = [
        `textarea-${title.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()}`,
        `textarea-${title.replace(/\s+/g, '').toLowerCase()}`,
        `textarea-${title.replace(/\W/g, '')}`,
        `textarea-${title}`
    ];
    ...
    if (!textarea) {
        console.error('Textarea not found. Tried IDs:', possibleIds);
        // Try to find textarea by looking in parent elements
        const buttons = document.querySelectorAll(`button[onclick*="${title}"]`);
        for (const button of buttons) {
            const section = button.closest('.editor-section') || button.closest('.section') || button.closest('div');
            if (section) {
                textarea = section.querySelector('textarea');
                if (textarea) {
                    console.log('Found textarea by traversing DOM from button');
                    break;
                }
            }
        }
        ...
        if (!textarea) {
            console.error('Still no textarea found. Cannot proceed.');
            return;
        }
    } else {
        console.log('Found textarea with ID:', foundId);
    }
    ...
    if (!window.sectionEditStacks[currentKey]) {
        const currentValue = textarea.value || '';
        window.sectionEditStacks[currentKey] = [currentValue];
    }
    ...
    const displayId = foundId ? foundId.replace('textarea-', 'display-') : `display-${title.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()}`;
    const display = document.getElementById(displayId);
    if (display) {
        const content = textarea.value || '<span style="color: #9ca3af; font-style: italic;">Click to add content...</span>';
        display.innerHTML = content;
    }
...
function resetSectionEdit(title) {
    console.log('Reset called for:', title);
    ...
    const currentKey = title;
    const originalValue = (typeof pdSections !== 'undefined' && pdSections[currentKey]) || '';
    
    window.sectionEditStacks[currentKey] = [originalValue];
    ...
    const display = document.getElementById(displayId);
    if (display) {
        const content = originalValue || '<span style="color: #9ca3af; font-style: italic;">Click to add content...</span>';
        display.innerHTML = content;
    }
...
function cancelSectionEdit(title) {
    console.log('Cancel called for:', title);
    ...
    // Call toggleSectionEdit if it exists
    if (typeof toggleSectionEdit === 'function') {
        try {
            toggleSectionEdit(currentKey);
        } catch (e) {
            console.warn('toggleSectionEdit failed:', e);
        }
    }
...
window.formatGeneratedPD = function(pdText) {
    return pdText; // No splitting or formatting, just return the raw text
};

// For updated PDs, keep splitting and formatting as before
function formatUpdatedPD(sectionsObject) {
    console.log('formatUpdatedPD called with:', Object.keys(sectionsObject || {}));
    
    if (!sectionsObject || typeof sectionsObject !== 'object') {
        console.error('Invalid sectionsObject provided');
        return '';
    }
    ...
    // AGGRESSIVE major duty deduplication
    const majorDutyPattern = /^major\s*dut(?:y|ies)\s*(\d+)/i;
    const majorDutyGroups = {};
    const duplicateKeys = [];
    
    // Group all major duty sections by number
    Object.keys(workingSections).forEach(key => {
        const match = key.match(majorDutyPattern);
        if (match) {
            const dutyNum = match[1];
            if (!majorDutyGroups[dutyNum]) {
                majorDutyGroups[dutyNum] = [];
            }
            majorDutyGroups[dutyNum].push(key);
        }
    });
    
    // For each group, keep only the first one and merge content if needed
    Object.entries(majorDutyGroups).forEach(([dutyNum, keys]) => {
        if (keys.length > 1) {
            console.log(`Found ${keys.length} versions of Major Duty ${dutyNum}:`, keys);
            
            const keepKey = keys[0];
            const duplicates = keys.slice(1);
            
            // Merge content from all duplicates
            let mergedContent = workingSections[keepKey] || '';
            duplicates.forEach(dupKey => {
                const dupContent = workingSections[dupKey] || '';
                if (dupContent.trim() && dupContent.trim() !== mergedContent.trim()) {
                    mergedContent = (mergedContent + '\n\n' + dupContent).trim();
                }
                duplicateKeys.push(dupKey);
            });
            
            workingSections[keepKey] = mergedContent;
            console.log(`Keeping "${keepKey}", removing:`, duplicates);
        }
    });
    
    // Remove duplicates
    duplicateKeys.forEach(key => delete workingSections[key]);
    
    // Build ordered sections
    const orderedSections = [];
    
    // Standard header sections
    ['HEADER', 'INTRODUCTION'].forEach(section => {
        const key = Object.keys(workingSections).find(k => 
            k.toUpperCase() === section || 
            k.toUpperCase().includes(section)
        );
        if (key && workingSections[key]?.trim()) {
            orderedSections.push(key);
        }
    });
    
    // Major duties overview
    const majorDutiesKey = Object.keys(workingSections).find(k => 
        /^major\s*duties$/i.test(k) && !/\d/.test(k)
    );
    if (majorDutiesKey && workingSections[majorDutiesKey]?.trim()) {
        orderedSections.push(majorDutiesKey);
    }
    
    // Individual major duties in order
    Object.keys(majorDutyGroups)
        .sort((a, b) => parseInt(a) - parseInt(b))
        .forEach(dutyNum => {
            const key = majorDutyGroups[dutyNum][0]; // We already kept only the first
            if (workingSections[key]?.trim()) {
                orderedSections.push(key);
            }
        });
    
    // Factor evaluation header
    const factorEvalKey = Object.keys(workingSections).find(k => 
        /factor\s*evaluation/i.test(k)
    );
    if (factorEvalKey && workingSections[factorEvalKey]?.trim()) {
        orderedSections.push(factorEvalKey);
    }
    
    // Factor sections in numerical order
    const factorKeys = Object.keys(workingSections)
        .filter(k => /^Factor\s*\d+/i.test(k))
        .sort((a, b) => {
    ...
    // CRITICAL: Summary sections with flexible matching
    const summaryPatterns = [
        { regex: /^Total\s*Points/i, name: 'Total Points' },
        { regex: /^Final\s*Grade/i, name: 'Final Grade' },
        { regex: /^Grade\s*Range/i, name: 'Grade Range' }
    ];
    
    summaryPatterns.forEach(({ regex, name }) => {
        // Try regex match first
        let summaryKey = Object.keys(workingSections).find(k => regex.test(k));
        
        // Try exact name match
        if (!summaryKey && workingSections[name]) {
            summaryKey = name;
        }
        
        if (summaryKey && workingSections[summaryKey]?.trim()) {
            console.log(`Adding summary section: ${summaryKey}`);
            orderedSections.push(summaryKey);
        } else {
            console.log(`Summary section "${name}" not found or empty`);
        }
    });
    ...
    ['CONDITIONS OF EMPLOYMENT', 'TITLE AND SERIES DETERMINATION', 'FAIR LABOR STANDARDS ACT DETERMINATION']
        .forEach(section => {
            const key = Object.keys(workingSections).find(k => 
                k.toUpperCase() === section || 
                k.toUpperCase().includes(section.replace(/\s+/g, ''))
            );
            if (key && workingSections[key]?.trim()) {
                orderedSections.push(key);
            }
    ...
    const usedKeys = new Set(orderedSections);
    Object.keys(workingSections).forEach(key => {
        if (!usedKeys.has(key) && workingSections[key]?.trim()) {
            console.log(`Adding remaining section: ${key}`);
            orderedSections.push(key);
        }
    });
    ...
    orderedSections.forEach(key => {
        const content = workingSections[key];
        if (content?.trim()) {
            result += `**${key}**\n${content.trim()}\n\n`;
        }
    });
    
    console.log('Final section order:', orderedSections);
    return result.trim();
}
...
function getGroupSeriesOptionsFromAPI() {
    // Map group codes to their series codes
    const groupMap = {};
    Object.entries(jobSeriesData).forEach(([seriesCode, series]) => {
        let groupCode = series.groupCode;
        if (!groupCode) {
            groupCode = seriesCode.substring(0, 2) + "00";
        }
    ...
    // Return sorted array of group series options (only code)
    return Object.values(groupMap).sort((a, b) => a.code.localeCompare(b.code));
}
