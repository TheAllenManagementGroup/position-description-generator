...
function renderSidebar(currentStep) {
    ...
    steps.forEach((step, idx) => {
        ...
        let iconHtml = '';
        if (isCompleted) {
            // Completed: white check in dark teal circle
            iconHtml = `<div class="step-icon completed" style="background:${darkTeal};color:#fff;">
                <i class="fas fa-check"></i>
            </div>`;
        } else if (isCurrent) {
            // Current: step number in green, white circle
            iconHtml = `<div class="step-icon current" style="background:#fff;color:${green};border:2px solid ${green};font-weight:700;">
                ${stepNum}
            </div>`;
        } else {
            // Upcoming: step number in gray, white circle
            iconHtml = `<div class="step-icon upcoming" style="background:#fff;color:#9ca3af;border:2px solid #d1d5db;font-weight:700;">
                ${stepNum}
            </div>`;
        }
        ...
        if (isCompleted || isCurrent) {
            highlightStyle = `background:${darkTeal}1a !important;border-color:${darkTeal} !important;`;
        }

        container.innerHTML += `
            <div class="${stepClass}" style="${highlightStyle}">
                ${iconHtml}
                <div class="step-content">
                    <h4 style="color:${isCurrent ? green : isCompleted ? darkTeal : '#6b7280'};">${step.name}</h4>
...
// Add this helper to go to the previous step
function goToPreviousStep(currentStep) {
    let prevStep = typeof currentStep === 'number' ? currentStep - 1 : null;
    // If using named steps, handle special cases
    if (currentStep === 'unknownSeries') prevStep = 2;
    if (prevStep && prevStep >= 1) {
        renderStep(prevStep);
    }
}

// Add Previous button to every step rendering function
function renderStep(step) {
    renderSidebar(step);
    const content = document.getElementById('stepContent');
    if (!content) return;

    // Helper to render Previous button
    function renderPreviousButton() {
        return `<button class="btn btn-outline" id="previousStepBtn" style="margin-bottom:1rem;">
    ...
    switch (step) {
        case 1:
            content.innerHTML = `
                ...
                <!-- rest of step 1 UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep(1);
        ...
        case 2:
            content.innerHTML = `
                ...
                <!-- rest of step 2 UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep(2);
        ...
        case 3:
            content.innerHTML = `
                ...
                <!-- rest of step 3 UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep(3);
            renderJobClassificationStep(content);
            break;
        case 'unknownSeries':
            content.innerHTML = `
                ${renderPreviousButton()}
                <!-- rest of unknownSeries UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep('unknownSeries');
        ...
        case 4:
            content.innerHTML = `
                ...
                <!-- rest of step 4 UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep(4);
        ...
        case 5:
            content.innerHTML = `
                ...
                <!-- rest of step 5 UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep(5);
        ...
        case 6:
            content.innerHTML = `
                ...
                <!-- rest of step 6 UI -->
            `;
            document.getElementById('previousStepBtn').onclick = () => goToPreviousStep(6);
...
function renderJobClassificationStep(content) {
    ...
    // Check if selected job series is 301
    const is301Series = formState.jobSeries === '301';
        ...
        <div class="card">
            ...
            <div class="card-content">
                ...
                <div class="form-group">
                    ...
                    <div class="dropdown-wrapper">
                        <input class="form-input" id="positionTitleInput" autocomplete="off" placeholder="Type position title..." value="${formState.positionTitle || ''}" ${!formState.jobSeries ? 'disabled' : ''}>
                        <div class="dropdown-list" id="positionTitleDropdownList" style="display:none;${is301Series ? 'display:none;' : ''}"></div>
                    </div>
                    <div id="positionTitleList" style="margin-top:0.5rem;"></div>
                </div>
                <div class="form-group">
                    <label class="form-label" for="orgTitleInput">Organizational Title <span style="color:#6b7280;">(optional)</span></label>
                    <input class="form-input" id="orgTitleInput" autocomplete="off" placeholder="Type organizational title..." value="${formState.orgTitle || ''}" ${!formState.jobSeries ? 'disabled' : ''}>
                </div>
    ...
    // For 301 series, do NOT show position title dropdown, only allow typing
    if (!is301Series) {
        setupPositionTitleDropdown();
    } else {
        // For 301, always allow typing and update formState on input
        document.getElementById('positionTitleInput').oninput = function() {
            formState.positionTitle = this.value;
        };
    }

    // Organizational title input always updates formState
    document.getElementById('orgTitleInput').oninput = function() {
        formState.orgTitle = this.value;
    };
...
function renderReviewGenerateStep(content) {
    ...
    if (formState.jobSeries) {
        ...
        if (jobSeriesData[code]) {
            ...
            title = formState.jobSeries.split(' - ')[1];
        }
        if (title) {
    ...
    // Only show organization title if it exists and is not empty
    const orgTitleRow = formState.orgTitle && formState.orgTitle.trim()
        ? `<li><strong>Organizational Title:</strong> ${formState.orgTitle}</li>`
        : '';
        ...
        <div class="card" style="max-width: 72rem; margin: 0 auto;">
            <div class="card-header">
                <div class="card-title" style="display: flex; align-items: center; gap: 8px;">
                    <span class="step-icon" style="background: #0f766e; color: #fff; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                        <i class="fas fa-building"></i>
                    </span>
                    <span>Agency & Organization Selection</span>
                    <span class="step-info" style="position:relative;">
                        <i class="fas fa-info-circle" style="color:#2563eb; margin-left:6px; cursor:help;"></i>
                        <div class="step-tooltip">
            ...
            <div class="card-content" style="max-width: 64rem; margin: 0 auto;">
                ...
                <div class="step-actions">
                    <button class="btn btn-primary btn-disabled" id="continueAgencyBtn" disabled style="opacity:0.6;cursor:not-allowed;">
...
function setupAgencyDropdowns(agencyData) {
    ...
    function autoDetectAgencyAndSubOrg(input, agencyData) {
            ...
            if (searchInput.includes(keyword)) {
                ...
                if (agencyMatch) {
                    ...
                    agencyMatch.subOrgs.forEach(subOrg => {
                        ...
                        let score = 0;
                        const subOrgMain = subOrgLower.split('(')[0].trim();
                        if (searchInput.includes(subOrgMain) && subOrgMain.length > 2) score += 100;
                        const inputWords = searchInput.split(/\s+/);
                        const subOrgWords = subOrgMain.split(/\s+/);
        ...
        agencyData.forEach(agency => {
            agency.subOrgs.forEach(subOrg => {
                ...
                let score = 0;
                const subOrgMain = subOrgLower.split('(')[0].trim();
                const subOrgWords = subOrgMain.replace(/[()]/g, '').split(/\s+/).filter(word => word.length > 2);
                ...
                if (matchedWords > 1) score += matchedWords * 5;
                const acronymMatch = subOrg.match(/\(([^)]+)\)/);
                if (acronymMatch) {
                    const acronym = acronymMatch[1].toLowerCase();
                    if (searchInput.includes(acronym)) score += 30;
                }
    ...
    // --- FIX: Add autocomplete event for lowestOrgInput ---
    let inputTimeout;
    ...
    function updateContinueButton() {
        ...
        const agencyFilled = federalAgencyDropdown.value && federalAgencyDropdown.value !== '';
        const subOrgFilled = subOrgDropdown.value && subOrgDropdown.value !== '';
    ...
    continueBtn.addEventListener('click', function() {
        ...
        const agencyFilled = federalAgencyDropdown.value && federalAgencyDropdown.value !== '';
        const subOrgFilled = subOrgDropdown.value && subOrgDropdown.value !== '';
        if (lowestOrgFilled && agencyFilled && subOrgFilled) {
            currentStep = 3;
...
function setupGenerateButton() {
    ...
    btn.onclick = async function() {
        ...
        // Combine position title and organizational title for the prompt
        let positionTitle = formState.positionTitle || '';
        if (formState.orgTitle && formState.orgTitle.trim()) {
            positionTitle += ` (${formState.orgTitle.trim()})`;
        }
        ...
        const payload = {
            ...
            supervisoryLevel: formState.supervisoryLevel,
            positionTitle // <-- use combined title
        };
...
function formatUpdatedPD(sectionsObject) {
    // Remove duplicates and clean up summary sections
    ...
    // 6. Summary sections (always include in this order)
    ...
    sectionOrder.forEach(section => {
        ...
        if (content !== undefined) {
            // Remove leading/trailing asterisks from content
            const cleanContent = (content || '').trim().replace(/^\*{1,2}/gm, '').replace(/\*{1,2}$/gm, '');
            formatted += `**${section}**\n${cleanContent}\n\n`;
        }
    });
...
function setupAIRewriteButton(textareaId, resultId, buttonId) {
    ...
    btn.onclick = async function() {
        ...
        try {
            ...
            // Display rewritten duties
            resultDiv.innerHTML = `
                <div style="margin-bottom:1em;">
                    <strong>Rewritten Duties:</strong>
                    <pre style="background:#f3f4f6;padding:1em;border-radius:0.5em;">${rewritten.join('\n')}</pre>
                </div>
            ...
            document.getElementById('useRewritesBtn').onclick = function() {
                ...
                // --- FIX: Show and enable the grade analysis button if 6+ duties ---
                const gradeBtn = document.getElementById('gradeAnalysisBtn');
                if (gradeBtn) {
                    if (formState.unknownDuties.filter(d => d.trim().length > 0).length >= 6) {
                        gradeBtn.style.display = '';
...
function splitPDSections(text) {
    ...
    // Fix headers that are merged with content (e.g. "**HEADER**Job Series: ...")
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)([A-Z][a-z])/g, '$1\n$2');
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)(\d+\.)/g, '$1\n$2');
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)(- )/g, '$1\n$2');
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)(Factor\d+)/g, '$1\n$2');
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)(Total Points:)/g, '$1\n$2');
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)(Final Grade:)/g, '$1\n$2');
    text = text.replace(/(\*\*[A-Z][A-Z0-9 \-:&(),%/]+?\*\*)(Grade Range:)/g, '$1\n$2');

    // Also fix plain headers merged with content
    [
        'HEADER',
        'INTRODUCTION',
        'MAJOR DUTIES',
    ...
    // --- NEW: Split summary sections if run together ---
    text = text.replace(/(Total Points:\s*\d+)(Final Grade:\s*GS-?\d+)?(Grade Range:\s*\d+-\d+)?/g, function(_, tp, fg, gr) {
        let out = '';
        if (tp) out += '\n' + tp + '\n';
        if (fg) out += fg + '\n';
        if (gr) out += gr + '\n';
    ...
    // Build a regex that matches both bolded and plain section headers, even if run together
    const sectionHeaderRegex = new RegExp(
        [
            '\\*\\*([A-Z][A-Z0-9 \\-:&(),%/]+)\\*\\*\\s*', // Bolded headers
            '(HEADER|INTRODUCTION|MAJOR DUTIES|FACTOR EVALUATION - COMPLETE ANALYSIS|EVALUATION SUMMARY|CONDITIONS OF EMPLOYMENT|TITLE AND SERIES DETERMINATION|FAIR LABOR STANDARDS ACT DETERMINATION)', // Plain headers
            'Factor\\s*\\d+\\s*[–-]?\\s*[^\\n]+', // Factor headers
            'Total\\s*Points\\s*:\\s*\\d+', // Total Points
            'Final\\s*Grade\\s*:\\s*GS-?\\d+', // Final Grade
            'Grade\\s*Range\\s*:\\s*\\d+-\\d+' // Grade Range
        ].join('|'),
        'gim'
    );
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        // If header and content are run together (no line break), try to split at first sentence or bullet
        if (!rawContent && matches[i + 1].index - headerEnd < 100) {
            const nextChunk = text.substring(headerEnd, matches[i + 1].index);
            const splitIdx = nextChunk.search(/(\. |- )/);
            if (splitIdx > 0) {
                rawContent = nextChunk.substring(splitIdx).trim();
            }
        }
...
window.resetUpdateProcess = resetUpdateProcess;
...
window.setupGroupSeriesDropdown = setupGroupSeriesDropdown;
