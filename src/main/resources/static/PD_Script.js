    ...
    btn.onclick = async function() {
        ...
        try {
            ...
            document.getElementById('useRewritesBtn').onclick = function() {
                formState.duties = [...rewritten];
                renderDutyInputs(formState.duties);

                // Re-enable AI Grade Analysis and Recommendation buttons
                const gradeBtn = document.getElementById('gradeAnalysisBtn');
                if (gradeBtn) {
                    gradeBtn.disabled = false;
                    gradeBtn.style.display = '';
                }
                const aiRecBtn = document.getElementById('getAIRecommendationBtn');
                if (aiRecBtn) {
                    aiRecBtn.disabled = false;
                    aiRecBtn.style.display = '';
                }
            };
        } catch (err) {
