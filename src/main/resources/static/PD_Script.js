...
function fixPDFormatting(pdText) {
    ...
    // --- Ensure section headers are bold and on their own line ---
    ...
    text = text.replace(
        /Factor\s*([1-9])\s*[-–]\s*([^\n]+?)\s*Level\s*([1-9]-[1-9]),\s*([\d,]+)\s*Points/gi,
        '\n\n**Factor $1 – $2 Level $3, $4 Points**\n\n'
    );
...
// Use this for displaying the PD
...
window.formatGeneratedPD = function(pdText) {
    ...
    // First, ensure **HEADER** is on its own line
    text = text.replace(/([^\n])\s*\*\*HEADER\*\*/gi, '$1\n\n**HEADER**\n\n');
    text = text.replace(/\*\*HEADER\*\*\s*([A-Z])/gi, '**HEADER**\n\n$1');
    
    // Fix header elements to be on separate lines with blank line after each
    text = text.replace(/Job Series:\s*GS-(\d+)\s*Position Title:/gi, 'Job Series: GS-$1\n\nPosition Title:');
    text = text.replace(/Position Title:\s*([^\n]+)\s*Agency:/gi, 'Position Title: $1\n\nAgency:');
    text = text.replace(/Agency:\s*([^\n]+?)\s*(?=Organization:|[A-Z][a-z]+ Division)/gi, 'Agency: $1\n\n');
    text = text.replace(/([A-Z][a-z]+ Division)\s*(?=[A-Z][a-z]+ [Dd]ivision|Supervisory Level:)/gi, '$1\n\n');
    text = text.replace(/Organization:\s*([^\n]+)\s*Supervisory Level:/gi, 'Organization: $1\n\nSupervisory Level:');
    text = text.replace(/Supervisory Level:\s*([^\n]+?)(?=\s*\*\*)/gi, 'Supervisory Level: $1\n\n');
    
    // Ensure clean separation between header info items
    text = text.replace(/(U\.S\.\s+[^\n]+?)(GS-\d+-\d+)/gi, '$1\n\n$2');
    text = text.replace(/(GS-\d+-\d+)\s*(Organizational Title:)/gi, '$1\n\nOrganizational Title:');
    
    // ===== SECTION HEADER FORMATTING =====
    // Ensure all major section headers are properly formatted with spacing
    const sectionHeaders = [
        'HEADER',
        'INTRODUCTION',
        'MAJOR DUTIES',
    ...
    sectionHeaders.forEach(header => {
        // Match the header with or without asterisks, with or without colon
        const pattern = new RegExp(`\\*{0,2}\\s*${header}\\s*:?\\s*\\*{0,2}`, 'gi');
        text = text.replace(pattern, `\n\n**${header}**\n\n`);
    });
    
    // ===== INTRODUCTION SECTION FIX =====
    // Ensure Introduction header is on its own line and content starts on new line after header
    text = text.replace(/([^\n])\s*\*\*INTRODUCTION\*\*/gi, '$1\n\n**INTRODUCTION**\n\n');
    text = text.replace(/\*\*INTRODUCTION\*\*\s*([A-Z])/g, '**INTRODUCTION**\n\n$1');
    
    // ===== FACTOR HEADER FORMATTING =====
    // Fix factor headers to be properly formatted with spacing
    // Pattern: Factor 2 – Supervisory Controls Level 2-4, 450 Points
    // Fix the spacing issues: Factor2 -> Factor 2, Level2-4 -> Level 2-4, no comma in numbers
    
    // First pass: Add space after "Factor" number
    text = text.replace(/Factor(\d+)\s*[–-]/gi, 'Factor $1 –');
    
    // Second pass: Add space in Level designation (2-4 -> 2 - 4)
    text = text.replace(/Level\s*(\d+)-(\d+)/gi, 'Level $1-$2');
    
    // Third pass: Format the complete factor header with proper spacing
    text = text.replace(
    ...
    // Format numbered duty sections with titles
    text = text.replace(/^\s*(\d+)\.\s*\*{0,2}([^*\n]{10,}?)\*{0,2}\s*\((\d+)%\)/gmi, '\n\n$1. **$2** ($3%)\n\n');
    text = text.replace(/^\s*(\d+)\.\s*([A-Z][^:\n]{10,}?):\s*/gmi, '\n\n$1. **$2:**\n\n');
    
    // ===== EVALUATION SUMMARY FORMATTING =====
    // Ensure each summary item is on its own line with proper spacing
    text = text.replace(/\*{0,2}Total Points:\s*(\d+)\*{0,2}/gi, '\n\n**Total Points:** $1\n\n');
    text = text.replace(/\*{0,2}Final Grade:\s*(GS-\d+)\*{0,2}/gi, '**Final Grade:** $1\n\n');
    text = text.replace(/\*{0,2}Grade Range:\s*([\d\-+]+)\*{0,2}/gi, '**Grade Range:** $1\n\n');
    ...
    // ===== PARAGRAPH SPACING =====
    // Add spacing between paragraphs (after sentence-ending periods followed by capital letters)
    text = text.replace(/([.!?])\s*\n([A-Z])/g, '$1\n\n$2');
    ...
    // Fix excessive blank lines (max 2 newlines = 1 blank line)
    text = text.replace(/\n{3,}/g, '\n\n');
