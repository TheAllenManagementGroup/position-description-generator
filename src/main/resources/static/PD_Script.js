...
async function updateFactorSections(factors) {
    console.log('Updating factor sections (preserving locked titles):', factors);

    // Determine if supervisory by presence of 4A/4B
    const has4A = Object.keys(factors).some(k => /4A/i.test(k));
    const has4B = Object.keys(factors).some(k => /4B/i.test(k));
    const isSupervisory = has4A || has4B;
    ...
    // CRITICAL: Do NOT delete existing factor keys - only update level/points
    order.forEach(id => {
        const factorKey = `Factor ${id}`;
        ...
        // LOCKED: Use canonical title from names map
        const factorName = names[id];
        const newSectionKey = `Factor ${id} - ${factorName}` + 
                              (level ? ` Level ${level}` : '') + 
                              (points ? `, ${points} Points` : '');

        // Find if this factor already exists with old level/points
        const oldKey = Object.keys(pdSections).find(k => {
            const m = k.match(/^Factor\s*(\d+[A-Za-z]?)\s*-\s*([^L]+)/);
            if (!m) return false;
            return m[1].toUpperCase() === id.toUpperCase();
        });

        // If exists with different key, update in place
        if (oldKey && oldKey !== newSectionKey) {
            pdSections[newSectionKey] = rationale;
            delete pdSections[oldKey]; // Remove old key
        } else if (!pdSections[newSectionKey]) {
            pdSections[newSectionKey] = rationale;
        } else {
            // Already exists with correct key, just update content
            pdSections[newSectionKey] = rationale;
        }
    });
