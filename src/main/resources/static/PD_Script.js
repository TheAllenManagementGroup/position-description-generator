...
async function saveSectionEdit(title) {
    ...
    // Only update pdSections for non-major duties edits
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            if (data.factors) await updateFactorSections(data.factors);
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Save the original major duties content before updating factors
            const originalMajorDuties = pdSections['MAJOR DUTIES'];

            // Send only the new major duties text
            ...
            // Update ONLY the factor sections and summary sections properly
            if (data.factors && typeof data.factors === 'object') {
                console.log('Updating factors with data:', data.factors);
                console.log('Total points from server:', data.totalPoints);
                
                // Update factor sections with proper header/content separation
                for (let i = 1; i <= 9; i++) {
                    const key = `Factor ${i}`;
                    if (data.factors[key]) {
                        ...
                        // Create new section key with proper header
                        const newSectionKey = `Factor ${i} - ${factorName} Level ${factor.level}, ${factor.points} Points`;
                        
                        // Store just the rationale in the content (not the header info)
                        pdSections[newSectionKey] = factor.rationale || '';
                        
                        // Remove old factor section if it exists with different key
                ...
                // Restore the original major duties content so it is not overwritten
                pdSections['MAJOR DUTIES'] = originalMajorDuties;
                
                // Re-render all sections to show updated headers
                renderEditableSections();
                
                console.log('Updated pdSections:', pdSections);
            }
            showFactorUpdateSuccess();
...
async function updateSummarySections(data) {
    ...
    const keysToRemove = Object.keys(pdSections).filter(key => 
        /^Total Points:|^Final Grade:|^Grade Range:/i.test(key) &&
        key !== `Total Points: ${data.totalPoints}` &&
        key !== `Final Grade: ${data.finalGrade}` &&
        key !== `Grade Range: ${data.gradeRange}`
    );
    keysToRemove.forEach(key => delete pdSections[key]);
}
