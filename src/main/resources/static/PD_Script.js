...
window.formatGeneratedPD = function(pdText) {
    ...
    // Fix merged header lines
    text = text.replace(/(U\.S\. Department of Justice)([A-Z])/g, '$1\n$2');
    text = text.replace(/(Criminal Division)([A-Z])/g, '$1\n$2');
    text = text.replace(/(GS-\d+)(Organizational Title:)/g, '$1\n$2');

    // Ensure section headers are bold and properly spaced
    const sections = [
    ...
    sections.forEach(section => {
        // Handle both with and without existing ** markers
        const escapedSection = section.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
        text = text.replace(new RegExp(`\\*\\*${escapedSection}\\*\\*`, 'g'), `\n\n**${section}**\n`);
        text = text.replace(new RegExp(`^${escapedSection}`, 'gm'), `\n\n**${section}**\n`);
    });

    // Major duties: ensure each starts on a new line with bold headers
    text = text.replace(/(\d+)\.\s*([A-Z][^:]+?)(\s*\([^)]+\))?\s*:/g, '\n\n$1. **$2$3:**');

    // Fix factor headers with proper spacing and make them bold
    // Handles: Factor1 - Knowledge Required by the Position Level1-7,1250 Points
    text = text.replace(/(\*\*)?Factor(\d+)\s*-\s*([^L]+?)\s*Level(\d+)-(\d+),(\d+)\s*Points(\*\*)?/g, 
        '\n\n**Factor $2 - $3 Level $4-$5, $6 Points**\n');

    // Handle evaluation summary components
    text = text.replace(/(\*\*EVALUATION SUMMARY:\*\*)\s*(\*\*)?Total Points:\s*(\d+)(\*\*)?\s*(\*\*)?Final Grade:\s*(GS-\d+)(\*\*)?\s*(\*\*)?Grade Range:\s*(\d+-\d+)(\*\*)?/gi,
        '$1\n\n**Total Points: $3**\n**Final Grade: $6**\n**Grade Range: $9**\n');
    
    // Also handle cases where these might be separate
    text = text.replace(/Total Points:\s*(\d+)/g, '**Total Points: $1**');
    text = text.replace(/Final Grade:\s*(GS-\d+)/g, '**Final Grade: $1**');
    text = text.replace(/Grade Range:\s*(\d+-\d+)/g, '**Grade Range: $1**');
