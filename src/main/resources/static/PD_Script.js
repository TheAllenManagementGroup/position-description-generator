...
function splitPDSections(text) {
    ...
    const sectionHeaderRegex = /(?:^\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*$|^(Introduction|Major Duties|Conditions of Employment|Title and Series Determination|Fair Labor Standards Act Determination)$|^(Factor\s*\d+\s*[–-]\s*[^\n]+?Level\s*\d+-\d+,\s*\d+\s*Points?)$|^(Total Points:\s*\d+)$|^(Final Grade:\s*GS-?\d+)$|^(Grade Range:\s*\d+-\d+)$)/gm;
    ...
    while ((match = sectionHeaderRegex.exec(text)) !== null) {
        ...
        let header = match[1] || match[2] || match[3] || match[4] || match[5] || match[6];
