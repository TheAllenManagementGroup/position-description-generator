...
async function saveSectionEdit(title) {
    ...
    // Always update pdSections for all edits, including MAJOR DUTIES
    ...
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Save the user's input for major duties
            const originalMajorDuties = newContent;
            ...
            // Update ONLY the factor sections and summary sections
            if (data.factors && typeof data.factors === 'object') {
                ...
                // Restore the user's input for major duties so it is not overwritten
