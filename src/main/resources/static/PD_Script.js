...
window.formatGeneratedPD = function(pdText) {
    ...
    // Step 2: Fix merged header lines
    // Add line breaks before Division, GS-Grade, and Organizational Title
    text = text.replace(/(U\.S\. Department of Justice)([A-Z])/g, '$1\n$2');
    text = text.replace(/([A-Za-z &]+ Division)([A-Z])/g, '$1\n$2');
    text = text.replace(/(GS-\d+)([A-Za-z])/g, '$1\n$2');
    text = text.replace(/(Organizational Title:)/g, '\n$1');

    // Step 3: Bold main sections and add spacing
    const mainSections = [
        'HEADER:', 'INTRODUCTION:', 'MAJOR DUTIES:', 'FACTOR EVALUATION - COMPLETE ANALYSIS:',
        'EVALUATION SUMMARY:', 'CONDITIONS OF EMPLOYMENT:', 'TITLE AND SERIES DETERMINATION:',
        'FAIR LABOR STANDARDS ACT DETERMINATION:'
    ];
    mainSections.forEach(section => {
        const re = new RegExp(`([a-zA-Z0-9])\\s*(${section.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'g');
        text = text.replace(re, '$1\n\n**$2**\n\n');
        const boldRe = new RegExp(section.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g');
        text = text.replace(boldRe, `**${section}**`);
    });

    // Step 4: Format Major Duties numbering and bold titles
    text = text.replace(/(\d+)\.\s*([A-Z][^:\n]+?)\s*:/g, '\n$1. **$2:** ');

    // Step 5: Format Factors properly
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*([^\n]*?)Level\s*(\d+)-(\d+)\s*,?\s*([\d,]+)\s*Points/gi,
        '\n**Factor $1 – $2(Level $3-$4, $5 Points)**\n');
    text = text.replace(/Factor\s*(\d+)\s*[-–—]?\s*-\s*([^\n*]+)/gi,
        '\n**Factor $1 – $2**\n');

    // Step 6: Split Evaluation Summary into separate lines
    text = text.replace(/(\*\*EVALUATION SUMMARY:\*\*)\s*Total Points:?\s*([\d,]+)\s*Final Grade:?\s*(GS-\d+)\s*Grade Range:?\s*([\d,-]+)/gi,
        '\n$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4');

    // Step 7: Fix bullets in Conditions of Employment
    text = text.replace(/-([^\n]+)/g, '\n- $1');

    // Step 8: Clean up spacing around Factors, Duties, and Sections
    text = text.replace(/\n{2,}/g, '\n\n');
    text = text.replace(/[ \t]+\n/g, '\n').replace(/\n[ \t]+/g, '\n');
