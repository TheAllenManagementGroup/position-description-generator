...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (disclaimerCheckbox && continueBtn) {
        setupDisclaimerModal(); // OK if it doesn't call renderStep()
    }

    // Only assign functions to window — safe
    window.renderStep = renderStep;
    window.showUpdateApp = showUpdateApp;
    window.showMainApp = showMainApp;

    // Delay anything that uses steps until after steps is declared
    // initializeJobSeriesData(); // <-- move this AFTER steps declaration
});
