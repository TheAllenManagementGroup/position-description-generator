...
async function saveSectionEdit(title) {
    ...
    // Save edited content to history
    let newTitle = title;
    if (!window.sectionEditHistory) window.sectionEditHistory = {};
    ...
    // If this is a factor section, regenerate ALL factors with correct evaluation type
    if (/^Factor\s*\d+[AB]?/i.test(newTitle)) {
        await regenerateAllFactorsPreservingType();
    }
}

// NEW: Regenerate ALL factors while preserving evaluation type
async function regenerateAllFactorsPreservingType() {
    try {
        // Detect if supervisory or non-supervisory
        const isSupervisory = detectEvaluationTypeFromSections(pdSections);
        console.log('Regenerating factors - Type: ' + (isSupervisory ? 'SUPERVISORY' : 'NON-SUPERVISORY'));

        // Get major duties
        let majorDuties = '';
        const dutyKeys = Object.keys(pdSections).filter(k => 
            /^MAJOR DUT(Y|IES)\s*\d*/i.test(k.trim())
        );
        
        if (dutyKeys.length > 0) {
            majorDuties = dutyKeys
                .sort((a, b) => {
            ...
            majorDuties = pdSections['MAJOR DUTIES'] || '';
        }

        if (!majorDuties || majorDuties.trim().isEmpty()) {
            console.warn('No major duties found for regeneration');
            return;
        }
        ...
        const body = {
            ...
            evaluationType: isSupervisory ? 'supervisory' : 'non-supervisory'
        };

        // Preserve existing levels
        if (isSupervisory) {
            for (let factorId of ['1', '2', '3', '4A', '4B', '5', '6']) {
                const key = Object.keys(pdSections).find(k => 
                    new RegExp(`^Factor\\s*${factorId.replace(/A|B/g, '[AB]?')}\\b`, 'i').test(k.trim())
                );
                if (key) {
                    body[`Factor ${factorId}_originalLevel`] = extractLevelFromHeader(pdSections[key] || '');
                }
            }
        } else {
            for (let i = 1; i <= 9; i++) {
                const key = Object.keys(pdSections).find(k => 
                    new RegExp(`^Factor\\s*${i}\\b`, 'i').test(k.trim())
                );
                if (key) {
                    body[`Factor ${i}_originalLevel`] = extractLevelFromHeader(pdSections[key] || '');
                }
            }
        }
        ...
        const data = await response.json();
        if (!data.success) {
            console.error('Factor regeneration failed:', data.error);
            return;
        }

        // Remove old factor sections
        Object.keys(pdSections).forEach(key => {
            if (/^Factor\s*\d+[AB]?/i.test(key.trim())) {
                delete pdSections[key];
            }
        });

        // Add new factors with CORRECT titles
        if (isSupervisory) {
            const gssgNames = {
            ...
            for (let i = 1; i <= 6; i++) {
                if (i === 4) {
                    ['4A', '4B'].forEach(fid => {
                        const f = data.factors[`Factor ${fid}`];
                        if (f) {
                            const header = `Factor ${fid} – ${gssgNames[fid]} Level ${f.level}, ${f.points} Points`;
                            pdSections[header] = f.rationale || '';
                        }
                    });
                } else {
                    const f = data.factors[`Factor ${i}`];
                    if (f) {
                        const header = `Factor ${i} – ${gssgNames[i]} Level ${f.level}, ${f.points} Points`;
                        pdSections[header] = f.rationale || '';
                    }
                }
            }
            ...
            for (let i = 1; i <= 9; i++) {
                const f = data.factors[`Factor ${i}`];
                if (f) {
                    const header = `Factor ${i} – ${fesNames[i]} Level ${f.level}, ${f.points} Points`;
                    pdSections[header] = f.rationale || '';
                }
            }
        }
...
// NEW: Detect evaluation type from sections
function detectEvaluationTypeFromSections(sections) {
    ...
    let gssgCount = 0, fesCount = 0;
    
    for (let key of Object.keys(sections || {})) {
        const upper = key.toUpperCase();
        gssgIndicators.forEach(ind => { if (upper.includes(ind)) gssgCount++; });
        fesIndicators.forEach(ind => { if (upper.includes(ind)) fesCount++; });
    }
    
    return gssgCount > fesCount;
}

// NEW: Extract level from header string
function extractLevelFromHeader(text) {
    if (!text) return null;
    const match = text.match(/Level\s+(\d+[AB]?-\d+)/i);
