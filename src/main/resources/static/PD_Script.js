...
window.formatGeneratedPD = function(pdText) {
    ...
    text = text.replace(/([A-Za-z &]+ Division)(GS-\d+)/g, '$1\n$2');
    text = text.replace(/(GS-\d+)(Organizational Title:)/g, '$1\n$2');
    text = text.replace(/(Organizational Title:[^\n]+)(\*\*INTRODUCTION:\*\*)/g, '$1\n\n$2');
    text = text.replace(/(\*\*[A-Z][^*]+:\*\*)/g, '\n\n$1');
    text = text.replace(/\n{3,}/g, '\n\n');

    // --- FACTOR HEADER FIXES ---
    text = text.replace(
        /Factor\s*(\d+)\s*-\s*([^\n]+?)\s*Level\s*(\d+)-(\d+),\s*([\d,]+)\s*Points/gi,
    ...
    sectionHeaders.forEach(section => {
        ...
        const boldPattern = new RegExp(`\\*\\*${escapedSection}\\*\\*`, 'gi');
        const plainPattern = new RegExp(`(?<!\\*)${escapedSection}(?!\\*)`, 'gi');
        text = text.replace(boldPattern, `\n\n**${section}**`);
        text = text.replace(plainPattern, `\n\n**${section}**`);
    });

    // --- MAJOR DUTIES FORMATTING ---
    text = text.replace(/(\d+)\.\s*\*\*([^*]+?)\*\*:\s*([^0-9]+?)(\d+\.)/g, '$1. **$2:**\n$3\n\n$4.');
    text = text.replace(/^\s*(\d+)\.\s*\*\*([^*]+?)\*\*:\s*/gmi, '\n\n$1. **$2:**\n');
    text = text.replace(/^\s*(\d+)\.\s*([A-Z][^:\n]{5,}?):\s*/gmi, '\n\n$1. **$2:**\n');

    // --- EVALUATION SUMMARY FORMATTING ---
    text = text.replace(
        /(\*\*EVALUATION SUMMARY:\*\*)\s*\*\*Total Points:\s*(\d+)\*\*\s*\*\*Final Grade:\s*(GS-\d+)\*\*\s*\*\*Grade Range:\s*(\d+[-–]\d+)\*\*/gi,
        '$1\n\n**Total Points:** $2\n**Final Grade:** $3\n**Grade Range:** $4\n'
    );
    text = text.replace(/Total Points:\s*(\d+)/gi, '**Total Points:** $1');
    text = text.replace(/Final Grade:\s*(GS-\d+)/gi, '**Final Grade:** $1');
    text = text.replace(/Grade Range:\s*(\d+[-–]\d+)/gi, '**Grade Range:** $1');
...
// Your markdownToHTML function (this works fine)
...
function renderGeneratedPDStep(content) {
    ...
    if (generatedText) {
        // 1. Format the text using your improved formatter
        ...
        // Style the strong elements (headers)
