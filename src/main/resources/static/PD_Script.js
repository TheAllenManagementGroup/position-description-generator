...
async function saveSectionEdit(title) {
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            // Detect system (GSSG vs FES) from existing keys and allow client override via formState if set
            const has4A = Object.keys(factorContents).some(k => /4A/i.test(k));
            const has4B = Object.keys(factorContents).some(k => /4B/i.test(k));
            const has7 = Object.keys(factorContents).some(k => /^Factor\s*7$/i.test(k));
            const has8 = Object.keys(factorContents).some(k => /^Factor\s*8$/i.test(k));
            const has9 = Object.keys(factorContents).some(k => /^Factor\s*9$/i.test(k));
            let isSupervisory = (has4A || has4B) ? true : (has7 || has8 || has9) ? false
                : Object.keys(factorContents).filter(k => /^Factor\s*\d+/i.test(k)).length <= 7;
    ...
    // MAJOR DUTIES path: regenerate all factors
    if (/^MAJOR DUTIES$/i.test(newTitle)) {
        try {
            ...
            // Preserve current factor structure (so server can detect system)
            const currentFactors = collectFactorContents();
            const has4A = Object.keys(currentFactors).some(k => /4A/i.test(k));
            const has4B = Object.keys(currentFactors).some(k => /4B/i.test(k));
            const has7 = Object.keys(currentFactors).some(k => /^Factor\s*7$/i.test(k));
            const has8 = Object.keys(currentFactors).some(k => /^Factor\s*8$/i.test(k));
            const has9 = Object.keys(currentFactors).some(k => /^Factor\s*9$/i.test(k));
            let isSupervisory = (has4A || has4B) ? true : (has7 || has8 || has9) ? false
                : Object.keys(currentFactors).filter(k => /^Factor\s*\d+/i.test(k)).length <= 7;
