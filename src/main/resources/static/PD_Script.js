...
function splitPDSections(text) {
    ...
    // Normalize missing spaces/letter-case issues like "Factor4A" -> "Factor 4A", "Factor3" -> "Factor 3"
    text = text.replace(/\bFactor\s*([0-9]+)([A-Za-z])\b/gi, (m, n, l) => `Factor ${n}${l.toUpperCase()}`);
    text = text.replace(/\bFactor\s*([0-9]+)\b/gi, (m, n) => `Factor ${n}`);

    // Normalize spacing issues in Factor 4A/4B/Level formatting
    text = text.replace(/Level\s*([0-9]+-[0-9]+)\s*,?\s*([0-9]+)\s*Points?/gi, 'Level $1, $2 Points');
    text = text.replace(/Level4-4,100/gi, 'Level 4-4, 100');
    text = text.replace(/Level4-4,125/gi, 'Level 4-4, 125');

    // Ensure "Factor 4A" / "Factor 4B" canonical form (in case of lower/upper/missing space)
    text = text.replace(/\bFactor\s*4\s*A\b/gi, 'Factor 4A');
    text = text.replace(/\bFactor\s*4\s*B\b/gi, 'Factor 4B');

    // Ensure we have a newline before factor headers that might be run together (bold or plain),
    // and collapse header + following "Level ..." that are split across lines into a single header line.
    // This will catch: "**Factor 3 – TITLE\nLevel 3-4, 900 Points**" and "Factor4B – ...\nLevel4-4,125"
    // Insert a newline before any Factor header sequence (safer than lookbehind for compat).
    text = text.replace(/(\bFactor\s*\d+[A-Z]?\b\s*[–-])/gi, '\n$1');

    // Join headers that are split across two lines: "Factor X – TITLE\nLevel X-Y, N Points" -> "Factor X – TITLE Level X-Y, N Points"
    text = text.replace(/(Factor\s*\d+[A-Z]?\s*[–-][^\n]*?)\n\s*(Level\s*\d+-\d+,\s*\d+\s*Points?)/gmi, '$1 $2');

    // Also add a break before Total/Final/Grade headers
    text = text.replace(/(Total Points:\s*\d+)/gi, '\n$1');
    text = text.replace(/(Final Grade:\s*GS-?\d+)/gi, '\n$1');
    text = text.replace(/(Grade Range:\s*\d+-\d+)/gi, '\n$1');

    // Keep bold-section separation as before
    ...
    // Enhanced section header regex - allow the header body to include newlines (we already joined Level lines above)
    // This pattern finds either bold section titles, bolded Factor lines, or plain Factor lines that include "Level ... Points"
    const sectionHeaderRegex = /(?:\*\*([A-Z][A-Z0-9 \-:&(),%/]+)\*\*|\*\*(Factor\s*\d+[AB]?\s*[–-][\s\S]*?Level\s*\d+-\d+,\s*\d+\s*Points)\*\*|^Factor\s*\d+[AB]?\s*[–-][\s\S]*?Level\s*\d+-\d+,\s*\d+\s*Points?|^Total Points:\s*\d+|^Final Grade:\s*GS-?\d+|^Grade Range:\s*\d+-\d+)/gim;

    // Find all section headers
    let matches = [];
    let match;
    while ((match = sectionHeaderRegex.exec(text)) !== null) {
        // match[1] is bold title, match[2] is bold Factor, otherwise use match[0]
    ...
    for (let i = 0; i < matches.length - 1; i++) {
        ...
        // Skip FACTOR EVALUATION and EVALUATION SUMMARY sections
        ...
        if (/^Total Points:/i.test(header) || /^Final Grade:/i.test(header) || /^Grade Range:/i.test(header)) {
            ...
            if (/^Factor\s*\d+/i.test(header) && cleanContent) {
                ...
                const m2 = cleanContent.match(/^(.+?Points\.?)([\s\S]*)$/s);
                if (m2 && m2[2].trim()) {
                    sections[header] = m2[2].trim();
                } else {
