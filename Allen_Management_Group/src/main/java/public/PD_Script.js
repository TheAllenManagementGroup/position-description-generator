...
async function handleAIRecommendation() {
    ...
    try {
        const response = await fetch('http://localhost:8080/api/recommend-series', {
            method: 'POST',
...
function renderGradeAnalysisResultStyled(data) {
    ...
    document.getElementById('generateEvalStatementBtn').onclick = async function() {
        ...
        try {
            const response = await fetch('http://localhost:8080/api/generate-evaluation-statement', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(payload)
            });
...
async function tryNonStreamingGeneration(payload) {
    ...
    try {
        const response = await fetch('http://localhost:8080/api/generate-sync', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload)
        });
...
async function saveSectionEdit(title) {
    ...
    if (/^Factor\s*\d+/i.test(newTitle)) {
        try {
            ...
            const response = await fetch('http://localhost:8080/api/update-factor-points', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function updateSummarySections(data) {
    ...
    for (const update of updates) {
        if (update.value) {
            ...
            pdSections[headerWithValue] = '';
            updateSectionDisplayAndTextarea(headerWithValue, '');
        }
    }
    // Re-render sections so the new headers show up
    renderEditableSections();
}
...
function undoSectionEdit(title) {
    ...
    if (!window.sectionEditStacks[title]) window.sectionEditStacks[title] = [pdSections[title] || ''];
    const stack = window.sectionEditStacks[title];

    // Only pop if there is more than one state in the stack
    if (stack.length > 1) {
        stack.pop();
        textarea.value = stack[stack.length - 1];
    } else {
        // If only one state, revert to initial content
...
function resetSectionEdit(title) {
    const textarea = document.getElementById(`textarea-${title}`);
    // Reset to the original loaded content (first in stack)
    if (!window.sectionEditStacks[title]) window.sectionEditStacks[title] = [pdSections[title] || ''];
    const stack = window.sectionEditStacks[title];
    if (stack.length > 0) {
        // Clear stack and reset to initial content
        window.sectionEditStacks[title] = [stack[0]];
        textarea.value = stack[0];
    }
    // Also reset display
...
function cancelSectionEdit(title) {
    const textarea = document.getElementById(`textarea-${title}`);
    textarea.value = pdSections[title] || '';
    toggleSectionEdit(title);
}
...
function setupAIRewriteButton(textareaId, resultId, buttonId) {
    ...
    btn.onclick = async function() {
        ...
        try {
            const response = await fetch('http://localhost:8080/api/rewrite-duties', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
function splitMajorDutiesIntoSections(sections) {
    // Find the main major duties section
    const majorDutiesKeys = Object.keys(sections).filter(k => {
        const normalized = k.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
        return normalized.match(/^MAJORDUT(Y|IES)\d*$/);
    });
    ...
    if (matches.length > 1) {
        console.log(`Splitting major duties into ${matches.length} sections`);
        
        // Remove ALL major duties variations before adding the split sections
        majorDutiesKeys.forEach(k => {
            console.log(`Removing original major duties section: "${k}"`);
            delete sections[k];
        });
        ...
        matches.forEach((match, idx) => {
            ...
            console.log(`Added: "${newKey}" with ${content.length} characters`);
        });
    }
...
function cleanupMajorDutyDuplicates(sections) {
    ...
    keys.forEach(key => {
        // More aggressive normalization - remove ALL non-alphanumeric and convert to uppercase
        let normalized = key.replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
        
        // Handle various patterns:
        // MAJORDUTIES, MAJORDUTIES1, MAJORDUTY1, etc.
        if (normalized.match(/^MAJORDUT(Y|IES)\d*$/)) {
            // Extract the number if present, default to '1' if no number
    ...
    Object.entries(duplicateGroups).forEach(([groupKey, variants]) => {
        if (variants.length > 1) {
            console.log(`Processing ${variants.length} variants for ${groupKey}:`, variants.map(v => v.originalKey));
            
            // Sort by preference: spaces > "DUTY" > shorter length
            variants.sort((a, b) => {
                // Prefer more spaces (better formatting)
                if (a.hasSpaces !== b.hasSpaces) return b.hasSpaces - a.hasSpaces;
                
                // Prefer "DUTY" over "DUTIES"
            ...
            console.log(`Keeping: "${keepKey}"`);
            console.log(`Deleting: ${deleteKeys.map(k => `"${k}"`).join(', ')}`);
    ...
    toDelete.forEach(key => {
        console.log(`Deleting duplicate section: "${key}"`);
        delete sections[key];
    });
    
    console.log('Cleanup complete. Remaining keys:', Object.keys(sections));
